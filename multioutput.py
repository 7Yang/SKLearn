...
they require a base estimator to be provided in their constructor. 
The meta-estimator extends single output estimators to multioutput estimators.
"""
...
def _available_if_estimator_has(attr):
    ...
    def _check(self):
        ...
        return  hasattr(self.estimator, attr) ? True : False
...
class _MultiOutputEstimator(MetaEstimatorMixin, BaseEstimator, metaclass=ABCMeta):
    ...
    def partial_fit(self, X, y, classes=None, sample_weight=None, **partial_fit_params):
        ...
        classes : list of ndarray of shape (n_outputs,), default=None
            Each array is unique classes for one output in str/int.  Can be obtained via ``[np.unique(y[:, i]) for i in range(y.shape[1])]``, 
        ...
        **partial_fit_params : dict of str -> object
        ...
        Returns
        -------
        self : object, Returns a fitted instance.
        """
        ...
        if _routing_enabled():
            routed_params = process_routing(self, "partial_fit", sample_weight=sample_weight, **partial_fit_params)
        else:
    ...
    def get_metadata_routing(self):
        ...
        router = MetadataRouter(owner=self.__class__.__name__) \
                .add(estimator=self.estimator, method_mapping=MethodMapping() \
...
class MultiOutputRegressor(RegressorMixin, _MultiOutputEstimator):
    ...
    n_jobs : int or None, optional (default=None)
        The number of jobs to run in parallel. :meth:`fit`, :meth:`predict` and :meth:`partial_fit` 
        (if supported by the passed estimator) will be parallelized for each target.

        When individual estimators are fast to train or predict, 
        using ``n_jobs > 1`` can result in slower performance due to the parallelism overhead.

        ``None`` means `1` unless in a :obj:`joblib.parallel_backend` context.
        ``-1`` means using all available processes / threads. See :term:`Glossary <n_jobs>` for more details.
    ...
    feature_names_in_ : ndarray of shape (`n_features_in_`,)
        Names of features seen during :term:`fit`. Only defined if the underlying estimators expose such an attribute when fit.
        .. versionadded:: 1.0
...
class MultiOutputClassifier(ClassifierMixin, _MultiOutputEstimator):
    ...
    estimator : estimator object
        An estimator object implementing :term:`fit` and :term:`predict`.
        A :term:`predict_proba` method will be exposed only if `estimator` implements it.

    n_jobs : int or None, optional (default=None)
        The number of jobs to run in parallel.
        :meth:`fit`, :meth:`predict` and :meth:`partial_fit` (if supported by the passed estimator) will be parallelized for each target.
        ...
        ``None`` means `1` unless in a :obj:`joblib.parallel_backend` context. ``-1`` means using all available processes / threads.
    ...
    def fit(self, X, Y, sample_weight=None, **fit_params):
        ...
        sample_weight : array-like of shape (n_samples,), default=None
            ...
            Only supported if the underlying classifier supports sample weights.
    ...
    def _check_predict_proba(self):
        # raise an AttributeError if `predict_proba` does not exist for each estimator
        if hasattr(self, "estimators_"):
            [getattr(est, "predict_proba") for est in self.estimators_]
            return True
        # raise an AttributeError if `predict_proba` does not exist for the unfitted estimator
    ...
    def predict_proba(self, X):
        ...
        p : array of shape (n_samples, n_classes), or a list of n_outputs such arrays if n_outputs > 1.
            ...
            .. versionchanged:: 0.19
                This function now returns a list of arrays where the length of the list is ``n_outputs``, 
                and each array is (``n_samples``, ``n_classes``) for that particular output.
        """
        check_is_fitted(self)
    ...
    def score(self, X, y):
        ...
        Returns
        -------
        scores : float, Mean accuracy of predicted target versus true target.
        """
        check_is_fitted(self)
    ...
    def fit(self, X, Y, **fit_params):
        ...
        **fit_params : dict of string -> object
        ...
        for chain_idx, estimator in enumerate(self.estimators_):
            message = self._log_message(estimator_idx=chain_idx + 1, n_estimators=len(self.estimators_),
...
class ClassifierChain(MetaEstimatorMixin, ClassifierMixin, _BaseChain):
    ...
    Each model makes a prediction in the order specified by the chain using all of the available features 
    provided to the model plus the predictions of models that are earlier in the chain.

    For an example of how to use ``ClassifierChain`` and benefit from its ensemble, 
    see :ref:`ClassifierChain on a yeast dataset <sphx_glr_auto_examples_multioutput_plot_classifier_chain_yeast.py>` example.
    ...
    order : array-like of shape (n_outputs,) or 'random', default=None
        ...
        the second model will make predictions for column 3, etc. If order is `random` a random ordering will be used.

    cv : int, cross-validation generator or an iterable, default=None
        Determines whether to use cross validated predictions or true labels for the results of previous estimators in the chain.
    ...
    random_state : int, RandomState instance or None, optional (default=None)
        ...
        In addition, it controls the random seed given at each `base_estimator` at each chaining iteration. 
        Thus, it is only used when `base_estimator` exposes a `random_state`.  Pass an int for reproducible output across multiple function calls.
    ...
    n_features_in_ : int
        Number of features seen during :term:`fit`. Only defined if the underlying `base_estimator` exposes such an attribute when fit.
        .. versionadded:: 0.24
    ...
    def predict_proba(self, X):
        ...
        Returns
        -------
        Y_prob : array-like of shape (n_samples, n_classes), The predicted probabilities.
        """
        X = self._validate_data(X, accept_sparse=True, reset=False)
...
class RegressorChain(MetaEstimatorMixin, RegressorMixin, _BaseChain):
    ...
    Each model makes a prediction in the order specified by the chain using all of the available features provided to the model 
    plus the predictions of models that are earlier in the chain.
