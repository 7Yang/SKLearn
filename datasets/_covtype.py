...
The dataset page is available from UCI Machine Learning Repository https://archive.ics.uci.edu/ml/datasets/Covertype
...
ARCHIVE = RemoteFileMetadata(filename="covtype.data.gz",
...
def fetch_covtype(*, data_home=None, download_if_missing=True, random_state=None, shuffle=False, return_X_y=False, as_frame=False):
    ...
    dataset : :class:`~sklearn.utils.Bunch`
        ...
        data : ndarray of shape (581012, 54) Each row corresponds to the 54 features in the dataset.
        target : ndarray of shape (581012,) Each value corresponds to one of the 7 forest covertypes with values ranging between 1 to 7.
        frame : dataframe of shape (581012, 55) Only present when `as_frame=True`. Contains `data` and `target`.
        DESCR : str Description of the forest covertype dataset.
    ...
    if download_if_missing and not available:
        ...
        # Creating temp_dir as a direct subdirectory of the target directory guarantees that both reside on the same filesystem, 
        # so that we can use os.rename to atomically move the data files to their target location.
