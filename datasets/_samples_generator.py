...
def _generate_hypercube(samples, dimensions, rng):
    ...
    if dimensions > 30: return np.hstack([rng.randint(2, size=(samples, dimensions - 30)), _generate_hypercube(samples, 30, rng)])
...
def make_classification(n_samples=100, n_features=20, *, n_informative=2, n_redundant=2, n_repeated=0, \
    ...
    about vertices of an ``n_informative``-dimensional hypercube with sides of length ``2*class_sep`` 
    and assigns an equal number of clusters to each class. 
    ...
    For an example of usage, see :ref:`sphx_glr_auto_examples_datasets_plot_random_dataset.py`.
    ...
    n_features : int, default=20
        ...
        ``n_repeated`` duplicated features and ``n_features-n_informative-n_redundant-n_repeated`` useless features drawn at random.

    n_informative : int, default=2
        The number of informative features. Each class is composed of a number of gaussian clusters each located around 
        the vertices of a hypercube in a subspace of dimension ``n_informative``. 
    ...
    scale : float, ndarray of shape (n_features,) or None, default=1.0
        ...
        If None, then features are scaled by a random value drawn in [1, 100]. Note that scaling happens after shifting.
...
def make_multilabel_classification(n_samples=100, n_features=20, *, n_classes=5, n_labels=2, length=50, \
    ...
    n_classes  : int, default=5,   The number of classes of the classification problem.

    n_labels : int, default=2
        The average number of labels per instance. More precisely, 
        the number of labels per sample is drawn from a Poisson distribution with ``n_labels`` as its expected value, 
        but samples are bounded (using rejection sampling) by ``n_classes``, and must be nonzero if ``allow_unlabeled`` is False.
    ...
    return_indicator : {'dense', 'sparse'} or False, default='dense'
        If ``'dense'``  return ``Y`` in the dense  binary indicator format. 
        If ``'sparse'`` return ``Y`` in the sparse binary indicator format.  
    ...
    p_c      /= p_c.sum()
    cumulative_p_c = np.cumsum(p_c) # cumulative sum
...
def make_regression(n_samples=100, n_features=100, *, n_informative=10, n_targets=1, bias=0.0, \
    ...
    tail_strength : float, default=0.5
        ...
        When a float, it should be between 0 and 1.
    ...
    if effective_rank is None:
        ...
        X = make_low_rank_matrix(n_samples=n_samples, n_features=n_features, effective_rank=effective_rank, \
            tail_strength=tail_strength, random_state=generator)

    # Generate a ground truth model with only n_informative features being non zeros 
    # (the other features are not correlated to y and should be ignored by a sparsifying regularizers such as L1 or elastic net)
...
def make_circles(n_samples=100, *, shuffle=True, noise=None, random_state=None, factor=0.8):
    ...
    if noise is not None: X += generator.normal(scale=noise, size=X.shape)
...
@validate_params({
    ...
    "shuffle"  : ["boolean"],
    "noise"    : [Interval(Real, 0, None, closed="left"), None],
    "random_state": ["random_state"] }, prefer_skip_nested_validation=True)
...
@validate_params({
    ...
    "shuffle": ["boolean"],
    "random_state": ["random_state"],
    "return_centers": ["boolean"]}, prefer_skip_nested_validation=True)
def make_blobs(n_samples=100, n_features=2, *, centers=None, cluster_std=1.0, center_box=(-10.0, 10.0), \
    ...
    if isinstance(n_samples, numbers.Integral):
        ...
        if len(centers) != n_centers:
            raise ValueError("Length of `n_samples` not consistent with number of centers. Got n_samples = {n_samples} and centers = {centers}")
        centers    = check_array(centers)
        n_features = centers.shape[1]

    # stds: if cluster_std is given as list, it must be consistent with the n_centers
    if hasattr(cluster_std, "__len__") and len(cluster_std) != n_centers:
        raise ValueError("Length of `clusters_std` not consistent with number of centers. Got centers = {} "
    ...
    if isinstance(n_samples, Iterable):
        ...
        for i in range(n_samples % n_centers): n_samples_per_center[i] += 1
...
def make_low_rank_matrix(n_samples=100, n_features=100, *, effective_rank=10, tail_strength=0.5, random_state=None):
    ...
    The low rank part of the profile can be considered the structured signal part of the data while the tail 
    can be considered the noisy part of the data that cannot be summarized by a low number of linear components (singular vectors).
    ...
    Returns
    -------
    X : ndarray of shape (n_samples, n_features), The matrix.
    """
    generator = check_random_state(random_state)
    ...
    # QR（正交三角）
    # 把矩阵分解成一个正交矩阵与一个上三角矩阵的积, 求一般矩阵全部特征值的最有效并广泛应用的方法
...
@validate_params({
    ...
    "random_state": ["random_state"],
    "data_transposed": ["boolean", Hidden(StrOptions({"deprecated"}))] }, prefer_skip_nested_validation=True)
def make_sparse_coded_signal(n_samples, *, n_components, n_features, n_nonzero_coefs, random_state=None, data_transposed="deprecated"):
    ...
    `X` is of shape `(n_components, n_samples)` and each column of `X` has exactly `n_nonzero_coefs` non-zero elements.
    ...
    code : ndarray of shape (n_components, n_samples) or (n_samples, n_components)
        ...
        The shape is `(n_samples, n_components)` if `data_transposed` is False, otherwise it's `(n_components, n_samples)`.
    """
    generator = check_random_state(random_state)
    ...
    # TODO(1.5) remove data_transposed raise warning if data_transposed is not passed explicitly
...
def make_sparse_uncorrelated(n_samples=100, n_features=10, *, random_state=None):
    ...
    .. [1] G. Celeux, M. El Anbari, J.-M. Marin, C. P. Robert,
           "Regularization in regression: comparing Bayesian and frequentist methods in a poorly informative situation", 2009.
    """
    generator = check_random_state(random_state)
...
def make_spd_matrix(n_dim, *, random_state=None):
    ...
    U, _, Vt = linalg.svd(np.dot(A.T, A), check_finite=False) # np.dot(A.T, A) 对称矩阵, 一定存在svd奇异值分解
...
def make_sparse_spd_matrix(dim=1, *, alpha=0.95, norm_diag=False, smallest_coef=0.1, largest_coef=0.9, random_state=None):
    ...
    Thus alpha does not translate directly into the filling fraction of the matrix itself.
    """
...
@validate_params({
    ...
    "noise": [Interval(Real, 0, None, closed="left")],
    "random_state": ["random_state"],
    "hole": ["boolean"] }, prefer_skip_nested_validation=True)
def make_swiss_roll(n_samples=100, *, noise=0.0, random_state=None, hole=False):
    ...
    References
    ----------
    .. [1] S. Marsland, "Machine Learning: An Algorithmic Perspective", 2nd edition, Chapter 6, 2014.
...
def make_gaussian_quantiles(*, mean=None, cov=1.0, n_samples=100, n_features=2, n_classes=3, shuffle=True, random_state=None):
    ...
    This classification dataset is constructed by taking a multi-dimensional standard normal distribution 
    and defining classes separated by nested concentric multi-dimensional spheres such that roughly equal numbers of samples 
...
def make_biclusters(shape, n_clusters, *, noise=0.0, minval=10, maxval=100, shuffle=True, random_state=None):
    ...
    .. [1] Dhillon, I. S. (2001, August). Co-clustering documents and words using bipartite spectral graph partitioning. 
        In Proceedings of the seventh ACM SIGKDD international conference on Knowledge discovery and data mining (pp. 269-274). ACM.
    """
    generator = check_random_state(random_state)
    n_rows, n_cols = shape
...
def make_checkerboard(shape, n_clusters, *, noise=0.0, minval=10, maxval=100, shuffle=True, random_state=None):
    ...
    .. [1] Kluger, Y., Basri, R., Chang, J. T., & Gerstein, M. (2003).
        Spectral biclustering of microarray data: coclustering genes and conditions. Genome research, 13(4), 703-716.
    """
    generator = check_random_state(random_state)
    ...
    # row and column clusters of approximately equal sizes
    n_rows, n_cols = shape
    row_sizes = generator.multinomial(n_rows, np.repeat(1./n_row_clusters, n_row_clusters))
    col_sizes = generator.multinomial(n_cols, np.repeat(1./n_col_clusters, n_col_clusters))
