...
def _split_sparse_columns(arff_data: ArffSparseDataType, include_columns: List) -> ArffSparseDataType:
    """
    Obtains several columns from sparse ARFF representation. 
    Additionally, the column indices are re-labelled, given the columns that are not included. 
    (e.g., when including [1, 2, 3], the columns will be relabelled to [0, 1, 2]).

    Parameters
    ----------
    arff_data : tuple
        A tuple of three lists of equal size; first list indicating the value, second the x coordinate and the third the y coordinate.
    ...
    Returns
    -------
    arff_data_new : tuple, Subset of arff data with only the include columns indicated by the include_columns argument.
    """
...
def _sparse_data_to_array(arff_data: ArffSparseDataType, include_columns: List) -> np.ndarray:
    # turns the sparse data back into an array (can't use toarray() function, as this does only work on numeric data)
...
def _liac_arff_parser(gzip_file, output_arrays_type, openml_columns_info, feature_names_to_select, target_names_to_select, shape=None):
    ...
    This parser is quite slow but consumes a generator. Currently it is needed to parse sparse datasets. 
    For dense datasets, it is recommended to instead use the pandas-based parser, although it does not always handles the dtypes exactly the same.
    ...
    # we should not let LIAC-ARFF to encode the nominal attributes with NumPy arrays to have only numerical values.
    ...
    if output_arrays_type == "pandas":
        ...
        # dfs[0] contains only one row, which may not have enough data to infer to column's dtype. 
        # Here we use `dfs[1]` to configure the dtype in dfs[0]
        if len(dfs) >= 2:
            dfs[0] = dfs[0].astype(dfs[1].dtypes)

        # liac-arff parser does not depend on NumPy and uses None to represent missing values. 
        # To be consistent with the pandas parser, we replace None with np.nan.
        ...
        for name in frame.columns:
            ...
            if column_dtype.lower() == "integer":
                # Use a pandas extension array instead of np.int64 to be able to support missing values.
                dtypes[name] = "Int64"
        ...
        if not is_classification:
            ...
            y = np.hstack([
                np.take(np.asarray(categories.pop(col_name), dtype="O"), y[:, i : i + 1].astype(int, copy=False))
        ...
        # reshape y back to 1-D array, if there is only 1 target column; back to None if there are not target columns
        if   y.shape[1] == 1: y = y.reshape((-1,))
        elif y.shape[1] == 0: y = None
...
def _pandas_arff_parser(gzip_file, output_arrays_type, openml_columns_info, feature_names_to_select, target_names_to_select, read_csv_kwargs=None):
    ...
    This parser uses the metadata fetched directly from OpenML and skips the metadata headers of ARFF file itself. The data is loaded as a CSV file.
    ...
    read_csv_kwargs : dict, default=None
        Keyword arguments to pass to `pandas.read_csv`. It allows to overwrite the default options.
    ...
    for name in openml_columns_info:
        ...
        if column_dtype.lower() == "integer":
            ...
            dtypes[name] = "category"
    # since we will not pass `names` when reading the ARFF file, 
    # we need to translate `dtypes` from column names to column indices to pass to `pandas.read_csv`
    ...
    try:
        # Setting the columns while reading the file will select the N first columns and not raise a ParserError. 
        # Instead, we set the columns after reading the file and raise a ParserError if the number of columns does not match the
    ...
    # `pd.read_csv` automatically handles double quotes for quoting non-numeric CSV cell values.
    # Contrary to LIAC-ARFF, `pd.read_csv` cannot be configured to consider either single quotes 
    # and double quotes as valid quoting chars at the same time since this case does not occur in regular (non-ARFF) CSV files.
    # To mimic the behavior of LIAC-ARFF parser, we manually strip single quotes on categories as a post-processing steps if needed.
    #
    # Note however that we intentionally do not attempt to do this kind of manual post-processing of (non-categorical) string-typed columns 
    # because we cannot resolve the ambiguity of the case of CSV cell with nesting quoting such as `"'some string value'"` with pandas.
...
def load_arff_from_gzip_file(gzip_file, parser, output_type, openml_columns_info, feature_names_to_select, \
    target_names_to_select, shape=None, read_csv_kwargs=None):
    """
    ...
    parser : {"pandas", "liac-arff"}
        The parser used to parse the ARFF file. "pandas" is recommended but only supports loading dense datasets.
