...
# The original data can be found in: http://vis-www.cs.umass.edu/lfw/lfw.tgz
ARCHIVE = RemoteFileMetadata(filename="lfw.tgz",
...
# The original funneled data can be found in: http://vis-www.cs.umass.edu/lfw/lfw-funneled.tgz
FUNNELED_ARCHIVE = RemoteFileMetadata(filename="lfw-funneled.tgz",
...
#
# Common private utilities for data fetching from the original LFW website local disk caching, and image decoding.
#
...
def _load_imgs(file_paths, slice_, color, resize):
    ...
    try:
        ...
        raise ImportError("The Python Imaging Library (PIL) is required to load data from jpeg files. Please refer to "
            "https://pillow.readthedocs.io/en/stable/installation.html for installing PIL.")

    # compute the portion of the images to load to respect the slice_ parameter given by the caller
...
def _fetch_lfw_people(data_folder_path, slice_=None, color=False, resize=None, min_faces_per_person=0):
    ...
    # shuffle the faces with a deterministic RNG scheme to avoid having all faces of the same person in a row, 
    # as it would break some cross validation and learning algorithms such as SGD and online k-means that make an IID assumption
...
def fetch_lfw_people(*, data_home=None, funneled=True, resize=0.5, min_faces_per_person=0, color=False, \
    slice_=(slice(70, 195), slice(78, 172)), download_if_missing=True, return_X_y=False):
    """
    ...
    color : bool, default=False
        Keep the 3 RGB channels instead of averaging them to a single gray level channel. 
        If color is True the shape of the data has one more dimension than the shape with color = False.

    slice_ : tuple of slice, default=(slice(70, 195), slice(78, 172))
        Provide a custom 2D slice (height, width) to extract the 'interesting' part of the jpeg files 
    ...
    return_X_y : bool, default=False
        ...
        See below for more information about the `dataset.data` and `dataset.target` object.
        .. versionadded:: 0.20
    ...
    (data, target) : tuple if ``return_X_y`` is True
        A tuple of two ndarray. The first containing a 2D array of shape (n_samples, n_features) with each row representing one sample 
...
def _fetch_lfw_pairs(index_file_path, data_folder_path, slice_=None, color=False, resize=None):
    ...
    This operation is meant to be cached by a joblib wrapper.
    """
    # parse the index file to find the number of pairs to be able to allocate the right amount of memory before starting to decode the jpeg files
    with open(index_file_path, "rb") as index_file:
        split_lines = [ln.decode().strip().split("\t") for ln in index_file]
    pair_specs = [sl for sl in split_lines if len(sl) > 2]
    n_pairs = len(pair_specs)

    # iterating over the metadata lines for each pair to find the filename to decode and load in memory
...
@validate_params({
    ...
    "color": ["boolean"],
    "slice_": [tuple, Hidden(None)],
    "download_if_missing": ["boolean"] }, prefer_skip_nested_validation=True)
def fetch_lfw_pairs(*, subset="train", data_home=None, funneled=True, resize=0.5, color=False, \
    slice_=(slice(70, 195), slice(78, 172)), download_if_missing=True):
    """
    ...
    subset : {'train', 'test', '10_folds'}, default='train'
        Select the dataset to load: 'train' for the development training set, 'test' for the development test set, 
        and '10_folds' for the official evaluation set that is meant to be used with a 10-folds cross validation.
    ...
    data : :class:`~sklearn.utils.Bunch`
        ...
        data : ndarray of shape (2200, 5828). Shape depends on ``subset``.
            ...
            Changing the ``slice_``, ``resize`` or ``subset`` parameters will change the shape of the output.
        pairs : ndarray of shape (2200, 2, 62, 47). Shape depends on ``subset``
            Each row has 2 face images corresponding to same or different person from the dataset containing 5749 people. Changing the ``slice_``,
