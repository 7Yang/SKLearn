...
# The original data can be found at: https://cs.nyu.edu/~roweis/data/olivettifaces.mat
FACES = RemoteFileMetadata(filename="olivettifaces.mat",
...
def fetch_olivetti_faces(*, data_home=None, shuffle=False, random_state=0, download_if_missing=True, return_X_y=False):
    ...
    (data, target) : tuple if `return_X_y=True` Tuple with the `data` and `target` objects described above.
        .. versionadded:: 0.22
    """
    ...
    # We want floating point data, but float32 is enough (there is only one byte of precision in the original uint8s anyway)
