...
# https://archive.ics.uci.edu/ml/machine-learning-databases/kddcup99-mld/kddcup.data.gz
ARCHIVE = RemoteFileMetadata(filename="kddcup99_data",
...
ARCHIVE_10_PERCENT = RemoteFileMetadata(filename="kddcup99_10_data",
...
def fetch_kddcup99(*, subset=None, data_home=None, shuffle=False, random_state=None, percent10=True, \
    download_if_missing=True, return_X_y=False, as_frame=False):
    """
    ...
    as_frame : bool, default=False
        If `True`, returns a pandas Dataframe for the ``data`` and ``target`` objects in the `Bunch` returned object; 
        `Bunch` return object will also have a ``frame`` member.
        .. versionadded:: 0.24
    ...
    (data, target) : tuple if ``return_X_y`` is True
        A tuple of two ndarray. The first containing a 2D array of shape (n_samples, n_features) 
        with each row representing one sample and each column representing the features. 
        The second ndarray of shape (n_samples,) containing the target samples.
...
def _fetch_brute_kddcup99(data_home=None, download_if_missing=True, percent10=True):
    ...
    dataset : :class:`~sklearn.utils.Bunch`
        ...
        data          : ndarray of shape (494021, 41), Each row corresponds to the 41 features in the dataset.
        target        : ndarray of shape (494021,),    Each value corresponds to one of the 21 attack types or to the label 'normal.'.
        feature_names : list,                          The names of the dataset columns
        target_names  : list,                          The names of the target columns
        DESCR         : str,                           Description of the kddcup99 dataset.
    ...
    dt = [
        ("duration", int), ("protocol_type", "S4"), ("service", "S11"), ("flag", "S6"), ("src_bytes", int),
        ("dst_bytes", int), ("land", int), ("wrong_fragment", int), ("urgent", int), ("hot", int), 
        ("num_failed_logins", int), ("logged_in", int), ("num_compromised", int), ("root_shell", int),
        ("su_attempted", int), ("num_root", int), ("num_file_creations", int), ("num_shells", int),
        ("num_access_files", int), ("num_outbound_cmds", int), ("is_host_login", int), ("is_guest_login", int),
        ("count", int), ("srv_count", int), ("serror_rate", float), ("srv_serror_rate", float), ("rerror_rate", float),
        ("srv_rerror_rate", float), ("same_srv_rate", float), ("diff_srv_rate", float), ("srv_diff_host_rate", float), 
        ("dst_host_count", int), ("dst_host_srv_count", int), ("dst_host_same_srv_rate", float), ("dst_host_diff_srv_rate", float),
        ("dst_host_same_src_port_rate", float), ("dst_host_srv_diff_host_rate", float), ("dst_host_serror_rate", float), 
        ("dst_host_srv_serror_rate", float), ("dst_host_rerror_rate", float), ("dst_host_srv_rerror_rate", float),
        ("labels", "S16"),
    ]
    ...
    if available:
        try:
            ...
            raise OSError("The cache for fetch_kddcup99 is invalid, please delete {str(kddcup_dir)} and run the fetch_kddcup99 again") from e
        ...
        # (error: 'Incorrect data length while decompressing[...] the file could be corrupted.')
