...
class DictVectorizer(TransformerMixin, BaseEstimator):
    ...
    For instance, a feature "f" that can take on the values "ham" and "spam" will become two features in the output, 
    one signifying "f=ham", the other "f=spam".

    If a feature value is a sequence or set of strings, this transformer will iterate over the values 
    and will count the occurrences of each string value.

    However, note that this transformer will only do a binary one-hot encoding when feature values are of type string. 
    If categorical features are represented as numeric values such as int or iterables of strings, 
    the DictVectorizer can be followed by :class:`~sklearn.preprocessing.OneHotEncoder` to complete binary one-hot encoding.

    Features that do not occur in a sample (mapping) will have a zero value in the resulting array/matrix.
    For an efficiency comparision of the different feature extractors, see :ref:`sphx_glr_auto_examples_text_plot_hashing_vs_dict_vectorizer.py`.
    ...
    def fit(self, X, y=None):
        ...
        Returns
        -------
        self : object, DictVectorizer class instance.
        """
        feature_names = []
        vocab = {}
    ...
    def _transform(self, X, fitting):
        # Sanity check: Python's array has no way of explicitly requesting the signed 32-bit integers that scipy.sparse needs, 
        # so we use the next best thing: typecode "i" (int). However, if that gives larger or smaller integers than 32-bit ones, 
        # np.frombuffer screws up.
        ...
        for x in X:
            for f, v in x.items():
                if isinstance(v, str):
                    ...
                    feature_name = None
                    self._add_iterable_element(f, v, feature_names, vocab, fitting=fitting, transforming=transforming, \
    ...
    def inverse_transform(self, X, dict_type=dict):
        ...
        X must have been produced by this DictVectorizer's transform or fit_transform method; 
        it may only have passed through transformers that preserve the number of features and their order.
