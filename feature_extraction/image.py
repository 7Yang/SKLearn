"""
The :mod:`sklearn.feature_extraction.image` submodule gathers utilities to extract features from images.
"""
...
def _make_edges_3d(n_x, n_y, n_z=1):
    ...
    n_z : integer, default=1, The size of the grid in the z direction, defaults to 1
    """
...
def _to_graph(n_x, n_y, n_z, mask=None, img=None, return_as=sparse.coo_matrix, dtype=None):
    ...
    graph = sparse.coo_matrix((np.hstack((weights, weights, diag)),
        (np.hstack((i_idx, diag_idx)), np.hstack((j_idx, diag_idx)))), (n_voxels, n_voxels), dtype=dtype)
...
def img_to_graph(img, *, mask=None, return_as=sparse.coo_matrix, dtype=None):
    ...
    Parameters
    ----------
    img       : array-like of shape (height, width) or (height, width, channel), 2D or 3D image.
    ...
    Returns
    -------
    graph : ndarray or a sparse matrix class, The computed adjacency matrix.

    Notes
    -----
    For scikit-learn versions 0.14.1 and prior, return_as=np.ndarray was handled by returning a dense np.matrix instance.  
    Going forward, np.ndarray returns an np.ndarray, as expected.
...
def grid_to_graph(n_x, n_y, n_z=1, *, mask=None, return_as=sparse.coo_matrix, dtype=int):
    ...
    Returns
    -------
    graph : np.ndarray or a sparse matrix class, The computed adjacency matrix.
...
def _compute_n_patches(i_h, i_w, p_h, p_w, max_patches=None):
    ...
    Parameters
    ----------
    i_h : int, The image height
    i_w : int, The image with
    p_h : int, The height of a patch
    p_w : int, The width of a patch
    max_patches : int or float, default=None
        The maximum number of patches to extract. If `max_patches` is a float between 0 and 1, 
        it is taken to be a proportion of the total number of patches. If `max_patches` is None, all possible patches are extracted.
    """
    n_h = i_h - p_h + 1
    n_w = i_w - p_w + 1
    all_patches = n_h * n_w
...
def _extract_patches(arr, patch_shape=8, extraction_step=1):
    ...
    Given an n-dimensional array it will return a 2n-dimensional array with the first n dimensions indexing patch position 
    and the last n indexing the patch content. This operation is immediate (O(1)). 
    A reshape performed on the first n dimensions will cause numpy to copy data, leading to a list of extracted patches.
    ...
    patch_shape : int or tuple of length arr.ndim.default=8
        Indicates the shape of the patches to be extracted. If an integer is given, the shape will be a hypercube of sidelength given by its value.
    ...
    patches : strided ndarray
        2n-dimensional array indexing patches on first n dimensions and containing patches on the last n dimensions. 
        These dimensions are fake, but this way no data is copied. 
        A simple reshape invokes a copying operation to obtain a list of patches: result.reshape([-1] + list(patch_shape))
    """
...
@validate_params({
    ...
    "max_patches": [Interval(RealNotInt, 0, 1, closed="neither"), Interval(Integral, 1, None, closed="left"), None ],
...
def extract_patches_2d(image, patch_size, *, max_patches=None, random_state=None):
    ...
    max_patches : int or float, default=None
        ...
        If `max_patches` is None it corresponds to the total number of patches that can be extracted.
    ...
    patches : array of shape (n_patches, patch_height, patch_width) or (n_patches, patch_height, patch_width, n_channels)
        The collection of patches extracted from the image, where `n_patches` is either `max_patches` or the total number of 
        patches that can be extracted.
...
def reconstruct_from_patches_2d(patches, image_size):
    ...
    Patches are assumed to overlap and the image is constructed by filling in the patches from left to right, top to bottom, 
    ...
    patches : ndarray of shape (n_patches, patch_height, patch_width) or (n_patches, patch_height, patch_width, n_channels)
        The complete set of patches. If the patches contain colour information, channels are indexed along the last dimension: 
...
class PatchExtractor(TransformerMixin, BaseEstimator):
    ...
    patch_size : tuple of int (patch_height, patch_width), default=None
        ...
        where `img_height` and `img_width` are the dimensions of the input images.

    max_patches : int or float, default=None
        The maximum number of patches per image to extract. If `max_patches` is a float in (0, 1), 
        it is taken to mean a proportion of the total number of patches. If set to None, extract all possible patches.
    ...
    However, we recommend to call :meth:`fit_transform` instead of :meth:`transform`, 
    as parameter validation is only performed in :meth:`fit`.
    ...
    _parameter_constraints: dict = {
        ...
        "max_patches": [None, Interval(RealNotInt, 0, 1, closed="neither"), Interval(Integral, 1, None, closed="left")],
    ...
    def fit(self, X, y=None):
        ...
        X : ndarray of shape (n_samples, image_height, image_width) or (n_samples, image_height, image_width, n_channels)
            Array of images from which to extract patches. For color images, the last dimension specifies the channel: 
    ...
    def transform(self, X):
        ...
        -------
        patches : array of shape (n_patches, patch_height, patch_width) or (n_patches, patch_height, patch_width, n_channels)
            The collection of patches extracted from the images, where `n_patches` is either `n_samples * max_patches` 
        ...
        if self.patch_size is None:
            ...
            if len(self.patch_size) != 2:
                raise ValueError("patch_size must be a tuple of two integers. Got {self.patch_size} instead.")
        ...
        for ii, image in enumerate(X):
            patches[ii * n_patches : (ii + 1) * n_patches] = \
                    extract_patches_2d(image, patch_size, max_patches=self.max_patches, random_state=random_state)
        return patches
