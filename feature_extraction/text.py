...
__all__ = [
    "HashingVectorizer", "CountVectorizer", "ENGLISH_STOP_WORDS", "TfidfTransformer", "TfidfVectorizer", "strip_accents_ascii", "strip_accents_unicode", "strip_tags" 
]

def _preprocess(doc, accent_function=None, lower=False):
    """
    Chain together an optional series of text preprocessing steps to apply to a document.

    Parameters
    ----------
    doc: str, The string to preprocess
    accent_function: callable, default=None
        Function for handling accented characters. Common strategies include normalizing and removing.
    lower: bool, default=False
        Whether to use str.lower to lowercase all of the text

    Returns
    -------
    doc: str, preprocessed string
    """
    if lower: doc = doc.lower()
    if accent_function is not None: doc = accent_function(doc)
    return doc

def _analyze(doc, analyzer=None, tokenizer=None, ngrams=None, preprocessor=None, decoder=None, stop_words=None):
    """
    Chain together an optional series of text processing steps to go from a single document to ngrams, 
    with or without tokenizing or preprocessing.

    If analyzer is used, only the decoder argument is used, as the analyzer is intended to replace the preprocessor, tokenizer, and ngrams steps.
    ...
    if decoder  is not None: doc = decoder(doc)
    if analyzer is not None: doc = analyzer(doc)
    else:
        if preprocessor is not None: doc = preprocessor(doc)
        if tokenizer    is not None: doc = tokenizer(doc)
...
def strip_accents_unicode(s):
    ...
    Warning: the python-level loop and join operations make this implementation 20 times slower 
    than the strip_accents_ascii basic normalization.
    ...
    """
    try:
        # If `s` is ASCII-compatible, then it does not contain any accented characters and we can avoid an expensive list comprehension
...
class _VectorizerMixin:
    ...
    def decode(self, doc):
        ...
        Parameters
        ----------
        doc : bytes or str, The string to decode.
    ...
    def _word_ngrams(self, tokens, stop_words=None):
        ...
        if max_n != 1:
            ...
            if min_n == 1:
                # no need to do any slicing for unigrams just iterate through the original tokens
    ...
    def _char_ngrams(self, text_document):
        ...
        min_n, max_n = self.ngram_range
        if min_n == 1:
            # no need to do any slicing for unigrams iterate through the string
    ...
    def _char_wb_ngrams(self, text_document):
        ...
        Tokenize text_document into a sequence of character n-grams operating only inside word boundaries. 
    ...
    def _check_stop_words_consistency(self, stop_words, preprocess, tokenize):
        ...
        Returns
        -------
        is_consistent : True if stop words are consistent with the preprocessor and tokenizer, False if they are not, 
            None if the check was previously performed, "error" if it could not be performed 
            (e.g. because of the use of a custom preprocessor / tokenizer)
        """
        ...
        try:
            ...
            if inconsistent:
                warnings.warn("Your stop_words may be inconsistent with your preprocessing. Tokenizing the stop "
                    "words generated tokens %r not in stop_words." % sorted(inconsistent))
...
class HashingVectorizer(TransformerMixin, _VectorizerMixin, BaseEstimator, auto_wrap_output_keys=None):
    ...
    - there can be collisions: distinct tokens can be mapped to the same feature index. 
      However in practice this is rarely an issue if n_features is large enough (e.g. 2 ** 18 for text classification problems).

    - no IDF weighting as this would render the transformer stateful.

    The hash function employed is the signed 32-bit version of Murmurhash3.

    For an efficiency comparision of the different feature extractors, 
    see :ref:`sphx_glr_auto_examples_text_plot_hashing_vs_dict_vectorizer.py`.

    Read more in the :ref:`User Guide <text_feature_extraction>`.
    Parameters
    ----------
    input : {'filename', 'file', 'content'}, default='content'
        - If `'filename'`, the sequence passed as an argument to fit is expected to be a list of filenames 
    ...
    strip_accents : {'ascii', 'unicode'} or callable, default=None
        ...
        'unicode' is a slightly slower method that works on any character. None (default) means no character normalization is performed.
    ...
    stop_words : {'english'}, list, default=None
        ...
        and you should consider an alternative (see :ref:`stop_words`).
    ...
    token_pattern : str or None, default=r"(?u)\\b\\w\\w+\\b"
        ...
        The default regexp selects tokens of 2 or more alphanumeric characters (punctuation is completely ignored
    ...
    analyzer : {'word', 'char', 'char_wb'} or callable, default='word'
        ...
        .. versionchanged:: 0.21
            Since v0.21, if ``input`` is ``'filename'`` or ``'file'``, 
    ...
    alternate_sign : bool, default=True
        When True, an alternating sign is added to the features as to approximately conserve the inner product in the hashed space 
        even for small n_features. This approach is similar to sparse random projection.
        .. versionadded:: 0.19
    ...
    def __init__(self, *, input="content", encoding="utf-8", decode_error="strict", strip_accents=None, lowercase=True, \
        ...
        analyzer="word", n_features=(2**20), binary=False, norm="l2", alternate_sign=True, dtype=np.float64):
        self.input          = input
    ...
    def partial_fit(self, X, y=None):
        ...
        Returns
        -------
        self : object, HashingVectorizer instance.
        """
        return self
    ...
    def transform(self, X):
        ...
        Parameters
        ----------
        X : iterable over raw text documents, length = n_samples Samples. 
            Each sample must be a text document (either bytes or unicode strings, 
            file name or file object depending on the constructor argument) which will be tokenized and hashed.
...
class CountVectorizer(_VectorizerMixin, BaseEstimator):
    ...
    input : {'filename', 'file', 'content'}, default='content'
        ...
        - If `'file'`, the sequence items must have a 'read' method (file-like object) 
          that is called to fetch the bytes in memory.

        - If `'content'`, the input is expected to be a sequence of items 
          that can be of type string or byte.
    ...
    strip_accents : {'ascii', 'unicode'} or callable, default=None
        Remove accents and perform other character normalization during the preprocessing step.
        'ascii' is a fast method that only works on characters that have a direct ASCII mapping.
        'unicode' is a slightly slower method that works on any characters. None (default) means no character normalization is performed.
    ...
    _parameter_constraints: dict = {
        ...
        "ngram_range": [tuple],
        "analyzer": [StrOptions({"word", "char", "char_wb"}), callable],
        "max_df": [Interval(RealNotInt, 0, 1, closed="both"), Interval(Integral, 1, None, closed="left") ],
        "min_df": [Interval(RealNotInt, 0, 1, closed="both"), Interval(Integral, 1, None, closed="left") ],
    ...
    def _limit_features(self, X, vocabulary, high=None, low=None, limit=None):
        ...
        Prune features that are non zero in more samples than high or less documents than low, 
        modifying the vocabulary, and restricting it to at most the limit most frequent.
...
class TfidfTransformer(OneToOneFeatureMixin, TransformerMixin, BaseEstimator, auto_wrap_output_keys=None):
    ...
    The formula that is used to compute the tf-idf for a term t of a document d in a document set is tf-idf(t, d) = tf(t, d) * idf(t), 
    and the idf is computed as idf(t) = log [ n / df(t) ] + 1 (if ``smooth_idf=False``),
    ...
    The effect of adding "1" to the idf in the equation above is that terms with zero idf, i.e., 
    ...
    norm : {'l1', 'l2'} or None, default='l2'
        ...
        - 'l1': Sum of absolute values of vector elements is 1. See :func:`~sklearn.preprocessing.normalize`.
    ...
    def fit(self, X, y=None):
        ...
        """
        # large sparse data is not supported for 32bit platforms because _document_frequency uses 
        # np.bincount which works on arrays of dtype NPY_INTP which is int32 for 32bit platforms. See #20923
    ...
    def transform(self, X, copy=True):
        ...
        Returns
        -------
        vectors : sparse matrix of shape (n_samples, n_features), Tf-idf-weighted document-term matrix.
        """
        ...
        if self.use_idf:
            # idf_ being a property, the automatic attributes detection does not work as usual and we need to specify the attribute name:
    ...
    def idf_(self):
        ...
        """
        # if _idf_diag is not set, this will raise an attribute error, which means hasattr(self, "idf_") is False
...
class TfidfVectorizer(CountVectorizer):
    ...
    For an example of usage, see :ref:`sphx_glr_auto_examples_text_plot_document_classification_20newsgroups.py`.
    ...
    max_df : float or int, default=1.0
        ...
        (corpus-specific stop words).  If float in range [0.0, 1.0], the parameter represents a proportion of documents, 
    ...
    binary : bool, default=False
        If True, all non-zero term counts are set to 1. This does not mean outputs will have only 0/1 values, 
        only that the tf term in tf-idf is binary. (Set `binary` to True, `use_idf` to False and `norm` to None to get 0/1 outputs).
    ...
    # Broadcast the TF-IDF parameters to the underlying transformer instance for easy grid search and repr
    ...
    def idf_(self, value):
        ...
        if not hasattr(self, "_tfidf"):
            # We should support transferring `idf_` from another `TfidfTransformer` and therefore, 
            # we need to create the transformer instance it does not exist yet.
    ...
    def _check_params(self):
        if self.dtype not in FLOAT_DTYPES:
            warnings.warn("Only {} 'dtype' should be used. {} 'dtype' will be converted to np.float64.".format(FLOAT_DTYPES, self.dtype),
                UserWarning)
