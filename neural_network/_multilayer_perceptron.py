...
class BaseMultilayerPerceptron(BaseEstimator, metaclass=ABCMeta):
    ...
    _parameter_constraints: dict = {
        ...
        "n_iter_no_change": [Interval(Integral, 1, None, closed="left"), Options(Real, {np.inf}) ],
    ...
    def __init__(self, hidden_layer_sizes, activation, solver, alpha, batch_size, learning_rate, learning_rate_init, \
        power_t, max_iter, loss, shuffle, random_state, tol, verbose, warm_start, momentum, nesterovs_momentum,      \
    ...
    def _forward_pass_fast(self, X, check_input=True):
        ...
        Returns
        -------
        y_pred : ndarray of shape (n_samples,) or (n_samples, n_outputs), The decision function of the samples for each class in the model.
        """
        if check_input:
    ...
    def _loss_grad_lbfgs(self, packed_coef_inter, X, y, activations, deltas, coef_grads, intercept_grads):
        ...
        Returned gradients are packed in a single vector so it can be used in lbfgs
        ...
        deltas : list, length = n_layers - 1
            The ith element of the list holds the difference between the activations of the i + 1 layer and the backpropagated error.
            More specifically, deltas are gradients of loss with respect to z in each layer, 
            where z = wx + b is the value of a particular layer before passing through the activation function
    ...
    def _backprop(self, X, y, activations, deltas, coef_grads, intercept_grads):
        ...
        # The calculation of delta[last] here works with following combinations of output activation and loss function:
        # sigmoid and binary cross entropy, softmax and categorical cross entropy, and identity with squared loss
    ...
    def _fit_lbfgs(self, X, y, activations, deltas, coef_grads, intercept_grads, layer_units):
        ...
        opt_res = scipy.optimize.minimize(self._loss_grad_lbfgs, packed_coef_inter, method="L-BFGS-B", jac=True, 
            options={ "maxfun": self.max_fun, "maxiter": self.max_iter, "iprint": iprint, "gtol": self.tol },
    ...
    def _fit_stochastic(self, X, y, activations, deltas, coef_grads, intercept_grads, layer_units, incremental):
        ...
        if not incremental or not hasattr(self, "_optimizer"):
            if self.solver == "sgd":
                self._optimizer = SGDOptimizer(params, self.learning_rate_init, self.learning_rate, self.momentum,\
        ...
        try:
            ...
            for it in range(self.max_iter):
                if self.shuffle:
                    # Only shuffle the sample indices instead of X and y to reduce the memory footprint. 
                    # These indices will be used to slice the X and y.
                ...
                if self._no_improvement_count > self.n_iter_no_change:
                    # not better than last `n_iter_no_change` iterations by tol stop or decrease learning rate
                    if early_stopping:
                ...
                if self.n_iter_ == self.max_iter:
                    warnings.warn("Stochastic Optimizer: Maximum iterations (%d) reached and the optimization hasn't converged yet."
    ...
    def fit(self, X, y):
        ...
        Returns
        -------
        self : object, Returns a trained MLP model.
        """
...
class MLPClassifier(ClassifierMixin, BaseMultilayerPerceptron):
    ...
    tol : float, default=1e-4
        Tolerance for the optimization. When the loss or score is not improving by at least ``tol`` for ``n_iter_no_change`` consecutive iterations,
    ...
    early_stopping : bool, default=False
        ...
        If early stopping is False, then the training stops when the training loss does not improve by more 
        than tol for n_iter_no_change consecutive passes over the training set.
    ...
    best_loss_ : float or None
        ...
        If `early_stopping=True`, this attribute is set to `None`. Refer to the `best_validation_score_` fitted attribute instead.
    ...
    Notes
    -----
    MLPClassifier trains iteratively since at each time step the partial derivatives of the loss function 
    with respect to the model parameters are computed to update the parameters.
    ...
    def __init__(self, hidden_layer_sizes=(100,), activation="relu", *, solver="adam", alpha=0.0001, batch_size="auto", \
        ...
        tol=1e-4, verbose=False, warm_start=False, momentum=0.9, nesterovs_momentum=True, early_stopping=False,         \ 
        ...
        super().__init__(hidden_layer_sizes=hidden_layer_sizes, activation=activation, solver=solver, alpha=alpha, \
            ...
            power_t=power_t, max_iter=max_iter, loss="log_loss", shuffle=shuffle, random_state=random_state,       \
            tol=tol, verbose=verbose, warm_start=warm_start, momentum=momentum, nesterovs_momentum=nesterovs_momentum, \
    ...
    def _validate_input(self, X, y, incremental, reset):
        ...
        # The case that incremental == True and classes_ not defined is already checked 
        # by _check_partial_fit_first_call that is called in _partial_fit below.
        ...
        #
        # Note the reliance on short-circuiting here, so that the second or part implies that classes_ is defined.
        if (not hasattr(self, "classes_")) or (not self.warm_start and not incremental):
            ...
            if self.warm_start:
                if set(classes) != set(self.classes_):
                    raise ValueError("warm_start can only be used where `y` has the same classes as in the previous call to fit. "
                        f"Previously got {self.classes_}, `y` has {classes}")
        ...
        # This downcast to bool is to prevent upcasting when working with float32 data
    ...
    def partial_fit(self, X, y, classes=None):
        ...
        classes : array of shape (n_classes,), default=None
            ...
            Can be obtained via `np.unique(y_all)`, where y_all is the target vector of the entire dataset.
    ...
    def predict_log_proba(self, X):
        ...
        log_y_prob : ndarray of shape (n_samples, n_classes)
            ...
            where classes are ordered as they are in `self.classes_`. Equivalent to `log(predict_proba(X))`.
        """
...
class MLPRegressor(RegressorMixin, BaseMultilayerPerceptron):
    ...
    batch_size : int, default='auto'
        Size of minibatches for stochastic optimizers.
        If the solver is 'lbfgs', the regressor will not use minibatch.  When set to "auto", `batch_size=min(200, n_samples)`.
    ...
    early_stopping : bool, default=False
        Whether to use early stopping to terminate training when validation score is not improving. 
        If set to True, it will automatically set aside ``validation_fraction`` of training data as validation 
        and terminate training when validation score is not improving by at least ``tol`` for ``n_iter_no_change`` consecutive epochs.
        Only effective when solver='sgd' or 'adam'.
    ...
    max_fun : int, default=15000
        ...
        Note that number of function calls will be greater than or equal to the number of iterations for the MLPRegressor.
        .. versionadded:: 0.22
    ...
    loss_curve_ : list of shape (`n_iter_`,)
        ...
        The ith element in the list represents the loss at the ith iteration.  Only accessible when solver='sgd' or 'adam'.

    validation_scores_ : list of shape (`n_iter_`,) or None
        The score at each iteration on a held-out validation set. 
        The score reported is the R2 score. Only available if `early_stopping=True`, otherwise the attribute is set to `None`.
        Only accessible when solver='sgd' or 'adam'.
    ...
    t_ : int
        The number of training samples seen by the solver during fitting.
        Mathematically equals `n_iters * X.shape[0]`, it means `time_step` and it is used by optimizer's learning rate scheduler.
    ...
    Notes
    -----
    MLPRegressor trains iteratively since at each time step the partial derivatives of the loss function with respect to 
    ...
    def __init__(self, hidden_layer_sizes=(100,), activation="relu", *, solver="adam", alpha=0.0001, batch_size="auto", \
        ...
        validation_fraction=0.1, beta_1=0.9, beta_2=0.999, epsilon=1e-8, n_iter_no_change=10, max_fun=15000):
        super().__init__(hidden_layer_sizes=hidden_layer_sizes, activation=activation, solver=solver, alpha=alpha,      \
            batch_size=batch_size, learning_rate=learning_rate, learning_rate_init=learning_rate_init, power_t=power_t, \
            max_iter=max_iter, loss="squared_error", shuffle=shuffle, random_state=random_state, tol=tol,               \
    ...
    def _validate_input(self, X, y, incremental, reset):
        X, y = self._validate_data(X, y, accept_sparse=["csr", "csc"], multi_output=True, y_numeric=True, \
            dtype=(np.float64, np.float32), reset=reset,)
        if y.ndim == 2 and y.shape[1] == 1:
            y = column_or_1d(y, warn=True)
        return X, y
