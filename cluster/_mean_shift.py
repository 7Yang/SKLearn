...
def estimate_bandwidth(X, *, quantile=0.3, n_samples=None, random_state=0, n_jobs=None):
    ...
    Alternatively, the parameter `bandwidth` can be set to a small value without estimating it.
...
def mean_shift(X, *, bandwidth=None, seeds=None, bin_seeding=False, min_bin_freq=1, cluster_all=True, max_iter=300, n_jobs=None):
    ...
    bandwidth : float, default=None
        ...
        If None, the bandwidth is determined using a heuristic based on the median of all pairwise distances. 
        This will take quadratic time in the number of samples. 
        The sklearn.cluster.estimate_bandwidth function can be used to do this more efficiently.

    seeds : array-like of shape (n_seeds, n_features) or None
        Point used as initial kernel locations. If None and bin_seeding=False, each data point is used as a seed. 
        If None and bin_seeding=True, see bin_seeding.
    ...
    """
    model = MeanShift(bandwidth=bandwidth, seeds=seeds, min_bin_freq=min_bin_freq, bin_seeding=bin_seeding, \
        cluster_all=cluster_all, n_jobs=n_jobs, max_iter=max_iter).fit(X)
...
def get_bin_seeds(X, bin_size, min_bin_freq=1):
    ...
    Finds seeds by first binning data onto a grid whose lines are spaced bin_size apart, 
    and then choosing those bins with at least min_bin_freq points.
    ...
    bin_size : float
        Controls the coarseness of the binning. Smaller values lead to more seeding (which is computationally more expensive). 
        If you're not sure how to set this, set it to the value of the bandwidth used in clustering.mean_shift.

    min_bin_freq : int, default=1
        Only bins with at least min_bin_freq will be selected as seeds.
        Raising this value decreases the number of seeds found, which makes mean_shift computationally cheaper.
...
class MeanShift(ClusterMixin, BaseEstimator):
    ...
    bandwidth : float, default=None
        ...
        see the documentation for that function for hints on scalability (see also the Notes, below).
    ...
    Because this implementation uses a flat kernel and a Ball Tree to look up members of each kernel, 
    the complexity will tend towards O(T*n*log(n)) in lower dimensions, with n the number of samples and T the number of points. 
    ...
    def fit(self, X, y=None):
        ...
        # We use n_jobs=1 because this will be used in nested calls under parallel calls to _mean_shift_single_seed 
        # so there is no need for for further parallelism.
        ...
        # POST PROCESSING: remove near duplicate points If the distance between two kernels is less than the bandwidth,
