...
def average_merge(IntFloatDict a, IntFloatDict b, const intp_t[:] mask, intp_t n_a, intp_t n_b):
    """
    Merge two IntFloatDicts with the average strategy: when the same key is present in the two dicts, 
    the weighted average of the two values is used.
    ...
    a, b : IntFloatDict object
        The IntFloatDicts to merge
    mask : ndarray array of dtype integer and of dimension 1
        a mask for keys to ignore: if not mask[key] the corresponding key is skipped in the output dictionary
    n_a, n_b : float
        n_a and n_b are weights for a and b for the merge strategy. They are used for a weighted mean.

    Returns
    =======
    out : IntFloatDict object, The IntFloatDict resulting from the merge
    """
    cdef IntFloatDict out_obj = IntFloatDict.__new__(IntFloatDict)
    cdef cpp_map[intp_t, float64_t].iterator a_it = a.my_map.begin()
    cdef cpp_map[intp_t, float64_t].iterator a_end = a.my_map.end()
    ...
    while b_it != b_end:
        ...
        if mask[key]:
            ...
            if out_it == out_end:
                ...
                deref(out_it).second = (n_a * deref(out_it).second + n_b * value) / n_out
        inc(b_it)
    return out_obj
...
def _single_linkage_label(const float64_t[:, :] L):
    ...
    See ``single_linkage_label`` for the user facing version of this function.

    Parameters
    ----------
    L: array of shape (n_samples - 1, 3)
        The linkage array or MST where each row specifies two samples to be merged and a distance or weight at which the merge occurs. 
        This array is assumed to be sorted by the distance/weight.
...
def single_linkage_label(L):
    """
    Convert an linkage array or MST to a tree by labelling clusters at merges.
    This is done by using a Union find structure to keep track of merges efficiently.
...
def mst_linkage_core(const float64_t [:, ::1] raw_data, DistanceMetric64 dist_metric):
    """
    Compute the necessary elements of a minimum spanning tree for computation of single linkage clustering. 
    This represents the MST-LINKAGE-CORE algorithm (Figure 6) from :arxiv:`Daniel Mullner, 
    "Modern hierarchical, agglomerative clustering algorithms" <1109.2378>`.

    In contrast to the scipy implementation is never computes a full distance matrix, 
    generating distances only as they are needed and releasing them when no longer needed.
    ...
    dist_metric: DistanceMetric64
        A DistanceMetric64 object conforming to the API from ``sklearn.metrics._dist_metrics.pxd`` 
        that will be used to compute distances.

    Returns
    -------
    mst_core_data: array of shape (n_samples, 3)
        An array providing information from which one can either compute an MST, 
        or the linkage hierarchy very efficiently. See :arxiv:`Daniel Mullner, "Modern hierarchical,
        agglomerative clustering algorithms" <1109.2378>` algorithm MST-LINKAGE-CORE for more details.
    """
    cdef:
    ...
    for i in range(n_samples - 1):
        ...
        for j in range(n_samples):
            ...
            left_value = dist_metric.dist(&raw_data[current_node, 0], &raw_data[j, 0], num_features)
