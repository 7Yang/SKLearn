...
These routines perform some hierarchical agglomerative clustering of some input data.
...
def _fix_connectivity(X, connectivity, affinity):
    ...
    connectivity : sparse matrix, default=None
        ...
        The matrix is assumed to be symmetric and only the upper triangular half is used.
    ...
    if n_connected_components > 1:
        warnings.warn("the number of connected components of the connectivity matrix is %d > 1. "
            "Completing it to avoid stopping the tree early." % n_connected_components, stacklevel=2)
        # XXX: Can we do without completing the matrix?
        connectivity = _fix_connected_components(X=X, graph=connectivity, n_connected_components=n_connected_components,\
            component_labels=labels, metric=affinity, mode="connectivity")
...
def _single_linkage_tree(connectivity, n_samples, n_nodes, n_clusters, n_connected_components, return_distance):
    """
    Perform single linkage clustering on sparse data via the minimum spanning tree from scipy.sparse.csgraph, 
    then using union-find to label.  The parent array is then generated by walking through the tree.
    """
...
@validate_params({
    ...
    "return_distance": ["boolean"]}, prefer_skip_nested_validation=True)
def ward_tree(X, *, connectivity=None, n_clusters=None, return_distance=False):
    ...
    n_clusters : int, default=None
        `n_clusters` should be less than `n_samples`.  Stop early the construction of the tree at `n_clusters.` 
        This is useful to decrease computation time if the number of clusters is not small compared to the number of samples. 
        In this case, the complete tree is not computed, thus the 'children' output is of limited use, 
        and the 'parents' output should rather be used. This option is valid only when specifying a connectivity matrix.
    ...
    distances : ndarray of shape (n_nodes-1,)
        ...
        The distances between the centers of the nodes. `distances[i]` corresponds to a weighted Euclidean distance 
        between the nodes `children[i, 1]` and `children[i, 2]`. 
        ...
        where :math:`u` is the newly joined cluster consisting of clusters :math:`s` and :math:`t`, :math:`v` 
        is an unused cluster in the forest, :math:`T=|v|+|s|+|t|`, and :math:`|*|` is the cardinality of its argument. 
    ...
    if connectivity is None:
        ...
        if n_clusters is not None:
            warnings.warn(("Partial build of the tree is implemented only for structured clustering (i.e. with explicit connectivity). "
                "The algorithm will build the full tree and only retain the lower branches required for the specified number of clusters"),
                stacklevel=2)
    ...
    if n_clusters is None:
        ...
        if n_clusters > n_samples:
            raise ValueError("Cannot provide more clusters than samples. %i n_clusters was asked, and there are %i samples." \
    ...
    for ind, row in enumerate(connectivity.rows):
        A.append(row)
        # We keep only the upper triangular for the moments Generator expressions are faster than arrays on the following
...
def linkage_tree(X, connectivity=None, n_clusters=None, linkage="complete", affinity="euclidean", return_distance=False):
    ...
    linkage : {"average", "complete", "single"}, default="complete"
        Which linkage criteria to use. The linkage criterion determines which distance to use between sets of observation.
            - "average" uses the average of the distances of each observation of the two sets.
            - "complete" or maximum linkage uses the maximum distances between all observations of the two sets.
            - "single" uses the minimum of the distances between all observations of the two sets.

    affinity : str or callable, default='euclidean'
        Which metric to use. Can be 'euclidean', 'manhattan', or any distance known to paired distance (see metric.pairwise).
    ...
    if connectivity is None:
        ...
        if n_clusters is not None:
            warnings.warn(("Partial build of the tree is implemented only for structured clustering "
                "(i.e. with explicit connectivity). The algorithm will build the full tree and only "
                "retain the lower branches required for the specified number of clusters"), stacklevel=2)

        if affinity == "precomputed":
            # for the linkage function of hierarchy to work on precomputed data, 
            # provide as first argument an ndarray of the shape returned by sklearn.metrics.pairwise_distances.
    ...
    # LIL seems to the best format to access the rows quickly, without the numpy overhead of slicing CSR indices and data.
    connectivity = connectivity.tolil()
    ...
    for k in range(n_samples, n_nodes):
        ...
        # update the structure matrix A and the inertia matrix a clever 'min', or 'max' operation between A[i] and A[j]
        coord_col = join_func(A[i], A[j], used_node, n_i, n_j)
        for col, d in coord_col:
            A[col].append(k, d)
            # Here we use the information from coord_col (containing the distances) to update the heap
...
def _hc_cut(n_clusters, children, n_leaves):
    ...
    if n_clusters > n_leaves:
        raise ValueError("Cannot extract more clusters than samples: %s clusters where given for a tree with %s leaves."
            % (n_clusters, n_leaves))
    # In this function, we store nodes as a heap to avoid recomputing the max of the nodes: 
    # the first element is always the smallest We use negated indices as heaps work on smallest elements, 
    # and we are interested in largest elements children[-1] is the root of the tree
...
class AgglomerativeClustering(ClusterMixin, BaseEstimator):
    ...
    affinity : str or callable, default='euclidean'
        ...
        .. deprecated:: 1.2
            `affinity` was deprecated in version 1.2 and will be renamed to `metric` in 1.4.
    ...
    metric : str or callable, default=None
        ...
        If set to `None` then "euclidean" is used. If linkage is "ward", only "euclidean" is accepted. 
    ...
    compute_full_tree : 'auto' or bool, default='auto'
        ...
        It must be ``True`` if ``distance_threshold`` is not ``None``. By default `compute_full_tree` is "auto",
        which is equivalent to `True` when `distance_threshold` is not `None` or that `n_clusters` is inferior to the maximum 
    ...
    See Also
    --------
    FeatureAgglomeration : Agglomerative clustering but for features instead of samples.
    ward_tree : Hierarchical clustering with ward linkage.
    ...
    _parameter_constraints: dict = {
        "n_clusters": [Interval(Integral, 1, None, closed="left"), None],
        "affinity": [Hidden(StrOptions({"deprecated"})), StrOptions(set(_VALID_METRICS) | {"precomputed"}), callable ],
    ...
    def __init__(self, n_clusters=2, *, affinity="deprecated", metric=None, memory=None, connectivity=None,
    ...
    def _fit(self, X):
        ...
        if self.affinity != "deprecated":
            if self.metric is not None:
                raise ValueError("Both `affinity` and `metric` attributes were set. "\
                    "Attribute `affinity` was deprecated in version 1.2 and will be removed in 1.4. "\
                    "To avoid this error, only set the `metric` attribute.")
        ...
        if compute_full_tree == "auto":
            if self.distance_threshold is not None:
                ...
                # Early stopping is likely to give a speed up only for a large number of clusters. 
                # The actual threshold implemented here is heuristic
...
class FeatureAgglomeration(ClassNamePrefixFeaturesOutMixin, AgglomerativeClustering, AgglomerationTransform):
    ...
    compute_full_tree : 'auto' or bool, default='auto'
        Stop early the construction of the tree at `n_clusters`. 
        This is useful to decrease computation time if the number of clusters is not small compared to the number of features. 
        This option is useful only when specifying a connectivity matrix. 
        Note also that when varying the number of clusters and using caching, it may be advantageous to compute the full tree. 
        It must be ``True`` if ``distance_threshold`` is not ``None``. 
        By default `compute_full_tree` is "auto", which is equivalent to `True` when `distance_threshold` is not `None` 
    ...
    pooling_func : callable, default=np.mean
        ...
        and should accept an array of shape [M, N] and the keyword argument `axis=1`, and reduce it to an array of size [M].
    ...
    children_ : array-like of shape (n_nodes-1, 2)
        The children of each non-leaf node. Values less than `n_features` correspond to leaves of the tree which are the original samples.
        A node `i` greater than or equal to `n_features` is a non-leaf node and has children `children_[i - n_features]`. 
    ...
    See Also
    --------
    AgglomerativeClustering : Agglomerative clustering samples instead of features.
    ward_tree : Hierarchical clustering with ward linkage.
    ...
    def __init__(self, n_clusters=2, *, affinity="deprecated", metric=None, memory=None, connectivity=None, \
        compute_full_tree="auto", linkage="ward", pooling_func=np.mean, distance_threshold=None, compute_distances=False):
        super().__init__(n_clusters=n_clusters, memory=memory, connectivity=connectivity, compute_full_tree=compute_full_tree,\
                linkage=linkage, affinity=affinity, metric=metric, distance_threshold=distance_threshold, compute_distances=compute_distances)
