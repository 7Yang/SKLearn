...
class _BisectingTree:
    ...
    def __init__(self, center, indices, score):
        ...
        The node holds the center of this cluster and the indices of the data points that belong to it.
        """
        self.center  = center
    ...
    def get_cluster_to_bisect(self):
        ...
        It's based on the score of the cluster, which can be either the number of data points assigned to that cluster 
        or the inertia of that cluster (see `bisecting_strategy` for details).
        """
        max_score = None
...
class BisectingKMeans(_BaseKMeans):
    ...
    n_init : int, default=1
        Number of time the inner k-means algorithm will be run with different centroid seeds in each bisection.
        That will result producing for each bisection best output of n_init consecutive runs in terms of inertia.

    random_state : int, RandomState instance or None, default=None
        Determines random number generation for centroid initialization in inner K-Means. 
    ...
    tol : float, default=1e-4
        Relative tolerance with regards to Frobenius norm of the difference in the cluster centers of 
        two consecutive iterations  to declare convergence. 
        Used in inner k-means algorithm at each bisection to pick best possible clusters.
    ...
    algorithm : {"lloyd", "elkan"}, default="lloyd"
        Inner K-means algorithm used in bisection.  The classical EM-style algorithm is `"lloyd"`.
    ...
    bisecting_strategy : {"biggest_inertia", "largest_cluster"}, default="biggest_inertia"
        Defines how bisection should be performed:
         ...
         - "biggest_inertia" means that BisectingKMeans will always check all calculated cluster for cluster with biggest SSE
            (Sum of squared errors) and bisect it. This approach concentrates on precision, 
            but may be costly in terms of execution time (especially for larger amount of data points).

         - "largest_cluster" - BisectingKMeans will always split cluster with largest amount of points assigned to it 
            from all clusters previously calculated. That should work faster than picking by SSE
    ...
    def __init__(self, n_clusters=8, *, init="random", n_init=1, random_state=None, max_iter=300, verbose=0,
        tol=1e-4, copy_x=True, algorithm="lloyd", bisecting_strategy="biggest_inertia"):
    ...
    def _warn_mkl_vcomp(self, n_active_threads):
        ...
        warnings.warn("BisectingKMeans is known to have a memory leak on Windows with MKL, when there are less chunks than available "
            "threads. You can avoid it by setting the environment variable OMP_NUM_THREADS={n_active_threads}.")
    ...
    def _bisect(self, X, x_squared_norms, sample_weight, cluster_to_bisect):
        ...
        # Split samples in X into 2 clusters. Repeating `n_init` times to obtain best clusters
        for _ in range(self.n_init):
            centers_init = self._init_centroids(X, x_squared_norms=x_squared_norms, init=self.init,
                random_state=self._random_state, n_centroids=2, sample_weight=sample_weight)

            labels, inertia, centers, _ = self._kmeans_single(X, sample_weight, centers_init, max_iter=self.max_iter,
                verbose=self.verbose, tol=self.tol, n_threads=self._n_threads)

            # allow small tolerance on the inertia to accommodate for non-deterministic rounding errors due to parallel computation
        ...
        if self.bisecting_strategy == "biggest_inertia":
            ...
            # Using minlength to make sure that we have the counts for both labels even if all samples are labelled 0.
    ...
    def fit(self, X, y=None, sample_weight=None):
        ...
        sample_weight : array-like of shape (n_samples,), default=None
            ...
            `sample_weight` is not used during initialization if `init` is a callable.
    ...
    def _predict_recursive(self, X, sample_weight, cluster_node):
        ...
        ----------
        X : {ndarray, csr_matrix} of shape (n_samples, n_features)
            The data points, currently assigned to `cluster_node`, to predict between the subclusters of this node.
