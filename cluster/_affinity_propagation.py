    ...
    for it in range(max_iter):
        ...
        if it >= convergence_iter:
            ...
            if (not unconverged and (K > 0)) or (it == max_iter):
                ...
                if verbose: print("Converged after %d iterations." % it)
                break
    else:
    ...
    if K > 0:
        if never_converged:
            warnings.warn(("Affinity propagation did not converge, " \
                "this model may return degenerate cluster centers and labels."), ConvergenceWarning)
...
def affinity_propagation(S, *, preference=None, convergence_iter=15, max_iter=200, damping=0.5, copy=True, \
        verbose=False, return_n_iter=False, random_state=None):
    """
    ...
    preference : array-like of shape (n_samples,) or float, default=None
        ...
        The number of exemplars, i.e. of clusters, is influenced by the input preferences value. 
        If the preferences are not passed as arguments, they will be set to the median of the input similarities 
        (resulting in a moderate number of clusters). For a smaller amount of clusters, this can be set to the minimum value of the similarities.
    ...
    and label ``0`` for every sample will be returned. 
    Otherwise, every training sample becomes its own cluster center and is assigned a unique label.

    References
    ----------
    Brendan J. Frey and Delbert Dueck, "Clustering by Passing Messages Between Data Points", Science Feb. 2007
    """
    estimator = AffinityPropagation(damping=damping, max_iter=max_iter, convergence_iter=convergence_iter, copy=copy,
        preference=preference, affinity="precomputed", verbose=verbose, random_state=random_state).fit(S)
...
class AffinityPropagation(ClusterMixin, BaseEstimator):
    ...
    ----------
    damping : float, default=0.5
        Damping factor in the range `[0.5, 1.0)` is the extent to which the current value is maintained relative to
        incoming values (weighted 1 - damping). This in order to avoid numerical oscillations when updating these values (messages).
    ...
    FeatureAgglomeration    : Similar to AgglomerativeClustering, but recursively merges features instead of samples.
    ...
    When the algorithm does not converge, it will still return a arrays of ``cluster_center_indices`` 
    and labels if there are any exemplars/clusters, however they may be degenerate and should be used with caution.

    When ``fit`` does not converge, ``cluster_centers_`` is still populated however it may be degenerate. 
    ...
    When all training samples have equal similarities and equal preferences, the assignment of cluster centers 
    and labels depends on the preference.  If the preference is smaller than the similarities, 
    ``fit`` will result in a single cluster center and label ``0`` for every sample. Otherwise, 
    every training sample becomes its own cluster center and is assigned a unique label.
    ...
    def __init__(self, *, damping=0.5, max_iter=200, convergence_iter=15, copy=True, preference=None, 
    ...
    def fit(self, X, y=None):
        ...
        if self.affinity_matrix_.shape[0] != self.affinity_matrix_.shape[1]:
            raise ValueError("The matrix of similarities must be a square array. Got {self.affinity_matrix_.shape} instead.")
        ...
        (self.cluster_centers_indices_, self.labels_, self.n_iter_) = _affinity_propagation(
            self.affinity_matrix_, max_iter=self.max_iter, convergence_iter=self.convergence_iter,
            preference=preference, damping=self.damping, verbose=self.verbose, return_n_iter=True, 
            random_state=random_state)
    ...
    def predict(self, X):
        ...
        if self.cluster_centers_.shape[0] > 0:
            ...
            warnings.warn(("This model does not have any cluster centers because affinity propagation did not converge. "
