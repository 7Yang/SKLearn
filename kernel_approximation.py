"""
The :mod:`sklearn.kernel_approximation` module implements several(几个)
...
class PolynomialCountSketch(ClassNamePrefixFeaturesOutMixin, TransformerMixin, BaseEstimator):
    ...
    Read more in the :ref:`User Guide <polynomial_kernel_approx>`.
    ...
    n_components : int, default=100
        ...
        Usually, `n_components` should be greater than the number of features in input samples in order to achieve good performance. 
        The optimal score / run time balance is typically achieved around `n_components` = 10 * `n_features`,
        but this depends on the specific dataset being used.

    random_state : int, RandomState instance, default=None
        Determines random number generation for indexHash and bitHash initialization. 
    ...
    def fit(self, X, y=None):
        ...
        Initializes the internal variables. The method needs no information about the distribution of data, 
        so we only care about n_features in X.
    ...
    def transform(self, X):
        ...
        Returns
        -------
        X_new : array-like, shape (n_samples, n_components), Returns the instance itself.
        """
        ...
        # For each same, compute a count sketch of phi(x) using the polynomial multiplication (via FFT) of p count sketches of x.
...
class RBFSampler(ClassNamePrefixFeaturesOutMixin, TransformerMixin, BaseEstimator):
    ...
    random_state : int, RandomState instance or None, default=None
        Pseudo-random number generator to control the generation of the random weights and random offset when fitting the training data.
    ...
    by A. Rahimi and Benjamin Recht.(https://people.eecs.berkeley.edu/~brecht/papers/08.rah.rec.nips.pdf)
    ...
    def fit(self, X, y=None):
        ...
        if X.dtype == np.float32:
            # Setting the data type of the fitted attribute will ensure the output data type during `transform`.
...
class SkewedChi2Sampler(ClassNamePrefixFeaturesOutMixin, TransformerMixin, BaseEstimator):
    """
    Approximate feature map for "skewed chi-squared" kernel.        Skewed:斜的、歪的
    ...
    def transform(self, X):
        ...
        X : array-like, shape (n_samples, n_features)
            New data, where `n_samples` is the number of samples and `n_features` is the number of features. 
            All values of X must be strictly greater than "-skewedness".
...
class AdditiveChi2Sampler(TransformerMixin, BaseEstimator):
    ...
    Since the kernel that is to be approximated is additive, the components of the input vectors can be treated separately.  
    Each entry in the original space is transformed into 2*sample_steps-1 features, where sample_steps is a parameter of the method. 
    Typical values of sample_steps include 1, 2 and 3.

    Optimal choices for the sampling interval for certain data ranges can be computed (see the reference). 
    ...
    References
    ----------
    See `"Efficient additive kernels via explicit feature maps" <http://www.robots.ox.ac.uk/~vedaldi/assets/pubs/vedaldi11efficient.pdf>`_
    ...
    def fit(self, X, y=None):
        ...
        if self.sample_interval is None:
            ...
            # A. Vedaldi and A. Zisserman, Pattern Analysis and Machine Intelligence, 2011
            if   self.sample_steps == 1: self._sample_interval = 0.8
            elif self.sample_steps == 2: self._sample_interval = 0.5
            elif self.sample_steps == 3: self._sample_interval = 0.4
            else: raise ValueError("If sample_steps is not in [1, 2, 3], you need to provide sample_interval")
        else:
    ...
    def transform(self, X):
        ...
        if hasattr(self, "_sample_interval"):
            ...
            if self.sample_interval is None:
                ...
                # <http://www.robots.ox.ac.uk/~vedaldi/assets/pubs/vedaldi11efficient.pdf>
                # A. Vedaldi and A. Zisserman, Pattern Analysis and Machine Intelligence, # noqa 2011
                if   self.sample_steps == 1: sample_interval = 0.8
                elif self.sample_steps == 2: sample_interval = 0.5
                elif self.sample_steps == 3: sample_interval = 0.4
                else: raise ValueError("If sample_steps is not in [1, 2, 3], you need to provide sample_interval")
            else:
...
class Nystroem(ClassNamePrefixFeaturesOutMixin, TransformerMixin, BaseEstimator):
    ...
    random_state : int, RandomState instance or None, default=None
        Pseudo-random number generator to control the uniform sampling without replacement of `n_components` of the training data 
        to construct the basis kernel.
        Pass an int for reproducible output across multiple function calls.  See :term:`Glossary <random_state>`.

    n_jobs : int, default=None
        The number of jobs to use for the computation. 
        This works by breaking down the kernel matrix into `n_jobs` even slices and computing them in parallel.
    ...
    * Williams, C.K.I. and Seeger, M.
      "Using the Nystroem method to speed up kernel machines", Advances in neural information processing systems 2001

    * T. Yang, Y. Li, M. Mahdavi, R. Jin and Z. Zhou
      "Nystroem Method vs Random Fourier Features: A Theoretical and Empirical Comparison",
    ...
    _parameter_constraints: dict = {
        "kernel": [StrOptions(set(PAIRWISE_KERNEL_FUNCTIONS.keys()) | {"precomputed"}), callable],
    ...
    def fit(self, X, y=None):
        ...
        if self.n_components > n_samples:
            ...
            warnings.warn("n_components > n_samples. This is not possible. n_components was set to n_samples,"\
                " which results in inefficient evaluation of the full kernel.")
        else:
    ...
    def transform(self, X):
        ...
        Returns
        -------
        X_transformed : ndarray of shape (n_samples, n_components), Transformed data.
        """
        check_is_fitted(self)
        X = self._validate_data(X, accept_sparse="csr", reset=False)
    ...
    def _more_tags(self):
        return {
            "_xfail_checks": { "check_transformer_preserve_dtypes": ("dtypes are preserved but not at a close enough precision") },
