...
class TransformedTargetRegressor(RegressorMixin, BaseEstimator):
    ...
    This transformation can be given as a Transformer such as the :class:`~sklearn.preprocessing.QuantileTransformer` 
    or as a function and its inverse such as `np.log` and `np.exp`.
    ...
    regressor : object, default=None
        ...
        :class:`~sklearn.base.RegressorMixin`. This regressor will automatically be cloned each time prior to fitting. 
        If `regressor is None`, :class:`~sklearn.linear_model.LinearRegression` is created and used.

    transformer : object, default=None
        Estimator object such as derived from :class:`~sklearn.base.TransformerMixin`. 
        Cannot be set at the same time as `func` and `inverse_func`. 
        If `transformer is None` as well as `func` and `inverse_func`, the transformer will be an identity transformer. 
        Note that the transformer will be cloned during fitting.  Also, the transformer is restricting `y` to be a numpy array.

    func : function, default=None
        Function to apply to `y` before passing to :meth:`fit`. Cannot be set at the same time as `transformer`. 
        The function needs to return a 2-dimensional array. If `func is None`, the function used will be the identity function.
    ...
    n_features_in_ : int
        Number of features seen during :term:`fit`. Only defined if the underlying regressor exposes such an attribute when fit.
        .. versionadded:: 0.24
    ...
    See Also
    --------
    sklearn.preprocessing.FunctionTransformer : Construct a transformer from an arbitrary callable.
    ...
    For a more detailed example use case refer to :ref:`sphx_glr_auto_examples_compose_plot_transformed_target.py`.
    """
    ...
    def _fit_transformer(self, y):
        ...
        Create the default transformer, fit it and make additional inverse check on a subset (optional).
        """
        ...
        # XXX: sample_weight is not currently passed to the transformer. 
        # However, if transformer starts using sample_weight, the code should be modified accordingly. 
        # At the time to consider the sample_prop feature, it is also a good use case to be considered.
        ...
        if self.check_inverse:
            ...
            if not np.allclose(y_sel, self.transformer_.inverse_transform(y_sel_t)):
                warnings.warn(("The provided functions or transformer are not strictly inverse of each other. "\
                        "If you are sure you want to proceed regardless, set 'check_inverse=False'"), UserWarning)
    ...
    def fit(self, X, y, **fit_params):
        ...
        **fit_params : dict
        ...
        # store the number of dimension of the target to predict an array of similar shape at predict
        self._training_dim = y.ndim

        # transformers are designed to modify X which is 2d dimensional, we need to modify y accordingly.
        if y.ndim == 1:
            y_2d = y.reshape(-1, 1)
        else:
            y_2d = y
        ...
        # FIXME: a FunctionTransformer can return a 1D array even when validate is set to True. 
        # Therefore, we need to check the number of dimension first.
    ...
    def n_features_in_(self):
        ...
        # For consistency with other estimators we raise a AttributeError so that hasattr() returns False the estimator isn't fitted.
        try:
            check_is_fitted(self)
