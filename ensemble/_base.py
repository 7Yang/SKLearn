...
def _set_random_states(estimator, random_state=None):
    ...
    Finds all parameters ending ``random_state`` and sets them to integers derived from ``random_state``.
    ...
    random_state : int, RandomState instance or None, default=None
        Pseudo-random number generator to control the generation of the random integers. 
        Pass an int for reproducible output across multiple function calls.  See :term:`Glossary <random_state>`.

    Notes
    -----
    This does not necessarily set *all* ``random_state`` attributes that control an estimator's randomness, 
    only those accessible through ``estimator.get_params()``.  ``random_state``s not controlled include those belonging to:
...
class BaseEnsemble(MetaEstimatorMixin, BaseEstimator, metaclass=ABCMeta):
    ...
    @abstractmethod
    def __init__(self, estimator=None, *, n_estimators=10, estimator_params=tuple(), base_estimator="deprecated"):
        # Set parameters
        ...
        # Don't instantiate estimators now! Parameters of base_estimator might still change. 
        # Eg., when grid-searching with the nested object syntax. self.estimators_ needs to be filled by the derived classes in fit.
    ...
    def _make_estimator(self, append=True, random_state=None):
        ...
        Warning: This method should be used to properly instantiate new sub-estimators.
        """
...
class _BaseHeterogeneousEnsemble(MetaEstimatorMixin, _BaseComposition, metaclass=ABCMeta):
    ...
    estimators : list of (str, estimator) tuples
        The ensemble of estimators to use in the ensemble. Each element of the list is defined as a tuple of string (i.e. name of the estimator) 
        and an estimator instance. An estimator can be set to `'drop'` using `set_params`.

    Attributes
    ----------
    estimators_ : list of estimators
        The elements of the estimators parameter, having been fitted on the training data. 
        If an estimator has been set to `'drop'`, it will not appear in `estimators_`.
    """
    ...
    def set_params(self, **params):
        ...
        **params : keyword arguments
            ...
            In addition, to setting the parameters of the estimator, the individual estimator of the estimators can also be set, 
            or can be removed by setting them to 'drop'.
