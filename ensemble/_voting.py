...
class _BaseVoting(TransformerMixin, _BaseHeterogeneousEnsemble):
    ...
    def fit(self, X, y, sample_weight=None):
        ...
        if self.weights is not None and len(self.weights) != len(self.estimators):
            raise ValueError("Number of `estimators` and weights must be equal;" \
                " got {len(self.weights)} weights, {len(self.estimators)} estimators")

        self.estimators_ = Parallel(n_jobs=self.n_jobs)(delayed(_fit_single_estimator)(
            clone(clf), X, y, sample_weight=sample_weight, message_clsname="Voting", message=self._log_message(names[idx], idx + 1, len(clfs)))
    ...
    def n_features_in_(self):
        ...
        # For consistency with other estimators we raise a AttributeError so that hasattr() fails if the estimator isn't fitted.
        try:
...
class VotingClassifier(ClassifierMixin, _BaseVoting):
    ...
    estimators : list of (str, estimator) tuples
        ...
        .. versionchanged:: 0.21
            ``'drop'`` is accepted. Using None was deprecated in 0.22 and support was removed in 0.24.
    ...
    n_features_in_ : int
        Number of features seen during :term:`fit`. Only defined if the underlying classifier exposes such an attribute when fit.
        .. versionadded:: 0.24
    ...
    def fit(self, X, y, sample_weight=None):
        ...
        sample_weight : array-like of shape (n_samples,), default=None
            ...
            Note that this is supported only if all underlying estimators support sample weights.
    ...
    def predict_proba(self, X):
        ...
        Returns
        -------
        avg : array-like of shape (n_samples, n_classes), Weighted average probability for each class per sample.
        """
        check_is_fitted(self)
...
class VotingRegressor(RegressorMixin, _BaseVoting):
    ...
    ----------
    estimators : list of (str, estimator) tuples
        Invoking the ``fit`` method on the ``VotingRegressor`` will fit clones of those original estimators 
        that will be stored in the class attribute ``self.estimators_``. An estimator can be set to ``'drop'`` using :meth:`set_params`.
    ...
    In the following example, we drop the `'lr'` estimator with :meth:`~VotingRegressor.set_params` and fit the remaining two estimators:
