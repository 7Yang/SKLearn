...
class IsolationForest(OutlierMixin, BaseBagging):
    ...
    n_jobs : int, default=None
        The number of jobs to run in parallel for both :meth:`fit` and :meth:`predict`. ``None`` means 1 
    ...
    warm_start : bool, default=False
        ...
        otherwise, just fit a whole new forest. See :term:`the Glossary <warm_start>`.
    ...
    offset_ : float
        ...
        When the contamination parameter is set to "auto", the offset is equal to -0.5 as the scores of inliers are close to 0 
        and the scores of outliers are close to -1. When a contamination parameter different than "auto" is provided, 
    ...
    The maximum depth of each tree is set to ``ceil(log_2(n))`` where :math:`n` is the number of samples used to 
    ...
    _parameter_constraints: dict = {
        ...
        "max_samples": [StrOptions({"auto"}), Interval(Integral, 1, None, closed="left"), Interval(RealNotInt, 0, 1, closed="right")],
    ...
    def __init__(self, *, n_estimators=100, max_samples="auto", contamination="auto", max_features=1.0, bootstrap=False, \
        ...
        super().__init__(estimator=ExtraTreeRegressor(max_features=1, splitter="random", random_state=random_state),
            # here above max_features has no links with self.max_features
            bootstrap=bootstrap, bootstrap_features=False, n_estimators=n_estimators, max_samples=max_samples,
            max_features=max_features, warm_start=warm_start, n_jobs=n_jobs, random_state=random_state, verbose=verbose)
    ...
    def _parallel_args(self):
        # ExtraTreeRegressor releases the GIL, so it's more efficient to use a thread-based backend rather 
        # than a process-based backend so as to avoid suffering from communication overhead and extra memory copies.
    ...
    def fit(self, X, y=None, sample_weight=None):
        ...
        if isinstance(self.max_samples, str) and self.max_samples == "auto":
            ...
            if self.max_samples > n_samples:
                warn("max_samples (%s) is greater than the total number of samples (%s). " \
                     "max_samples will be set to n_samples for estimation." % (self.max_samples, n_samples))
        ...
        self._average_path_length_per_tree, self._decision_path_lengths = zip(*[
            (_average_path_length(tree.tree_.n_node_samples), tree.tree_.compute_node_depths()) for tree in self.estimators_])

        if self.contamination == "auto":
            # 0.5 plays a special role as described in the original paper. we take the opposite as we consider the opposite of their score.
            self.offset_ = -0.5
            return self

        # Else, define offset_ wrt contamination parameter
        # To avoid performing input validation a second time we call _score_samples rather than score_samples
    ...
    def _compute_chunked_score_samples(self, X):
        ...
        # (defined by sklearn.get_config()['working_memory']) to store self._max_features in each row during computation.
        #
        # Note:
        #  - this will get at least 1 row, even if 1 row of score will exceed working_memory.
        #  - this does only account for temporary memory usage while loading the data needed to compute the scores 
        # -- the returned scores themselves are 1D.
    ...
    def _compute_score_samples(self, X, subsample_features):
        ...
        # For a single training sample, denominator and depth are 0. Therefore, we set the score manually to 1.
...
def _average_path_length(n_samples_leaf):
    ...
    which is equal to the average path length of an unsuccessful BST search since the latter has the same structure as an isolation tree.
    Parameters
    ----------
    n_samples_leaf : array-like of shape (n_samples,)
        The number of training samples in each test sample leaf, for each estimators.
    ...
    average_path_length[not_mask] = (2.0 * (np.log(n_samples_leaf[not_mask] - 1.0) + np.euler_gamma)
