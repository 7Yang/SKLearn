...
- The ``BaseGradientBoosting`` base class implements a common ``fit`` method
  for all the estimators in the module. Regression and classification only differ in the concrete ``LossFunction`` used.
- ``GradientBoostingClassifier`` implements gradient boosting for classification problems.
- ``GradientBoostingRegressor`` implements gradient boosting for regression problems.
"""
...
def _safe_divide(numerator, denominator):
    ...
    try:
        # Cast to Python float to trigger a ZeroDivisionError without relying on `np.errstate` that is not supported by Pyodide.
...
def _init_raw_predictions(X, estimator, loss, use_predict_proba):
    ...
    raw_predictions : ndarray of shape (n_samples, K)
        The initial raw predictions. K is equal to 1 for binary classification and regression, 
        and equal to the number of classes for multiclass classification. ``raw_predictions`` is casted into float64.
    """
    # TODO: Use loss.fit_intercept_only where appropriate instead of DummyRegressor which is the default given by the `init` parameter, 
    # see also _init_state.
    if use_predict_proba:
        # Our parameter validation, set via _fit_context and _parameter_constraints already guarantees that estimator has a predict_proba method.
...
def _update_terminal_regions(loss, tree, X, y, neg_gradient, raw_prediction, sample_weight, sample_mask, learning_rate=0.1, k=0):
    ...
    This corresponds to the line search step in "Greedy Function Approximation" by Friedman, Algorithm 1 step 5.

    Update equals:
        argmin_{x} loss(y_true, raw_prediction_old + x * tree.value)

    For non-trivial cases like the Binomial loss, the update has no closed formula and is an approximation, again, see the Friedman paper.
    Also note that the update formula for the SquaredError is the identity. 
    Therefore, in this case, the leaf values don't need an update and only the raw_predictions are updated (with the learning rate included).

    Parameters
    ----------
    loss : BaseLoss
    tree : tree.Tree, The tree object.
    X    : ndarray of shape (n_samples, n_features), The data array.
    y    : ndarray of shape (n_samples),             The target labels.
    neg_gradient   : ndarray of shape (n_samples,), The negative gradient.
    raw_prediction : ndarray of shape (n_samples, n_trees_per_iteration) The raw predictions (i.e. values from the tree leaves) of the tree ensemble at iteration ``i - 1``.
    sample_weight  : ndarray of shape (n_samples,), The weight of each sample.
    sample_mask    : ndarray of shape (n_samples,), The sample mask to be used.
    learning_rate  : float, default=0.1, Learning rate shrinks the contribution of each tree by ``learning_rate``.
    k              : int,   default=0, The index of the estimator being updated.
    """
...
class VerboseReporter:
    ...
    verbose : int
        Verbosity level. 
        If ``verbose==1`` output is printed once in a while (when iteration mod verbose_mod is zero).; 
        if larger than 1 then output is printed for each update.
    """
    ...
    def update(self, j, est):
        ...
        Parameters
        ----------
        j   : int, The new iteration.
        est : Estimator, The estimator.
        """
...
class BaseGradientBoosting(BaseEnsemble, metaclass=ABCMeta):
    ...
    @abstractmethod
    def __init__(self, *, loss, learning_rate, n_estimators, criterion, min_samples_split, min_samples_leaf,  \
        min_weight_fraction_leaf, max_depth, min_impurity_decrease, init, subsample, max_features, ccp_alpha, \
        random_state, alpha=0.9, verbose=0, max_leaf_nodes=None, warm_start=False, validation_fraction=0.1,   \
    ...
    def _fit_stage(self, i, X, y, raw_predictions, sample_weight, sample_mask, random_state, X_csc=None, X_csr=None):
        ...
        neg_gradient = -self._loss.gradient(y_true=y, raw_prediction=raw_predictions, sample_weight=None)# We pass sample_weights to the tree directly.
        ...
        for k in range(self.n_trees_per_iteration_):
            ...
            _update_terminal_regions(self._loss, tree.tree_, X_for_tree_update, y, neg_g_view[:, k], raw_predictions, \
    ...
    def _clear_state(self):
        ...
        if hasattr(self, "estimators_"     ): self.estimators_ = np.empty((0, 0), dtype=object)
        if hasattr(self, "train_score_"    ): del self.train_score_
        if hasattr(self, "oob_improvement_"): del self.oob_improvement_
        if hasattr(self, "oob_scores_"     ): del self.oob_scores_
        if hasattr(self, "oob_score_"      ): del self.oob_score_
        if hasattr(self, "init_"           ): del self.init_
        if hasattr(self, "_rng"            ): del self._rng
    ...
    def fit(self, X, y, sample_weight=None, monitor=None):
        ...
        sample_weight : array-like of shape (n_samples,), default=None
            ...
            In the case of classification, splits are also ignored if they would result in any 
            single class carrying a negative weight in either child node.

        monitor : callable, default=None
            The monitor is called after each iteration with the current iteration, 
            a reference to the estimator and the local variables of ``_fit_stages`` as keyword arguments ``callable(i, self, locals())``. 
            If the callable returns ``True`` the fitting procedure is stopped. 
            The monitor can be used for various things such as computing held-out estimates, early stopping, model introspect, and snapshoting.
        ...
        # Check input
        # Since check_array converts both X and y to the same dtype, but the trees use different types for X and y, checking them separately.
        ...
        if self.n_iter_no_change is not None:
            ...
            (X_train, X_val, y_train, y_val, sample_weight_train, sample_weight_val) = train_test_split( \
                X, y, sample_weight, random_state=self.random_state, test_size=self.validation_fraction, stratify=stratify)
            if is_classifier(self):
                if self.n_classes_ != np.unique(y_train).shape[0]:
                    # We choose to error here. The problem is that the init estimator would be trained on y, 
                    # which has some missing classes now, so its predictions would not have the correct shape.
                    raise ValueError("The training data after the early stopping split is missing some classes. " \
                        "Try using another random seed.")
        else:
            ...
            if self.init_ == "zero":
                ...
                if sample_weight_is_none:
                    ...
                    try:
                        ...
                        if ("pass parameters to specific steps of your pipeline using the stepname__parameter" in str(e)):  # pipeline
                            raise ValueError(msg) from e
                        else:  # regular estimator whose input checking failed
                            raise
        ...
        # warm start: this is not the first time fit was called
            ...
            # add more estimators to fitted model invariant: warm_start = True
            if self.n_estimators < self.estimators_.shape[0]:
                raise ValueError("n_estimators=%d must be larger or equal to estimators_.shape[0]=%d when warm_start==True" 
                    % (self.n_estimators, self.estimators_.shape[0]))
            begin_at_stage = self.estimators_.shape[0]
            # The requirements of _raw_predict are more constrained than fit. 
            # It accepts only CSR matrices. Finite values have already been checked in _validate_data.
        ...
        n_stages = self._fit_stages(X_train, y_train, raw_predictions, sample_weight_train, self._rng, X_val, \
            y_val, sample_weight_val, begin_at_stage, monitor)
    ...
    def _fit_stages(self, X, y, raw_predictions, sample_weight, random_state, X_val, y_val, sample_weight_val, begin_at_stage=0, monitor=None):
        ...
        # Older versions of GBT had its own loss functions. With the new common private loss function submodule _loss, 
        # we often are a factor of 2 away from the old version. Here we keep backward compatibility for
        # oob_scores_ and oob_improvement_, even if the old way is quite inconsistent (sometimes the gradient is half the gradient, sometimes not).
        ...
        for i in range(begin_at_stage, self.n_estimators):
            ...
            if do_oob:
                ...
                if i == 0:  # store the initial loss to compute the OOB score
                    initial_loss = factor * self._loss(y_true=y_oob_masked, raw_prediction=raw_predictions[~sample_mask],\
            ...
            # track loss
            if do_oob:
                self.train_score_[i] = factor * self._loss(y_true=y[sample_mask], raw_prediction=raw_predictions[sample_mask], \
                    sample_weight=sample_weight[sample_mask])
                self.oob_scores_[i] = factor * self._loss(y_true=y_oob_masked, raw_prediction=raw_predictions[~sample_mask],   \
                    sample_weight=sample_weight_oob_masked)
            ...
            # We also provide an early stopping based on the score from validation set (X_val, y_val), if n_iter_no_change is set
            if self.n_iter_no_change is not None:
                # By calling next(y_val_pred_iter), we get the predictions for X_val after the addition of the current stage
    ...
    def _staged_raw_predict(self, X, check_input=True):
        ...
        raw_predictions : generator of ndarray of shape (n_samples, k)
            The raw predictions of the input samples. The order of the classes corresponds to that in the attribute :term:`classes_`.
            Regression and binary classification are special cases with ``k == 1``, otherwise ``k==n_classes``.
        """
        if check_input:
    ...
    def _compute_partial_dependence_recursion(self, grid, target_features):
        ...
        if self.init is not None:
            warnings.warn("Using recursion method with a non-constant init predictor " 
                "will lead to incorrect partial dependence values. Got init=%s." % self.init, UserWarning)
    ...
    def apply(self, X):
        ...
        X : {array-like, sparse matrix} of shape (n_samples, n_features)
            ...
            If a sparse matrix is provided, it will be converted to a sparse ``csr_matrix``.
...
class GradientBoostingClassifier(ClassifierMixin, BaseGradientBoosting):
    ...
    e.g. binary or multiclass log loss. Binary classification is a special case where only a single regression tree is induced.
    ...
    criterion : {'friedman_mse', 'squared_error'}, default='friedman_mse'
        The function to measure the quality of a split. Supported criteria are 'friedman_mse' for the mean squared error 
        with improvement score by Friedman, 'squared_error' for mean squared error. 
    ...
    min_weight_fraction_leaf : float, default=0.0
        ...
        Samples have equal weight when sample_weight is not provided.  Values must be in the range `[0.0, 0.5]`.
    ...
    init : estimator or 'zero', default=None
        ...
        ``init`` has to provide :term:`fit` and :term:`predict_proba`. If 'zero', the initial raw predictions are set to zero.
    ...
    max_features : {'sqrt', 'log2'}, int or float, default=None
        ...
        - If int, values must be in the range `[1, inf)`.
        - If float, values must be in the range `(0.0, 1.0]` and the features considered at each split will be 
    ...
    verbose : int, default=0
        ...
        If greater than 1 then it prints progress and performance for every tree.  Values must be in the range `[0, inf)`.

    max_leaf_nodes : int, default=None
        Grow trees with ``max_leaf_nodes`` in best-first fashion.  Best nodes are defined as relative reduction in impurity.
        Values must be in the range `[2, inf)`.  If `None`, then unlimited number of leaf nodes.
    ...
    validation_fraction : float, default=0.1
        The proportion of training data to set aside as validation set for early stopping. Values must be in the range `(0.0, 1.0)`.
    ...
    n_iter_no_change : int, default=None
        ...
        If set to a number, it will set aside ``validation_fraction`` size of the training data as validation and terminate training 
        when validation score is not improving in all of the previous ``n_iter_no_change`` numbers of iterations. 
        The split is stratified.  Values must be in the range `[1, inf)`.
    ...
    ccp_alpha : non-negative float, default=0.0
        ...
        By default, no pruning is performed.  Values must be in the range `[0.0, inf)`. 
    ...
    RandomForestClassifier : A meta-estimator that fits a number of decision tree classifiers on various sub-samples of the dataset 
                             and uses averaging to improve the predictive accuracy and control over-fitting.
    AdaBoostClassifier : A meta-estimator that begins by fitting a classifier on the original dataset and then fits additional copies of the
        classifier on the same dataset where the weights of incorrectly classified instances are adjusted such that subsequent classifiers
        focus more on difficult cases.

    Notes
    -----
    The features are always randomly permuted at each split. 
    Therefore, the best found split may vary, even with the same training data and ``max_features=n_features``, 
    ...
    def __init__(self, *, loss="log_loss", learning_rate=0.1, n_estimators=100, subsample=1.0, criterion="friedman_mse", \
        ...
        super().__init__( loss=loss, learning_rate=learning_rate, n_estimators=n_estimators, criterion=criterion, \
            min_samples_split=min_samples_split, min_samples_leaf=min_samples_leaf, min_weight_fraction_leaf=min_weight_fraction_leaf,  \
            max_depth=max_depth, init=init, subsample=subsample, max_features=max_features, random_state=random_state, verbose=verbose, \
            max_leaf_nodes=max_leaf_nodes, min_impurity_decrease=min_impurity_decrease, warm_start=warm_start, \
    ...
    def _encode_y(self, y, sample_weight):
        # encode classes into 0 ... n_classes - 1 and sets attributes classes_ and n_trees_per_iteration_
        ...
        # only 1 tree for binary classification. For multiclass classification, we build 1 tree per class.
        self.n_trees_per_iteration_ = 1 if n_classes <= 2 else n_classes
        encoded_y = encoded_y_int.astype(float, copy=False)

        # From here on, it is additional to the HGBT case. expose n_classes_ attribute
        ...
        if n_trim_classes < 2:
            raise ValueError("y contains %d class after sample_weight trimmed classes with zero weights, "
                "while a minimum of 2 classes are required." % n_trim_classes)
        return encoded_y
    ...
    def _get_loss(self, sample_weight):
        if self.loss == "log_loss":
            ...
            if self.n_classes_ > 2:
                raise ValueError(f"loss='{self.loss}' is only suitable for a binary classification "
                    f"problem, you have n_classes={self.n_classes_}. Please use loss='log_loss' instead.")
            else:
    ...
    def decision_function(self, X):
        ...
        score : ndarray of shape (n_samples, n_classes) or (n_samples,)
            The decision function of the input samples, which corresponds to the raw values predicted from the trees of the ensemble . 
            The order of the classes corresponds to that in the attribute :term:`classes_`. Regression and binary classification produce an
    ...
    def staged_decision_function(self, X):
        ...
        score : generator of ndarray of shape (n_samples, k)
            The decision function of the input samples, which corresponds to the raw values predicted from the trees of the ensemble . 
            The classes corresponds to that in the attribute :term:`classes_`.  Regression and binary classification are special cases with
...
class GradientBoostingRegressor(RegressorMixin, BaseGradientBoosting):
    ...
    n_estimators : int, default=100
        ...
        usually results in better performance.  Values must be in the range `[1, inf)`.
    ...
    init : estimator or 'zero', default=None
        ...
        By default a ``DummyEstimator`` is used, predicting either the average target value (for loss='squared_error'), 
    ...
    alpha : float, default=0.9
        ...
        Only if ``loss='huber'`` or ``loss='quantile'``.  Values must be in the range `(0.0, 1.0)`.
    ...
    See Also
    --------
    HistGradientBoostingRegressor : Histogram-based Gradient Boosting Classification Tree.
    ...
    def __init__(self, *, loss="squared_error", learning_rate=0.1, n_estimators=100, subsample=1.0, criterion="friedman_mse",       \
        ...
        super().__init__(loss=loss, learning_rate=learning_rate, n_estimators=n_estimators, criterion=criterion,                        \
            min_samples_split=min_samples_split, min_samples_leaf=min_samples_leaf, min_weight_fraction_leaf=min_weight_fraction_leaf,  \
            max_depth=max_depth, init=init, subsample=subsample, max_features=max_features, min_impurity_decrease=min_impurity_decrease,\
            random_state=random_state, alpha=alpha, verbose=verbose, max_leaf_nodes=max_leaf_nodes, warm_start=warm_start,              \
            validation_fraction=validation_fraction, n_iter_no_change=n_iter_no_change, tol=tol, ccp_alpha=ccp_alpha)
    ...
    def predict(self, X):
        ...
        ----------
        X : {array-like, sparse matrix} of shape (n_samples, n_features)
            The input samples. Internally, 
            it will be converted to ``dtype=np.float32`` and if a sparse matrix is provided to a sparse ``csr_matrix``.
