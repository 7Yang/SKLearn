...
# Each estimator extracts 6 components as vectors :math:`h \in \mathbb{R}^{4096}`.
...
#
# .. note::
#     The Eigenfaces estimator, via the :py:mod:`sklearn.decomposition.PCA`, also provides a scalar `noise_variance_` 
...
# ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
#
# Mini-batch sparse PCA (:class:`~sklearn.decomposition.MiniBatchSparsePCA`) extracts the set of sparse components that best reconstruct the data. 
# This variant is faster but less accurate than the similar :class:`~sklearn.decomposition.SparsePCA`.
...
# ^^^^^^^^^^^^^^^^^^^
#
# By default, :class:`~sklearn.decomposition.MiniBatchDictionaryLearning` divides the data into mini-batches and optimizes in an online manner by
...
# ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
#
# :class:`sklearn.cluster.MiniBatchKMeans` is computationally efficient and implements on-line learning with a
# :meth:`~sklearn.decomposition.MiniBatchKMeans.partial_fit` method. 
# That is why it could be beneficial to enhance some time-consuming algorithms with :class:`~sklearn.cluster.MiniBatchKMeans`.

# %%
kmeans_estimator = cluster.MiniBatchKMeans(n_clusters=n_components, tol=1e-3, batch_size=20, max_iter=50, random_state=rng, n_init="auto")
...
# ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
#
# :class:`~sklearn.decomposition.FactorAnalysis` is similar to :class:`~sklearn.decomposition.PCA` but has the advantage of modelling the
# variance in every direction of the input space independently (heteroscedastic noise). Read more in the :ref:`User Guide <FA>`.
...
# ----------------------------------
#
# In the further section, let's consider :ref:`DictionaryLearning` more precisely. Dictionary learning is a problem that amounts to finding a sparse representation
# of the input data as a combination of simple elements. These simple elements form a dictionary. It is possible to constrain the dictionary and/or coding coefficients
...
# is better suited for large datasets. Read more in the :ref:`User Guide <MiniBatchDictionaryLearning>`.
...
# %%
# Similar to the previous examples, we change parameters and train :class:`~sklearn.decomposition.MiniBatchDictionaryLearning` estimator on all images. 
# Generally, the dictionary learning and sparse encoding decompose input data into the dictionary and the coding coefficients matrices. 
# :math:`X
