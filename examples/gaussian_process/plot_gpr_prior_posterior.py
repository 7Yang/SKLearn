...
This example illustrates the prior and posterior of a :class:`~sklearn.gaussian_process.GaussianProcessRegressor` 
with different kernels. Mean, standard deviation, and 5 samples are shown for both prior and posterior distributions.

Here, we only give some illustration. To know more about kernels' formulation, refer to the :ref:`User Guide <gp_kernels>`.
...
# we will define an helper function allowing us plotting samples drawn from the Gaussian process.
#
# This function will take a :class:`~sklearn.gaussian_process.GaussianProcessRegressor` model and will drawn sample from the Gaussian process. 
# If the model was not fit, the samples are drawn from the prior distribution while after model fitting, the samples are drawn from the posterior distribution.
import numpy as np
...
def plot_gpr_samples(gpr_model, n_samples, ax):
    ...
    If the Gaussian process model is not trained then the drawn samples are drawn from the prior distribution. 
    Otherwise, the samples are drawn from the posterior distribution. Be aware that a sample here corresponds to a function.

    Parameters
    ----------
    gpr_model : `GaussianProcessRegressor` A :class:`~sklearn.gaussian_process.GaussianProcessRegressor` model.
    n_samples : int, The number of samples to draw from the Gaussian process distribution.
