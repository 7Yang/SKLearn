...
This example shows the use of a forest of trees to evaluate the impurity based importance of the pixels in an image classification task 
on the faces dataset. The hotter the pixel, the more important it is.
...
# ----------------------------------
# First, we load the olivetti faces dataset and limit the dataset to contain only the first five classes. 
# Then we train a random forest on the dataset and evaluate the impurity-based feature importance. 
# One drawback of this method is that it cannot be evaluated on a separate test set. 
# For this example, we are interested in representing the information learned from the full dataset. 
...
#     See :ref:`permutation_importance` as an alternative.
import time
...
#
# If these two conditions are not met, it is recommended to instead use the :func:`~sklearn.inspection.permutation_importance`.
