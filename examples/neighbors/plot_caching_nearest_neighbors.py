...
Here the durations are small since the dataset is small, but the gain can be more substantial when the dataset grows larger, 
or when the grid of parameter to search is large.
...
# The transformer computes the nearest neighbors graph using the maximum number of neighbors necessary in the grid search. 
# The classifier model filters the nearest neighbors graph as required by its own n_neighbors parameter.
graph_model = KNeighborsTransformer(n_neighbors=max(n_neighbors_list), mode="distance")
classifier_model = KNeighborsClassifier(metric="precomputed")

# Note that we give `memory` a directory to cache the graph computation that will be used several times when tuning the hyperparameters of the classifier.
