...
such as the :class:`~sklearn.covariance.EmpiricalCovariance`. It is unbiased, i.e. 
it converges to the true (population) covariance when given many observations. However, 
it can also be beneficial to regularize it, in order to reduce its variance; this, in turn, introduces some bias. 
This example illustrates the simple regularization used in :ref:`shrunk_covariance` estimators. In particular, 
it focuses on how to set the amount of regularization, i.e. how to choose the bias-variance trade-off.
"""
...
#
# * A close formula proposed by Ledoit and Wolf to compute the asymptotically optimal regularization parameter (minimizing a MSE criterion), 
#   yielding the :class:`~sklearn.covariance.LedoitWolf` covariance estimate.
#
# * An improvement of the Ledoit-Wolf shrinkage, the :class:`~sklearn.covariance.OAS`, proposed by Chen et al. 
...
# .. note::
#
#    The maximum likelihood estimate corresponds to no shrinkage, and thus performs poorly. 
#    The Ledoit-Wolf estimate performs really well, as it is close to the optimal and is not computationally costly. In this example, the OAS estimate is a bit further away. 
#    Interestingly, both approaches outperform cross-validation, which is significantly most computationally costly.
