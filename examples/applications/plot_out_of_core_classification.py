...
learning from data that doesn't fit into main memory. We make use of an online classifier, i.e., 
one that supports the partial_fit method, that will be fed with batches of examples. 
To guarantee that the features space remains the same over time we leverage a HashingVectorizer that will project 
each example into the same feature space. This is especially useful in the case of text classification where new
...
# --------------------------------
#
# The dataset used in this example is Reuters-21578 as provided by the UCI ML repository. 
# It will be automatically downloaded and uncompressed on first run.
...
def stream_reuters_documents(data_path=None):
    ...
    The Reuters archive will automatically be downloaded and uncompressed if the `data_path` directory does not exist.
    Documents are represented as dictionaries with 'body' (str), 'title' (str), 'topics' (list(str)) keys.
    ...
    if not data_path.exists():
        ...
        if _not_in_sphinx(): sys.stdout.write("\r")
...
# For other datasets, one should take care of creating a test set with a realistic portion of positive instances.
...
def get_minibatch(doc_iter, size, pos_class=positive_class):
    ...
    if not len(data): return np.asarray([], dtype=int), np.asarray([], dtype=int)
...
# We will feed the classifier with mini-batches of 1000 documents; this means we have at most 1000 docs in memory at any time.  
# The smaller the document batch, the bigger the relative overhead of the partial fit methods.
minibatch_size = 1000
...
for i, (X_train_text, y_train) in enumerate(minibatch_iterators):
    ...
    for cls_name, cls in partial_fit_classifiers.items():
        ...
        if i % 3 == 0: print(progress(cls_name, cls_stats[cls_name]))
