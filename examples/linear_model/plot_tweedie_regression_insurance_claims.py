...
This example illustrates the use of Poisson, Gamma and Tweedie regression on the `French Motor Third-Party Liability 
Claims dataset <https://www.openml.org/d/41214>`_, and is inspired by an R tutorial [1]_.

In this dataset, each sample corresponds to an insurance policy, i.e. a contract within an insurance company 
and an individual (policyholder).  Available features include driver age, vehicle age, vehicle power, etc.

A few definitions: a *claim* is the request made by a policyholder to the insurer to compensate for 
a loss covered by the insurance. The *claim amount* is the amount of money that the insurer must pay. 
The *exposure* is the duration of the insurance coverage of a given policy, in years.

Here our goal is to predict the expected value, i.e. the mean, of the total claim amount per exposure 
unit also referred to as the pure premium.

There are several possibilities to do that, two of which are:

1. Model the number of claims with a Poisson distribution, and the average claim amount per claim, also known as severity, 
   as a Gamma distribution and multiply the predictions of both in order to get the total claim amount.
2. Model the total claim amount per exposure directly, typically with a Tweedie distribution of Tweedie power :math:`p \\in (1, 2)`.
...
def plot_obs_pred(df, feature, weight, observed, predicted, y_label=None, title=None, ax=None, fill_legend=False):
    ...
    feature: str, a column name of df for the feature to be plotted
    weight : str, column name of df with the values of weights or exposure
    observed : str, a column name of df with the observed target
    predicted : DataFrame, a dataframe, with the same index as df, with the predicted target
    fill_legend : bool, default=False, whether to show fill_between legend
    """
    ...
    for subset_label, X, df in [("train", X_train, df_train), ("test", X_test, df_test) ]:
        ...
        for score_label, metric in metrics:
            if isinstance(estimator, tuple) and len(estimator) == 2:
                # Score the model consisting of the product of frequency and severity models.
            ...
            if metric is None:
                if not hasattr(estimator, "score"): continue
...
# containing the claim amount (``ClaimAmount``) for the same policy ids (``IDpol``).
...
# Insurances companies are interested in modeling the Pure Premium, 
# that is the expected total claim amount per unit of exposure for each policyholder in their portfolio:
...
#
# The parameters of the model are estimated by minimizing the Poisson deviance on the training set via a Newton solver. 
# Some of the features are collinear (e.g. because we did not drop any categorical level in the `OneHotEncoder`), we use a weak L2 penalization to avoid numerical issues.
...
# %%
#
# Note that the score measured on the test set is surprisingly better than on the training set. This might be specific to this random train-test split.
# Proper cross-validation could help us to assess the sampling variability of these results.
#
...
# %%
# According to the observed data, the frequency of accidents is higher for drivers younger than 30 years old, and is positively correlated with the `BonusMalus` variable. 
# Our model is able to mostly correctly model this behaviour.
#
# Severity Model -  Gamma distribution
# ------------------------------------
# The mean claim amount or severity (`AvgClaimAmount`) can be empirically shown to follow approximately a Gamma distribution.
# We fit a GLM model for the severity with the same features as the frequency model.
#
# Note:
# - We filter out ``ClaimAmount == 0`` as the Gamma distribution has support on :math:`(0, \infty)`, not :math:`[0, \infty)`.
...
# It can be insightful to compare them with a model that does not use any input features and always predicts a constant value, i.e. the average claim amount, in the same setting:
...
# %%
#
# We conclude that the claim amount is very challenging to predict. Still, 
# the :class:`~sklearn.linear_model.GammaRegressor` is able to leverage some information from the input features to slightly improve upon the mean baseline in terms of D².
#
# Note that the resulting model is the average claim amount per claim. As such, it is conditional on having at least one claim, 
# and cannot be used to predict the average claim amount per policy. For this, it needs to be combined with a claims frequency model.
...
plot_obs_pred(df=df_train.loc[mask_train], feature="DrivAge", weight="Exposure", observed="AvgClaimAmount", predicted=glm_sev.predict(X_train[mask_train.values]), 
    y_label="Average Claim Severity", title="train data", ax=ax[0])
plot_obs_pred(df=df_test.loc[mask_test], feature="DrivAge", weight="Exposure", observed="AvgClaimAmount", predicted=glm_sev.predict(X_test[mask_test.values]),
    y_label="Average Claim Severity", title="test data", ax=ax[1], fill_legend=True)
plt.tight_layout()
...
# --------------------------------------------------------------------
# As mentioned in the introduction, the total claim amount per unit of exposure can be modeled as the product of the prediction of 
# the frequency model by the prediction of the severity model.
#
...
# This model is a special case of the Tweedie GLM with a "power" parameter :math:`p \in (1, 2)`. Here, we fix apriori the `power` parameter of 
# the Tweedie model to some arbitrary value (1.9) in the valid range. Ideally one would select this value via grid-search by minimizing the negative
# log-likelihood of the Tweedie model, but unfortunately the current implementation does not allow for this (yet).
#
# We will compare the performance of both approaches. To quantify the performance of both models, 
# one can compute the mean deviance of the train and test data assuming a Compound Poisson-Gamma distribution of the total claim amount. 
# This is equivalent to a Tweedie distribution with a `power` parameter between 1 and 2.
#
# The :func:`sklearn.metrics.mean_tweedie_deviance` depends on a `power` parameter. 
# As we do not know the true value of the `power` parameter, we here compute the mean deviances for a grid of possible values, 
# and compare the models side by side, i.e. we compare them at identical values of `power`.  Ideally, we hope that one model will be consistently better than the other,
# regardless of `power`.
...
# %%
# In this example, both modeling approaches yield comparable performance metrics. 
# For implementation reasons, the percentage of explained variance :math:`D^2` is not available for the product model.
#
...
# We see that, on average, both model tend to underestimate the total claim (but this behavior depends on the amount of regularization).

res = []
for subset_label, X, df in [("train", X_train, df_train), ("test", X_test, df_test) ]:
    exposure = df["Exposure"].values
    res.append({ "subset": subset_label, "observed": df["ClaimAmount"].values.sum(), "predicted, frequency*severity model": np.sum(exposure * glm_freq.predict(X) * glm_sev.predict(X)),
...
# %%
#
# Finally, we can compare the two models using a plot of cumulated claims: for each model, 
# the policyholders are ranked from safest to riskiest based on the model predictions and the fraction of observed total cumulated claims is plotted on the y axis. 
# This plot is often called the ordered Lorenz curve of the model.
#
# The Gini coefficient (based on the area between the curve and the diagonal) can be used as a model selection metric to quantify the ability of 
# the model to rank policyholders. Note that this metric does not reflect the ability of the models to make accurate predictions in terms of 
# absolute value of total claim amounts but only in terms of relative amounts as a ranking metric. 
...
#
# We observe that both models are able to rank policyholders by risky-ness significantly better than chance although they are also both far from the
# oracle model due to the natural difficulty of the prediction problem from a few features: most accidents are not predictable and can be caused by
# environmental circumstances that are not described at all by the input features of the models.
#
# Note that the Gini index only characterizes the ranking performance of the model but not its calibration: any monotonic transformation of the predictions
# leaves the Gini index of the model unchanged.
#
# Finally one should highlight that the Compound Poisson Gamma model that is directly fit on the pure premium is operationally simpler to develop 
# and maintain as it consists of a single scikit-learn estimator instead of a pair of models, each with its own set of hyperparameters.
