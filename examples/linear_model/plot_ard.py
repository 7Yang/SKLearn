...
In the first part, we use an :ref:`ordinary_least_squares` (OLS) model as a baseline for comparing the models' coefficients 
with respect to the true coefficients. Thereafter, we show that the estimation of such models is done by iteratively maximizing 
the marginal log-likelihood of the observations.

In the last section we plot predictions and uncertainties for the ARD and the Bayesian Ridge regressions using a polynomial feature 
expansion to fit a non-linear relationship between `X` and `y`.
...
# --------------------------
#
# We generate a dataset where `X` and `y` are linearly linked: 10 of the features of `X` will be used to generate `y`. 
# The other features are not useful at predicting `y`. In addition, we generate a dataset where `n_samples == n_features`. 
# Such a setting is challenging for an OLS model and leads potentially to arbitrary large weights. 
...
df = pd.DataFrame({ "Weights of true generative process": true_weights, "ARDRegression": ard.coef_, "BayesianRidge": brr.coef_, "LinearRegression": olr.coef_ })
...
ax = sns.heatmap(df.T, norm=SymLogNorm(linthresh=10e-4, vmin=-80, vmax=80), cbar_kws={"label": "coefficients' values"}, cmap="seismic_r")
...
# %%
# Due to the added noise, none of the models recover the true weights. Indeed, all models always have more than 10 non-zero coefficients. 
...
# The ARD regression provides a sparser solution: some of the non-informative coefficients are set exactly to zero, while shifting others closer to zero. 
# Some non-informative coefficients are still present and retain large values.
...
# then :class:`~sklearn.preprocessing.PolynomialFeatures` should not introduce an additional bias feature. By setting `return_std=True`, 
...
ax.fill_between(X_plot.ravel(), y_ard - y_ard_std, y_ard + y_ard_std, color="navy", alpha=0.3)
ax.fill_between(X_plot.ravel(), y_brr - y_brr_std, y_brr + y_brr_std, color="red",  alpha=0.3)
ax.legend()
_ = ax.set_title("Polynomial fit of a non-linear feature")

# %%
# The error bars represent one standard deviation of the predicted gaussian distribution of the query points. 
# Notice that the ARD regression captures the ground truth the best when using the default parameters in both models, 
# but further reducing the `lambda_init` hyperparameter of the Bayesian Ridge can reduce its bias 
# (see example :ref:`sphx_glr_auto_examples_linear_model_plot_bayesian_ridge_curvefit.py`).  
