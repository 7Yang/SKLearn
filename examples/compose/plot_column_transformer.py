...
This example demonstrates how to use :class:`~sklearn.compose.ColumnTransformer` on a dataset containing different types of features. 
...
# ---------------------
#
# We will use the :ref:`20 newsgroups dataset <20newsgroups_dataset>`, which comprises posts from newsgroups on 20 topics. 
# This dataset is split into train and test subsets based on messages posted before and after a specific date. 
# We will only use posts from 2 categories to speed up running time.
...
# ---------------------
#
# First, we would like a transformer that extracts the subject and body of each post. 
# Since this is a stateless transformation (does not require state information from training data), we can define a function that
# performs the data transformation then use :class:`~sklearn.preprocessing.FunctionTransformer` to create a scikit-learn transformer.
...
# This array is then used to compute standard bag-of-words features for the subject and body as well as text length and number of sentences on the body, 
# using ``ColumnTransformer``. We combine them, with weights, then train a classifier on the combined set of features.
...
pipeline = Pipeline([
        ...
        ("union", ColumnTransformer([ 
                   ...
                   ("body_stats", Pipeline([("stats", text_stats_transformer), ("vect", DictVectorizer())]), 1),
                ],
