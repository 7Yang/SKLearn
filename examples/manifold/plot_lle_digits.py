...
# ---------------------------------
# Below, we will use different techniques to embed the digits dataset. We will plot the projection of the original data onto each embedding. 
# It will allow us to check whether or digits are grouped together in the embedding space, or scattered across it.
import numpy as np
...
def plot_embedding(X, title):
    ...
    for i in range(X.shape[0]):
        # plot every digit on the embedding show an annotation box for a group of digits
...
#
# * the :class:`~sklearn.ensemble.RandomTreesEmbedding` is not technically a manifold embedding method, 
#   as it learn a high-dimensional representation on which we apply a dimensionality reduction method.
#   However, it is often useful to cast a dataset into a representation in which the classes are linearly-separable.
# * the :class:`~sklearn.discriminant_analysis.LinearDiscriminantAnalysis` and the :class:`~sklearn.neighbors.NeighborhoodComponentsAnalysis`, 
#   are supervised dimensionality reduction method, i.e. they make use of the provided labels, contrary to other methods.
# * the :class:`~sklearn.manifold.TSNE` is initialized with the embedding that is generated by PCA in this example. It ensures global stability of the embedding,
...
embeddings = {
    ...
    "t-SNE embeedding": TSNE(n_components=2, n_iter=500, n_iter_without_progress=150, n_jobs=2, random_state=0),
    "NCA embedding": NeighborhoodComponentsAnalysis(n_components=2, init="pca", random_state=0),
}

# %%
# Once we declared all the methodes of interest, we can run and perform the projection of the original data. 
# We will store the projected data as well as the computational time needed to perform each projection.
