...
def yield_namespace_device_dtype_combinations():
    ...
    device : str
        The name of the device on which to allocate the arrays. Can be None to indicate that the default value should be used.

    dtype : str
        The name of the data type to use for arrays. Can be None to indicate that the default value should be used.
    """
    for array_namespace in [
        # The following is used to test the array_api_compat wrapper 
        # when array_api_dispatch is enabled: in particular, the arrays used in the tests are regular numpy arrays without any "device" attribute.
        "numpy",
        # Stricter NumPy-based Array API implementation. The numpy.array_api.Array instances always a dummy "device" attribute.
        "numpy.array_api",
        "cupy",
        "cupy.array_api",
        "torch",
...
def _check_array_api_dispatch(array_api_dispatch):
    ...
    array_api_compat follows NEP29, which has a higher minimum NumPy version than scikit-learn.
    """
...
def size(x):
    ...
    Returns
    -------
    out : int, Total number of elements.
    """
...
def _isdtype_single(dtype, kind, *, xp):
    if isinstance(kind, str):
        if kind == "bool":
            ...
            # Some name spaces do not have complex, such as cupy.array_api and numpy.array_api
            complex_dtypes = set()
...
def supported_float_dtypes(xp):
    ...
    Note: float16 is not officially part of the Array API spec at the time of writing but scikit-learn estimators 
    and functions can choose to accept it when xp.float16 is defined.
...
class _ArrayAPIWrapper:
    ...
    This wrapper makes it possible for scikit-learn maintainers to deal with discrepancies between different implementations of the
    Python Array API standard and its evolution over time.

    The Python Array API standard specification: https://data-apis.org/array-api/latest/
    Documentation of the NumPy implementation: https://numpy.org/neps/nep-0047-array-api-standard.html
    """
    ...
    def take(self, X, indices, *, axis=0):
        ...
        # We only support axis in (0, 1) and ndim in (1, 2) because that is all we need in scikit-learn
        if axis not in {0, 1}:
...
class _NumPyAPIWrapper:
    ...
    NumPy < 1.22 does not expose the numpy.array_api namespace. 
    This wrapper makes it possible to write code that uses the standard Array API 
    while working with any version of NumPy supported by scikit-learn.
...
def get_namespace(*arrays):
    ...
    Introspect(反省) `arrays` arguments and return their common Array API compatible namespace object, 
    if any. NumPy 1.22 and later can construct such containers using the `numpy.array_api` namespace for instance.

    See: https://numpy.org/neps/nep-0047-array-api-standard.html

    If `arrays` are regular numpy arrays, an instance of the `_NumPyAPIWrapper` compatibility wrapper is returned instead.
    ...
    Otherwise an instance of the `_NumPyAPIWrapper` compatibility wrapper is always returned irrespective of 
    the fact that arrays implement the `__array_namespace__` protocol or not.

    Parameters
    ----------
    *arrays : array objects, Array objects.

    Returns
    -------
    namespace : module
        Namespace shared by array objects. If any of the `arrays` are not arrays, the namespace defaults to NumPy.

    is_array_api_compliant : bool
        True if the arrays are containers that implement the Array API spec.  Always False when array_api_dispatch=False.
    """
    ...
    # array-api-compat is a required dependency of scikit-learn only when configuring `array_api_dispatch=True`. 
    # Its import should therefore be protected by _check_array_api_dispatch to display an informative error message in case it is missing.
    import array_api_compat

    namespace, is_array_api_compliant = array_api_compat.get_namespace(*arrays), True

    # These namespaces need additional wrapping to smooth out small differences between implementations
...
def _add_to_diagonal(array, value, xp):
    # Workaround for the lack of support for xp.reshape(a, shape, copy=False) in numpy.array_api: https://github.com/numpy/numpy/issues/23410
...
def _weighted_sum(sample_score, sample_weight, normalize=False, xp=None):
    # XXX: this function accepts Array API input but returns a Python scalar float. 
    # The call to float() is convenient because it removes the need to move back results from device to host memory 
    # (e.g. calling `.cpu()` on a torch tensor). However, this might interact in unexpected ways (break?)
    # with lazy Array API implementations. See: https://github.com/data-apis/array-api/issues/642
    if xp is None:
    ...
    if not xp.isdtype(sample_score.dtype, "real floating"):
        # We move to cpu device ahead of time since certain devices may not support float64, 
...
def _nanmin(X, axis=None):
    # TODO: refactor once nan-aware reductions are standardized: https://github.com/data-apis/array-api/issues/621
    xp, _ = get_namespace(X)
    if _is_numpy_namespace(xp):
...
def _asarray_with_order(array, dtype=None, order=None, copy=None, *, xp=None):
    ...
    however some input validation code in scikit-learn needs to work both for classes 
    and functions that will leverage Array API only operations
    and for code that inherently relies on NumPy backed data containers with
    specific memory layout constraints (e.g. our own Cython code). 
    The purpose of this helper is to make it possible to share code for data container validation without memory copies 
    for both downstream use cases: 
    the `order` parameter is only enforced if the input array implementation is NumPy based, 
    otherwise `order` is just silently ignored.
    """
    if xp is None:
    ...
    if _is_numpy_namespace(xp):
        ...
        # At this point array is a NumPy ndarray. We convert it to an array container that is consistent with the input's namespace.
...
def _estimator_with_converted_arrays(estimator, converter):
    ...
    The converter is called on all NumPy arrays and arrays that support the `DLPack interface <https://dmlc.github.io/dlpack/latest/>`__.
