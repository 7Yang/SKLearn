...
def _wrap_in_pandas_container(data_to_wrap, *, columns, index=None):
    ...
    columns : callable, ndarray, or None
        ...
        The callable is useful if the column names require some computation.
        If `columns` is a callable that raises an error, `columns` will have the same semantics as `None`. 
        If `None` and `data_to_wrap` is already a dataframe, then the column names are not changed. 
        If `None` and `data_to_wrap` is **not** a dataframe, then columns are `range(n_features)`.
    ...
    Returns
    -------
    dataframe : DataFrame, Container with column names or unchanged `output`.
    """
...
def _get_output_config(method, estimator=None):
    ...
    estimator : estimator instance or None
        Estimator to get the output configuration from. If `None`, check global configuration is used.
    ...
    config : dict
        ...
        - "dense": specifies the dense container for `method`. This can be `"default"` or `"pandas"`.
    """
...
def _wrap_data_with_container(method, data_to_wrap, original_input, estimator):
    ...
    output : {ndarray, dataframe}
        If the output config is "default" or the estimator is not configured for wrapping return `data_to_wrap` unchanged.
        If the output config is "pandas", return `data_to_wrap` as a pandas DataFrame.
    """
...
def _auto_wrap_is_configured(estimator):
    ...
    `_SetOutputMixin` sets `_sklearn_auto_wrap_output_keys` to `set()` if auto wrapping is manually disabled.
    """
...
class _SetOutputMixin:
    ...
    Currently `_SetOutputMixin` wraps `transform` and `fit_transform` and configures it based on `set_output` of the global configuration.

    `set_output` is only defined if `get_feature_names_out` is defined and `auto_wrap_output_keys` is the default value.
    """

    def __init_subclass__(cls, auto_wrap_output_keys=("transform",), **kwargs):
        super().__init_subclass__(**kwargs)

        # Dynamically wraps `transform` and `fit_transform` and configure it's output based on `set_output`.
...
def _safe_set_output(estimator, *, transform=None):
    ...
    Returns
    -------
    estimator : estimator instance, Estimator instance.
    """
    set_output_for_transform = (hasattr(estimator, "transform") or hasattr(estimator, "fit_transform") and transform is not None)
    if not set_output_for_transform:
        # If estimator can not transform, then `set_output` does not need to be called.
        return
