"""
    Utilities to get the response values of a classifier or a regressor.  It allows to make uniform checks and validation.
"""
import numpy as np
...
def _process_predict_proba(*, y_pred, target_type, classes, pos_label):
    ...
    In the binary case, it selects the column corresponding to the positive class. 
    In the multi-label case, it stacks the predictions if they are not in the "compressed" format `(n_samples, n_outputs)`.
    ...
    y_pred : ndarray
        ...
        - for multilabel classification, it is either a list of 2d arrays of shape `(n_samples, 2)` 
          (e.g. `RandomForestClassifier` or `KNeighborsClassifier`) or an array of shape `(n_samples, n_outputs)` 
          (e.g. `MLPClassifier` or `RidgeClassifier`).
    ...
    if target_type == "binary":
        ...
        # Only `MLPClassifier` and `RidgeClassifier` return an array of shape `(n_samples, n_outputs)`.
...
def _process_decision_function(*, y_pred, target_type, classes, pos_label):
    ...
    This function process the `y_pred` array in the binary and multi-label cases.
    In the binary case, it inverts the sign of the score if the positive label is not `classes[1]`. 
    In the multi-label case, it stacks the predictions if they are not in the "compressed" format `(n_samples, n_outputs)`.
...
def _get_response_values(estimator, X, response_method, pos_label=None):
    ...
    estimator : estimator instance
        Fitted classifier or regressor or a fitted :class:`~sklearn.pipeline.Pipeline` in which the last estimator is a classifier or a regressor.
    ...
    response_method : {"predict_proba", "decision_function", "predict"} or list of such str
        ...
        (i.e. :term:`predict_proba`, :term:`decision_function` or :term:`predict`). Possible choices are:
    ...
    pos_label : int, float, bool, str or None
        The class considered as the positive class when computing the metrics. Returns `None` if `estimator` is a regressor.
    ...
    ValueError
        ...
        If the response method can be applied to a classifier only and `estimator` is a regressor.
    """
    ...
    if is_classifier(estimator):
        ...
        if response_method != "predict":
            raise ValueError(f"{estimator.__class__.__name__} should either be a classifier to be used with response_method={response_method} "
                "or the response_method should be 'predict'. Got a regressor with response_method={response_method} instead.")
...
def _get_response_values_binary(estimator, X, response_method, pos_label=None):
    ...
    ----------
    estimator : estimator instance
        Fitted classifier or a fitted :class:`~sklearn.pipeline.Pipeline` in which the last estimator is a binary classifier.
