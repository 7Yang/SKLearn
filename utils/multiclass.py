...
def unique_labels(*ys):
    ...
    Parameters
    ----------
    *ys : array-likes, Label values.

    Returns
    -------
    out : ndarray of shape (n_unique_labels,), An ordered array of unique labels.
...
def is_multilabel(y):
    ...
    if hasattr(y, "__array__") or isinstance(y, Sequence) or is_array_api_compliant:
        ...
        check_y_kwargs = dict(accept_sparse=True, allow_nd=True, force_all_finite=False, ensure_2d=False,\
        ...
        with warnings.catch_warnings():
            ...
            try:
                ...
                # dtype=object should be provided explicitly for ragged arrays, see NEP 34
    ...
    if issparse(y):
        ...
        return (len(y.data) == 0 or (labels.size == 1 or (labels.size == 2) and (0 in labels)) 
...
def check_classification_targets(y):
    ...
    if y_type not in [ "binary", "multiclass", "multiclass-multioutput", "multilabel-indicator", "multilabel-sequences" ]:
        raise ValueError(f"Unknown label type: {y_type}. Maybe you are trying to fit a classifier, which expects discrete classes on a "
...
def type_of_target(y, input_name=""):
    ...
    # DeprecationWarning will be replaced by ValueError, see NEP 34
    # https://numpy.org/neps/nep-0034-infer-dtype-is-object.html
    # We therefore catch both deprecation (NumPy < 1.24) warning and value error (NumPy >= 1.24).
    ...
    try:
        ...
        if (not hasattr(first_row, "__array__") and isinstance(first_row, Sequence) and not isinstance(first_row, str)):
            raise ValueError("You appear to be using a legacy multi-label data representation. "
...
def _check_partial_fit_first_call(clf, classes=None):
    ...
    Estimators that implement the ``partial_fit`` API need to be provided with the list of possible classes at the first call to partial_fit.
    Subsequent calls to partial_fit should check that ``classes`` is still consistent with a previous value of ``clf.classes_`` when provided.

    This function returns True if it detects that this was the first call to ``partial_fit`` on ``clf``. 
    In that case the ``classes_`` attribute is also set on ``clf``.
    ...
    # classes is None and clf.classes_ has already previously been set: nothing to do
    return False
...
def _ovr_decision_function(predictions, confidences, n_classes):
    ...
    n_classes : int
        Number of classes. n_classifiers must be ``n_classes * (n_classes - 1 ) / 2``.
    """
    ...
    # Monotonically transform the sum_of_confidences to (-1/3, 1/3) and add it with votes. 
    # The monotonic transformation is f: x -> x / (3 * (|x| + 1)), it uses 1/3 instead of 1/2 to ensure 
    # that we won't reach the limits and change vote order.  
    # The motivation is to use confidence levels as a way to break ties in the votes without switching 
    # any decision made based on a difference of 1 vote.
