...
def _random_choice_csc(n_samples, classes, class_probability=None, random_state=None):
    ...
    class_probability : list of size n_outputs of arrays of shape (n_classes,), default=None
        Class distribution of each column. If None, uniform distribution is assumed.

    random_state : int, RandomState instance or None, default=None
        Controls the randomness of the sampled classes.  See :term:`Glossary <random_state>`.
    ...
    for j in range(len(classes)):
        ...
        if class_prob_j.shape[0] != classes[j].shape[0]:
            raise ValueError("classes[{0}] (length {1}) and class_probability[{0}] (length {2}) have different length."
