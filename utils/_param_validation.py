...
def validate_parameter_constraints(parameter_constraints, params, caller_name):
    ...
    params : dict
        A dictionary `param_name: param_value`. The parameters to validate against the constraints.
    ...
    for param_name, param_val in params.items():
        ...
        # can inherit from sklearn estimators without having to necessarily use the validation tools.
...
def make_constraint(constraint):
    ...
    Returns
    -------
    constraint : instance of _Constraint, The converted constraint.
    """
...
def validate_params(parameter_constraints, *, prefer_skip_nested_validation):
    ...
    parameter_constraints : dict
        ...
        Note that the *args and **kwargs parameters are not validated and must not be present in the parameter_constraints dictionary.

    prefer_skip_nested_validation : bool
        If True, the validation of parameters of inner estimators or functions called by the decorated function will be skipped.

        This is useful to avoid validating many times the parameters passed by the user from the public facing API. 
        It's also useful to avoid validating parameters that we pass internally to inner functions that are guaranteed to
        be valid by the test suite.

        It should be set to True for most functions, except for those that receive non-validated objects as parameters 
        or that are just wrappers around classes because they only perform a partial validation.

    Returns
    -------
    decorated_function : function or method, The decorated function.
    """

    def decorator(func):
        # The dict of parameter constraints is set as an attribute of the function to make it possible to dynamically 
        # introspect the constraints for automatic testing.
        ...
        def wrapper(*args, **kwargs):
            ...
            try:
                ...
                # we allow the function to delegate validation to the estimator, 
                # but we replace the name of the estimator by the name of the function in the error message to avoid confusion.
                msg = re.sub(r"parameter of \w+ must be", f"parameter of {func.__qualname__} must be", str(e))
...
class _Constraint(ABC):
    ...
    def is_satisfied_by(self, val):
        ...
        Returns
        -------
        is_satisfied : bool, Whether or not the constraint is satisfied by this value.
        """
...
class Options(_Constraint):
    ...
    deprecated : set or None, default=None, A subset of the `options` to mark as deprecated in the string representation of the constraint.
    """
...
class StrOptions(Options):
    ...
    deprecated : set of str or None, default=None
        A subset of the `options` to mark as deprecated in the string representation of the constraint.
    """
...
class Interval(_Constraint):
    ...
    type : {numbers.Integral, numbers.Real, RealNotInt}
        ...
        If RealNotInt, only reals that don't have the integer type are allowed. For example 1.0 is allowed but 1 is not.
    ...
    closed : {"left", "right", "both", "neither"}
        ...
        - `"left"`: the interval is closed on the left and open on the right.   It is equivalent to the interval `[ left, right )`.
        - `"right"`: the interval is closed on the right and open on the left.  It is equivalent to the interval `( left, right ]`.
        - `"both"`: the interval is closed.   It is equivalent to the interval `[ left, right ]`.
        - `"neither"`: the interval is open.  It is equivalent to the interval `( left, right )`.

    Notes
    -----
    Setting a bound to `None` and setting the interval closed is valid. 
    For instance, strictly speaking, `Interval(Real, 0, None, closed="both")` corresponds to `[0, +∞) U {+∞}`.
    """
    ...
    def _check_params(self):
        if self.type not in (Integral, Real, RealNotInt):
            raise ValueError("type must be either numbers.Integral, numbers.Real or RealNotInt. Got {self.type} instead.")

        if self.closed not in ("left", "right", "both", "neither"):
            raise ValueError("closed must be either 'left', 'right', 'both' or 'neither'. Got {self.closed} instead.")
...
class _RandomStates(_Constraint):
    ...
    Convenience class for [Interval(Integral, 0, 2**32 - 1, closed="both"), np.random.RandomState, None]
    """
...
class _Booleans(_Constraint):
    ...
    Convenience class for [bool, np.bool_, Integral (deprecated)]
    """
    ...
    def is_satisfied_by(self, val):
        ...
        if isinstance(val, Integral) and not isinstance(val, bool):
            warnings.warn(("Passing an int for a boolean parameter is deprecated in version 1.2 and won't be supported anymore in version 1.4."),
                FutureWarning)
...
class _VerboseHelper(_Constraint):
    ...
    Convenience class for [Interval(Integral, 0, None, closed="left"), bool, numpy.bool_]
    """
...
class HasMethods(_Constraint):
    ...
    It is useful for parameters following a protocol and where we don't want to impose an affiliation to a specific module or class.

    Parameters
    ----------
    methods : str or list of str, The method(s) that the object is expected to expose.
    """
...
class Hidden:
    ...
    Parameters
    ----------
    constraint : str or _Constraint instance, The constraint to be used internally.
    """
...
def generate_invalid_param_val(constraint):
    ...
    Parameters
    ----------
    constraint : _Constraint instance, The constraint to generate a value for.

    Returns
    -------
    val : object, A value that does not satisfy the constraint.
    """
...
def generate_valid_param(constraint):
    ...
    Returns
    -------
    val : object, A value that does satisfy the constraint.
    """
