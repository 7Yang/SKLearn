...
class _VisualBlock:
    ...
    estimators : list of estimators or `_VisualBlock`s or a single estimator
        If kind != 'single', then `estimators` is a list of estimators.
        If kind == 'single', then `estimators` is a single estimator.

    names : list of str, default=None
        If kind != 'single', then `names` corresponds to estimators.
        If kind == 'single', then `names` is a single string corresponding to the single estimator.

    name_details : list of str, str, or None, default=None
        If kind != 'single', then `name_details` corresponds to `names`.
        If kind == 'single', then `name_details` is a single string corresponding to the single estimator.

    dash_wrapped : bool, default=True
        If true, wrapped HTML element will be wrapped with a dashed border.  Only active when kind != 'single'.
    """
...
def _write_label_html(out, name, name_details, outer_class="sk-label-container", inner_class="sk-label", checked=False):
    ...
    if name_details is not None:
        ...
        out.write('<input class="sk-toggleable__control sk-hidden--visually" '
...
def _get_visual_block(estimator):
    ...
    if hasattr(estimator, "get_params") and not isclass(estimator):
        ...
        if estimators:
            return _VisualBlock("parallel", \
                [est for _, est in estimators], names=[f"{key}: {est.__class__.__name__}" for key, est in estimators], \
...
def _write_estimator_html(out, estimator, estimator_label, estimator_label_details, first_call=False):
    ...
    if est_block.kind in ("serial", "parallel"):
        ...
        _write_label_html(out, est_block.names, est_block.name_details, outer_class="sk-item", inner_class="sk-estimator", checked=first_call)
...
def estimator_html_repr(estimator):
    ...
    with closing(StringIO()) as out:
        ...
        #
        # If the notebook is trusted, the CSS is loaded which hides the fallback message. 
        # If the notebook is not trusted, then the CSS is not loaded and the fallback message is shown by default.
        #
        ...
        _write_estimator_html(out, estimator, estimator.__class__.__name__, estimator_str, first_call=True)
        out.write("</div></div>")
