...
def inplace_csr_row_scale(X, scale):
    ...
    Parameters
    ----------
    X     : sparse matrix of shape (n_samples, n_features), Matrix to be scaled. It should be of CSR format.
    scale : ndarray of float of shape (n_samples,), Array of precomputed sample-wise values to use for scaling.
    """
...
def mean_variance_axis(X, axis, weights=None, return_sum_weights=False):
    ...
    weights : ndarray of shape (n_samples,) or (n_features,), default=None
        If axis is set to 0 shape is (n_samples,) or if axis is set to 1 shape is (n_features,).
    ...
    sum_weights : ndarray of shape (n_features,), dtype=floating, Returned if `return_sum_weights` is `True`.
    """
...
def incr_mean_variance_axis(X, *, axis, last_mean, last_var, last_n, weights=None):
    ...
    last_mean, last_var are the statistics computed at the last step by this function. 
    Both must be initialized to 0-arrays of the proper size, i.e. the number of features in X. last_n is the number of samples encountered until now.
    ...
    last_n : float or ndarray of shape (n_features,) or (n_samples,), dtype=floating
        Sum of the weights seen so far, excluding the current weights If not float, 
        it should be of shape (n_features,) if axis=0 or (n_samples,) if axis=1. 
        If float it corresponds to having same weights for all samples (or features).
    ...
    if axis == 1:
        if np.size(last_mean) != X.shape[0]:
            raise ValueError("If axis=1, then last_mean, last_n, last_var should be of size n_samples {X.shape[0]} (Got {np.size(last_mean)}).")
    else:  # axis == 0
        if np.size(last_mean) != X.shape[1]:
            raise ValueError("If axis=0, then last_mean, last_n, last_var should be of size n_features {X.shape[1]} (Got {np.size(last_mean)}).")
...
def inplace_row_scale(X, scale):
    ...
    Parameters
    ----------
    X : sparse matrix of shape (n_samples, n_features), Matrix to be scaled. It should be of CSR or CSC format.
    scale : ndarray of shape (n_features,), dtype={np.float32, np.float64}, Array of precomputed sample-wise values to use for scaling.
    """
    if sp.issparse(X) and X.format == "csc":
...
def inplace_swap_row_csc(X, m, n):
    ...
    ----------
    X : sparse matrix of shape (n_samples, n_features)
        Matrix whose two rows are to be swapped. It should be of CSC format.
...
def inplace_swap_row_csr(X, m, n):
    ...
    ----------
    X : sparse matrix of shape (n_samples, n_features)
        Matrix whose two rows are to be swapped. It should be of CSR format.
...
def _minor_reduce(X, ufunc):
    ...
    # reduceat tries casts X.indptr to intp, which errors if it is int64 on a 32 bit system.
...
def min_max_axis(X, axis, ignore_nan=False):
    ...
    Returns
    -------
    mins : ndarray of shape (n_features,), dtype={np.float32, np.float64}, Feature-wise minima.
    maxs : ndarray of shape (n_features,), dtype={np.float32, np.float64}, Feature-wise maxima.
    """
...
def count_nonzero(X, axis=None, sample_weight=None):
    ...
    # We rely here on the fact that np.diff(Y.indptr) for a CSR will return the number of nonzero entries in each row.
    # A bincount over Y.indices will return the number of nonzeros in each column. See ``csr_matrix.getnnz`` in scipy >= 0.14.
    if axis is None:
...
def csc_median_axis_0(X):
    ...
    Returns
    -------
    median : ndarray of shape (n_features,), Median.
    """
