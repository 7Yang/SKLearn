...
If you add content to this file, please give the version of the package at which the fix is no longer needed.
"""
...
# since version 3 it's possible to setup a global threadpool controller to avoid looping through all loaded shared libraries each time.
...
def _mode(a, axis=0):
    if sp_version >= parse_version("1.9.0"):
        ...
        if sp_version >= parse_version("1.10.999"):
            # scipy.stats.mode has changed returned array shape with axis=None and keepdims=True, see https://github.com/scipy/scipy/pull/17561
            if axis is None:
                mode = np.ravel(mode)
        return mode
...
# For +1.25 NumPy versions exceptions and warnings are being moved to a dedicated submodule.
