...
class CheckingClassifier(ClassifierMixin, BaseEstimator):
    ...
    This allows testing whether pipelines / cross-validation or metaestimators changed the input.
    ...
    check_y, check_X : callable, default=None
        The callable used to validate `X` and `y`. These callable should return a bool where `False` will trigger an `AssertionError`. 
        If `None`, the data is not validated. Default is `None`.

    check_y_params, check_X_params : dict, default=None
        The optional parameters to pass to `check_X` and `check_y`. If `None`, then no parameters are passed in.

    methods_to_check : "all" or list of str, default="all"
        The methods in which the checks should be applied. 
        By default, all checks will be done on all methods (`fit`, `predict`, `predict_proba`, `decision_function` and `score`).

    foo_param : int, default=0
        A `foo` param. When `foo > 1`, the output of :meth:`score` will be 1 otherwise it is 0.
    ...
    This helper allow to assert to specificities regarding `X` or `y`. 
    In this case we expect `check_X` or `check_y` to return a boolean.
    ...
    We can also provide a check which might raise an error. In this case, we expect `check_X` to return `X` and `check_y` to return `y`.
    ...
    def __init__(self, *, check_y=None, check_y_params=None, check_X=None, check_X_params=None, methods_to_check="all", \
        foo_param=0, expected_sample_weight=None, expected_fit_params=None):
    ...
    def _check_X_y(self, X, y=None, should_be_fitted=True):
        ...
        Parameters
        ----------
        X : array-like of shape (n_samples, n_features) The data set.
            `X` is checked only if `check_X` is not `None` (default is None).
        y : array-like of shape (n_samples), default=None
            The corresponding(相应的、相关的) target, by default `None`.
            `y` is checked only if `check_y` is not `None` (default is None). 
        should_be_fitted : bool, default=True
            Whether or not the classifier should be already fitted.  By default True.
    ...
    def predict(self, X):
        ...
        Returns
        -------
        preds : ndarray of shape (n_samples,), Predictions of the first class seens in `classes_`.
        """
    ...
    def predict_proba(self, X):
        ...
        Here, the dummy classifier will provide a probability of 1 for the first class of `classes_` and 0 otherwise.
        ...
        Returns
        -------
        proba : ndarray of shape (n_samples, n_classes), The probabilities for each sample and class.
        """
    ...
    def decision_function(self, X):
        ...
        if len(self.classes_) == 2:
            # for binary classifier, the confidence score is related to classes_[1] and therefore should be null.
    ...
    def score(self, X=None, Y=None):
        ...
        -------
        score : float
            Either 0 or 1 depending of `foo_param` (i.e. `foo_param > 1 => score=1` otherwise `score=0`).
        """
...
# Deactivate key validation for CheckingClassifier because we want to be able to call fit with arbitrary fit_params and record them. 
# Without this change, we would get an error because those arbitrary params are not expected.
...
class NoSampleWeightWrapper(BaseEstimator):
    ...
    Parameters
    ----------
    est : estimator, default=None, The estimator to wrap.
    """
...
class _MockEstimatorOnOffPrediction(BaseEstimator):
    ...
    List containing the response implemented by the estimator. 
    When, the response is in the list, it will return the name of the response method when called.
    Otherwise, an `AttributeError` is raised. It allows to use `getattr` as any conventional estimator. 
