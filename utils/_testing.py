...
from numpy.testing import (assert_almost_equal, assert_approx_equal, assert_array_almost_equal, assert_array_equal, \
        assert_array_less, assert_no_warnings)

import sklearn
from sklearn.utils import (_IS_32BIT, IS_PYPY, _in_unstable_openblas_configuration)
...
__all__ = [
    "assert_raises", "assert_raises_regexp", "assert_array_equal", "assert_almost_equal", "assert_array_almost_equal",
    "assert_array_less", "assert_approx_equal", "assert_allclose", "assert_run_python_script", "assert_no_warnings", "SkipTest" ]
...
# assert_raises_regexp is deprecated in Python 3.4 in favor of assert_raises_regex 
# but lets keep the backward compat in scikit-learn with the old name for now
...
def ignore_warnings(obj=None, category=Warning):
    ...
    Note: Using this (in both variants) will clear all warnings from all python modules loaded. 
    In case you need to test cross-module-warning-logging, this is not your tool of choice.
    ...
    >>> with ignore_warnings(): warnings.warn('buhuhuhu')
    ...
    if isinstance(obj, type) and issubclass(obj, Warning):
        # Avoid common pitfall of passing category as the first positional argument which result in the test not being run
        warning_name = obj.__name__
        raise ValueError("'obj' should be a callable where you want to ignore warnings. "
            "You passed a warning class instead: 'obj={warning_name}'. If you want to pass a warning class to ignore_warnings, "
...
class _IgnoreWarnings:
    ...
    This class allows the user to ignore the warnings raised by a function.  Copied from Python 2.7.5 and modified as required.

    Parameters
    ----------
    category : tuple of warning class, default=Warning, The category to filter. By default, all the categories will be muted.
    """
...
def assert_raise_message(exceptions, message, function, *args, **kwargs):
    ...
    Given an exception, a callable to raise the exception, and a message string, 
    tests that the correct exception is raised and that the message is a substring of the error thrown. 
    Used to test that the specific message thrown during an exception is correct.
...
def assert_allclose(actual, desired, rtol=None, atol=0.0, equal_nan=True, err_msg="", verbose=True):
    ...
    This variant introspects the least precise floating point dtype in the input argument 
    and automatically sets the relative tolerance parameter to 1e-4 float32 
    and use 1e-7 otherwise (typically float64 in scikit-learn).

    `atol` is always left to 0. by default. It should be adjusted manually to an assertion-specific value in case 
    there are null values expected in `desired`.
    ...
    rtol : float, optional, default=None
        Relative tolerance.  If None, it is set based on the provided arrays' dtypes.
...
def set_random_state(estimator, random_state=0):
    ...
    random_state : int, RandomState instance or None, default=0
        Pseudo random number generator state.  Pass an int for reproducible results across multiple function calls.  
...
try:
    ...
    skip_if_array_api_compat_not_configured = pytest.mark.skipif(not ARRAY_API_COMPAT_FUNCTIONAL, 
        reason="requires array_api_compat installed and a new enough version of NumPy")

    #  Decorator for tests involving both BLAS calls and multiprocessing.
    #
    #  Under POSIX (e.g. Linux or OSX), using multiprocessing in conjunction with some implementation of BLAS 
    #  (or other libraries that manage an internal posix thread pool) can cause a crash or a freeze of the Python process.
    #
    #  In practice all known packaged distributions (from Linux distros or Anaconda) of BLAS under Linux seems to be safe. 
    #  So we this problem seems to only impact OSX users.
    #
    #  This wrapper makes it possible to skip tests that can possibly cause this crash under OS X with.
    #
    #  Under Python 3.4+ it is possible to use the `forkserver` start method for multiprocessing to avoid this issue. 
    #  However it can cause pickling errors on interactively defined functions. It therefore not enabled by default.
...
def _create_aligned_memmap_backed_arrays(data, mmap_mode, folder):
    ...
    if isinstance(data, Sequence) and all(isinstance(each, np.ndarray) for each in data):
        return [_create_memmap_backed_array(array, op.join(folder, f"data{index}.dat"), mmap_mode) for index, array in enumerate(data)]
...
def create_memmap_backed_data(data, mmap_mode="r", return_folder=False, aligned=False):
    ...
    aligned : bool, default=False
        If True, if input is a single numpy array and if the input array is aligned, the memory mapped array will also be aligned. 
        This is a workaround for https://github.com/joblib/joblib/issues/563.
    """
    temp_folder = tempfile.mkdtemp(prefix="sklearn_testing_")
    atexit.register(functools.partial(_delete_folder, temp_folder, warn=True))
    # OpenBLAS is known to segfault with unaligned data on the Prescott architecture so force aligned=True on Prescott. 
    # For more details, see: https://github.com/scipy/scipy/issues/14886
    has_prescott_openblas = any(
        True for info in threadpool_info() if info["internal_api"] == "openblas"
...
def check_docstring_parameters(func, doc=None, ignore=None):
    ...
    Returns
    -------
    incorrect : list, A list of string describing the incorrect results.
    """
        ...
        with warnings.catch_warnings(record=True):
            ...
            try:
                ...
                if "potentially wrong underline length" in str(exp):
                    # Catch warning raised as of numpydoc 1.2 when the underline length for a section of a docstring is not consistent.
    ...
    for name, type_definition, param_doc in doc["Parameters"]:
        ...
        # Create a list of parameters to compare with the parameters gotten from the func signature
        if "*" not in name:
            param_docs.append(name.split(":")[0].strip("` "))

    # If one of the docstring's parameters had an error then return that incorrect message
    if len(incorrect) > 0:
        return incorrect
    ...
    # Krekel and others, Licensed under MIT License. See https://github.com/pytest-dev/pytest

    message = []
    for i in range(min(len(param_docs), len(param_signature))):
        if param_signature[i] != param_docs[i]:
            message += ["There's a parameter name mismatch in function docstring w.r.t. function signature, at index %s" 
                " diff: %r != %r" % (i, param_signature[i], param_docs[i])]
            break
    if len(param_signature) > len(param_docs):
        message += ["Parameters in function docstring have less items w.r.t. function signature, first missing item: %s" 
            % param_signature[len(param_docs)]]

    elif len(param_signature) < len(param_docs):
        message += ["Parameters in function docstring have more items w.r.t. function signature, first extra item: %s"
            % param_docs[len(param_signature)]]

    # If there wasn't any difference in the parameters themselves between docstring 
    # and signature including having the same length then return empty list
    if len(message) == 0:
        return []
...
def assert_run_python_script(source_code, timeout=60):
    ...
    The script provided in the source code should return 0 and not print anything on stderr or stdout.
...
def _convert_container(container, constructor_name, columns_name=None, dtype=None, minversion=None):
    """
    ...
    columns_name : index or array-like, default=None
        For pandas container supporting `columns_names`, it will affect specific names.
    dtype : dtype, default=None
        Force the dtype of the container. Does not apply to `"slice"` container.
    ...
    if constructor_name == "list":
        ...
        raise ValueError(f"sparse_csr_array is only available with scipy>=1.8.0, got {sp_version}")
    elif constructor_name == "sparse_csc":
        return sp.sparse.csc_matrix(container, dtype=dtype)
    elif constructor_name == "sparse_csc_array":
        if sp_version >= parse_version("1.8"):
            return sp.sparse.csc_array(container, dtype=dtype)
        raise ValueError(f"sparse_csc_array is only available with scipy>=1.8.0, got {sp_version}")


def raises(expected_exc_type, match=None, may_pass=False, err_msg=None):
    """
    Context manager to ensure exceptions are raised within a code block.
    This is similar to and inspired from pytest.raises, but supports a few other cases.

    This is only intended to be used in estimator_checks.py where we don't want to use pytest. 
    In the rest of the code base, just use pytest.raises instead.

    Parameters
    ----------
    excepted_exc_type : Exception or list of Exception
        The exception that should be raised by the block. If a list, the block should raise one of the exceptions.
    match : str or list of str, default=None
        A regex that the exception message should match. If a list, one of the entries must match. If None, match isn't enforced.
    may_pass : bool, default=False
        If True, the block is allowed to not raise an exception. 
        Useful in cases where some estimators may support a feature but others must fail with an appropriate error message. 
        By default, the context manager will raise an exception if the block does not raise an exception.
    err_msg : str, default=None
        If the context manager fails (e.g. the block fails to raise the proper exception, or fails to match), 
        then an AssertionError is raised with this message. 
        By default, an AssertionError is raised with a default error message (depends on the kind of failure). 
        Use this to indicate how users should fix their estimators to pass the checks.
...
class _Raises(contextlib.AbstractContextManager):
    ...
    def __exit__(self, exc_type, exc_value, _):
        # see https://docs.python.org/2.5/whatsnew/pep-343.html#SECTION000910000000000000000
...
class MinimalTransformer:
    """
    Minimal transformer implementation with inheriting from BaseEstimator.
...
def _array_api_for_tests(array_namespace, device, dtype):
    ...
    # This is because `cupy` is not the same as the compatibility wrapped namespace of a CuPy array.
    ...
    if array_namespace == "torch" and device == "cuda" and not xp.has_cuda:
        ...
        if os.getenv("PYTORCH_ENABLE_MPS_FALLBACK") != "1":
            # For now we need PYTORCH_ENABLE_MPS_FALLBACK=1 for all estimators to work when using the MPS device.
        ...
        if not xp.has_mps:
            if not xp.backends.mps.is_built():
                ...
                raise SkipTest("MPS is not available because the current MacOS version is not 12.3+ and/or you do not have "
