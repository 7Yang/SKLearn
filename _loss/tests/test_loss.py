...
from sklearn._loss.loss import (_LOSSES, AbsoluteError, BaseLoss, HalfBinomialLoss, HalfGammaLoss,
    HalfMultinomialLoss, HalfPoissonLoss, HalfSquaredError, HalfTweedieLoss, HalfTweedieLossIdentity, HuberLoss, PinballLoss)
...
@pytest.mark.parametrize("loss, y_true, raw_prediction, loss_true",
    [
        ...
        (HalfMultinomialLoss(n_classes=3), 0.0, [0.2, 0.5, 0.3], logsumexp([0.2, 0.5, 0.3]) - 0.2),
        (HalfMultinomialLoss(n_classes=3), 1.0, [0.2, 0.5, 0.3], logsumexp([0.2, 0.5, 0.3]) - 0.5),
        (HalfMultinomialLoss(n_classes=3), 2.0, [0.2, 0.5, 0.3], logsumexp([0.2, 0.5, 0.3]) - 0.3),
    ],
...
def test_loss_dtype(loss, readonly_memmap, dtype_in, dtype_out, sample_weight, out1, out2, n_threads):
    ...
    Check that loss accepts if all input arrays are either all float32 or all float64, 
    and all output arrays are either all float32 or all float64.
    ...
    if out1 is not None and loss.is_multiclass: out1 = np.empty_like(raw_prediction, dtype=dtype_out)
    ...
    if hasattr(loss, "predict_proba"): loss.predict_proba(raw_prediction=raw_prediction)
    if hasattr(loss, "gradient_proba"): loss.gradient_proba( y_true=y_true, raw_prediction=raw_prediction, sample_weight=sample_weight, gradient_out=out1, proba_out=out2, n_threads=n_threads)
...
def test_loss_same_as_C_functions(loss, sample_weight):
    ...
    if sample_weight == "range": sample_weight = np.linspace(1, y_true.shape[0], num=y_true.shape[0])
    ...
    assert_allclose(
        loss.loss(y_true=y_true, raw_prediction=raw_prediction, sample_weight=sample_weight, loss_out=out_l1),
        loss.closs.loss(y_true=y_true, raw_prediction=raw_prediction, sample_weight=sample_weight, loss_out=out_l2),
    )
    assert_allclose(
        loss.gradient(y_true=y_true, raw_prediction=raw_prediction, sample_weight=sample_weight, gradient_out=out_g1),
        loss.closs.gradient(y_true=y_true, raw_prediction=raw_prediction, sample_weight=sample_weight, gradient_out=out_g2),
    )
    loss.closs.loss_gradient(y_true=y_true, raw_prediction=raw_prediction, sample_weight=sample_weight, loss_out=out_l1, gradient_out=out_g1)
    loss.closs.loss_gradient(y_true=y_true, raw_prediction=raw_prediction, sample_weight=sample_weight, loss_out=out_l2, gradient_out=out_g2)
    assert_allclose(out_l1, out_l2)
    assert_allclose(out_g1, out_g2)
    loss.gradient_hessian(y_true=y_true, raw_prediction=raw_prediction, sample_weight=sample_weight, gradient_out=out_g1, hessian_out=out_h1)
    loss.closs.gradient_hessian(y_true=y_true, raw_prediction=raw_prediction, sample_weight=sample_weight, gradient_out=out_g2, hessian_out=out_h2)
    assert_allclose(out_g1, out_g2)
...
def test_sample_weight_multiplies(loss, sample_weight, global_random_seed):
    ...
    assert_allclose(
        loss.loss(y_true=y_true, raw_prediction=raw_prediction, sample_weight=sample_weight),
        sample_weight * loss.loss(y_true=y_true, raw_prediction=raw_prediction, sample_weight=None))
...
def test_loss_of_perfect_prediction(loss, sample_weight):
    ...
    Loss of y_pred = y_true plus constant_to_optimal_zero should sums up to zero.
    """
    if not loss.is_multiclass:
        ...
        # raw_prediction with entries -exp(10), but +exp(10) on the diagonal this is close enough to np.inf which would produce nan
    ...
    # Comparing loss_value + constant_term to zero would result in large round-off errors.
...
def test_gradients_hessians_numerically(loss, sample_weight, global_random_seed):
    ...
    g, h = loss.gradient_hessian(y_true=y_true, raw_prediction=raw_prediction, sample_weight=sample_weight)
    ...
    if not loss.is_multiclass:
        def loss_func(x):
            return loss.loss(y_true=y_true, raw_prediction=x, sample_weight=sample_weight)

        g_numeric = numerical_derivative(loss_func, raw_prediction, eps=1e-6)
        assert_allclose(g, g_numeric, rtol=5e-6, atol=1e-10)

        def grad_func(x):
            return loss.gradient(y_true=y_true, raw_prediction=x, sample_weight=sample_weight)
        ...
        # For multiclass loss, we should only change the predictions of the class for which the derivative is taken for, 
        # e.g. offset[:, k] = eps for class k. As a softmax is computed, offsetting the whole array by a constant
        ...
        for k in range(loss.n_classes):
            ...
            def loss_func(x):
                ...
                raw[:, k] = x
                return loss.loss(y_true=y_true, raw_prediction=raw, sample_weight=sample_weight)
            ...
            def grad_func(x):
                raw = raw_prediction.copy()
                raw[:, k] = x
                return loss.gradient(y_true=y_true, raw_prediction=raw, sample_weight=sample_weight,)[:, k]
...
def test_derivatives(loss, x0, y_true):
    ...
    We check this on a single value/sample using Halley's method with the first and second order derivatives computed by the Loss instance.
    Note that methods of Loss instances operate on arrays while the newton root finder expects a scalar or a one-element array for this purpose.
    """
    ...
    def func(x: np.ndarray) -> np.ndarray:
        ...
        The constant term is such that the minimum function value is zero, which is required by the Newton method.
        """
...
def test_loss_intercept_only(loss, sample_weight):
    ...
    if not loss.is_multiclass:
        ...
        # The constraint corresponds to sum(raw_prediction) = 0. 
        # Without it, we would need to apply loss.symmetrize_raw_prediction to opt.x before comparing.
...
@pytest.mark.parametrize("loss, func, random_dist",
    [
...
def test_specific_fit_intercept_only(loss, func, random_dist, global_random_seed):
    ...
    # Make sure baseline prediction is the expected functional=func, e.g. mean or median.
...
def test_binomial_and_multinomial_loss(global_random_seed):
    ...
    assert_allclose(binom.loss(y_true=y_train, raw_prediction=raw_prediction), multinom.loss(y_true=y_train, raw_prediction=raw_multinom))
...
def test_binomial_vs_alternative_formulation(y_true, y_pred, global_dtype):
    ...
    Often, the binomial deviance or log loss is written in terms of a variable z in {-1, +1}, 
    but we use y in {0, 1}, hence z = 2 * y - 1.  ESL II Eq. (10.18):
...
def test_predict_proba(loss, global_random_seed):
    ...
    if hasattr(loss, "gradient_proba"):
        for grad, proba in ((None, None), (None, np.empty_like(raw_prediction)), (np.empty_like(raw_prediction), None), (np.empty_like(raw_prediction), np.empty_like(raw_prediction))):
            grad, proba = loss.gradient_proba(y_true=y_true, raw_prediction=raw_prediction, sample_weight=None, gradient_out=grad, proba_out=proba)
            assert proba.shape == (n_samples, loss.n_classes)
            assert np.sum(proba, axis=1) == approx(1, rel=1e-11)
            assert_allclose(grad, loss.gradient(y_true=y_true, raw_prediction=raw_prediction, sample_weight=None, gradient_out=None))
...
@pytest.mark.parametrize("loss, params, err_type, err_msg", [
        (PinballLoss, {"quantile": None}, TypeError,  "quantile must be an instance of float, not NoneType."),
        (PinballLoss, {"quantile": 0},    ValueError, "quantile == 0, must be > 0."),
        (PinballLoss, {"quantile": 1.1},  ValueError, "quantile == 1.1, must be < 1."),
        (HuberLoss,   {"quantile": None}, TypeError,  "quantile must be an instance of float, not NoneType."),
...
def test_tweedie_log_identity_consistency(p):
    ...
    # Let's compare the loss values, up to some constant term that is dropped in HalfTweedieLoss but not in HalfTweedieLossIdentity.
    ...
    # Note that HalfTweedieLoss ignores different constant terms than HalfTweedieLossIdentity. 
    # Constant terms means terms not depending on raw_prediction. By adding these terms, `constant_to_optimal_zero`, both losses give the same values.
