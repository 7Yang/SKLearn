...
def test_link_inverse_identity(link, global_random_seed):
    ...
    # The values for `raw_prediction` are limited from -20 to 20 because in the class `LogitLink` 
    # the term `expit(x)` comes very close to 1 for large positive x and therefore loses precision.
...
def test_link_out_argument(link):
    ...
    if link.is_multiclass:
        ...
        # So far, the valid interval of raw_prediction is (-inf, inf) and we do not need to distinguish.
        raw_prediction = rng.uniform(low=-10, high=10, size=(n_samples))
