...
class _BaseNB(ClassifierMixin, BaseEstimator, metaclass=ABCMeta):
    ...
    def _joint_log_likelihood(self, X):
        """
        Compute the unnormalized posterior log probability of X I.e. ``log P(c) + log P(x|c)`` for all rows x of X, 
        as an array-like of shape (n_samples, n_classes).

        Public methods predict, predict_proba, predict_log_proba, and predict_joint_log_proba 
        pass the input through _check_X before handing it over to _joint_log_likelihood. 
        The term "joint log likelihood" is used interchangibly with "joint log probability".
        """
    ...
    def predict_joint_log_proba(self, X):
        ...
        where ``log P(y)`` is the class prior probability and ``log P(x|y)`` is the class-conditional probability.
        ...
        C : ndarray of shape (n_samples, n_classes)
            Returns the joint log-probability of the samples for each class in the model. 
...
class GaussianNB(_BaseNB):
    ...
    def _update_mean_variance(n_past, mu, var, X, sample_weight=None):
        ...
        Given starting sample count, mean, and variance, a new set of points X, and optionally sample weights, 
        return the updated mean and variance. (NB - each dimension (column) in X is treated as independent 
        ...
        ----------
        n_past : int
            Number of samples represented in old mean and variance. If sample weights were given, 
            this should contain the sum of sample weights represented in old mean and variance.
        ...
        if sample_weight is not None:
            ...
            if np.isclose(n_new, 0.0): return mu, var
        ...
        if n_past == 0: return new_mu, new_var
        ...
        # Combine variance of old and new data, taking into consideration (weighted) number of observations. 
        # This is achieved by combining the sum-of-squared-differences (ssd)
        old_ssd   = n_past  * var
    ...
    def partial_fit(self, X, y, classes=None, sample_weight=None):
        ...
        This method has some performance and numerical stability overhead, hence it is better to call partial_fit on chunks of data 
        that are as large as possible (as long as fitting in the memory budget) to hide the overhead.
    ...
    def _partial_fit(self, X, y, classes=None, _refit=False, sample_weight=None):
        ...
        _refit : bool, default=False
            If true, act as though this were the first time we called _partial_fit (ie, throw away any past fitting and start over).
        ...
        # If the ratio of data variance between dimensions is too small, it will cause numerical errors. 
        # To address this, we artificially boost the variance by epsilon, a small fraction of the standard deviation of the largest dimension.
        self.epsilon_   = self.var_smoothing * np.var(X, axis=0).max()

        if first_call:
            # This is the first call to partial_fit: initialize various cumulative counters
            ...
            # Initialise the class prior Take into account the priors
            if self.priors is not None:
                priors = np.asarray(self.priors)
                # Check that the provided prior matches the number of classes
                if len(priors) != n_classes:          raise ValueError("Number of priors must match number of classes.")
                # Check that the sum is 1
                if not np.isclose(priors.sum(), 1.0): raise ValueError("The sum of the priors should be 1.")
                # Check that the priors are non-negative
                if (priors < 0).any():                raise ValueError("Priors must be non-negative.")
...
class _BaseDiscreteNB(_BaseNB):
    """
    Abstract base class for naive Bayes on discrete/categorical data Any estimator based on this class should provide:
    ...
    def _count(self, X, Y):
        ...
        Accordingly, this method is called each time `fit` or `partial_fit` update the model. 
        `class_count_` and `feature_count_` must be updated here along with any model specific counts.
    ...
    def _update_class_log_prior(self, class_prior=None):
        ...
        if class_prior is not None:
            ...
            with warnings.catch_warnings():
                # silence the warning when count is 0 because class was not yet observed
    ...
    def _check_alpha(self):
        ...
        if isinstance(alpha, np.ndarray):
            if not alpha.shape[0] == self.n_features_in_:
                raise ValueError("When alpha is an array, it should contains `n_features`. Got {alpha.shape[0]} elements instead of {self.n_features_in_}.")
        ...
        if alpha_min < alpha_lower_bound and not _force_alpha:
            warnings.warn("alpha too small will result in numeric errors, setting alpha = {alpha_lower_bound:.1e}. Use `force_alpha=True` to keep alpha unchanged.")
    ...
    def partial_fit(self, X, y, classes=None, sample_weight=None):
        ...
        This method has some performance overhead hence it is better to call partial_fit on chunks of data 
        that are as large as possible (as long as fitting in the memory budget) to hide the overhead.
        ...
        # XXX: OPTIM: we could introduce a public finalization method to be called by the user explicitly just once after several consecutive
        # calls to partial_fit and prior any call to predict[_[log_]proba] to avoid computing the smooth log probas at each call to partial fit
    ...
    def fit(self, X, y, sample_weight=None):
        ...
        # We convert it to np.float64 to support sample_weight consistently; this means we also don't have to cast X to floating point
        if sample_weight is not None:
            Y             = Y.astype(np.float64, copy=False)
            sample_weight = _check_sample_weight(sample_weight, X)
            sample_weight = np.atleast_2d(sample_weight)
...
class MultinomialNB(_BaseDiscreteNB):
    """
    Naive Bayes classifier for multinomial(多项) models.
...
class ComplementNB(_BaseDiscreteNB):
    ...
    norm : bool, default=False
        Whether or not a second normalization of the weights is performed. The default behavior mirrors the implementations found in Mahout and Weka,
...
class CategoricalNB(_BaseDiscreteNB):
    ...
    def fit(self, X, y, sample_weight=None):
        ...
        X : {array-like, sparse matrix} of shape (n_samples, n_features)
            ...
            It is further assumed that all categories of each feature are represented by the numbers 0, ..., n - 1, where n refers to the total number of categories for the given feature. 
            This can, for instance, be achieved with the help of OrdinalEncoder.
    ...
    def _validate_n_categories(X, min_categories):
        ...
        if min_categories is not None:
            if not np.issubdtype(min_categories_.dtype, np.signedinteger):
                raise ValueError("'min_categories' should have integral type. Got {min_categories_.dtype} instead.")
