...
def _smacof_single(dissimilarities, metric=True, n_components=2, init=None, max_iter=300, verbose=0, eps=1e-3, \
        random_state=None, normalized_stress=False):
    """
    ...
    eps : float, default=1e-3
        ...
        The value of `eps` should be tuned separately depending on whether or not `normalized_stress` is being used.
    ...
    normalized_stress : bool, default=False
        ...
        Only supported in non-metric MDS. The caller must ensure that if `normalized_stress=True` then `metric=False`
        .. versionadded:: 1.2
    ...
    stress : float
        ...
        If `normalized_stress=True`, and `metric=False` returns Stress-1.  A value of 0 indicates "perfect" fit, 0.025 excellent, 0.05 good,
...
def smacof(dissimilarities, *, metric=True, n_components=2, init=None, n_init=8, n_jobs=None, \
    max_iter=300, verbose=0, eps=1e-3, random_state=None, return_n_iter=False, normalized_stress="warn"):
    """
    Compute multidimensional scaling using the SMACOF algorithm.

    The SMACOF (Scaling by MAjorizing a COmplicated Function) algorithm is a multidimensional scaling algorithm 
    which minimizes an objective function (the *stress*) using a majorization technique. Stress majorization, 
    also known as the Guttman Transform, guarantees a monotone convergence of stress, 
    and is more powerful than traditional techniques such as gradient descent.
    ...
    init : array-like of shape (n_samples, n_components), default=None
        ...
        By default, the algorithm is initialized with a randomly chosen array.
    ...
    n_init : int, default=8
        ...
        If ``init`` is provided, this option is overridden and a single run is performed.

    n_jobs : int, default=None
        The number of jobs to use for the computation. 
        If multiple initializations are used (``n_init``), each run of the algorithm is computed in parallel.
    ...
    if normalized_stress == "warn":
        warnings.warn(("The default value of `normalized_stress` will change to `'auto'` in version 1.4. "
            "To suppress this warning, manually set the value of `normalized_stress`."), FutureWarning)
    ...
    if normalized_stress and metric:
        raise ValueError("Normalized stress is not supported for metric MDS. "
            "Either set `normalized_stress=False` or use `metric=False`.")
    ...
    if effective_n_jobs(n_jobs) == 1:
        for it in range(n_init):
            pos, stress, n_iter_ = _smacof_single(dissimilarities, metric=metric, n_components=n_components, init=init, \
                max_iter=max_iter, verbose=verbose, eps=eps, random_state=random_state, normalized_stress=normalized_stress)
        ...
        results = Parallel(n_jobs=n_jobs, verbose=max(verbose - 1, 0))(delayed(_smacof_single)(      \
            dissimilarities, metric=metric, n_components=n_components, init=init, max_iter=max_iter, \
            verbose=verbose, eps=eps, random_state=seed, normalized_stress=normalized_stress)
            for seed in seeds
        )
...
class MDS(BaseEstimator):
    ...
    Attributes
    ----------
    embedding_ : ndarray of shape (n_samples, n_components), Stores the position of the dataset in the embedding space.
    ...
    def __init__(self, n_components=2, *, metric=True, n_init=4, max_iter=300, verbose=0, eps=1e-3, \
        n_jobs=None, random_state=None, dissimilarity="euclidean", normalized_stress="warn"):
    ...
    def fit_transform(self, X, y=None, init=None):
        ...
        Returns
        -------
        X_new : ndarray of shape (n_samples, n_components), X transformed in the new space.
        """
        X = self._validate_data(X)
        if X.shape[0] == X.shape[1] and self.dissimilarity != "precomputed":
            warnings.warn("The MDS API has changed. ``fit`` now constructs an dissimilarity matrix from data. "
                "To use a custom dissimilarity matrix, set ``dissimilarity='precomputed'``.")
        ...
        self.embedding_, self.stress_, self.n_iter_ = smacof(self.dissimilarity_matrix_, metric=self.metric, \
            n_components=self.n_components, init=init, n_init=self.n_init, n_jobs=self.n_jobs, max_iter=self.max_iter, \
            verbose=self.verbose, eps=self.eps, random_state=self.random_state, return_n_iter=True, normalized_stress=self.normalized_stress)
