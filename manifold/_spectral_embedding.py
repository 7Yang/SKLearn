...
def _graph_is_connected(graph):
    ...
    Returns
    -------
    is_connected : bool, True means the graph is fully connected and False means not.
    """
    if sparse.issparse(graph):
...
def _set_diag(laplacian, value, norm_laplacian):
    ...
    if not sparse.issparse(laplacian):
        ...
        # If the matrix has a small number of diagonals (as in the case of structured matrices coming from images), 
        # the dia format might be best suited for matvec products:
        n_diags = np.unique(laplacian.row - laplacian.col).size
        if n_diags <= 7:
            # 3 or less outer diagonals on each side
            laplacian = laplacian.todia()
        else:
            # csr has the fastest matvec and is thus best suited to arpack
...
def spectral_embedding(adjacency, *, n_components=8, eigen_solver=None, random_state=None, eigen_tol="auto", norm_laplacian=True, drop_first=True):
    ...
    The adjacency matrix is used to compute a normalized graph Laplacian whose spectrum (especially the eigenvectors associated to 
    the smallest eigenvalues) has an interpretation in terms of minimal number of cuts necessary to split the graph into 
    comparably sized components.

    This embedding can also 'work' even if the ``adjacency`` variable is not strictly the adjacency matrix of a graph but more generally
    an affinity or similarity matrix between samples (for instance the heat kernel of a euclidean distance matrix or a k-NN matrix).
    ...
    eigen_solver : {'arpack', 'lobpcg', 'amg'}, default=None
        ...
        It can be faster on very large, sparse problems, but may also lead to instabilities. If None, then ``'arpack'`` is used.

    random_state : int, RandomState instance or None, default=None
        A pseudo random number generator used for the initialization of the lobpcg eigen vectors decomposition 
    ...
    drop_first : bool, default=True
        Whether to drop the first eigenvector. For spectral embedding, 
        this should be True as the first eigenvector should be constant vector for connected graph, 
    ...
    Notes
    -----
    Spectral Embedding (Laplacian Eigenmaps) is most useful when the graph has one connected component. 
    If there graph has many components, the first few eigenvectors will simply uncover the connected components of the graph.

    References
    ----------
    * https://en.wikipedia.org/wiki/LOBPCG
    * :doi:`"Toward the Optimal Preconditioned Eigensolver: Locally Optimal Block Preconditioned Conjugate Gradient Method",
      Andrew V. Knyazev <10.1137/S1064827500366124>`
    """
    ...
    if (eigen_solver == "arpack" or eigen_solver != "lobpcg" and (not sparse.issparse(laplacian) or n_nodes < 5 * n_components)):
        ...
        # or matlab: https://www.mathworks.com/matlabcentral/fileexchange/48-lobpcg-m
        ...
        try:
            ...
            # When submatrices are exactly singular, an LU decomposition in arpack fails. We fallback to lobpcg
            eigen_solver = "lobpcg"
        ...
        # The Laplacian matrix is always singular, having at least one zero eigenvalue, 
        # corresponding to the trivial eigenvector, which is a constant. 
        # Using a singular matrix for preconditioning may result in random failures in LOBPCG and is not supported by the existing
        # theory:
        #     see https://doi.org/10.1007/s10208-015-9297-1
        # Shift the Laplacian so its diagononal is not all ones. 
        # The shift does change the eigenpairs however, so we'll feed the shifted matrix to the solver and afterward set it back to the original.
    ...
    if eigen_solver == "lobpcg":
        ...
        if n_nodes < 5 * n_components + 1:
            # see note above under arpack why lobpcg has problems with small number of nodes
            ...
            laplacian = _set_diag(laplacian, 1, norm_laplacian)
            # We increase the number of eigenvectors requested, 
            # as lobpcg doesn't behave well in low dimension and create initial approximation X to eigenvectors
            X = random_state.standard_normal(size=(laplacian.shape[0], n_components + 1))
            X[:, 0] = dd.ravel()
            X = X.astype(laplacian.dtype)
            tol = None if eigen_tol == "auto" else eigen_tol
...
class SpectralEmbedding(BaseEstimator):
    ...
    eigen_tol : float, default="auto"
        ...
        - If `eigen_solver="arpack"`, then `eigen_tol=0.0`;
        - If `eigen_solver="lobpcg"` or `eigen_solver="amg"`, 
          then `eigen_tol=None` which configures the underlying `lobpcg` solver to automatically resolve the value according to their heuristics. 
          See, :func:`scipy.sparse.linalg.lobpcg` for details.
    ...
    - :doi:`A Tutorial on Spectral Clustering, 2007 Ulrike von Luxburg <10.1007/s11222-007-9033-z>`
    ...
    - :doi:`Normalized cuts and image segmentation, 2000 Jianbo Shi, Jitendra Malik
      <10.1109/34.868688>`
    ...
    _parameter_constraints: dict = {
        ...
        "affinity": [ StrOptions({ "nearest_neighbors", "rbf", "precomputed", "precomputed_nearest_neighbors"}), callable ],
    ...
    def __init__(self, n_components=2, *, affinity="nearest_neighbors", gamma=None, random_state=None, \
    ...
    def _get_affinity_matrix(self, X, Y=None):
        ...
        X : array-like of shape (n_samples, n_features)
            ...
            If affinity is "precomputed" X : array-like of shape (n_samples, n_samples),
            Interpret X as precomputed adjacency graph computed from samples.
    ...
    def fit(self, X, y=None):
        ...
        X : {array-like, sparse matrix} of shape (n_samples, n_features)
            ...
            If affinity is "precomputed" X : {array-like, sparse matrix}, shape (n_samples, n_samples),
            Interpret X as precomputed adjacency graph computed from samples.
        ...
        self.embedding_ = spectral_embedding(affinity_matrix, n_components=self.n_components, eigen_solver=self.eigen_solver,
            eigen_tol=self.eigen_tol, random_state=random_state)
        return self
    ...
    def fit_transform(self, X, y=None):
        ...
        X : {array-like, sparse matrix} of shape (n_samples, n_features)
            ...
            X : {array-like, sparse matrix} of shape (n_samples, n_samples), Interpret X as precomputed adjacency graph computed from samples.
        ...
        Returns
        -------
        X_new : array-like of shape (n_samples, n_components), Spectral embedding of the training matrix.
        """
        self.fit(X)
