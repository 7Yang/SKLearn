...
# This is the exact and Barnes-Hut t-SNE implementation. There are other modifications of the algorithm:
# * Fast Optimization for t-SNE: https://cseweb.ucsd.edu/~lvdmaaten/workshops/nips2010/papers/vandermaaten.pdf
...
def _joint_probabilities(distances, desired_perplexity, verbose):
    ...
    distances : ndarray of shape (n_samples * (n_samples-1) / 2,)
        ...
        we omit the diagonal and duplicate entries and store everything in a one-dimensional array.

    desired_perplexity : float, Desired perplexity of the joint probability distributions.

    verbose : int, Verbosity level.

    Returns
    -------
    P : ndarray of shape (n_samples * (n_samples-1) / 2,), Condensed joint probability matrix.
    """
...
def _joint_probabilities_nn(distances, desired_perplexity, verbose):
    ...
    This method is approximately equal to _joint_probabilities. 
    The latter is O(N), but limiting the joint probability to nearest neighbors improves this substantially to O(uN).

    Parameters
    ----------
    distances : sparse matrix of shape (n_samples, n_samples)
        Distances of samples to its n_neighbors nearest neighbors. 
        All other distances are left to zero (and are not materialized in memory).  Matrix should be of CSR format.
    ...
    P : sparse matrix of shape (n_samples, n_samples)
        Condensed joint probability matrix with only nearest neighbors. Matrix will be of CSR format.
    """
    t0 = time()
    # Compute conditional probabilities such that they approximately match the desired perplexity
...
def _kl_divergence(params, P, degrees_of_freedom, n_samples, n_components, skip_num_points=0, compute_error=True):
    ...
    P : ndarray of shape (n_samples * (n_samples-1) / 2,) Condensed joint probability matrix.
    degrees_of_freedom : int, Degrees of freedom of the Student's-t distribution.
    n_samples    : int, Number of samples.
    n_components : int, Dimension of the embedded space.

    skip_num_points : int, default=0
        This does not compute the gradient for points with indices below `skip_num_points`. 
        This is useful when computing transforms of new data where you'd like to keep the old data fixed.
...
def _kl_divergence_bh(params, P, degrees_of_freedom, n_samples, n_components, angle=0.5,
    skip_num_points=0, verbose=False, compute_error=True, num_threads=1):
    """
    t-SNE objective function: KL divergence of p_ijs and q_ijs.
    Uses Barnes-Hut tree methods to calculate the gradient that runs in O(NlogN) instead of O(N^2).
    ...
    P : sparse matrix of shape (n_samples, n_sample)
        Sparse approximate joint probability matrix, computed only for the k nearest-neighbors and symmetrized. Matrix should be of CSR format.
    ...
    num_threads : int, default=1
        ...
        This is set here to avoid calling _openmp_effective_n_threads for each gradient step.
    ...
    error = _barnes_hut_tsne.gradient(val_P, X_embedded, neighbors, indptr, grad, angle, \
        n_components, verbose, dof=degrees_of_freedom, compute_error=compute_error, num_threads=num_threads)
    c = 2.0 * (degrees_of_freedom + 1.0) / degrees_of_freedom
    grad = grad.ravel()
    grad *= c
...
def _gradient_descent(objective, p0, it, n_iter, n_iter_check=1, n_iter_without_progress=300, momentum=0.8, \
    learning_rate=200.0, min_gain=0.01, min_grad_norm=1e-7, verbose=0, args=None, kwargs=None):
    """
    ...
    ----------
    objective : callable
        Should return a tuple of cost and gradient for a given parameter vector.
        When expensive to compute, the cost can optionally be None and can be computed every n_iter_check steps using the objective_error function.

    p0 : array-like of shape (n_params,)
        Initial parameter vector.

    it : int
        Current number of iterations (this function will be called more than once during the optimization).

    n_iter : int
        Maximum number of gradient descent iterations.

    n_iter_check : int, default=1
        Number of iterations before evaluating the global error. If the error is sufficiently low, we abort the optimization.

    n_iter_without_progress : int, default=300
        Maximum number of iterations without progress before we abort the optimization.

    momentum : float within (0.0, 1.0), default=0.8
        The momentum generates a weight for previous gradients that decays exponentially.
...
def trustworthiness(X, X_embedded, *, n_neighbors=5, metric="euclidean"):
    ...
    where for each sample i, :math:`\mathcal{N}_{i}^{k}` are its k nearest neighbors in the output space, 
    and every sample j is its :math:`r(i, j)`-th nearest neighbor in the input space. 
    ...
    n_neighbors : int, default=5
        ...
        Should be fewer than `n_samples / 2` to ensure the trustworthiness to lies within [0, 1], as mentioned in [1]_. 
        An error will be raised otherwise.

    metric : str or callable, default='euclidean'
        Which metric to use for computing pairwise distances between samples from the original input space. 
        If metric is 'precomputed', X must be a matrix of pairwise distances or squared distances. 
        Otherwise, for a list of available metrics, see the documentation of argument metric in
    ...
    References
    ----------
    .. [1] Jarkko Venna and Samuel Kaski. 2001. Neighborhood Preservation in Nonlinear Projection Methods: An Experimental Study.
           In Proceedings of the International Conference on Artificial Neural Networks (ICANN '01). Springer-Verlag, Berlin, Heidelberg, 485-491.

    .. [2] Laurens van der Maaten. Learning a Parametric Embedding by Preserving Local Structure. 
           Proceedings of the Twelfth International Conference on Artificial Intelligence and Statistics, PMLR 5:384-391, 2009.
    """
    ...
    if metric == "precomputed":
        dist_X = dist_X.copy()
    # we set the diagonal to np.inf to exclude the points themselves from their own neighborhood
    ...
    # we define `inverted_index[i]` as the inverted index of sorted distances: inverted_index[i][ind_X[i]] = np.arange(1, n_sample + 1)
...
class TSNE(ClassNamePrefixFeaturesOutMixin, TransformerMixin, BaseEstimator):
    ...
    (e.g. PCA for dense data or TruncatedSVD for sparse data) 
    to reduce the number of dimensions to a reasonable amount (e.g. 50)
    if the number of features is very high. This will suppress some noise 
    and speed up the computation of pairwise distances between samples. 
    For more tips see Laurens van der Maaten's FAQ [2].
    ...
    early_exaggeration : float, default=12.0
        ...
        Again, the choice of this parameter is not very critical. If the cost function increases during initial optimization, 
    ...
    learning_rate : float or "auto", default="auto"
        ...
        Note that many other t-SNE implementations (bhtsne, FIt-SNE, openTSNE, etc.) 
        use a definition of learning_rate that is 4 times smaller than ours. 
        So our learning_rate=200 corresponds to learning_rate=800 in those other implementations. 
        The 'auto' option sets the learning_rate to `max(N / early_exaggeration / 4, 50)` where N is the sample size, following [4] and [5].
    ...
    n_jobs : int, default=None
        ...
        This parameter has no impact when ``metric="precomputed"`` or (``metric="euclidean"`` and ``method="exact"``).
    ...
    [4] Belkina, A. C., Ciccolella, C. O., Anno, R., Halpert, R., Spidlen, J., & Snyder-Cappione, J. E. (2019). 
    ...
    _parameter_constraints: dict = {
        ...
        "learning_rate": [ StrOptions({"auto"}), Interval(Real, 0, None, closed="neither") ],
    ...
    def __init__(self, n_components=2, *, perplexity=30.0, early_exaggeration=12.0, learning_rate="auto", n_iter=1000,  \
        n_iter_without_progress=300, min_grad_norm=1e-7, metric="euclidean", metric_params=None, init="pca", verbose=0, \
    ...
    def _fit(self, X, skip_num_points=0):
        ...
        if self.metric == "precomputed":
            ...
            if self.method == "exact" and issparse(X):
                raise TypeError('TSNE with method="exact" does not accept sparse precomputed distance matrix. Use method="barnes_hut" '
                    "or provide the dense distance matrix.")

        if self.method == "barnes_hut" and self.n_components > 3:
            raise ValueError("'n_components' should be inferior to 4 for the barnes_hut algorithm as it relies on "
        ...
        if self.method == "exact":
            # Retrieve the distance matrix, either using the precomputed one or computing it.
            if self.metric == "precomputed":
                distances = X
            else:
                if self.verbose:
                ...
                if self.metric == "euclidean":
                    ...
                    # because euclidean_distances already calculates squared distances, and returns np.sqrt(dist) for squared=False.
            ...
            # Compute the number of nearest neighbors to find. LvdM uses 3 * perplexity as the number of neighbors.
            # In the event that we have very small # of points set the neighbors to n - 1.
            ...
            knn = NearestNeighbors(algorithm="auto", n_jobs=self.n_jobs, n_neighbors=n_neighbors, \
            ...
            # knn return the euclidean distance but we need it squared to be consistent with the 'exact' method. 
            # Note that the the method was derived using the euclidean method as in the input space. 
            # Not sure of the implication of using a different metric.
            distances_nn.data **= 2
        ...
        if isinstance(self.init, np.ndarray):
            ...
            # PCA is rescaled so that PC1 has standard deviation 1e-4 which is the default value for random initialization. See issue #18018.
        ...
        # Degrees of freedom of the Student's t-distribution. 
        # The suggestion degrees_of_freedom = n_components - 1 comes from "Learning a Parametric Embedding by Preserving Local Structure"
    ...
    def _tsne(self, P, degrees_of_freedom, n_samples, X_embedded, neighbors=None, skip_num_points=0):
        """Runs t-SNE."""
        # t-SNE minimizes the Kullback-Leiber divergence of the Gaussians P and the Student's t-distributions Q. 
        # The optimization algorithm that we use is batch gradient descent with two stages:
        ...
        if self.method == "barnes_hut":
            ...
            # Get the number of threads for gradient computation here to avoid recomputing it at each iteration.
        ...
        # Learning schedule (part 2): disable early exaggeration and finish optimization with a higher momentum at 0.8
        P /= self.early_exaggeration
    ...
    def fit_transform(self, X, y=None):
        ...
        X : {array-like, sparse matrix} of shape (n_samples, n_features) or (n_samples, n_samples)
            If the metric is 'precomputed' X must be a square distance matrix. Otherwise it contains a sample per row. 
            If the method is 'exact', X may be a sparse matrix of type 'csr', 'csc' or 'coo'. 
            If the method is 'barnes_hut' and the metric is 'precomputed', X may be a precomputed sparse graph.
        ...
        Returns
        -------
        X_new : ndarray of shape (n_samples, n_components), Embedding of the training data in low-dimensional space.
        """
    ...
    def fit(self, X, y=None):
        ...
        Returns
        -------
        X_new : array of shape (n_samples, n_components), Embedding of the training data in low-dimensional space.
        """
