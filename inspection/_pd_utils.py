def _check_feature_names(X, feature_names=None):
    ...
    feature_names : list of str
        ...
        i.e. the column names of a pandas dataframe or a generic list of feature names (e.g. `["x0", "x1", ...]`) for a NumPy array.
    """
...
def _get_feature_index(fx, feature_names=None):
    ...
    if isinstance(fx, str):
        if feature_names is None:
            raise ValueError(f"Cannot plot partial dependence for feature {fx!r} since the list of feature names was not provided, "\
                "neither as column names of a pandas data-frame nor via the feature_names parameter.")
        try:
