...
def _calculate_permutation_scores(estimator, X, y, sample_weight, col_idx, random_state, n_repeats, scorer, max_samples):
    ...
    # Work on a copy of X to ensure thread-safety in case of threading based parallelism. 
    # Furthermore, making a copy is also useful when the joblib backend is 'loky' (default) or the old 'multiprocessing': 
    # in those cases, if X is large it will be automatically be backed by a readonly memory map (memmap). 
    # X.copy() on the other hand is always guaranteed to return a writable data-structure whose columns can be shuffled inplace.
...
def _create_importances_bunch(baseline_score, permuted_score):
    ...
    Parameters
    ----------
    baseline_score : ndarray of shape (n_features,), The baseline score without permutation.
    permuted_score : ndarray of shape (n_features, n_repeats), The permuted scores for the `n` repetitions.
...
def permutation_importance(estimator, X, y, *, scoring=None, n_repeats=5, n_jobs=None, random_state=None, sample_weight=None, max_samples=1.0):
    ...
    The :term:`estimator` is required to be a fitted estimator. `X` can be the data set used to train the estimator or a hold-out set. 
    The permutation importance of a feature is calculated as follows. First, a baseline metric,
    defined by :term:`scoring`, is evaluated on a (potentially different) dataset defined by the `X`. 
    ...
    ----------
    estimator : object
        An estimator that has already been :term:`fitted` and is compatible with :term:`scorer`.
    ...
    scoring : str, callable, list, tuple, or dict, default=None
        ...
        Passing multiple scores to `scoring` is more efficient than calling `permutation_importance` for each of the scores 
        as it reuses predictions to avoid redundant computation.
    ...
    max_samples : int or float, default=1.0
        ...
        it keeps the method tractable when evaluating feature importance on large datasets. 
    ...
    result : :class:`~sklearn.utils.Bunch` or dict of such instances
        ...
        If there are multiple scoring metrics in the scoring parameter `result` is a dict with scorer names as keys (e.g. 'roc_auc') 
        and `Bunch` objects like above as values.

    References
    ----------
    .. [BRE] :doi:`L. Breiman, "Random Forests", Machine Learning, 45(1), 5-32, 2001. <10.1023/A:1010933404324>`
    ...
    # Precompute random seed from the random state to be used to get a fresh independent RandomState instance 
    # for each parallel call to _calculate_permutation_scores, 
    # irrespective of the fact that variables are shared or not depending on the active joblib backend (sequential, thread-based or process-based).
    random_state = check_random_state(random_state)
    ...
    scores = Parallel(n_jobs=n_jobs)(delayed(_calculate_permutation_scores)(
            estimator, X, y, sample_weight, col_idx, random_seed, n_repeats, scorer, max_samples) for col_idx in range(X.shape[1]))

    if isinstance(baseline_score, dict):
        return {
            # unpack the permuted scores
            name: _create_importances_bunch(baseline_score[name], np.array([scores[col_idx][name] for col_idx in range(X.shape[1])]))
