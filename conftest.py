# Even if empty this file is useful so that when running from the root folder ./sklearn is added to sys.path by pytest. 
# See https://docs.pytest.org/en/latest/explanation/pythonpath.html for more details. 
# For example, this allows to build extensions in place and run pytest doc/modules/clustering.rst and use sklearn from the local folder rather than the one from site-packages.
import sys
import builtins
import platform
...
from sklearn.datasets import (fetch_20newsgroups, fetch_20newsgroups_vectorized, fetch_california_housing, \
        fetch_covtype, fetch_kddcup99, fetch_olivetti_faces, fetch_rcv1)

if parse_version(pytest.__version__) < parse_version(PYTEST_MIN_VERSION):
    raise ImportError(f"Your version of pytest is too old. Got version {pytest.__version__}, you should have pytest >= {PYTEST_MIN_VERSION} installed.")
...
_SKIP32_MARK = pytest.mark.skipif(environ.get("SKLEARN_RUN_FLOAT32_TESTS", "0") != "1", reason="Set SKLEARN_RUN_FLOAT32_TESTS=1 to run float32 dtype tests")
...
def pytest_collection_modifyitems(config, items):
    ...
    # download datasets during collection to avoid thread unsafe behavior when running pytest in parallel with pytest-xdist
    dataset_features_set = set(dataset_fetchers)
    datasets_to_download = set()

    for item in items:
        if not hasattr(item, "fixturenames"): continue

        item_fixtures = set(item.fixturenames)
        dataset_to_fetch = item_fixtures & dataset_features_set
        if not dataset_to_fetch: continue
    ...
    # Only download datasets on the first worker spawned by pytest-xdist to avoid thread unsafe behavior. 
    # If pytest-xdist is not used, we still download before tests run.
    ...
    if skip_doctests:
        ...
        for item in items:
            if isinstance(item, DoctestItem):
                # work-around an internal error with pytest if adding a skip mark to a doctest in a contextmanager, 
                # see https://github.com/pytest-dev/pytest/issues/8796 for more details.
...
def pyplot():
    ...
    This fixture checks if we can import matplotlib. 
    If not, the tests will be skipped. Otherwise, we close the figures before and after running the functions.
...
def pytest_configure(config):
    ...
    if xdist_worker_count is not None:
        # Set the number of OpenMP and BLAS threads based on the number of workers xdist is using to prevent oversubscription.
