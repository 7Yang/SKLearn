...
class _BinaryGaussianProcessClassifierLaplace(BaseEstimator):
    ...
    optimizer : 'fmin_l_bfgs_b' or callable, default='fmin_l_bfgs_b'
        ...
        If None is passed, the kernel's parameters are kept fixed. Available internal optimizers are::
    ...
    warm_start : bool, default=False
        If warm-starts are enabled, the solution of the last Newton iteration on the Laplace approximation of the posterior mode 
        is used as initialization for the next call of _posterior_mode(). 
    ...
    copy_X_train : bool, default=True
        ...
        Otherwise, just a reference to the training data is stored, 
    ...
    pi_ : array-like of shape (n_samples,)
        The probabilities of the positive class for the training points X_train_

    W_sr_ : array-like of shape (n_samples,)
        Square root of W, the Hessian of log-likelihood of the latent function values for the observed labels. 
        Since W is diagonal, only the diagonal of sqrt(W) is stored.
    ...
    References
    ----------
    .. [RW2006] `Carl E. Rasmussen and Christopher K.I. Williams, "Gaussian Processes for Machine Learning",
    ...
    def predict(self, X):
        ...
        # As discussed on Section 3.4.2 of GPML, for making hard binary decisions, 
        # it is enough to compute the MAP of the posterior and pass it through the link function
    ...
    def predict_proba(self, X):
        ...
        # Approximation is due to Williams & Barber, 
        # "Bayesian Classification with Gaussian Processes", 
        # Appendix A: Approximate the logistic sigmoid by a linear combination of 5 error functions.
        # For information on how this integral can be computed see blitiri.blogspot.de/2012/11/gaussian-integral-of-error-function.html
    ...
    def log_marginal_likelihood(self, theta=None, eval_gradient=False, clone_kernel=True):
        ...
        eval_gradient : bool, default=False
            If True, the gradient of the log-marginal likelihood with respect to the kernel hyperparameters 
            at position theta is returned additionally. If True, theta must not be None.
        ...
        log_likelihood_gradient : ndarray of shape (n_kernel_params), optional
            Gradient of the log-marginal likelihood with respect to the kernel hyperparameters at position theta.
    ...
    def _posterior_mode(self, K, return_temporaries=False):
        ...
        This approximates the posterior of the latent function values for given inputs and target observations with a Gaussian approximation 
        and uses Newton's iteration to find the mode of this approximation.
        """
        # Based on Algorithm 3.1 of GPML

        # If warm_start are enabled, we reuse the last solution for the posterior mode as initialization; otherwise, we initialize with 0
        ...
        for _ in range(self.max_iter_predict):
            ...
            # Check if we have converged (log marginal likelihood does not decrease)
...
class GaussianProcessClassifier(ClassifierMixin, BaseEstimator):
    ...
    kernel : kernel instance, default=None
        ...
        If None is passed, the kernel "1.0 * RBF(1.0)" is used as default. 
        Note that the kernel's hyperparameters are optimized during fitting. 
        Also kernel cannot be a `CompoundKernel`.
    ...
    optimizer : 'fmin_l_bfgs_b', callable or None, default='fmin_l_bfgs_b'
        ...
        If a callable is passed, it must have the  signature::
            ...
            def optimizer(obj_func, initial_theta, bounds):
                ...
                #   which takes the hyperparameters theta as parameter and an optional flag eval_gradient, 
                #   which determines if the gradient is returned additionally to the function value
                # * 'initial_theta': the initial value for theta, which can be used by local optimizers
                # * 'bounds': the bounds on the values of theta
                ....
                # Returned are the best found hyperparameters theta and the corresponding value of the target function.
                return theta_opt, func_min
    ...
    multi_class : {'one_vs_rest', 'one_vs_one'}, default='one_vs_rest'
        Specifies how multi-class classification problems are handled.  Supported are 'one_vs_rest' and 'one_vs_one'. In 'one_vs_rest',
        one binary Gaussian process classifier is fitted for each class, which is trained to separate this class from the rest. 
    ...
    kernel_ : kernel instance
        ...
        In case of multi-class classification, a CompoundKernel is returned which consists of the different kernels 
        used in the one-versus-rest classifiers.
    ...
    def fit(self, X, y):
        ...
        self.base_estimator_ = _BinaryGaussianProcessClassifierLaplace(kernel=self.kernel, \
            optimizer=self.optimizer, n_restarts_optimizer=self.n_restarts_optimizer,      \
            max_iter_predict=self.max_iter_predict, warm_start=self.warm_start,            \
            copy_X_train=self.copy_X_train, random_state=self.random_state)

        self.classes_ = np.unique(y)
        self.n_classes_ = self.classes_.size
        if self.n_classes_ == 1:
            raise ValueError("GaussianProcessClassifier requires 2 or more distinct classes; got %d class (only class %s is present)" 
    ...
    def predict_proba(self, X):
        ...
        if self.n_classes_ > 2 and self.multi_class == "one_vs_one":
            raise ValueError("one_vs_one multi-class mode does not support predicting probability estimates. "
                "Use one_vs_rest mode instead.")
    ...
    def log_marginal_likelihood(self, theta=None, eval_gradient=False, clone_kernel=True):
        ...
        theta : array-like of shape (n_kernel_params,), default=None
            ...
            theta may be the  hyperparameters of the compound kernel or of an individual kernel. 
            In the latter case, all individual kernel get assigned the same theta values. 
            If None, the precomputed log_marginal_likelihood of ``self.kernel_.theta`` is returned.

        eval_gradient : bool, default=False
            If True, the gradient of the log-marginal likelihood with respect to the kernel hyperparameters 
            at position theta is returned additionally. 
            Note that gradient computation is not supported for non-binary classification. If True, theta must not be None.

        clone_kernel : bool, default=True
            If True, the kernel attribute is copied. If False, the kernel attribute is modified, 
        ...
        if self.n_classes_ == 2:
            ...
            if theta.shape[0] == n_dims:  # use same theta for all sub-kernels
                return np.mean([estimator.log_marginal_likelihood(theta, clone_kernel=clone_kernel)
                    for i, estimator in enumerate(estimators)])
            elif theta.shape[0] == n_dims * self.classes_.shape[0]:
                # theta for compound kernel
                return np.mean([estimator.log_marginal_likelihood(theta[n_dims * i : n_dims * (i + 1)], clone_kernel=clone_kernel)
                    for i, estimator in enumerate(estimators)]
                )
            else:
                raise ValueError("Shape of theta must be either %d or %d. Obtained theta with shape %d."
