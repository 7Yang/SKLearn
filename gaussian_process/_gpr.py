...
class GaussianProcessRegressor(MultiOutputMixin, RegressorMixin, BaseEstimator):
    ...
    In addition to standard scikit-learn estimator API, :class:`GaussianProcessRegressor`:
       ...
       * exposes a method `log_marginal_likelihood(theta)`, which can be used externally for other ways of selecting hyperparameters, 
           e.g., via Markov chain Monte Carlo.

    To learn the difference between a point-estimate approach vs. 
    a more Bayesian modelling approach, refer to the example entitled :ref:`sphx_glr_auto_examples_gaussian_process_plot_compare_gpr_krr.py`.
    ...
    kernel : kernel instance, default=None
        ...
        If None is passed, the kernel ``ConstantKernel(1.0, constant_value_bounds="fixed") * RBF(1.0, length_scale_bounds="fixed")`` 
        is used as default. Note that the kernel hyperparameters are optimized during fitting unless the bounds are marked as "fixed".
    ...
    alpha : float or ndarray of shape (n_samples,), default=1e-10
        ...
        Note that this is different from using a `WhiteKernel`. 
        If an array is passed, it must have the same number of entries as the data used for fitting 
        and is used as datapoint-dependent noise level. 
        Allowing to specify the noise level directly as a parameter is mainly for convenience 
        and for consistency with :class:`~sklearn.linear_model.Ridge`.
    ...
    optimizer : "fmin_l_bfgs_b", callable or None, default="fmin_l_bfgs_b"
        ...
        If a callable is passed, it must have the signature::
            ...
            def optimizer(obj_func, initial_theta, bounds):
                ...
                #   which takes the hyperparameters theta as a parameter 
                #   and an optional flag eval_gradient, which determines if the gradient is returned additionally to the function value
        ...
        If None is passed, the kernel's parameters are kept fixed.  Available internal optimizers are: `{'fmin_l_bfgs_b'}`.
    ...
    n_targets : int, default=None
        ...
        Used to decide the number of outputs when sampling from the prior distributions (i.e. calling
        :meth:`sample_y` before :meth:`fit`). This parameter is ignored once :meth:`fit` has been called.
    ...
    kernel_ : kernel instance
        The kernel used for prediction. The structure of the kernel is the same as the one passed as parameter 
    ...
    def fit(self, X, y):
        ...
        if self.n_targets is not None and n_targets_seen != self.n_targets:
            raise ValueError("The number of targets seen in `y` is different from the parameter `n_targets`. "\
                "Got {n_targets_seen} != {self.n_targets}.")
        ...
        if np.iterable(self.alpha) and self.alpha.shape[0] != y.shape[0]:
            if self.alpha.shape[0] == 1:
                ...
                raise ValueError("alpha must be a scalar or an array with same number of entries as y. ({self.alpha.shape[0]} != {y.shape[0]})")
        ...
        try:
            ...
            exc.args = ((f"The kernel, {self.kernel_}, is not returning a positive definite matrix. " \
                "Try gradually increasing the 'alpha' parameter of your GaussianProcessRegressor estimator.")) + exc.args
            raise
    ...
    def predict(self, X, return_std=False, return_cov=False):
        ...
        optionally also returns its standard deviation (`return_std=True`) or covariance (`return_cov=True`). 
        ...
        if not hasattr(self, "X_train_"):  # Unfitted;predict based on GP prior
            ...
            if return_cov:
                ...
                # Use einsum to avoid explicitly forming the large matrix V^T @ V just to extract its diagonal afterward.
    ...
    def sample_y(self, X, n_samples=1, random_state=0):
        ...
        if y_mean.ndim == 1:
            ...
            y_samples = [rng.multivariate_normal(y_mean[:, target], y_cov[..., target], n_samples).T[:, np.newaxis]
    ...
    def log_marginal_likelihood(self, theta=None, eval_gradient=False, clone_kernel=True):
        ...
        eval_gradient : bool, default=False
            If True, the gradient of the log-marginal likelihood with respect to the kernel hyperparameters at position theta 
            is returned additionally. If True, theta must not be None.
        ...
        # y is originally thought to be a (1, n_samples) row vector.
        # However, in multioutputs, y is of shape (n_samples, 2) and we need to compute y^T . 
        # alpha for each output, independently using einsum. Thus, it is equivalent to:
        # for output_idx in range(n_outputs):
        #     log_likelihood_dims[output_idx] = (y_train[:, [output_idx]] @ alpha[:, [output_idx]])
        ...
        if eval_gradient:
            ...
            # alpha is supposed to be a vector of (n_samples,) elements. 
            # With multioutputs, alpha is a matrix of size (n_samples, n_outputs).
            # Therefore, we want to construct a matrix of (n_samples, n_samples, n_outputs) equivalent to
            # for output_idx in range(n_outputs):
            ...
            # Since we are interested about the trace of inner_term @ K_gradient, we don't explicitly compute the
            # matrix-by-matrix operation and instead use an einsum. Therefore it is equivalent to:
