...
The kernels in this module allow kernel-engineering, 
i.e., they can be combined via the "+" and "*" operators or be exponentiated with a scalar via "**". 
These sum and product expressions can also contain scalar values, which are automatically converted to a constant kernel.

All kernels allow (analytic) gradient-based hyperparameter optimization.
The space of hyperparameters can be specified by giving lower und upper boundaries for the value of each hyperparameter 
(the search space is thus rectangular). Instead of specifying bounds, hyperparameters can also be declared to be "fixed", 
which causes these hyperparameters to be excluded from optimization.
"""

# Author: Jan Hendrik Metzen <jhm@informatik.uni-bremen.de>
# License: BSD 3 clause

# Note: this module is strongly inspired by the kernel module of the george package.
...
class Hyperparameter(namedtuple("Hyperparameter", ("name", "value_type", "bounds", "n_elements", "fixed"))):
    ...
    bounds : pair of floats >= 0 or "fixed"
        The lower and upper bound on the parameter. If n_elements>1, a pair of 1d array with n_elements each may be given alternatively. 
        If the string "fixed" is passed as bounds, the hyperparameter's value cannot be changed.

    n_elements : int, default=1
        The number of elements of the hyperparameter value. Defaults to 1, which corresponds to a scalar hyperparameter. 
        n_elements > 1 corresponds to a hyperparameter which is vector-valued, such as, e.g., anisotropic length-scales.

    fixed : bool, default=None
        Whether the value of this hyperparameter is fixed, i.e., cannot be changed during hyperparameter tuning. 
        If None is passed, the "fixed" is derived based on the given bounds.
    ...
    # in a struct to get rid of the __dict__ of attributes in particular 
    # it does not copy the string for the keys on each instance.
    # By deriving a namedtuple class just to introduce the __init__ method 
    # we would also reintroduce the __dict__ on the instance. 
    # By telling the Python interpreter that this subclass uses static __slots__ instead of dynamic attributes. 
    # Furthermore we don't need any additional slot in the subclass so we set __slots__ to the empty tuple.
    __slots__ = ()
    ...
    # This is mainly a testing utility to check that two hyperparameters are equal.
    def __eq__(self, other):
        return (self.name == other.name and self.value_type == other.value_type and np.all(self.bounds == other.bounds)
            and self.n_elements == other.n_elements and self.fixed == other.fixed)
...
class Kernel(metaclass=ABCMeta):
    ...
    def get_params(self, deep=True):
        ...
        if len(varargs) != 0:
            raise RuntimeError("scikit-learn kernels should always specify their parameters in the signature"
                " of their __init__ (no varargs). %s doesn't follow this convention." % (cls,))
        for arg in args:
        ...
        for key, value in params.items():
            ...
            if len(split) > 1:
                ...
                if name not in valid_params:
                    raise ValueError("Invalid parameter %s for kernel %s. Check the list of available parameters "
    ...
    def theta(self):
        ...
        Note that theta are typically the log-transformed values of the kernel's hyperparameters as 
        this representation of the search space is more amenable for hyperparameter search, 
        as hyperparameters like length-scales naturally live on a log-scale.

        Returns
        -------
        theta : ndarray of shape (n_dims,), The non-fixed, log-transformed hyperparameters of the kernel
        """
        theta = []
    ...
    def bounds(self):
        ...
        Returns
        -------
        bounds : ndarray of shape (n_dims, 2), The log-transformed bounds on the kernel's hyperparameters theta
        """
    ...
    def diag(self, X):
        ...
        however, it can be evaluated more efficiently since only the diagonal is evaluated.
    ...
    def _check_bounds_params(self):
        ...
        for hyp in self.hyperparameters:
            ...
            for dim in range(hyp.n_elements):
                if list_close[idx, 0]:
                    warnings.warn("The optimal value found for dimension %s of parameter %s is "
                        "close to the specified lower bound %s. Decreasing the bound and calling fit again may find a better value." 
                        % (dim, hyp.name, hyp.bounds[dim][0]), ConvergenceWarning)
                elif list_close[idx, 1]:
                    warnings.warn("The optimal value found for dimension %s of parameter %s is close to the specified upper "
                        "bound %s. Increasing the bound and calling fit again may find a better value." 
                        % (dim, hyp.name, hyp.bounds[dim][1]), ConvergenceWarning)
                idx += 1
...
class CompoundKernel(Kernel):
    ...
    def theta(self, theta):
        ...
        Parameters
        ----------
        theta : array of shape (n_dims,), The non-fixed, log-transformed hyperparameters of the kernel
        """
    ...
    def bounds(self):
        ...
        Returns
        -------
        bounds : array of shape (n_dims, 2), The log-transformed bounds on the kernel's hyperparameters theta
        """
...
class KernelOperator(Kernel):
    ...
    def hyperparameters(self):
        ...
        r = [Hyperparameter("k1__" + hyperparameter.name, hyperparameter.value_type, hyperparameter.bounds, hyperparameter.n_elements)
...
class WhiteKernel(StationaryKernelMixin, GenericKernelMixin, Kernel):
    ...
    The main use-case of this kernel is as part of a sum-kernel where it explains the noise of the signal 
    as independently and identically normally-distributed. The parameter noise_level equals the variance of this noise.
...
class RBF(StationaryKernelMixin, NormalizedKernelMixin, Kernel):
    ...
    which can either be a scalar (isotropic variant of the kernel) or a vector with the same number of dimensions 
    as the inputs X (anisotropic variant of the kernel). The kernel is given by:

    .. math::
        k(x_i, x_j) = \\exp\\left(- \\frac{d(x_i, x_j)^2}{2l^2} \\right)

    where :math:`l` is the length scale of the kernel and :math:`d(\\cdot,\\cdot)` is the Euclidean distance.
    ...
    length_scale : float or ndarray of shape (n_features,), default=1.0
        ...
        If an array, an anisotropic kernel is used where each dimension of l defines the length-scale of the respective feature dimension.
    ...
    References
    ----------
    .. [1] `David Duvenaud (2014). "The Kernel Cookbook: Advice on Covariance functions".
        <https://www.cs.toronto.edu/~duvenaud/cookbook/>`_

    .. [2] `Carl Edward Rasmussen, Christopher K. I. Williams (2006).  "Gaussian Processes for Machine Learning".  The MIT Press. 
...
class Matern(RBF):
    ...
    As :math:`\\nu\\rightarrow\\infty`, the kernel becomes equivalent to the :class:`RBF` kernel. 
    ...
    See [1]_, Chapter 4, Section 4.2, for details regarding the different variants of the Matern kernel.
    ...
    length_scale_bounds : pair of floats >= 0 or "fixed", default=(1e-5, 1e5)
        ...
        If set to "fixed", 'length_scale' cannot be changed during hyperparameter tuning.
    ...
    nu : float, default=1.5
        ...
        since they require to evaluate the modified Bessel function. 
    ...
    def __repr__(self):
        if self.anisotropic:
            return "{0}(length_scale=[{1}], nu={2:.3g})"   \
                    .format(self.__class__.__name__, ", ".join(map("{0:.3g}".format, self.length_scale)), self.nu)
        else:
            return "{0}(length_scale={1:.3g}, nu={2:.3g})" \
                    .format(self.__class__.__name__, np.ravel(self.length_scale)[0], self.nu)
...
class RationalQuadratic(StationaryKernelMixin, NormalizedKernelMixin, Kernel):
    ...
    It is parameterized by a length scale parameter :math:`l>0` and a scale mixture parameter :math:`\\alpha>0`. 
    Only the isotropic variant where length_scale :math:`l` is a scalar is supported at the moment.  The kernel is given by:

    .. math::
        k(x_i, x_j) = \\left(1 + \\frac{d(x_i, x_j)^2 }{ 2\\alpha  l^2}\\right)^{-\\alpha}

    where :math:`\\alpha` is the scale mixture parameter, :math:`l` is the length scale of the kernel and :math:`d(\\cdot,\\cdot)` 
    is the Euclidean distance.  For advice on how to set the parameters, see e.g. [1]_.
    ...
    alpha_bounds : pair of floats >= 0 or "fixed", default=(1e-5, 1e5)
        ...
        If set to "fixed", 'alpha' cannot be changed during hyperparameter tuning.
...
class ExpSineSquared(StationaryKernelMixin, NormalizedKernelMixin, Kernel):
    ...
    The ExpSineSquared kernel allows one to model functions which repeat themselves exactly. 
    It is parameterized by a length scale parameter :math:`l>0` and a periodicity parameter :math:`p>0`.
    Only the isotropic variant where :math:`l` is a scalar is supported at the moment. The kernel is given by:

    .. math::
        k(x_i, x_j) = \text{exp}\left(-
        \frac{ 2\sin^2(\pi d(x_i, x_j)/p) }{ l^ 2} \right)

    where :math:`l` is the length scale of the kernel, :math:`p` the periodicity of the kernel 
    and :math:`d(\\cdot,\\cdot)` is the Euclidean distance.
...
class DotProduct(Kernel):
    ...
    The DotProduct kernel is non-stationary and can be obtained from linear regression by putting :math:`N(0, 1)`
    priors on the coefficients of :math:`x_d (d = 1, . . . , D)` and a prior of :math:`N(0, \sigma_0^2)` on the bias. 
    The DotProduct kernel is invariant to a rotation of the coordinates about the origin, but not translations.
    It is parameterized by a parameter sigma_0 :math:`\sigma` which controls the inhomogenity of the kernel. 
    For :math:`\sigma_0^2 =0`, the kernel is called the homogeneous linear kernel, otherwise it is inhomogeneous. 
    The kernel is given by
    .. math::
...
class PairwiseKernel(Kernel):
    ...
    Note: Evaluation of eval_gradient is not analytic but numeric and all kernels support only isotropic distances. 
        The parameter gamma is considered to be a hyperparameter and may be optimized. 
        The other kernel parameters are set directly at initialization and are kept fixed.
