...
def _deprecate_n_iter(n_iter, max_iter):
    """
    Deprecates n_iter in favour of max_iter. Checks if the n_iter has been used instead of max_iter 
    and generates a deprecation warning if True.
    ...
    max_iter : int, default=None
        Value of max_iter attribute passed by the estimator.  If `None`, it corresponds to `max_iter=300`.
    ...
    if n_iter != "deprecated":
        if max_iter is not None:
            raise ValueError("Both `n_iter` and `max_iter` attributes were set. "
                "Attribute `n_iter` was deprecated in version 1.3 and will be removed in 1.5. "
                "To avoid this error, only set the `max_iter` attribute.")
...
class BayesianRidge(RegressorMixin, LinearModel):
    ...
    Fit a Bayesian ridge model. See the Notes section for details on this implementation 
    and the optimization of the regularization parameters lambda (precision of the weights) and alpha (precision of the noise).
    ...
    scores_ : array-like of shape (n_iter_+1,)
        ...
        The array starts with the value of the log marginal likelihood obtained for the initial values of alpha and lambda 
        and ends with the value obtained for the estimated alpha and lambda.
    ...
    Notes
    -----
    There exist several strategies to perform Bayesian ridge regression. 
    ...
    Note that according to A New View of Automatic Relevance Determination (Wipf and Nagarajan, 2008) 
    these update rules do not guarantee that the marginal likelihood is increasing between two consecutive iterations of the optimization.
    ...
    _parameter_constraints: dict = {
        ...
        "copy_X": ["boolean"],
        "verbose": ["verbose"],
        "n_iter": [Interval(Integral, 1, None, closed="left"), Hidden(StrOptions({"deprecated"})) ],
    }

    def __init__(self, *, max_iter=None, tol=1.0e-3, alpha_1=1.0e-6, alpha_2=1.0e-6, lambda_1=1.0e-6, lambda_2=1.0e-6, \
        alpha_init=None, lambda_init=None, compute_score=False, fit_intercept=True, copy_X=True, verbose=False, n_iter="deprecated"):
    ...
    def fit(self, X, y, sample_weight=None):
        ...
        for iter_ in range(max_iter):
            # update posterior mean coef_ based on alpha_ and lambda_ and compute corresponding rmse
        ...
        # return regularization parameters and corresponding posterior mean, log marginal likelihood and posterior covariance
        self.alpha_ = alpha_
    ...
    def _update_coef_(self, X, y, n_samples, n_features, XT_y, U, Vh, eigen_vals_, alpha_, lambda_):
        ...
        scaled_sigma_ = (lambda_/alpha_ * np.eye(n_features) + np.dot(X.T, X))^-1
        """
    ...
    def _log_marginal_likelihood(self, n_samples, n_features, eigen_vals, alpha_, lambda_, coef, rmse):
        ...
        # compute the log of the determinant of the posterior covariance. posterior covariance is given by
        ...
        score += 0.5 * (n_features * log(lambda_) + n_samples * log(alpha_) - alpha_ * rmse
            - lambda_ * np.sum(coef**2) + logdet_sigma - n_samples * log(2 * np.pi))
...
class ARDRegression(RegressorMixin, LinearModel):
    ...
    Also estimate the parameters lambda (precisions of the distributions of the weights) 
    ...
    Notes
    -----
    For an example, see :ref:`examples/linear_model/plot_ard.py <sphx_glr_auto_examples_linear_model_plot_ard.py>`.
    ...
    which ``self.lambda_ < self.threshold_lambda`` are kept and the rest are discarded.
    ...
    def __init__(self, *, max_iter=None, tol=1.0e-3, alpha_1=1.0e-6, alpha_2=1.0e-6, lambda_1=1.0e-6, lambda_2=1.0e-6,   \
        compute_score=False, threshold_lambda=1.0e4, fit_intercept=True, copy_X=True, verbose=False, n_iter="deprecated" ):
        self.max_iter         = max_iter
        self.tol              = tol
        self.fit_intercept    = fit_intercept
        self.alpha_1          = alpha_1
    ...
    def _update_sigma_woodbury(self, X, alpha_, lambda_, keep_lambda):
        ...
        # a matrix of shape (n_samples, n_samples) making use of the woodbury formula:
    ...
    def _update_sigma(self, X, alpha_, lambda_, keep_lambda):
        # See slides as referenced in the docstring note this function is used when n_samples >= n_features 
        # and will invert a matrix of shape (n_features, n_features)
        X_keep = X[:, keep_lambda]
