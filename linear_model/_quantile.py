...
class QuantileRegressor(LinearModel, RegressorMixin, BaseEstimator):
    ...
    The linear :class:`QuantileRegressor` optimizes the pinball loss for a desired `quantile` and is robust to outliers.
    This model uses an L1 regularization like :class:`~sklearn.linear_model.Lasso`.
    ...
    quantile : float, default=0.5
        The quantile that the model tries to predict. It must be strictly between 0 and 1. 
    ...
    solver : {'highs-ds', 'highs-ipm', 'highs', 'interior-point', 'revised simplex'}, default='interior-point'
        Method used by :func:`scipy.optimize.linprog` to solve the linear programming formulation.

        From `scipy>=1.6.0`, it is recommended to use the highs methods because they are the fastest ones. 
        Solvers "highs-ds", "highs-ipm" and "highs" support sparse input data and, in fact, always convert to sparse csc.
    ...
    solver_options : dict, default=None
        ...
        If `None` and if `solver='interior-point'`, then `{"lstsq": True}` is passed to :func:`scipy.optimize.linprog` for the sake of stability.

    Attributes
    ----------
    coef_ : array of shape (n_features,), Estimated coefficients for the features.
    intercept_ : float, The intercept of the model, aka bias term.
    ...
    n_iter_ : int, The actual number of iterations performed by the solver.
    ...
    _parameter_constraints: dict = {
        ...
        "solver": [StrOptions( { "highs-ds", "highs-ipm", "highs", "interior-point", "revised simplex" }), Hidden(StrOptions({"warn"})) ],
    ...
    def fit(self, X, y, sample_weight=None):
        ...
        if self.fit_intercept:
            n_params += 1
            # Note that centering y and X with _preprocess_data does not work for quantile regression.

        # The objective is defined as 1/n * sum(pinball loss) + alpha * L1. So we rescale the penalty term, which is equivalent.
        alpha = np.sum(sample_weight) * self.alpha

        if self.solver == "warn":
            warnings.warn(("The default solver will change from 'interior-point' to 'highs' in version 1.4. Set `solver='highs'` "
                "or to the desired solver to silence this warning."), FutureWarning)
        ...
        #
        # Filtering out zero sample weights from the beginning makes life easier for the linprog solver.
        ...
        if solver in ["highs", "highs-ds", "highs-ipm"]:
            ...
            # Therefore, we work with CSC matrices as early as possible to limit unnecessary repeated memory copies.
        ...
        if not result.success:
            ...
            warnings.warn("Linear programming for QuantileRegressor did not succeed.\n Status is {result.status}: "
                + failure.setdefault(result.status, "unknown reason") + "\n" + "Result message of linprog:\n"
                + result.message, ConvergenceWarning)

        # positive slack - negative slack solution is an array with (params_pos, params_neg, u, v)
