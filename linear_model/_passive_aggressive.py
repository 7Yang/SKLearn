...
class PassiveAggressiveClassifier(BaseSGDClassifier):
    ...
    max_iter : int, default=1000
        ...
        It only impacts the behavior in the ``fit`` method, and not the :meth:`PassiveAggressive.partial_fit` method.
        .. versionadded:: 0.19
    ...
    early_stopping : bool, default=False
        ...
        If set to True, it will automatically set aside a stratified fraction of training data as validation and 
    ...
    def __init__(self, *, C=1.0, fit_intercept=True, max_iter=1000, tol=1e-3, early_stopping=False, validation_fraction=0.1,   \
        n_iter_no_change=5, shuffle=True, verbose=0, loss="hinge", n_jobs=None, random_state=None, warm_start=False,           \
        class_weight=None, average=False):
        super().__init__(penalty=None, fit_intercept=fit_intercept, max_iter=max_iter, tol=tol, early_stopping=early_stopping, \
            validation_fraction=validation_fraction, n_iter_no_change=n_iter_no_change, shuffle=shuffle, verbose=verbose,      \
            random_state=random_state, eta0=1.0, warm_start=warm_start, class_weight=class_weight, average=average, n_jobs=n_jobs)
    ...
    def partial_fit(self, X, y, classes=None):
        ...
        classes : ndarray of shape (n_classes,)
            ...
            Can be obtained by via `np.unique(y_all)`, where y_all is the target vector of the entire dataset.
        ...
        if not hasattr(self, "classes_"):
            ...
            if self.class_weight == "balanced":
                raise ValueError("class_weight 'balanced' is not supported for partial_fit. For 'balanced' weights, "
                    "use `sklearn.utils.compute_class_weight` with `class_weight='balanced'`. "
                    "In place of y you can use a large enough subset of the full training set target to properly "
                    "estimate the class frequency distributions. Pass the resulting weights as the class_weight parameter.")

        lr = "pa1" if self.loss == "hinge" else "pa2"
        return self._partial_fit(X, y, alpha=1.0, C=self.C, loss="hinge", learning_rate=lr, max_iter=1, classes=classes, \
            sample_weight=None, coef_init=None, intercept_init=None)
...
class PassiveAggressiveRegressor(BaseSGDRegressor):
    ...
    max_iter : int, default=1000
        The maximum number of passes over the training data (aka epochs).  It only impacts the behavior in the ``fit`` method, and not the
    ...
    early_stopping : bool, default=False
        ...
        score is not improving. If set to True, it will automatically set aside a fraction of training data 
        as validation and terminate training when validation score is not improving by at least tol for n_iter_no_change consecutive epochs.
        .. versionadded:: 0.20
    ...
    loss : str, default="epsilon_insensitive"
        The loss function to be used: epsilon_insensitive: equivalent to PA-I in the reference paper.
    ...
    def __init__(self, *, C=1.0, fit_intercept=True, max_iter=1000, tol=1e-3, early_stopping=False,        \
        validation_fraction=0.1, n_iter_no_change=5, shuffle=True, verbose=0, loss="epsilon_insensitive",  \
        epsilon=DEFAULT_EPSILON, random_state=None, warm_start=False, average=False):
        super().__init__(penalty=None, l1_ratio=0, epsilon=epsilon, eta0=1.0, fit_intercept=fit_intercept, \
            max_iter=max_iter, tol=tol, early_stopping=early_stopping, validation_fraction=validation_fraction, \
            n_iter_no_change=n_iter_no_change, shuffle=shuffle, verbose=verbose, random_state=random_state, \
    ...
    def partial_fit(self, X, y):
        ...
        return self._partial_fit(X, y, alpha=1.0, C=self.C, loss="epsilon_insensitive", learning_rate=lr,
            max_iter=1, sample_weight=None, coef_init=None, intercept_init=None)
    ...
    def fit(self, X, y, coef_init=None, intercept_init=None):
        ...
        lr = "pa1" if self.loss == "epsilon_insensitive" else "pa2"
        return self._fit(X, y, alpha=1.0, C=self.C, loss="epsilon_insensitive", learning_rate=lr, \
