...
class NewtonSolver(ABC):
    ...
    This class implements Newton/2nd-order optimization routines for GLMs. 
    Each Newton iteration aims at finding the Newton step which is done by the inner solver. 
    With Hessian H, gradient g and coefficients coef, one step solves:
    ...
    Backtracking line search updates coef = coef_old + t * coef_newton for some t in (0, 1].
    This is a base class, actual implementations (child classes) may deviate from the above pattern and use structure specific tricks.
    ...
    References
    ----------
    - Jorge Nocedal, Stephen J. Wright. (2006) "Numerical Optimization" 2nd edition https://doi.org/10.1007/978-0-387-40065-5

    - Stephen P. Boyd, Lieven Vandenberghe. (2004) "Convex Optimization." Cambridge University Press, 2004.
      https://web.stanford.edu/~boyd/cvxbook/bv_cvxbook.pdf

    Parameters
    ----------
    coef : ndarray of shape (n_dof,), (n_classes, n_dof) or (n_classes * n_dof,), Initial coefficients of a linear model.
        If shape (n_classes * n_dof,), the classes of one feature are contiguous, 
        i.e. one reconstructs the 2d-array via coef.reshape((n_classes, -1), order="F").
    ...
    tol : float, default=1e-4
        The optimization problem is solved when each of the following condition is fulfilled:
    ...
    n_threads : int, default=1
        Number of OpenMP threads to use for the computation of the Hessian and gradient of the loss function.
    ...
    use_fallback_lbfgs_solve : bool
        If set to True, the solver will resort to call LBFGS to finish the optimisation procedure in case of convergence issues.

    gradient_times_newton : float
        gradient @ coef_newton, set in inner_solve and used by line_search. If the Newton step is a descent direction, this is negative.
    """

    def __init__(self, *, coef, linear_loss=LinearModelLoss(base_loss=HalfSquaredError(), fit_intercept=True), \
        l2_reg_strength=0.0, tol=1e-4, max_iter=100, n_threads=1, verbose=0):
        self.coef            = coef
    ...
    def setup(self, X, y, sample_weight):
        ...
        self.loss_value = self.linear_loss.loss(coef=self.coef, X=X, y=y, sample_weight=sample_weight, \
            l2_reg_strength=self.l2_reg_strength, n_threads=self.n_threads, raw_prediction=self.raw_prediction)
    ...
    def fallback_lbfgs_solve(self, X, y, sample_weight):
        ...
        If a solver detects convergence problems, it may fall back to this methods in the hope to exit with success instead of raising an error.

        Sets:
            - self.coef
            - self.converged
        """
        opt_res = scipy.optimize.minimize(self.linear_loss.loss_gradient, self.coef, method="L-BFGS-B", jac=True,
            options={ "maxiter": self.max_iter, "maxls": 50, "iprint": self.verbose - 1, "gtol": self.tol, "ftol": 64 * np.finfo(np.float64).eps },
    ...
    def line_search(self, X, y, sample_weight):
        ...
        # gradient_times_newton = self.gradient @ self.coef_newton was computed in inner_solve.
        ...
        for i in range(21):  # until and including t = beta**20 ~ 1e-6
            ...
            self.loss_value, self.gradient = self.linear_loss.loss_gradient(coef=self.coef, X=X, y=y, \
                sample_weight=sample_weight, l2_reg_strength=self.l2_reg_strength, n_threads=self.n_threads, raw_prediction=raw)
            ...
            if is_verbose:
                print("      check loss |improvement| <= eps * |loss_old|: {np.abs(loss_improvement)} <= {tiny_loss} {check}")
            if check:
                ...
                if is_verbose:
                    print("      check sum(|gradient|) < sum(|gradient_old|): {sum_abs_grad} < {sum_abs_grad_old} {check}")
                if check:
                    break

            t *= beta
        else:
            warnings.warn((f"Line search of Newton solver {self.__class__.__name__} at iteration #{self.iteration} "\
                "did no converge after 21 line search refinement iterations. It will now resort to lbfgs instead."), ConvergenceWarning)
            if self.verbose:
    ...
    def check_convergence(self, X, y, sample_weight):
        ...
        # Note: Checking maximum relative change of coefficient <= tol is a bad convergence criterion 
        # because even a large step could have brought us close to the true minimum.
        ...
        if self.verbose:
            loss_value = self.linear_loss.loss(coef=self.coef, X=X, y=y, sample_weight=sample_weight, \
                l2_reg_strength=self.l2_reg_strength, n_threads=self.n_threads)
    ...
    def solve(self, X, y, sample_weight):
        ...
        Returns
        -------
        coef : ndarray of shape (n_dof,), (n_classes, n_dof) or (n_classes * n_dof,), Solution of the optimization problem.
        """
        # setup usually:
        ...
        while self.iteration <= self.max_iter and not self.converged:
            ...
            # TODO:
            # if iteration == 1:
            # We might stop early, e.g. we already are close to the optimum, usually detected by zero gradients at this stage.
        ...
        if not self.converged:
            if self.use_fallback_lbfgs_solve:
                # Note: The fallback solver circumvents check_convergence and relies on the convergence checks of lbfgs instead. 
                # Enough warnings have been raised on the way.
...
class NewtonCholeskySolver(NewtonSolver):
    ...
    Inner solver for finding the Newton step H w_newton = -g uses Cholesky based linear solver.
    """

    def setup(self, X, y, sample_weight):
        super().setup(X=X, y=y, sample_weight=sample_weight)
        n_dof = X.shape[1]
        if self.linear_loss.fit_intercept: n_dof += 1
    ...
    def update_gradient_hessian(self, X, y, sample_weight):
        _, _, self.hessian_warning = self.linear_loss.gradient_hessian(                                  \
            coef=self.coef, X=X, y=y, sample_weight=sample_weight, l2_reg_strength=self.l2_reg_strength, \
            n_threads=self.n_threads, gradient_out=self.gradient, hessian_out=self.hessian,              \
            raw_prediction=self.raw_prediction,  # this was updated in line_search
        )

    def inner_solve(self, X, y, sample_weight):
        if self.hessian_warning:
            warnings.warn((f"The inner solver of {self.__class__.__name__} detected a pointwise hessian "\
                "with many negative values at iteration #{self.iteration}. It will now resort to lbfgs instead."), ConvergenceWarning)
            if self.verbose:
        ...
        try:
            ...
            # Most of them add, explicitly or implicitly, a matrix to the hessian to make it positive definite, 
            # confer to Chapter 3.4 of Nocedal & Wright 2nd ed. Instead, we resort to lbfgs.
            if self.verbose:
