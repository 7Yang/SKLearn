...
class _GeneralizedLinearRegressor(RegressorMixin, BaseEstimator):
    ...
    GLMs based on a reproductive Exponential Dispersion Model (EDM) aim at fitting and predicting the mean of the target y 
    as y_pred=h(X*w) with coefficients w.  Therefore, the fit minimizes the following objective function with L2 priors as regularizer::

        1/(2*sum(s_i)) * sum(s_i * deviance(y_i, h(x_i*w)) + 1/2 * alpha * ||w||_2^2

    with inverse link function h, s=sample_weight and per observation (unit) deviance deviance(y_i, h(x_i*w)). 
    Note that for an EDM, 1/2 * deviance is the negative log-likelihood up to a constant (in w) term.
    The parameter ``alpha`` corresponds to the lambda parameter in glmnet.

    Instead of implementing the EDM family and a link function separately, 
    we directly use the loss functions `from sklearn._loss` which have the link functions included in them for performance reasons. 
    We pick the loss functions that implement (1/2 times) EDM deviances.
    ...
    alpha : float, default=1
        ...
        ``alpha = 0`` is equivalent to unpenalized GLMs. In this case, the design matrix `X` must have full column rank (no collinearities).
    ...
    solver : {'lbfgs', 'newton-cholesky'}, default='lbfgs'
        ...
        'newton-cholesky'
            Uses Newton-Raphson steps (in arbitrary precision arithmetic equivalent to iterated reweighted least squares) 
            with an inner Cholesky based solver.  This solver is a good choice for `n_samples` >> `n_features`, 
    ...
    _base_loss : BaseLoss, default=HalfSquaredError()
        ...
        A `_base_loss` contains a specific loss function as well as the link function. 
        The loss to be minimized specifies the distributional assumption of the GLM, i.e. the distribution from the EDM. Here are some examples:
    ...
    # We allow for NewtonSolver classes for the "solver" parameter but do not make them public in the docstrings. 
    # This facilitates testing and benchmarking.
    ...
    def __init__(self, *, alpha=1.0, fit_intercept=True, solver="lbfgs", max_iter=100, tol=1e-4, warm_start=False, verbose=0):
        self.alpha         = alpha
    ...
    def fit(self, X, y, sample_weight=None):
        ...
        if self.solver == "lbfgs":
            # lbfgs will force coef and therefore raw_prediction to be float64. 
            # The base_loss needs y, X @ coef and sample_weight all of same dtype (and contiguous).
        ...
        # Note that _check_sample_weight calls check_array(order="C") required by losses.
        ...
        if not linear_loss.base_loss.in_y_true_range(y):
            raise ValueError("Some value(s) of y are out of the valid range of the loss {self._base_loss.__class__.__name__!r}.")
        ...
        if self.solver == "lbfgs":
            ...
            opt_res = scipy.optimize.minimize(func, coef, method="L-BFGS-B", jac=True,
                options={
                    ...
                    # The point is that ftol is very small, but a bit larger than machine precision for float64, which is the dtype used by lbfgs.
            ...
            sol = NewtonCholeskySolver(coef=coef, linear_loss=linear_loss, l2_reg_strength=l2_reg_strength, tol=self.tol, \
                max_iter=self.max_iter, n_threads=n_threads, verbose=self.verbose)
            coef = sol.solve(X, y, sample_weight)
            self.n_iter_ = sol.iteration
        elif issubclass(self.solver, NewtonSolver):
            sol = self.solver(coef=coef, linear_loss=linear_loss, l2_reg_strength=l2_reg_strength, tol=self.tol,          \
                max_iter=self.max_iter, n_threads=n_threads)
            coef = sol.solve(X, y, sample_weight)
            self.n_iter_ = sol.iteration
        else:
    ...
    def _linear_predictor(self, X):
        ...
        Returns
        -------
        y_pred : array of shape (n_samples,), Returns predicted values of linear predictor.
        """
        check_is_fitted(self)
        X = self._validate_data(X, accept_sparse=["csr", "csc", "coo"], dtype=[np.float64, np.float32], ensure_2d=True, \
    ...
    def score(self, X, y, sample_weight=None):
        ...
        R^2 uses squared error and D^2 uses the deviance of this GLM, see the :ref:`User Guide <regression_metrics>`.
        ...
        # TODO: make D^2 a score function in module metrics (and thereby get input validation and so on)
        ...
        if not base_loss.in_y_true_range(y):
            raise ValueError("Some value(s) of y are out of the valid range of the loss {base_loss.__name__}.")
    ...
    def _more_tags(self):
        try:
            ...
            # This is necessary as TweedieRegressor might set the used loss during fit different from self._base_loss.
            base_loss = self._get_loss()
            return {"requires_positive_y": not base_loss.in_y_true_range(-1.0)}
        except (ValueError, AttributeError, TypeError):
            # This happens when the link or power parameter of TweedieRegressor is invalid. We fallback on the default tags in that case.
            return {}

    def _get_loss(self):
        """
        This is only necessary because of the link and power arguments of the TweedieRegressor.

        Note that we do not need to pass sample_weight to the loss class 
        as this is only needed to set loss.constant_hessian on which GLMs do not rely.
        """
...
class TweedieRegressor(_GeneralizedLinearRegressor):
    ...
    link : {'auto', 'identity', 'log'}, default='auto'
        The link function of the GLM, i.e. mapping from linear predictor `X @ coeff + intercept` to prediction `y_pred`. 
        Option 'auto' sets the link depending on the chosen `power` parameter as follows:
    ...
    def __init__(self, *, power=0.0, alpha=1.0, fit_intercept=True, link="auto", solver="lbfgs", max_iter=100, tol=1e-4, warm_start=False, verbose=0):
        super().__init__(alpha=alpha, fit_intercept=fit_intercept, solver=solver, max_iter=max_iter, tol=tol, warm_start=warm_start, verbose=verbose)
        self.link = link
        self.power = power
