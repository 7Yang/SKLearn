...
from ._sgd_fast import (EpsilonInsensitive, Hinge, Huber, Log, ModifiedHuber, SquaredEpsilonInsensitive, \
    SquaredHinge, SquaredLoss, _plain_sgd32, _plain_sgd64)
...
class _ValidationScoreCallback:
    ...
    def __init__(self, estimator, X_val, y_val, sample_weight_val, classes=None):
        ...
        if classes is not None: self.estimator.classes_ = classes
        self.X_val        = X_val
        self.y_val        = y_val
...
class BaseSGD(SparseCoefMixin, BaseEstimator, metaclass=ABCMeta):
    ...
    def __init__(self, loss, *, penalty="l2", alpha=0.0001, C=1.0, l1_ratio=0.15, \
    ...
    def _make_validation_split(self, y, sample_mask):
        ...
        sample_mask : ndarray of shape (n_samples, )
            A boolean array indicating whether each sample should be included for validation set.
        ...
        if idx_train.shape[0] == 0 or idx_val.shape[0] == 0:
            raise ValueError("Splitting %d samples into a train set and a validation set "
                "with validation_fraction=%r led to an empty set (%d and %d samples)."
                "Please either change validation_fraction, increase number of samples, or disable early_stopping."
                % (n_samples, self.validation_fraction, idx_train.shape[0], idx_val.shape[0])
            )
...
def fit_binary(est, i, X, y, alpha, C, learning_rate, max_iter, pos_weight, neg_weight, sample_weight, validation_mask=None, random_state=None):
    ...
    learning_rate : str
        The learning rate. Accepted values are 'constant', 'optimal', 'invscaling', 'pa1' and 'pa2'.
    ...
    validation_mask : numpy array of shape [n_samples, ], default=None 
        Precomputed validation mask in case _fit_binary is called in the context of a one-vs-rest reduction.
    ...
    """
    # if average is not true, average_coef, and average_intercept will be unused
    ...
    coef, intercept, average_coef, average_intercept, n_iter_ = _plain_sgd( \
        coef, intercept, average_coef, average_intercept, est.loss_function_, penalty_type, alpha, C, \
        est.l1_ratio, dataset, validation_mask, est.early_stopping, validation_score_cb, int(est.n_iter_no_change), \
        max_iter, tol, int(est.fit_intercept), int(est.verbose), int(est.shuffle), seed, pos_weight, neg_weight,    \
        learning_rate_type, est.eta0, est.power_t, 0, est.t_, intercept_decay, est.average)
...
class BaseSGDClassifier(LinearClassifierMixin, BaseSGD, metaclass=ABCMeta):
    ...
    def __init__(self, loss="hinge", *, penalty="l2", alpha=0.0001, l1_ratio=0.15, fit_intercept=True, max_iter=1000, \
        tol=1e-3, shuffle=True, verbose=0, epsilon=DEFAULT_EPSILON, n_jobs=None, random_state=None, learning_rate="optimal", \
        eta0=0.0, power_t=0.5, early_stopping=False, validation_fraction=0.1, n_iter_no_change=5, class_weight=None, \
        warm_start=False, average=False):
        super().__init__(loss=loss, penalty=penalty, alpha=alpha, l1_ratio=l1_ratio, fit_intercept=fit_intercept, max_iter=max_iter, \
            tol=tol, shuffle=shuffle, verbose=verbose, epsilon=epsilon, random_state=random_state, learning_rate=learning_rate,      \
    ...
    def _partial_fit(self, X, y, alpha, C, loss, learning_rate, max_iter, classes, sample_weight, coef_init, intercept_init):
        ...
        if getattr(self, "coef_", None) is None or coef_init is not None:
            self._allocate_parameter_mem(n_classes=n_classes, n_features=n_features, input_dtype=X.dtype, \
    ...
    def _fit_binary(self, X, y, alpha, C, sample_weight, learning_rate, max_iter):
        ...
        coef, intercept, n_iter_ = fit_binary(self, 1, X, y, alpha, C, learning_rate, max_iter, \
            self._expanded_class_weight[1], self._expanded_class_weight[0], sample_weight,      \
    ...
    def _fit_multiclass(self, X, y, alpha, C, learning_rate, sample_weight, max_iter):
        ...
        Each binary classifier predicts one class versus all others. This strategy is called OvA (One versus All) or OvR (One versus Rest).
        """
        # Precompute the validation split using the multiclass labels to ensure proper balancing of the classes.
        validation_mask = self._make_validation_split(y, sample_mask=sample_weight > 0)

        # Use joblib to fit OvA in parallel.
        # Pick the random seed for each job outside of fit_binary to avoid sharing the estimator random state between threads 
        # which could lead to non-deterministic behavior
        random_state = check_random_state(self.random_state)
        seeds = random_state.randint(MAX_INT, size=len(self.classes_))
        result = Parallel(n_jobs=self.n_jobs, verbose=self.verbose, require="sharedmem")(delayed(fit_binary)(
            self, i, X, y, alpha, C, learning_rate, max_iter, self._expanded_class_weight[i], 1.0, sample_weight, \
            validation_mask=validation_mask, random_state=seed)
    ...
    def partial_fit(self, X, y, classes=None, sample_weight=None):
        ...
        Therefore, it is not guaranteed that a minimum of the cost function is reached after calling it once. 
        Matters such as objective convergence, early stopping, and learning rate adjustments should be handled by the user.
        ...
        if not hasattr(self, "classes_"):
            ...
            if self.class_weight == "balanced":
                raise ValueError("class_weight '{0}' is not supported for partial_fit. In order to use 'balanced' weights,"
                    " use compute_class_weight('{0}', classes=classes, y=y). In place of y you can use a large enough sample "
                    "of the full training set target to properly estimate the class frequency distributions. "
                    "Pass the resulting weights as the class_weight parameter.".format(self.class_weight))

        return self._partial_fit(X, y, alpha=self.alpha, C=1.0, loss=self.loss, learning_rate=self.learning_rate, max_iter=1, \
            classes=classes, sample_weight=sample_weight, coef_init=None, intercept_init=None)
    ...
    def fit(self, X, y, coef_init=None, intercept_init=None, sample_weight=None):
        ...
        sample_weight : array-like, shape (n_samples,), default=None
            Weights applied to individual samples.  If not provided, uniform weights are assumed. 
            These weights will be multiplied with class_weight (passed through the constructor) if class_weight is specified.
        ...
        return self._fit(X, y, alpha=self.alpha, C=1.0, loss=self.loss, learning_rate=self.learning_rate, \
            coef_init=coef_init, intercept_init=intercept_init, sample_weight=sample_weight)
...
class SGDClassifier(BaseSGDClassifier):
    ...
    a decreasing strength schedule (aka learning rate). SGD allows minibatch (online/out-of-core) 
    learning via the `partial_fit` method.  For best results using the default learning rate schedule, 
    ...
    loss : {'hinge', 'log_loss', 'modified_huber', 'squared_hinge', 'perceptron', 'squared_error', 'huber', 'epsilon_insensitive',\
        ...
        - The other losses, 'squared_error', 'huber', 'epsilon_insensitive' 
          and 'squared_epsilon_insensitive' are designed for regression but can be useful in classification as well; 
          see :class:`~sklearn.linear_model.SGDRegressor` for a description.
    ...
    l1_ratio : float, default=0.15
        ...
        Only used if `penalty` is 'elasticnet'.  Values must be in the range `[0.0, 1.0]`.
    ...
    max_iter : int, default=1000
        ...
        and not the :meth:`partial_fit` method.  Values must be in the range `[1, inf)`.
        .. versionadded:: 0.19
    ...
    epsilon : float, default=0.1
        ...
        if they are less than this threshold.  Values must be in the range `[0.0, inf)`.
    ...
    random_state : int, RandomState instance, default=None
        ...
        See :term:`Glossary <random_state>`.  Integer values must be in the range `[0, 2**32 - 1]`.
    ...
    learning_rate : str, default='optimal'
        ...
        - 'adaptive': `eta = eta0`, as long as the training keeps decreasing.  
          Each time n_iter_no_change consecutive epochs fail to decrease the training loss by tol or fail to increase validation score 
          by tol if `early_stopping` is `True`, the current learning rate is divided by 5.
            .. versionadded:: 0.20
    ...
    early_stopping : bool, default=False
        ...
        If set to `True`, it will automatically set aside a stratified fraction of training data as validation 
        and terminate training when validation score returned by the `score` method is not improving 
    ...
    validation_fraction : float, default=0.1
        ...
        Must be between 0 and 1.  Only used if `early_stopping` is True.  Values must be in the range `(0.0, 1.0)`.
    ...
    Perceptron : Inherits from SGDClassifier. ``Perceptron()`` is equivalent to ``SGDClassifier(loss="perceptron", eta0=1, learning_rate="constant",
        penalty=None)``.
    ...
    _parameter_constraints: dict = {
        **BaseSGDClassifier._parameter_constraints,
        ...
        "learning_rate": [StrOptions({"constant", "optimal", "invscaling", "adaptive"}), Hidden(StrOptions({"pa1", "pa2"})) ],
        "eta0": [Interval(Real, 0, None, closed="left")],
    }

    def __init__(self, loss="hinge", *, penalty="l2", alpha=0.0001, l1_ratio=0.15, fit_intercept=True, \
        max_iter=1000, tol=1e-3, shuffle=True, verbose=0, epsilon=DEFAULT_EPSILON, n_jobs=None,        \
        ...
        super().__init__(loss=loss, penalty=penalty, alpha=alpha, l1_ratio=l1_ratio, fit_intercept=fit_intercept, \
    ...
    def predict_proba(self, X):
        ...
        Binary probability estimates for loss="modified_huber" are given by (clip(decision_function(X), -1, 1) + 1) / 2. 
        For other loss functions it is necessary to perform proper probability calibration by wrapping the classifier with
        ...
        if self.loss == "log_loss":
            ...
            if binary:
                ...
                # the above might assign zero to all classes, which doesn't normalize neatly; 
                # work around this to produce uniform probabilities
...
class BaseSGDRegressor(RegressorMixin, BaseSGD):
    ...
    @abstractmethod
    def __init__(self, loss="squared_error", *, penalty="l2", alpha=0.0001, l1_ratio=0.15, fit_intercept=True, \
        max_iter=1000, tol=1e-3, shuffle=True, verbose=0, epsilon=DEFAULT_EPSILON, random_state=None,          \
        learning_rate="invscaling", eta0=0.01, power_t=0.25, early_stopping=False, validation_fraction=0.1,    \
        n_iter_no_change=5, warm_start=False, average=False):
        super().__init__(loss=loss, penalty=penalty, alpha=alpha, l1_ratio=l1_ratio, fit_intercept=fit_intercept,     \
            max_iter=max_iter, tol=tol, shuffle=shuffle, verbose=verbose, epsilon=epsilon, random_state=random_state, \
    ...
    def _partial_fit(self, X, y, alpha, C, loss, learning_rate, max_iter, sample_weight, coef_init, intercept_init):
        ...
        X, y = self._validate_data(X, y, accept_sparse="csr", copy=False, order="C", dtype=[np.float64, np.float32], \
    ...
    def partial_fit(self, X, y, sample_weight=None):
        ...
        Internally, this method uses ``max_iter = 1``. 
        Therefore, it is not guaranteed that a minimum of the cost function is reached after calling it once. 
        Matters such as objective convergence and early stopping should be handled by the user.
        ...
        return self._partial_fit(X, y, self.alpha, C=1.0, loss=self.loss, learning_rate=self.learning_rate, \
            max_iter=1, sample_weight=sample_weight, coef_init=None, intercept_init=None)
    ...
    def _fit(self, X, y, alpha, C, loss, learning_rate, coef_init=None, intercept_init=None, sample_weight=None):
        ...
        if (self.tol is not None and self.tol > -np.inf and self.n_iter_ == self.max_iter):
            warnings.warn(("Maximum number of iteration reached before convergence. "\
                "Consider increasing max_iter to improve the fit."), ConvergenceWarning)
    ...
    def _fit_regressor(self, X, y, alpha, C, loss, learning_rate, sample_weight, max_iter):
        ...
        if not hasattr(self, "t_"): self.t_ = 1.0
        ...
        coef, intercept, average_coef, average_intercept, self.n_iter_ = _plain_sgd( \
            coef, intercept[0], average_coef, average_intercept[0], loss_function, penalty_type, \
            alpha, C, self.l1_ratio, dataset, validation_mask, self.early_stopping, validation_score_cb, \
            int(self.n_iter_no_change), max_iter, tol, int(self.fit_intercept), int(self.verbose), int(self.shuffle), \
            seed, 1.0, 1.0, learning_rate_type, self.eta0, self.power_t, 0, self.t_, intercept_decay, self.average)
...
class SGDRegressor(BaseSGDRegressor):
    ...
    loss : str, default='squared_error'
        ...
        'huber' modifies 'squared_error' to focus less on getting outliers correct by switching 
        ...
        this is the loss function used in SVR.  'squared_epsilon_insensitive' is the same but becomes squared loss past a tolerance of epsilon.
    ...
    max_iter : int, default=1000
        ...
        It only impacts the behavior in the ``fit`` method, and not the :meth:`partial_fit` method.
        .. versionadded:: 0.19
    ...
    early_stopping : bool, default=False
        ...
        and terminate training when validation score returned by the `score` method is not
    ...
    def __init__(self, loss="squared_error", *, penalty="l2", alpha=0.0001, l1_ratio=0.15, fit_intercept=True, \
        ...
        super().__init__(loss=loss, penalty=penalty, alpha=alpha, l1_ratio=l1_ratio, fit_intercept=fit_intercept,     \
            max_iter=max_iter, tol=tol, shuffle=shuffle, verbose=verbose, epsilon=epsilon, random_state=random_state, \
            learning_rate=learning_rate, eta0=eta0, power_t=power_t, early_stopping=early_stopping,                   \
            validation_fraction=validation_fraction, n_iter_no_change=n_iter_no_change, warm_start=warm_start,        \
            average=average)
...
class SGDOneClassSVM(BaseSGD, OutlierMixin):
    ...
    This implementation is meant to be used with a kernel approximation technique 
    (e.g. `sklearn.kernel_approximation.Nystroem`) to obtain results similar to `sklearn.svm.OneClassSVM` which uses a Gaussian kernel by default.
    ...
    nu : float, default=0.5
        The nu parameter of the One Class SVM: an upper bound on the fraction of training errors 
        and a lower bound of the fraction of support vectors. Should be in the interval (0, 1]. By default 0.5 will be taken.
    ...
    max_iter : int, default=1000
        The maximum number of passes over the training data (aka epochs).
        It only impacts the behavior in the ``fit`` method, and not the `partial_fit`. Defaults to 1000.
    ...
    learning_rate : {'constant', 'optimal', 'invscaling', 'adaptive'}, default='optimal'
        ...
        - 'optimal': `eta = 1.0 / (alpha * (t + t0))` where t0 is chosen by a heuristic proposed by Leon Bottou.
        - 'invscaling': `eta = eta0 / pow(t, power_t)`
        - 'adaptive': eta = eta0, as long as the training keeps decreasing.  Each time n_iter_no_change consecutive epochs fail to decrease the
          training loss by tol or fail to increase validation score by tol if early_stopping is True, the current learning rate is divided by 5.
    ...
    Notes
    -----
    This estimator has a linear complexity in the number of training samples and is thus better suited 
    than the `sklearn.svm.OneClassSVM` implementation for datasets with a large number of training samples (say > 10,000).
    ...
    def __init__(self, nu=0.5, fit_intercept=True, max_iter=1000, tol=1e-3, shuffle=True, verbose=0,       \
        random_state=None, learning_rate="optimal", eta0=0.0, power_t=0.5, warm_start=False, average=False):
        self.nu = nu
        super(SGDOneClassSVM, self).__init__(loss="hinge", penalty="l2", C=1.0, l1_ratio=0, fit_intercept=fit_intercept,      \
            max_iter=max_iter, tol=tol, shuffle=shuffle, verbose=verbose, epsilon=DEFAULT_EPSILON, random_state=random_state, \
            learning_rate=learning_rate, eta0=eta0, power_t=power_t, early_stopping=False, validation_fraction=0.1,           \
            n_iter_no_change=5, warm_start=warm_start, average=average)
    ...
    def _fit_one_class(self, X, alpha, C, sample_weight, learning_rate, max_iter):
        ...
        # The One-Class SVM uses the SGD implementation with y=np.ones(n_samples).
        n_samples = X.shape[0]
        ...
        # early stopping is set to False for the One-Class SVM. 
        # thus validation_mask and validation_score_cb will be set to values associated to early_stopping=False 
        # in _make_validation_split and _make_validation_score_cb respectively.
        validation_mask = self._make_validation_split(y, sample_mask=sample_weight > 0)
        validation_score_cb = self._make_validation_score_cb(validation_mask, X, y, sample_weight)
        ...
        one_class = 1
        # There are no class weights for the One-Class SVM and they are therefore set to 1.
        pos_weight = 1
        neg_weight = 1
        ...
        _plain_sgd = _get_plain_sgd_function(input_dtype=coef.dtype)
        coef, intercept, average_coef, average_intercept, self.n_iter_ = _plain_sgd( \
            coef, intercept[0], average_coef, average_intercept[0], self.loss_function_, penalty_type,   \
            alpha, C, self.l1_ratio, dataset, validation_mask, self.early_stopping, validation_score_cb, \
            int(self.n_iter_no_change), max_iter, tol, int(self.fit_intercept), int(self.verbose),       \
            int(self.shuffle), seed, neg_weight, pos_weight, learning_rate_type, self.eta0, self.power_t,\
            one_class, self.t_, offset_decay, self.average)
    ...
    def _partial_fit(self, X, alpha, C, loss, learning_rate, max_iter, sample_weight, coef_init, offset_init):
        first_call = getattr(self, "coef_", None) is None
        X = self._validate_data(X, None, accept_sparse="csr", dtype=[np.float64, np.float32], order="C", \
            accept_large_sparse=False, reset=first_call)
        ...
        # We use intercept = 1 - offset where intercept is the intercept of the SGD implementation 
        # and offset is the offset of the One-Class SVM optimization problem.
        if getattr(self, "coef_", None) is None or coef_init is not None:
            self._allocate_parameter_mem(n_classes=1, n_features=n_features, input_dtype=X.dtype, \
                coef_init=coef_init, intercept_init=offset_init, one_class=1)
        elif n_features != self.coef_.shape[-1]:
            raise ValueError("Number of features %d does not match previous data %d." % (n_features, self.coef_.shape[-1]))
    ...
    def partial_fit(self, X, y=None, sample_weight=None):
        ...
        Returns
        -------
        self : object, Returns a fitted instance of self.
        """
        if not hasattr(self, "coef_"):
            self._more_validate_params(for_partial_fit=True)

        alpha = self.nu / 2
        return self._partial_fit(X, alpha, C=1.0, loss=self.loss, learning_rate=self.learning_rate, \
            max_iter=1, sample_weight=sample_weight, coef_init=None, offset_init=None)
    ...
    def fit(self, X, y=None, coef_init=None, offset_init=None, sample_weight=None):
        ...
        This solves an equivalent optimization problem of the One-Class SVM primal optimization problem 
        and returns a weight vector w and an offset rho such that the decision function is given by <w, x> - rho.
        ...
        alpha = self.nu / 2
        self._fit(X, alpha=alpha, C=1.0, loss=self.loss, learning_rate=self.learning_rate, coef_init=coef_init, \
            offset_init=offset_init, sample_weight=sample_weight)
    ...
    def decision_function(self, X):
        ...
        Returns
        -------
        dec : array-like, shape (n_samples,), Decision function values of the samples.
        """
    ...
    def score_samples(self, X):
        ...
        Returns
        -------
        score_samples : array-like, shape (n_samples,), Unshiffted scoring function values of the samples.
        """
    ...
    def predict(self, X):
        ...
        Returns
        -------
        y : array, shape (n_samples,), Labels of the samples.
        """
