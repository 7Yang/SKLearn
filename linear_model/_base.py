...
# TODO: bayesian_ridge_regression and bayesian_regression_ard should be squashed into its respective objects.

SPARSE_INTERCEPT_DECAY = 0.01
# For sparse data intercept updates are scaled by this decay factor to avoid intercept oscillation.
...
def _deprecate_normalize(normalize, estimator_name):
    """
    Normalize is to be deprecated from linear models and a use of a pipeline with a StandardScaler is to be recommended instead.
    Here the appropriate message is selected to be displayed to the user depending on the default normalize value 
    (as it varies between the linear models and normalize value selected by the user).
    ...
    estimator_name : str
        name of the linear estimator which calls this function.  The name will be used for writing the deprecation warnings

    Returns
    -------
    normalize : bool,
        normalize value which should further be used by the estimator at this stage of the depreciation process
    ...
    pipeline_msg = ("If you wish to scale the data, use Pipeline with a StandardScaler in a preprocessing stage. " \
        "To reproduce the previous behavior:\n\n from sklearn.pipeline import make_pipeline\n\n"                   \
        "model = make_pipeline(StandardScaler(with_mean=False), "                                                  \
        f"{estimator_name}())\n\n If you wish to pass a sample_weight parameter, "                                 \
        "you need to pass it as a fit parameter to each step of the pipeline as follows:\n\n"                      \
    ...
    if normalize != "deprecated" and normalize:
        warnings.warn("'normalize' was deprecated in version 1.2 and will be removed in 1.4.\n"
...
def make_dataset(X, y, sample_weight, random_state=None):
    ...
    This also returns the ``intercept_decay`` which is different for sparse datasets.
    ...
    random_state : int, RandomState instance or None (default)
        Determines random number generation for dataset random sampling. It is not used for dataset shuffling.
...
def _preprocess_data(X, y, fit_intercept, normalize=False, copy=True, copy_y=True, sample_weight=None, check_input=True):
    """
    Center and scale data.

    Centers data to have mean zero along axis 0. If fit_intercept=False or if the X is a sparse matrix, no centering is done, 
    but normalization can still be applied. The function returns the statistics necessary to reconstruct the input data, 
    which are X_offset, y_offset, X_scale, such that the output

        X = (X - X_offset) / X_scale

    X_scale is the L2 norm of X - X_offset. If sample_weight is not None, then the weighted mean of X and y is zero, 
    and not the mean itself. If fit_intercept=True, the mean, eventually weighted, is returned, 
    independently of whether X was centered (option used for optimization with sparse data in coordinate_descend).

    This is here because nearly all linear models will want their data to be centered. 
    This function also systematically makes y consistent with X.dtype

    Returns
    -------
    X_out    : {ndarray, sparse matrix} of shape (n_samples, n_features)
        If copy=True a copy of the input X is triggered, otherwise operations are inplace.
        If input X is dense, then X_out is centered.
        If normalize is True, then X_out is rescaled (dense and sparse case)
    y_out    : {ndarray, sparse matrix} of shape (n_samples,) or (n_samples, n_targets) Centered version of y. Likely performed inplace on input y.
    X_offset : ndarray of shape (n_features,) The mean per column of input X.
    y_offset : float or ndarray of shape (n_features,)
    X_scale  : ndarray of shape (n_features,) The standard deviation per column of input X.
    """
    ...
    if fit_intercept:
        if sp.issparse(X):
            ...
            if normalize:
                X_offset, X_var, _ = _incremental_mean_and_var( \ 
                    X, last_mean=0.0, last_variance=0.0, last_sample_count=0.0, sample_weight=sample_weight)
            else:
        ...
        if normalize:
            ...
            # Detect constant features on the computed variance, before taking the np.sqrt. 
            # Otherwise constant features cannot be detected with sample weights.
...
# Currently, the fact that sag implements its own way to deal with sample_weight makes the refactoring tricky.
...
def _rescale_data(X, y, sample_weight, inplace=False):
    ...
    """
    # Assume that _validate_data and _check_sample_weight have been called by the caller.
    n_samples = X.shape[0]
...
class LinearModel(BaseEstimator, metaclass=ABCMeta):
    ...
    def _set_intercept(self, X_offset, y_offset, X_scale):
        ...
        if self.fit_intercept:
            # We always want coef_.dtype=X.dtype. For instance, X.dtype can differ from coef_.dtype if warm_start=True.
...
class LinearClassifierMixin(ClassifierMixin):
    ...
    def _predict_proba_lr(self, X):
        ...
        Positive class probabilities are computed as 1. / (1. + np.exp(-self.decision_function(X)));
...
class SparseCoefMixin:
    ...
    def densify(self):
        ...
        Converts the ``coef_`` member (back) to a numpy.ndarray. 
        This is the default format of ``coef_`` and is required for fitting, 
        so calling this method is only required on models that have previously been sparsified; 
    ...
    def sparsify(self):
        ...
        which for L1-regularized models can be much more memory- and storage-efficient than the usual numpy.ndarray representation.
        ...
        A rule of thumb is that the number of zero elements, 
        which can be computed with ``(coef_ == 0).sum()``, 
...
class LinearRegression(MultiOutputMixin, RegressorMixin, LinearModel):
    ...
    or Non Negative Least Squares (scipy.optimize.nnls) wrapped as a predictor object.
    ...
    def fit(self, X, y, sample_weight=None):
        ...
        # LinearRegression can benefit from in-place operations when X is a sparse matrix. 
        # Therefore, let's not copy X when it is sparse.
        ...
        X, y, X_offset, y_offset, X_scale = _preprocess_data(X, y, fit_intercept=self.fit_intercept, \
            copy=copy_X_in_preprocess_data, sample_weight=sample_weight)

        if has_sw:
            # Sample weight can be implemented via a simple rescaling. 
            # Note that we safely do inplace rescaling when _preprocess_data has already made a copy if requested.
...
def _check_precomputed_gram_matrix(X, precompute, X_offset, X_scale, rtol=None, atol=1e-5):
    """
    Computes a single element of the gram matrix and compares it to the corresponding element of the user supplied gram matrix.
    ...
    rtol : float, default=None
        Relative tolerance; see numpy.allclose If None, it is set to 1e-4 for arrays of dtype numpy.float32 and 1e-7 otherwise.

    atol : float, default=1e-5
        absolute tolerance; see :func`numpy.allclose`. 
        Note that the default here is more tolerant than the default for :func:`numpy.testing.assert_allclose`, where `atol=0`.

    Raises
    ------
    ValueError, Raised when the provided Gram matrix is not consistent.
    """
    ...
    if not np.isclose(expected, actual, rtol=rtol, atol=atol):
        raise ValueError("Gram matrix passed in via 'precompute' parameter did not pass validation when a single element was "
            "checked - please check that it was computed properly. For element ({f1},{f2}) we computed {expected} "
            "but the user-supplied value was {actual}.")
...
def _pre_fit(X, y, Xy, precompute, normalize, fit_intercept, copy, check_input=True, sample_weight=None):
    ...
    This function applies _preprocess_data and additionally computes the gram matrix `precompute` as needed as well as `Xy`.
    """
    ...
    if sparse.issparse(X):
        ...
        precompute = False
        X, y, X_offset, y_offset, X_scale = _preprocess_data(X, y, fit_intercept=fit_intercept, normalize=normalize, \ 
            copy=False, check_input=check_input, sample_weight=sample_weight)
    else:
        ...
        # Rescale only in dense case. Sparse cd solver directly deals with sample_weight.
        if sample_weight is not None:
    ...
    if hasattr(precompute, "__array__"):
        if (fit_intercept and not np.allclose(X_offset, np.zeros(n_features)) or normalize and not np.allclose(X_scale, np.ones(n_features))):
            warnings.warn(("Gram matrix was provided but X was centered to fit intercept, or X was normalized : recomputing Gram matrix."), 
                UserWarning,)
            # recompute Gram
            precompute = "auto"
            Xy = None
        elif check_input:
            # If we're going to use the user's precomputed gram matrix, we do a quick check to make sure its not totally bogus.
    ...
    if hasattr(precompute, "__array__") and Xy is None:
        ...
        if y.ndim == 1:
            ...
            # Make sure that Xy is always F contiguous even if X or y are not contiguous: 
            # the goal is to make it fast to extract the data for a specific target.
            n_targets = y.shape[1]
