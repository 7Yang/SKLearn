...
def _modified_weiszfeld_step(X, x_old):
    ...
    This function defines one iteration step in order to approximate the spatial median (L1 median). 
    It is a form of an iteratively re-weighted least squares method.
    ...
    References
    ----------
    - On Computation of Spatial Median for Robust Data Mining, 2005 T. Kärkkäinen and S. Äyrämö
...
def _spatial_median(X, max_iter=300, tol=1.0e-3):
    ...
    The spatial median is member of a class of so-called M-estimators which are defined by an optimization problem. 
    Given a number of p points in an n-dimensional space, the point x minimizing the sum of all distances to 
    the p other points is called spatial median.
    ...
    for n_iter in range(max_iter):
        ...
        warnings.warn("Maximum number of iterations {max_iter} reached in spatial median for TheilSen regressor."
...
def _breakdown_point(n_samples, n_subsamples):
    ...
    Returns
    -------
    breakdown_point : float, Approximation of breakdown point.
    """
...
def _lstsq(X, y, indices, fit_intercept):
    ...
    This function calculates the least squares method on a subset of rows of X and y defined by the indices array. 
    Optionally, an intercept column is added if intercept is set to true.
    ...
    Returns
    -------
    weights : ndarray of shape (n_subpopulation, n_features + intercept), Solution matrix of n_subpopulation solved least square problems.
    """
...
class TheilSenRegressor(RegressorMixin, LinearModel):
    ...
    The algorithm calculates least square solutions on subsets with size n_subsamples of the samples in X. 
    Any value of n_subsamples between the number of features and samples leads to an estimator with a compromise
    between robustness and efficiency. Since the number of least square solutions is "n_samples choose n_subsamples", 
    it can be extremely large and can therefore be limited with max_subpopulation. 
    If this limit is reached, the subsets are chosen randomly. 
    In a final step, the spatial median (or L1 median) is calculated of all least square solutions.
    ...
    max_subpopulation : int, default=1e4
        ...
        Note that the data type should be int but floats such as 1e4 can be accepted too.
    ...
    n_subsamples : int, default=None
        ...
        A lower number leads to a higher breakdown point and a low efficiency while a high number leads to 
        a low breakdown point and a high efficiency. 
    ...
    def __init__(self, *, fit_intercept=True, copy_X=True, max_subpopulation=1e4, n_subsamples=None, max_iter=300, \
        tol=1.0e-3, random_state=None, n_jobs=None, verbose=False):
        ...
        if n_subsamples is not None:
            ...
            if n_samples >= n_features:
                if n_dim > n_subsamples:
                    ...
                    raise ValueError("Invalid parameter since n_features{0} > n_subsamples ({1} > {2})."
                        .format(plus_1, n_dim, n_subsamples))
            else:  # if n_samples < n_features
                if n_subsamples != n_samples:
                    raise ValueError("Invalid parameter since n_subsamples != n_samples ({0} != {1}) while n_samples < n_features."
    ...
    def fit(self, X, y):
        ...
        Returns
        -------
        self : returns an instance of self.  Fitted `TheilSenRegressor` estimator.
        """
