...
class LinearModelLoss:
    ...
    The loss is the sum of per sample losses and includes a term for L2 regularization::
        loss = sum_i s_i loss(y_i, X_i @ coef + intercept) + 1/2 * l2_reg_strength * ||coef||_2^2
    ...
    The option order="F" makes coef[:, i] contiguous. 
    This, in turn, makes the coefficients without intercept, coef[:, :-1], contiguous and speeds up matrix-vector computations.

    Note: If the average loss per sample is wanted instead of the sum of the loss per sample, 
    one can simply use a rescaled sample_weight such that sum(sample_weight) = 1.
    ...
    def init_zero_coef(self, X, dtype=None):
        ...
        dtype : data-type, default=None, Overrides the data type of coef. With dtype=None, coef will have the same dtype as X.
    ...
    def weight_intercept(self, coef):
        ...
        Parameters
        ----------
        coef : ndarray of shape (n_dof,), (n_classes, n_dof) or (n_classes * n_dof,) Coefficients of a linear model.
            If shape (n_classes * n_dof,), the classes of one feature are contiguous, 
            i.e. one reconstructs the 2d-array via coef.reshape((n_classes, -1), order="F").

        Returns
        -------
        weights : ndarray of shape (n_features,) or (n_classes, n_features), Coefficients without intercept term.
    ...
    def loss(self, coef, X, y, sample_weight=None, l2_reg_strength=0.0, n_threads=1, raw_prediction=None):
        ...
        raw_prediction : C-contiguous array of shape (n_samples,) or array of shape (n_samples, n_classes)
            Raw prediction values (in link space). If provided, these are used. If None, then raw_prediction = X @ coef + intercept is calculated.

        Returns
        -------
        loss : float, Sum of losses per sample plus penalty.
        """
        ...
        loss = self.base_loss.loss(y_true=y, raw_prediction=raw_prediction, sample_weight=sample_weight, n_threads=n_threads)
        loss = loss.sum()
    ...
    def loss_gradient(self, coef, X, y, sample_weight=None, l2_reg_strength=0.0, n_threads=1, raw_prediction=None):
        ...
        loss, grad_pointwise = self.base_loss.loss_gradient( \
            y_true=y, raw_prediction=raw_prediction, sample_weight=sample_weight, n_threads=n_threads)
        loss = loss.sum()
    ...
    def gradient(self, coef, X, y, sample_weight=None, l2_reg_strength=0.0, n_threads=1, raw_prediction=None):
        ...
        coef : ndarray of shape (n_dof,), (n_classes, n_dof) or (n_classes * n_dof,) Coefficients of a linear model.
            ...
            i.e. one reconstructs the 2d-array via coef.reshape((n_classes, -1), order="F").
        X : {array-like, sparse matrix} of shape (n_samples, n_features), Training data.
        y : contiguous array of shape (n_samples,), Observed, true target values.
        sample_weight   : None or contiguous array of shape (n_samples,), default=None, Sample weights.
        l2_reg_strength : float, default=0.0, L2 regularization strength
        n_threads       : int, default=1, Number of OpenMP threads to use.
        raw_prediction  : C-contiguous array of shape (n_samples,) or array of shape (n_samples, n_classes)
            Raw prediction values (in link space). If provided, these are used. If None, then raw_prediction = X @ coef + intercept is calculated.
    ...
    def gradient_hessian(self, coef, X, y, sample_weight=None, l2_reg_strength=0.0, n_threads=1, gradient_out=None, \
        hessian_out=None, raw_prediction=None):
        """
        ...
        l2_reg_strength : float, default=0.0, L2 regularization strength
        n_threads       : int, default=1, Number of OpenMP threads to use.
        gradient_out    : None or ndarray of shape coef.shape A location into which the gradient is stored. If None, a new array might be created.
        hessian_out     : None or ndarray,  A location into which the hessian is stored. If None, a new array might be created.
        raw_prediction  : C-contiguous array of shape (n_samples,) or array of shape (n_samples, n_classes)
            Raw prediction values (in link space). If provided, these are used. If None, then raw_prediction = X @ coef + intercept is calculated.
        ...
        -------
        gradient : ndarray of shape coef.shape, The gradient of the loss.
        hessian  : ndarray, Hessian matrix.
        hessian_warning : bool, True if pointwise hessian has more than half of its elements non-positive.
        """
        ...
        grad_pointwise, hess_pointwise = self.base_loss.gradient_hessian(y_true=y, raw_prediction=raw_prediction,\
            sample_weight=sample_weight, n_threads=n_threads)

        # For non-canonical link functions and far away from the optimum, the pointwise hessian can be negative. 
        # We take care that 75% of the hessian entries are positive.
        ...
        if not self.base_loss.is_multiclass:
            ...
            # TODO: This "sandwich product", X' diag(W) X, is the main computational bottleneck for solvers. 
            # A dedicated Cython routine might improve it exploiting the symmetry (as opposed to, e.g., BLAS gemm).
            if sparse.issparse(X):
                hess[:n_features, :n_features] = (X.T @ sparse.dia_matrix( (hess_pointwise, 0), shape=(n_samples, n_samples)) @ X).toarray()
            else:
                # np.einsum may use less memory but the following, using BLAS matrix multiplication (gemm), is by far faster.
                WX = hess_pointwise[:, None] * X
                hess[:n_features, :n_features] = np.dot(X.T, WX)

            if l2_reg_strength > 0:
                # The L2 penalty enters the Hessian on the diagonal only. To add those terms, we use a flattened view on the array.
            ...
            if self.fit_intercept:
                ...
                #      = (X' @ diag(h) @ X, X' @ h)
                #        (           h @ X, sum(h))
                # The left upper part has already been filled, it remains to compute the last row and the last column.
                Xh = X.T @ hess_pointwise
                hess[:-1, -1] = Xh
                hess[-1, :-1] = Xh
                hess[-1, -1] = hess_pointwise.sum()
        else:
            # Here we may safely assume HalfMultinomialLoss aka categorical cross-entropy.
    ...
    def gradient_hessian_product(self, coef, X, y, sample_weight=None, l2_reg_strength=0.0, n_threads=1):
        ...
        if not self.base_loss.is_multiclass:
            ...
            grad = np.empty_like(coef, dtype=weights.dtype)
            grad[:n_features] = X.T @ grad_pointwise + l2_reg_strength * weights
            if self.fit_intercept: grad[-1] = grad_pointwise.sum()
            ...
            # Here we may safely assume HalfMultinomialLoss aka categorical cross-entropy.
            # HalfMultinomialLoss computes only the diagonal part of the hessian, i.e. diagonal in the classes. 
            # Here, we want the matrix-vector product of the full hessian. Therefore, we call gradient_proba.
            grad_pointwise, proba = self.base_loss.gradient_proba(y_true=y, raw_prediction=raw_prediction,
                sample_weight=sample_weight, n_threads=n_threads)
            grad = np.empty((n_classes, n_dof), dtype=weights.dtype, order="F")
            grad[:, :n_features] = grad_pointwise.T @ X + l2_reg_strength * weights
            if self.fit_intercept: grad[:, -1] = grad_pointwise.sum(axis=0)

            # Full hessian-vector product, i.e. not only the diagonal part of the hessian.
            # Derivation with some index battle for input vector s:
            #   - sample index i
            #   - feature indices j, m
            #   - class indices k, l
            #   - 1_{k=l} is one if k=l else 0
            #   - p_i_k is the (predicted) probability that sample i belongs to class k for all i: sum_k p_i_k = 1
            ...
            #   = sum_{i, l, m} (X')_{ji} * p_i_k * (1_{k=l} - p_i_l) * X_{im} s_l_m
            #   = sum_{i, m} (X')_{ji} * p_i_k * (X_{im} * s_k_m - sum_l p_i_l * X_{im} * s_l_m)
            #
            ...
            def hessp(s):
                ...
                # hess_prod = empty_like(grad), but we ravel grad below and this function is run after that.
