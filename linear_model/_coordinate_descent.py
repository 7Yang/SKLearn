...
from ..utils.validation import (_check_sample_weight, check_consistent_length, check_is_fitted, check_random_state, column_or_1d)
...
def _set_order(X, y, order="C"):
    ...
    order : {None, 'C', 'F'}
        If 'C', dense arrays are returned as C-ordered, sparse matrices in csr format. 
        If 'F', dense arrays are return as F-ordered, sparse matrices in csc format.
...
def _alpha_grid(X, y, Xy=None, l1_ratio=1.0, fit_intercept=True, eps=1e-3, n_alphas=100, copy_X=True):
    ...
    l1_ratio : float, default=1.0
        ...
        For ``l1_ratio = 0`` the penalty is an L2 penalty. (currently not supported) ``
...
def lasso_path(X, y, *, eps=1e-3, n_alphas=100, alphas=None, precompute="auto", Xy=None, copy_X=True,
    coef_init=None, verbose=False, return_n_iter=False, positive=False, **params):
    """
    ...
    **params : kwargs
    ...
    Notes
    -----
    For an example, see :ref:`examples/linear_model/plot_lasso_coordinate_descent_path.py
    <sphx_glr_auto_examples_linear_model_plot_lasso_coordinate_descent_path.py>`.

    To avoid unnecessary memory duplication the X argument of the fit method should be directly passed as a Fortran-contiguous numpy array.

    Note that in certain cases, the Lars solver may be significantly faster to implement this functionality. 
    In particular, linear interpolation can be used to retrieve model coefficients between the values output by lars_path
    ...
    """
    return enet_path(X, y, l1_ratio=1.0, eps=eps, n_alphas=n_alphas, alphas=alphas, precompute=precompute, Xy=Xy, copy_X=copy_X, \
        coef_init=coef_init, verbose=verbose, positive=positive, return_n_iter=return_n_iter, **params)
...
def enet_path(X, y, *, l1_ratio=0.5, eps=1e-3, n_alphas=100, alphas=None, precompute="auto", Xy=None, copy_X=True, coef_init=None, \
    verbose=False, return_n_iter=False, positive=False, check_input=True, **params):
    """
    ...
    precompute : 'auto', bool or array-like of shape (n_features, n_features), default='auto'
        ...
        calculations. If set to ``'auto'`` let us decide. The Gram matrix can also be passed as argument.
    ...
    check_input : bool, default=True
        If set to False, the input validation checks are skipped (including the Gram matrix when provided). 
        It is assumed that they are handled by the caller.
    ...
    **params : kwargs
    ...
    # We expect X and y to be already Fortran ordered when bypassing checks
    if check_input:
    ...
    if not multi_output and sparse.issparse(X):
        if X_offset_param is not None:
            # As sparse matrices are not actually centered we need this to be passed to the CD solver.
            X_sparse_scaling = X_offset_param / X_scale_param
            X_sparse_scaling = np.asarray(X_sparse_scaling, dtype=X.dtype)
        else:
            X_sparse_scaling = np.zeros(n_features, dtype=X.dtype)

    # X should have been passed through _pre_fit already if function is called from ElasticNet.fit
    if check_input:
    ...
    for i, alpha in enumerate(alphas):
        ...
        if not multi_output and sparse.issparse(X):
            model = cd_fast.sparse_enet_coordinate_descent(w=coef_, alpha=l1_reg, beta=l2_reg, X_data=X.data, X_indices=X.indices, \
                X_indptr=X.indptr, y=y, sample_weight=sample_weight, X_mean=X_sparse_scaling, max_iter=max_iter, tol=tol, rng=rng, \
        ...
        coefs[..., i] = coef_
        # we correct the scale of the returned dual gap, as the objective in cd_fast is n_samples * the objective in this docstring.
...
class ElasticNet(MultiOutputMixin, RegressorMixin, LinearModel):
    ...
    Specifically, l1_ratio = 1 is the lasso penalty. Currently, l1_ratio <= 0.01 is not reliable, unless you supply your own sequence of alpha.
    ...
    tol : float, default=1e-4
        ...
        the optimization code checks the dual gap for optimality and continues until it is smaller than ``tol``, see Notes below.
    ...
    The precise stopping criteria based on `tol` are the following: First, check that that maximum coordinate update, 
    i.e. :math:`\\max_j |w_j^{new} - w_j^{old}|` is smaller than `tol` times the maximum absolute coefficient, 
    :math:`\\max_j |w_j|`.  If so, then additionally check whether the dual gap is smaller than `tol` times
    ...
    def __init__(self, alpha=1.0, *, l1_ratio=0.5, fit_intercept=True, precompute=False, max_iter=1000, \
        copy_X=True, tol=1e-4, warm_start=False, positive=False, random_state=None, selection="cyclic"):
        self.alpha         = alpha
    ...
    def fit(self, X, y, sample_weight=None, check_input=True):
        ...
        Notes
        -----
        Coordinate descent is an algorithm that considers each column of data at a time hence it will automatically convert the X input
        as a Fortran-contiguous numpy array if necessary.

        To avoid memory re-allocation it is advised to allocate the initial data in memory directly using that format.
        """
        if self.alpha == 0:
            warnings.warn(("With alpha=0, this algorithm does not converge well. You are advised to use the LinearRegression estimator"),
                stacklevel=2)

        # Remember if X is copied
        X_copied = False
        # We expect X and y to be float64 or float32 Fortran ordered arrays when bypassing checks
        if check_input:
            X_copied = self.copy_X and self.fit_intercept
            X, y = self._validate_data(X, y, accept_sparse="csc", order="F", dtype=[np.float64, np.float32], copy=X_copied, \
        ...
        if sample_weight is not None:
            ...
            #
            # inside its function body, which results in objective (2) being equivalent to (1) in case of no sw.
            # With sw, however, enet_path should set
            #
            #     alpha' = sum(sw) * alpha                                     (4)
            #
            # Therefore, we use the freedom of Eq. (1) to rescale sw before calling enet_path, i.e.
            #
            #     sw *= n_samples / sum(sw)
            #
            # such that sum(sw) = n_samples. This way, (3) and (4) are the same.
            sample_weight = sample_weight * (n_samples / np.sum(sample_weight))
            # Note: Alternatively, we could also have rescaled alpha instead of sample_weight:
            #
            #     alpha *= np.sum(sample_weight) / n_samples

        # Ensure copying happens only once, don't do it again if done above.
        # X and y will be rescaled if sample_weight is not None, order='F' ensures that the returned X and y are still F-contiguous.
        should_copy = self.copy_X and not X_copied
        X, y, X_offset, y_offset, X_scale, precompute, Xy = _pre_fit(X, y, None, self.precompute, normalize=False,  \
            fit_intercept=self.fit_intercept, copy=should_copy, check_input=check_input, sample_weight=sample_weight)
        # coordinate descent needs F-ordered arrays and _pre_fit might have called _rescale_data
        ...
        for k in range(n_targets):
            if Xy is not None:
                ...
                this_Xy = None
            _, this_coef, this_dual_gap, this_iter = self.path(X, y[:, k], l1_ratio=self.l1_ratio, eps=None, n_alphas=None, alphas=[alpha],    \
                precompute=precompute, Xy=this_Xy, copy_X=True, coef_init=coef_[k], verbose=False, return_n_iter=True, positive=self.positive, \
                check_input=False, tol=self.tol, X_offset=X_offset, X_scale=X_scale, max_iter=self.max_iter, random_state=self.random_state,   \
                selection=self.selection, sample_weight=sample_weight)
            coef_[k]       = this_coef[:, 0]
        ...
        if not all(np.isfinite(w).all() for w in [self.coef_, self.intercept_]):
            raise ValueError("Coordinate descent iterations resulted in non-finite parameter values. "
    ...
    def _decision_function(self, X):
        ...
        Returns
        -------
        T : ndarray of shape (n_samples,), The predicted decision function.
        """
...
class Lasso(ElasticNet):
    ...
    alpha : float, default=1.0
        ...
        For numerical reasons, using `alpha = 0` with the `Lasso` object is not advised. Instead, you should use the :class:`LinearRegression` object.
    ...
    warm_start : bool, default=False
        ...
        otherwise, just erase the previous solution. See :term:`the Glossary <warm_start>`.
    ...
    random_state : int, RandomState instance, default=None
        ...
        Used when ``selection`` == 'random'. Pass an int for reproducible output across multiple function calls.
    ...
    or :class:`~sklearn.svm.LinearSVC`. If an array is passed, penalties are assumed to be specific to the targets. 
    Hence they must correspond in number.

    The precise stopping criteria based on `tol` are the following: First, check that that maximum coordinate update, 
    i.e. :math:`\\max_j |w_j^{new} - w_j^{old}|` is smaller than `tol` times the maximum absolute coefficient, :math:`\\max_j |w_j|`.
    If so, then additionally check whether the dual gap is smaller than `tol` times :math:`||y||_2^2 / n_{\\text{samples}}`.
    ...
    It should not be confused with :class:`~sklearn.linear_model.MultiTaskLasso` 
    which instead penalizes the :math:`L_{2,1}` norm of the coefficients, yielding row-wise sparsity in the coefficients.
    ...
    def __init__(self, alpha=1.0, *, fit_intercept=True, precompute=False, copy_X=True, max_iter=1000, tol=1e-4,       \
        warm_start=False, positive=False, random_state=None, selection="cyclic"):
        super().__init__(alpha=alpha, l1_ratio=1.0, fit_intercept=fit_intercept, precompute=precompute, copy_X=copy_X, \
...
def _path_residuals(X, y, sample_weight, train, test, fit_intercept, path, path_params, \
    ...
    path : callable
        Function returning a list of models on the path. See enet_path for an example of signature.

    path_params : dictionary
        Parameters passed to the path function.

    alphas : array-like, default=None
        Array of float that is used for cross-validation. If not provided, computed using 'path'.
    ...
    X_order : {'F', 'C'}, default=None
        The order of the arrays expected by the path function to avoid memory copies.

    dtype : a numpy dtype, default=None
        The dtype of the arrays expected by the path function to avoid memory copies.
    """
    X_train = X[train]
    y_train = y[train]
    X_test = X[test]
    ...
    if not sparse.issparse(X):
        for array, array_input in ((X_train, X), (y_train, y), (X_test, X), (y_test, y)):
            if array.base is not array_input and not array.flags["WRITEABLE"]:
                # fancy indexing should create a writable copy but it doesn't for read-only memmaps (cf. numpy#14132).
    ...
    X_train, y_train, X_offset, y_offset, X_scale, precompute, Xy = _pre_fit(X_train, y_train, \
        None, precompute, normalize=False, fit_intercept=fit_intercept, copy=False, sample_weight=sw_train)
...
class LinearModelCV(MultiOutputMixin, LinearModel, ABC):
    ...
    @abstractmethod
    def __init__(self, eps=1e-3, n_alphas=100, alphas=None, fit_intercept=True, precompute="auto", max_iter=1000, tol=1e-4, \
    ...
    def fit(self, X, y, sample_weight=None):
        ...
        X : {array-like, sparse matrix} of shape (n_samples, n_features)
            Training data. Pass directly as Fortran-contiguous data to avoid unnecessary memory duplication. 
            If y is mono-output, X can be sparse.

        y : array-like of shape (n_samples,) or (n_samples, n_targets)
            Target values.

        sample_weight : float or array-like of shape (n_samples,), default=None
            Sample weights used for fitting and evaluation of the weighted mean squared error of each cv-fold. 
            Note that the cross validated MSE that is finally used to find the best model 
            is the unweighted mean over the (weighted) MSEs of each test fold.

        Returns
        -------
        self : object, Returns an instance of fitted model.
        """
        # This makes sure that there is no duplication in memory. Dealing right with copy_X is important in the following:
        # Multiple functions touch X and subsamples of X and can induce a lot of duplication of memory
        ...
        if isinstance(X, np.ndarray) or sparse.issparse(X):
            ...
            # Let us not impose fortran ordering so far: 
            # it is not useful for the cross-validation loop and will be done by the model fitting itself

            # Need to validate separately here.
            # We can't pass multi_output=True because that would allow y to be csr. 
            # We also want to allow y to be 64 or 32 but check_X_y only allows to convert for 64.
        ...
        if alphas is None:
            alphas = [_alpha_grid(X, y, l1_ratio=l1_ratio, fit_intercept=self.fit_intercept, eps=self.eps, \
                n_alphas=self.n_alphas, copy_X=self.copy_X) for l1_ratio in l1_ratios]
        else:
        ...
        # We do a double for loop folded in one, in order to be able to iterate in parallel on l1_ratio and folds
        jobs = (delayed(_path_residuals)(X, y, sample_weight, train, test, self.fit_intercept, self.path, path_params, \
                alphas=this_alphas, l1_ratio=this_l1_ratio, X_order="F", dtype=X.dtype.type)
        ...
        if sample_weight is None:
            # MultiTaskElasticNetCV does not (yet) support sample_weight, even not sample_weight=None.
            model.fit(X, y)
        else:
    ...
    def _more_tags(self):
        # Note: check_sample_weights_invariance(kind='ones') should work, but currently we can only mark a whole test as xfail.
...
class LassoCV(RegressorMixin, LinearModelCV):
    ...
    To avoid unnecessary memory duplication the `X` argument of the `fit` method should be directly passed as a Fortran-contiguous numpy array.
     ...
     For an example, see
     :ref:`examples/linear_model/plot_lasso_model_selection.py <sphx_glr_auto_examples_linear_model_plot_lasso_model_selection.py>`.

    :class:`LassoCV` leads to different results than a hyperparameter search using :class:`~sklearn.model_selection.GridSearchCV` with a
    :class:`Lasso` model. In :class:`LassoCV`, a model for a given penalty `alpha` is warm started using the coefficients of the
    closest model (trained at the previous iteration) on the regularization path. It tends to speed up the hyperparameter search.
    ...
    def __init__(self, *, eps=1e-3, n_alphas=100, alphas=None, fit_intercept=True, precompute="auto", max_iter=1000, tol=1e-4, \
        copy_X=True, cv=None, verbose=False, n_jobs=None, positive=False, random_state=None, selection="cyclic"):
        super().__init__(eps=eps, n_alphas=n_alphas, alphas=alphas, fit_intercept=fit_intercept, precompute=precompute,        \
            max_iter=max_iter, tol=tol, copy_X=copy_X, cv=cv, verbose=verbose, n_jobs=n_jobs, positive=positive,               \
...
class ElasticNetCV(RegressorMixin, LinearModelCV):
    ...
    Notes
    -----
    In `fit`, once the best parameters `l1_ratio` and `alpha` are found through cross-validation, 
    the model is fit again using the entire training set.
    ...
    def __init__(self, *, l1_ratio=0.5, eps=1e-3, n_alphas=100, alphas=None, fit_intercept=True, precompute="auto", \
        max_iter=1000, tol=1e-4, cv=None, copy_X=True, verbose=0, n_jobs=None, positive=False, random_state=None, selection="cyclic"):
        self.l1_ratio      = l1_ratio
        self.eps           = eps
        self.n_alphas      = n_alphas
        self.alphas        = alphas
...
class MultiTaskElasticNet(Lasso):
    ...
    l1_ratio : float, default=0.5
        The ElasticNet mixing parameter, with 0 < l1_ratio <= 1. For l1_ratio = 1 the penalty is an L1/L2 penalty. 
        For l1_ratio = 0 it is an L2 penalty.  For ``0 < l1_ratio < 1``, the penalty is a combination of L1/L2 and L2.
    ...
    def __init__(self, alpha=1.0, *, l1_ratio=0.5, fit_intercept=True, copy_X=True, max_iter=1000, tol=1e-4, warm_start=False,
    ...
    def fit(self, X, y):
        ...
        y : ndarray of shape (n_samples, n_targets ), Target. Will be cast to X's dtype if necessary.
        ...
        To avoid memory re-allocation it is advised to allocate the initial data in memory directly using that format.
        """
        # Need to validate separately here. We can't pass multi_output=True because that would allow y to be csr.
        ...
        (self.coef_, self.dual_gap_, self.eps_, self.n_iter_) = cd_fast.enet_coordinate_descent_multi_task( \
            self.coef_, l1_reg, l2_reg, X, y, self.max_iter, self.tol, check_random_state(self.random_state), random)
...
class MultiTaskLasso(MultiTaskElasticNet):
    ...
    def __init__(self, alpha=1.0, *, fit_intercept=True, copy_X=True, max_iter=1000, tol=1e-4, warm_start=False, \
        random_state=None, selection="cyclic"):
        self.alpha         = alpha
        self.fit_intercept = fit_intercept
        self.max_iter      = max_iter
...
class MultiTaskElasticNetCV(RegressorMixin, LinearModelCV):
    """
    Multi-task L1/L2 ElasticNet with built-in cross-validation. See glossary entry for :term:`cross-validation estimator`.
    ...
    l1_ratio : float or list of float, default=0.5
        The ElasticNet mixing parameter, with 0 < l1_ratio <= 1.  For l1_ratio = 1 the penalty is an L1/L2 penalty. 
        For l1_ratio = 0 it is an L2 penalty.  For ``0 < l1_ratio < 1``, the penalty is a combination of L1/L2 and L2.
        This parameter can be a list, in which case the different values are tested by cross-validation 
    ...
    To avoid unnecessary memory duplication the `X` and `y` arguments of the `fit` method should be 
    directly passed as Fortran-contiguous numpy arrays.
    ...
    def __init__(self, *, l1_ratio=0.5, eps=1e-3, n_alphas=100, alphas=None, fit_intercept=True, max_iter=1000, \
        tol=1e-4, cv=None, copy_X=True, verbose=0, n_jobs=None, random_state=None, selection="cyclic"):
        self.l1_ratio      = l1_ratio
        self.eps           = eps
        self.n_alphas      = n_alphas
        self.alphas        = alphas
    ...
    def fit(self, X, y):
        ...
        Returns
        -------
        self : object, Returns MultiTaskElasticNet instance.
        """
...
class MultiTaskLassoCV(RegressorMixin, LinearModelCV):
    """
    Multi-task Lasso model trained with L1/L2 mixed-norm as regularizer. See glossary entry for :term:`cross-validation estimator`.
    ...
    def __init__(self, *, eps=1e-3, n_alphas=100, alphas=None, fit_intercept=True, max_iter=1000, tol=1e-4, \
        copy_X=True, cv=None, verbose=False, n_jobs=None, random_state=None, selection="cyclic"):
        super().__init__(eps=eps, n_alphas=n_alphas, alphas=alphas, fit_intercept=fit_intercept,            \
            max_iter=max_iter, tol=tol, copy_X=copy_X, cv=cv, verbose=verbose, n_jobs=n_jobs,               \
            random_state=random_state, selection=selection)
