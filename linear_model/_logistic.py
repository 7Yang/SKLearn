...
def _logistic_regression_path(X, y, pos_class=None, Cs=10, fit_intercept=True, max_iter=100, tol=1e-4,        \
    verbose=0, solver="lbfgs", coef=None, class_weight=None, dual=False, penalty="l2", intercept_scaling=1.0, \
    multi_class="auto", random_state=None, check_input=True, max_squared_sum=None, sample_weight=None,        \
    ...
    making it faster than sequentially calling LogisticRegression for the different parameters.
    Note that there will be no speedup with liblinear solver, since it does not handle warm-starting.
    ...
    Cs : int or array-like of shape (n_cs,), default=10
        List of values for the regularization parameter or integer specifying the number of regularization parameters that should be used. 
        In this case, the parameters will be chosen in a logarithmic scale between 1e-4 and 1e4.
    ...
    tol : float, default=1e-4
        Stopping criterion. For the newton-cg and lbfgs solvers, the iteration will stop when ``max{|g_i | i = 1, ..., n} <= tol``
    ...
    multi_class : {'ovr', 'multinomial', 'auto'}, default='auto'
        ...
        *even when the data is binary*. 'multinomial' is unavailable when solver='liblinear'.
        ...
        .. versionadded:: 0.18,   Stochastic Average Gradient descent solver for 'multinomial' case.
        .. versionchanged:: 0.22, Default changed from 'ovr' to 'auto' in 0.22.
    ...
    max_squared_sum : float, default=None
        ...
        If None, it will be computed, going through all the samples. The value should be precomputed to speed up cross validation.
    ...
    coefs : ndarray of shape (n_cs, n_features) or (n_cs, n_features + 1)
        List of coefficients for the Logistic Regression model. 
        If fit_intercept is set to True then the second dimension will be n_features + 1, where the last item represents the intercept. 
        For ``multiclass='multinomial'``, the shape is (n_classes, n_cs, n_features) or (n_classes, n_cs, n_features + 1).
    ...
    .. versionchanged:: 0.19, The "copy" parameter was removed.
    """
    if isinstance(Cs, numbers.Integral): Cs = np.logspace(-4, 4, Cs)
    ...
    # If sample weights exist, convert them to array (support for lists) and check length
    ...
    if solver == "newton-cholesky":
        ...
        # We want to minimize
        #     obj = 1/(2*sum(sample_weight)) * sum(sample_weight * deviance) + 1/2 * alpha * L2,
        # with
        #     deviance = 2 * log_loss.
        # The objective is invariant to multiplying sample_weight by a constant. 
        # We choose this constant such that sum(sample_weight) = 1. Thus, we end up with
    ...
    # If class_weights is a dict (provided by the user), 
    # the weights are assigned to the original labels. 
    # If it is "balanced", then the class_weights are assigned after masking the labels with a OvR.
    le = LabelEncoder()
    if isinstance(class_weight, dict) or multi_class == "multinomial":
        class_weight_  = compute_class_weight(class_weight, classes=classes, y=y)
        sample_weight *= class_weight_[le.fit_transform(y)]

    # For doing a ovr, we need to mask the labels first. For the multinomial case this is not necessary.
    if multi_class == "ovr":
        w0 = np.zeros(n_features + int(fit_intercept), dtype=X.dtype)
        mask = y == pos_class
        y_bin = np.ones(y.shape, dtype=X.dtype)
        if solver in ["lbfgs", "newton-cg", "newton-cholesky"]:
            # HalfBinomialLoss, used for those solvers, represents y in [0, 1] instead of in [-1, 1].
        ...
        if solver in ["sag", "saga", "lbfgs", "newton-cg"]:
            ...
            # LabelEncoder also saves memory compared to LabelBinarizer, especially when n_classes is large.
            le = LabelEncoder()
    ...
    if coef is not None:
        ...
        if multi_class == "ovr":
            ...
            # For binary problems coef.shape[0] should be 1, otherwise it should be classes.size.
            n_classes = classes.size
            if n_classes == 2:
                n_classes = 1

            if coef.shape[0] != n_classes or coef.shape[1] not in (n_features, n_features + 1):
                raise ValueError("Initialization coef is of shape (%d, %d), expected shape (%d, %d) or (%d, %d)"
                    % (coef.shape[0], coef.shape[1], classes.size, n_features, classes.size, n_features + 1))
    ...
    for i, C in enumerate(Cs):
        if solver == "lbfgs":
            ...
            opt_res = optimize.minimize(func, w0, method="L-BFGS-B", jac=True,
            ...
            # The division by sw_sum is a consequence of the rescaling of sample_weight, see comment above.
            l2_reg_strength = 1.0 / C / sw_sum
            sol = NewtonCholeskySolver(coef=w0, linear_loss=loss, l2_reg_strength=l2_reg_strength, tol=tol, max_iter=max_iter, \
                n_threads=n_threads, verbose=verbose)
            w0 = sol.solve(X=X, y=target, sample_weight=sample_weight)
            n_iter_i = sol.iteration
        elif solver == "liblinear":
            (coef_, intercept_, n_iter_i) = _fit_liblinear(X, target, C, fit_intercept, intercept_scaling, None, penalty, dual, \
                verbose, max_iter, tol, random_state, sample_weight=sample_weight)
            if fit_intercept:
            ...
            w0, n_iter_i, warm_start_sag = sag_solver(X, target, sample_weight, loss, alpha, beta, \
                max_iter, tol, verbose, random_state, False, max_squared_sum, warm_start_sag, is_saga=(solver == "saga"))
...
def _log_reg_scoring_path(X, y, train, test, *, pos_class, Cs, scoring, fit_intercept, max_iter, tol, class_weight, verbose, solver, \
    penalty, dual, intercept_scaling, multi_class, random_state, max_squared_sum, sample_weight, l1_ratio, score_params):
    """
    ...
    scoring : callable, default=None
        ...
        For a list of scoring functions that can be used, look at :mod:`sklearn.metrics`. The default scoring option used is accuracy_score.

    fit_intercept : bool, default=False
        If False, then the bias term is set to zero. Else the last term of each coef_ gives us the intercept.
    ...
    intercept_scaling : float, default=1.
        Useful only when the solver 'liblinear' is used and self.fit_intercept is set to True. 
        In this case, x becomes [x, self.intercept_scaling], i.e. a "synthetic" feature with constant value equals to
        intercept_scaling is appended to the instance vector.  The intercept becomes intercept_scaling * synthetic feature weight Note! 
        the synthetic feature weight is subject to l1/l2 regularization as all other features.
        To lessen the effect of regularization on synthetic feature weight (and therefore on the intercept) intercept_scaling has to be increased.
    ...
    coefs, Cs, n_iter = _logistic_regression_path(X_train, y_train, Cs=Cs, l1_ratio=l1_ratio, fit_intercept=fit_intercept, solver=solver,    \
        max_iter=max_iter, class_weight=class_weight, pos_class=pos_class, multi_class=multi_class, tol=tol, verbose=verbose, dual=dual,     \
        penalty=penalty, intercept_scaling=intercept_scaling, random_state=random_state, check_input=False, max_squared_sum=max_squared_sum, \
...
class LogisticRegression(LinearClassifierMixin, SparseCoefMixin, BaseEstimator):
    ...
    **Note that regularization is applied by default**. It can handle both dense and sparse input. 
    ...
    penalty : {'l1', 'l2', 'elasticnet', None}, default='l2'
        ...
        .. deprecated:: 1.2
           The 'none' option was deprecated in version 1.2, and will be removed in 1.4. Use `None` instead.
    ...
    intercept_scaling : float, default=1
        Useful only when the solver 'liblinear' is used and self.fit_intercept is set to True. 
        In this case, x becomes [x, self.intercept_scaling], i.e. a "synthetic" feature with constant value equal to
    ...
    solver : {'lbfgs', 'liblinear', 'newton-cg', 'newton-cholesky', 'sag', 'saga'}, default='lbfgs'
        ...
        Algorithm to use in the optimization problem. Default is 'lbfgs'. To choose a solver, you might want to consider the following aspects:
            ...
            - 'newton-cholesky' is a good choice for `n_samples` >> `n_features`,
              ...
              Be aware that the memory usage of this solver has a quadratic dependency on `n_features` 
        ...
        .. seealso::
           Refer to the User Guide for more information regarding :class:`LogisticRegression` 
           and more specifically the :ref:`Table <Logistic_regression>` summarizing solver/penalty supports.
    ...
    intercept_ : ndarray of shape (1,) or (n_classes,)
        ...
        In particular, when `multi_class='multinomial'`, `intercept_` corresponds to outcome 1 (True) and `-intercept_` 
    ...
    See :ref:`differences from liblinear <liblinear_differences>` in the narrative documentation.
    ...
    SAGA -- Defazio, A., Bach F. & Lacoste-Julien S. (2014).  :arxiv:`"SAGA: A Fast Incremental Gradient Method With Support
            for Non-Strongly Convex Composite Objectives" <1407.0202>`

    Hsiang-Fu Yu, Fang-Lan Huang, Chih-Jen Lin (2011). Dual coordinate descent methods for logistic regression and maximum entropy models.
        Machine Learning 85(1-2):41-75.  https://www.csie.ntu.edu.tw/~cjlin/papers/maxent_dual.pdf
    ...
    def fit(self, X, y, sample_weight=None):
        ...
        if self.penalty == "none":
            warnings.warn(("`penalty='none'`has been deprecated in 1.2 and will be removed in 1.4. "
                "To keep the past behaviour, set `penalty=None`."), FutureWarning)
        ...
        if solver == "liblinear":
            if effective_n_jobs(self.n_jobs) != 1:
                warnings.warn("'n_jobs' > 1 does not have any effect when 'solver' is set to 'liblinear'. Got 'n_jobs' = {}."
                        .format(effective_n_jobs(self.n_jobs)))
            self.coef_, self.intercept_, self.n_iter_ = _fit_liblinear(X, y, self.C, self.fit_intercept, self.intercept_scaling,\
                self.class_weight, self.penalty, self.dual, self.verbose, self.max_iter, self.tol, self.random_state,           \
                sample_weight=sample_weight)
            return self
        ...
        # and multinomial multiclass classification and use joblib only for the one-vs-rest multiclass case.
        if (solver in ["lbfgs", "newton-cg", "newton-cholesky"] and len(classes_) == 1 and effective_n_jobs(self.n_jobs) == 1):
            # In the future, we would like n_threads = _openmp_effective_n_threads() For the time being, we just do
            n_threads = 1
        else:
            n_threads = 1

        fold_coefs_ = Parallel(n_jobs=self.n_jobs, verbose=self.verbose, prefer=prefer)(path_func(     \
            X, y, pos_class=class_, Cs=[C_], l1_ratio=self.l1_ratio, fit_intercept=self.fit_intercept, \
                tol=self.tol, verbose=self.verbose, solver=solver, multi_class=multi_class,            \
                max_iter=self.max_iter, class_weight=self.class_weight, check_input=False,             \
                random_state=self.random_state, coef=warm_start_coef_, penalty=penalty,                \
                max_squared_sum=max_squared_sum, sample_weight=sample_weight, n_threads=n_threads)
    ...
    def predict_proba(self, X):
        ...
        For a multi_class problem, if multi_class is set to be "multinomial" the softmax function 
        is used to find the predicted probability of each class.
        Else use a one-vs-rest approach, i.e calculate the probability of each class assuming it to be positive 
        ...
        ovr = self.multi_class in ["ovr", "warn"] or (self.multi_class == "auto" and (
                self.classes_.size <= 2 or self.solver in ("liblinear", "newton-cholesky")))
        if ovr:
            return super()._predict_proba_lr(X)
        else:
            decision = self.decision_function(X)
            if decision.ndim == 1:
                # Workaround for multi_class="multinomial" and binary outcomes which requires softmax prediction with only a 1D decision.
...
class LogisticRegressionCV(LogisticRegression, LinearClassifierMixin, BaseEstimator):
    ...
    :class:`~sklearn.model_selection.StratifiedKFold`, but it can be changed using the :term:`cv` parameter. 
    The 'newton-cg', 'sag', 'saga' and 'lbfgs' solvers can warm-start the coefficients (see :term:`Glossary<warm_start>`).
    ...
    cv : int or cross-validation generator, default=None
        ...
        See the module :mod:`sklearn.model_selection` module for the list of possible cross-validation objects.
    ...
    scoring : str or callable, default=None
        A string (see model evaluation documentation) or a scorer callable object / function with signature ``scorer(estimator, X, y)``. 
        For a list of scoring functions that can be used, look at :mod:`sklearn.metrics`. The default scoring option used is 'accuracy'.
    ...
    solver : {'lbfgs', 'liblinear', 'newton-cg', 'newton-cholesky', 'sag', 'saga'}, default='lbfgs'
        ...
        To choose a solver, you might want to consider the following aspects:
            ...
            - For small datasets, 'liblinear' is a good choice, whereas 'sag' and 'saga' are faster for large ones;
            - For multiclass problems, only 'newton-cg', 'sag', 'saga' and 'lbfgs' handle multinomial loss;
            - 'liblinear' might be slower in :class:`LogisticRegressionCV` because it does not handle warm-starting. 
              'liblinear' is limited to one-versus-rest schemes.
            - 'newton-cholesky' is a good choice for `n_samples` >> `n_features`, 
              especially with one-hot encoded categorical features with rare categories. 
              Note that it is limited to binary classification and the one-versus-rest reduction for multiclass classification. 
    ...
    refit : bool, default=True
        If set to True, the scores are averaged across all folds, and the coefs and the C that corresponds to the best score is taken, 
    ...
    random_state : int, RandomState instance, default=None
        ...
        Note that this only applies to the solver and not the cross-validation generator. See :term:`Glossary <random_state>` for details.

    l1_ratios : list of float, default=None
        The list of Elastic-Net mixing parameter, with ``0 <= l1_ratio <= 1``.  Only used if ``penalty='elasticnet'``. 
    ...
    coefs_paths_ : ndarray of shape (n_folds, n_cs, n_features) or (n_folds, n_cs, n_features + 1)
        dict with classes as the keys, and the path of coefficients obtained during cross-validating across each fold 
        and then across each Cs after doing an OvR for the corresponding class as values.
        If the 'multi_class' option is set to 'multinomial', then the coefs_paths are the coefficients corresponding to each class.
        Each dict value has shape ``(n_folds, n_cs, n_features)`` or ``(n_folds, n_cs, n_features + 1)`` depending on whether the
        intercept is fit or not. 
        If ``penalty='elasticnet'``, the shape is ``(n_folds, n_cs, n_l1_ratios_, n_features)`` or ``(n_folds, n_cs, n_l1_ratios_, n_features + 1)``.

    scores_ : dict
        dict with classes as the keys, and the values as the grid of scores obtained during cross-validating each fold, 
        after doing an OvR for the corresponding class. 
        If the 'multi_class' option given is 'multinomial' then the same scores are repeated across all classes, since this is the multinomial class.
        Each dict value has shape ``(n_folds, n_cs)`` or ``(n_folds, n_cs, n_l1_ratios)`` if ``penalty='elasticnet'``.

    C_ : ndarray of shape (n_classes,) or (n_classes - 1,)
        Array of C that maps to the best scores across every class. If refit is set to False, then for each class, 
        the best C is the average of the C's that correspond to the best scores for each fold.
        `C_` is of shape(n_classes,) when the problem is binary.

    l1_ratio_ : ndarray of shape (n_classes,) or (n_classes - 1,)
        Array of l1_ratio that maps to the best scores across every class. 
        If refit is set to False, then for each class, the best l1_ratio is the average of the l1_ratio's 
        that correspond to the best scores for each fold.  
        `l1_ratio_` is of shape(n_classes,) when the problem is binary.
    ...
    def __init__(self, *, Cs=10, fit_intercept=True, cv=None, dual=False, penalty="l2", scoring=None, solver="lbfgs", \
        tol=1e-4, max_iter=100, class_weight=None, n_jobs=None, verbose=0, refit=True, intercept_scaling=1.0,         \
    ...
    def fit(self, X, y, sample_weight=None, **params):
        ...
        **params : dict
        ...
        Returns
        -------
        self : object, Fitted LogisticRegressionCV estimator.
        """
        ...
        if self.penalty == "elasticnet":
            if (self.l1_ratios is None or len(self.l1_ratios) == 0 or any((
                not isinstance(l1_ratio, numbers.Number) or l1_ratio < 0 or l1_ratio > 1) for l1_ratio in self.l1_ratios)):
        ...
        fold_coefs_ = Parallel(n_jobs=self.n_jobs, verbose=self.verbose, prefer=prefer)(path_func(       \
            X, y, train, test, pos_class=label, Cs=self.Cs, fit_intercept=self.fit_intercept,            \
            penalty=self.penalty, dual=self.dual, solver=solver, tol=self.tol, max_iter=self.max_iter,   \
            verbose=self.verbose, class_weight=class_weight, scoring=self.scoring,                       \
            multi_class=multi_class, intercept_scaling=self.intercept_scaling,                           \
            random_state=self.random_state, max_squared_sum=max_squared_sum, sample_weight=sample_weight,\
            l1_ratio=l1_ratio, score_params=routed_params.scorer.score)
            for label in iter_encoded_labels for train, test in folds for l1_ratio in l1_ratios_)

        # _log_reg_scoring_path will output different shapes depending on the multi_class param, so we need to reshape the outputs accordingly.
        ...
        # - n_iter is of shape (n_classes, n_folds, n_Cs . n_l1_ratios) or (1, n_folds, n_Cs . n_l1_ratios)
        ...
        for index, (cls, encoded_label) in enumerate(zip(iter_classes, iter_encoded_labels)):
            if multi_class == "ovr":
                ...
                scores = scores[0]
                # coefs_paths will keep its original shape because logistic_regression_path expects it this way

            if self.refit:
                # best_index is between 0 and (n_Cs . n_l1_ratios - 1) for example, with n_cs=2 and n_l1_ratios=3 the layout of scores is
                # [c1, c2, c1, c2, c1, c2]
                #   l1_1 ,  l1_2 ,  l1_3
                ...
                # Note that y is label encoded and hence pos_class must be the encoded label / None (for 'multinomial')
                w, _, _ = _logistic_regression_path(X, y, pos_class=encoded_label, Cs=[C_], solver=solver, fit_intercept=self.fit_intercept, \
                    coef=coef_init, max_iter=self.max_iter, tol=self.tol, penalty=self.penalty, class_weight=class_weight,                   \
                    multi_class=multi_class, verbose=max(0, self.verbose - 1), random_state=self.random_state, check_input=False,            \
                    max_squared_sum=max_squared_sum, sample_weight=sample_weight, l1_ratio=l1_ratio_)
                w = w[0]

            else:
                # Take the best scores across every fold and the average of all coefficients corresponding to the best scores.
        ...
        # attributes
        if self.l1_ratios is not None:
            # with n_cs=2 and n_l1_ratios=3 the layout of scores is
            # [c1, c2, c1, c2, c1, c2]
            #   l1_1 ,  l1_2 ,  l1_3
            ...
            # c1 [[ .  ,  .  ,  .  ],
            # c2  [ .  ,  .  ,  .  ]]
            # We need to first reshape and then transpose. The same goes for the other arrays
    ...
    def score(self, X, y, sample_weight=None, **score_params):
        ...
        **score_params : dict
        ...
        Returns
        -------
        score : float, Score of self.predict(X) w.r.t. y.
        """
    ...
    def get_metadata_routing(self):
        ...
        routing : MetadataRouter
            A :class:`~utils.metadata_routing.MetadataRouter` encapsulating routing information.
        """

        router = (MetadataRouter(owner=self.__class__.__name__)
            .add_self_request(self)
            .add(splitter=self.cv, method_mapping=MethodMapping().add(callee="split", caller="fit"))
            .add(scorer=self._get_scorer(), method_mapping=MethodMapping()
    ...
    def _get_scorer(self):
        """
        Get the scorer based on the scoring method specified.  The default scoring method is `accuracy`.
        """
