...
def _huber_loss_and_gradient(w, X, y, epsilon, alpha, sample_weight=None):
    ...
    Parameters
    ----------
    w : ndarray, shape (n_features + 1,) or (n_features + 2,) Feature vector.
        w[:n_features] gives the coefficients
        w[-1] gives the scale factor and if the intercept is fit w[-2] gives the intercept factor.
    ...
    gradient : ndarray, shape (len(w))
        Returns the derivative of the Huber loss with respect to each coefficient, intercept and the scale as a vector.
    """
    ...
    if fit_intercept: linear_loss -= intercept
...
class HuberRegressor(LinearModel, RegressorMixin, BaseEstimator):
    ...
    where the model coefficients ``w``, the intercept ``c`` and the scale ``sigma`` are parameters to be optimized. 
    ...
    epsilon : float, default=1.35
        ...
        The smaller the epsilon, the more robust it is to outliers. Epsilon must be in the range `[1, inf)`.
    ...
    warm_start : bool, default=False
        ...
        If set to False, then the coefficients will be rewritten for every call to fit.  See :term:`the Glossary <warm_start>`.
    ...
    def fit(self, X, y, sample_weight=None):
        ...
        Returns
        -------
        self : object, Fitted `HuberRegressor` estimator.
        """
        ...
        # Setting it to be zero might cause undefined bounds hence we set it to a value close to zero.
        bounds = np.tile([-np.inf, np.inf], (parameters.shape[0], 1))
        bounds[-1][0] = np.finfo(np.float64).eps * 10

        opt_res = optimize.minimize(_huber_loss_and_gradient, parameters, method="L-BFGS-B", jac=True,
            args=(X, y, self.epsilon, self.alpha, sample_weight), options={"maxiter": self.max_iter, "gtol": self.tol, "iprint": -1}, bounds=bounds)
