...
def _assess_dimension(spectrum, rank, n_samples):
    ...
    The dataset is assumed to be embedded in gaussian noise of shape(n, dimf) having spectrum ``spectrum``. 
    This implements the method of T. P. Minka.
    ...
    spectrum : ndarray of shape (n_features,)
        Data spectrum.
    rank : int
        Tested rank value. It should be strictly lower than n_features, otherwise the method isn't specified 
        (division by zero in equation (31) from the paper).
    n_samples : int
        Number of samples.
    ...
    References
    ----------
    This implements the method of `Thomas P. Minka: Automatic Choice of Dimensionality for PCA. NIPS 2000: 598-604
    ...
    if spectrum[rank - 1] < eps:
        ...
        # there's no point in computing the log-likelihood: it's going to be very small and won't be the max anyway. 
        # Also, it can lead to numerical issues below when computing pa, in particular in log((spectrum[i] - spectrum[j]) 
...
class PCA(_BasePCA):
    ...
    It uses the LAPACK implementation of the full SVD or a randomized truncated SVD by the method of Halko et al. 
    2009, depending on the shape of the input data and the number of components to extract.
    ...
    svd_solver : {'auto', 'full', 'arpack', 'randomized'}, default='auto'
        If auto :
            ...
            and the number of components to extract is lower than 80% of the smallest dimension of the data, 
    ...
    n_oversamples : int, default=10
        ...
        It corresponds to the additional number of random vectors to sample the range of `X` so as to ensure proper conditioning. 
    ...
    n_components_ : int
        ...
        this number is estimated from input data. Otherwise it equals the parameter n_components, 
    ...
    References
    ----------
    For n_components == 'mle', this class uses the method from: `Minka, T. P.. "Automatic choice of dimensionality for PCA".
    ...
    <http://www.miketipping.com/papers/met-mppca.pdf>`_ via the score and score_samples methods.

    For svd_solver == 'arpack', refer to `scipy.sparse.linalg.svds`.
    For svd_solver == 'randomized', see: :doi:`Halko, N., Martinsson, P. G., and Tropp, J. A. (2011).
    "Finding structure with randomness: Probabilistic algorithms for constructing approximate matrix decompositions".
    SIAM review, 53(2), 217-288.  <10.1137/090771806>` and also :doi:`Martinsson, P. G., Rokhlin, V., and Tygert, M. (2011).
    "A randomized algorithm for the decomposition of matrices".  Applied and Computational Harmonic Analysis, 30(1), 47-68. 
    ...
    _parameter_constraints: dict = {
        "n_components": [Interval(Integral, 0, None, closed="left"), Interval(RealNotInt, 0, 1, closed="neither"), StrOptions({"mle"}), None ],
        "copy": ["boolean"],
        "whiten": ["boolean"],
    ...
    def _fit(self, X):
        ...
        # Raise an error for sparse input. This is more informative than the generic one raised by check_array.
        if issparse(X):
        ...
        X = self._validate_data(X, dtype=[xp.float64, xp.float32], ensure_2d=True, copy=self.copy)
    ...
    def _fit_full(self, X, n_components):
        ...
        if n_components == "mle":
            ...
            raise ValueError("n_components=%r must be between 0 and min(n_samples, n_features)=%r with svd_solver='full'" 
                % (n_components, min(n_samples, n_features)))
        ...
        if not is_array_api_compliant:
            # Use scipy.linalg with NumPy/SciPy inputs for the sake of not introducing unanticipated behavior changes. 
            # In the long run we could instead decide to always use xp.linalg.svd for all inputs,
            # but that would make this code rely on numpy's SVD instead of scipy's. It's not 100% clear whether they use the same 
            # LAPACK solver by default though (assuming both are built against the same BLAS).
        ...
        if n_components == "mle":
            ...
            # number of components for which the cumulated explained variance percentage is superior to the desired threshold
            # side='right' ensures that number of features selected their variance is always greater than n_components float passed. 
            # More discussion in issue: #15669
    ...
    def _fit_truncated(self, X, n_components, svd_solver):
        ...
        if isinstance(n_components, str):
            ...
            raise ValueError("n_components=%r must be between 1 and min(n_samples, n_features)=%r with svd_solver='%s'"
                % (n_components, min(n_samples, n_features), svd_solver))
        elif svd_solver == "arpack" and n_components == min(n_samples, n_features):
            raise ValueError("n_components=%r must be strictly less than min(n_samples, n_features)=%r with svd_solver='%s'"
                % (n_components, min(n_samples, n_features), svd_solver))
        ...
        if svd_solver == "arpack":
            ...
            U, S, Vt = randomized_svd(X, n_components=n_components, n_oversamples=self.n_oversamples, n_iter=self.iterated_power, \
                power_iteration_normalizer=self.power_iteration_normalizer, flip_sign=True, random_state=random_state)
        ...
        # Workaround in-place variance calculation since at the time numpy did not have a way to calculate variance in-place.
        N = X.shape[0] - 1
        X **= 2
    ...
    def score_samples(self, X):
        ...
        Returns
        -------
        ll : ndarray of shape (n_samples,), Log-likelihood of each sample under the current model.
        """
