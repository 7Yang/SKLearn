...
class _BaseSparsePCA(ClassNamePrefixFeaturesOutMixin, TransformerMixin, BaseEstimator):
    ...
    def __init__(self, n_components=None, *, alpha=1, ridge_alpha=0.01, max_iter=1000, tol=1e-8, \
        method="lars", n_jobs=None, verbose=False, random_state=None):
    ...
    def transform(self, X):
        ...
        regularization can be applied (Ridge regression) via the `ridge_alpha` parameter.
    ...
    def inverse_transform(self, X):
        ...
        Returns
        -------
        X_original : ndarray of shape (n_samples, n_features), Reconstructed data in the original space.
        """
        check_is_fitted(self)
        X = check_array(X)
...
class SparsePCA(_BaseSparsePCA):
    ...
    def __init__(self, n_components=None, *, alpha=1, ridge_alpha=0.01, max_iter=1000, tol=1e-8, method="lars", \
        n_jobs=None, U_init=None, V_init=None, verbose=False, random_state=None):
        """
        """
        super().__init__(n_components=n_components, alpha=alpha, ridge_alpha=ridge_alpha, max_iter=max_iter, tol=tol, \
            method=method, n_jobs=n_jobs, verbose=verbose, random_state=random_state)
        self.U_init = U_init
        self.V_init = V_init
    ...
    def _fit(self, X, n_components, random_state):
        ...
        code, dictionary, E, self.n_iter_ = dict_learning(X.T, n_components, alpha=self.alpha, tol=self.tol,
            max_iter=self.max_iter, method=self.method, n_jobs=self.n_jobs, verbose=self.verbose, random_state=random_state,
            code_init=code_init, dict_init=dict_init, return_n_iter=True)
...
class MiniBatchSparsePCA(_BaseSparsePCA):
    ...
    n_iter : int, default=100
        ...
        .. deprecated:: 1.2
           `n_iter` is deprecated in 1.2 and will be removed in 1.4. Use `max_iter` instead.
    ...
    method : {'lars', 'cd'}, default='lars'
        Method to be used for optimization.
        lars: uses the least angle regression method to solve the lasso problem (linear_model.lars_path)
        cd  : uses the coordinate descent method to compute the Lasso solution (linear_model.Lasso). Lars will be faster if
    ...
    max_no_improvement : int or None, default=10
        Control early stopping based on the consecutive number of mini batches that does not yield 
        an improvement on the smoothed cost function. Used only if `max_iter` is not None.
    ...
    mean_   : ndarray of shape (n_features,), Per-feature empirical mean, estimated from the training set.  Equal to ``X.mean(axis=0)``.
    ...
    >>> X, _ = make_friedman1(n_samples=200, n_features=30, random_state=0)
    >>> transformer = MiniBatchSparsePCA(n_components=5, batch_size=50, max_iter=10, random_state=0)
    >>> transformer.fit(X)
    >>> X_transformed = transformer.transform(X)
    >>> X_transformed.shape
    >>> # most values in the components_ are zero (sparsity)
    ...
    def __init__(self, n_components=None, *, alpha=1, ridge_alpha=0.01, n_iter="deprecated", max_iter=None, \
        callback=None, batch_size=3, verbose=False, shuffle=True, n_jobs=None, method="lars", random_state=None, \
    ...
    def _fit(self, X, n_components, random_state):
        ...
        est = MiniBatchDictionaryLearning(n_components=n_components, alpha=self.alpha, n_iter=self.n_iter, max_iter=self.max_iter, \
            dict_init=None, batch_size=self.batch_size, shuffle=self.shuffle, n_jobs=self.n_jobs, fit_algorithm=self.method,       \
            random_state=random_state, transform_algorithm=transform_algorithm, transform_alpha=self.alpha, verbose=self.verbose,  \
            callback=self.callback, tol=self.tol, max_no_improvement=self.max_no_improvement).fit(X.T)
