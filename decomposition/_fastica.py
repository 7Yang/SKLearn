...
def _gs_decorrelation(w, W, j):
    ...
    j : int < p
        The no of (from the first) rows of Null space W wrt which w is orthogonalized.

    Notes
    -----
    Assumes that W is orthogonal w changed in place
    """
...
def _sym_decorrelation(W):
    ...
    # Avoid sqrt of negative values because of rounding errors.
    # Note that np.sqrt(tiny) is larger than tiny and therefore this clipping also prevents division by zero in the next step.
...
def _ica_par(X, tol, g, fun_args, max_iter, w_init):
    ...
    for ii in range(max_iter):
        ...
        # np.einsum allows having the lowest memory footprint. It is faster than np.diag(np.dot(W1, W.T)).
...
def fastica(X, n_components=None, *, algorithm="parallel", whiten="unit-variance", fun="logcosh", fun_args=None, max_iter=200, \
    tol=1e-04, w_init=None, whiten_solver="svd", random_state=None, return_X_mean=False, compute_sources=True, return_n_iter=False):
    """
    ...
    fun : {'logcosh', 'exp', 'cube'} or callable, default='logcosh'
        ...
        You can also provide your own function. It should return a tuple containing the value of the function, and of its derivative, 
        in the point. The derivative should be averaged along its last dimension.
        Example::
    ...
    K : ndarray of shape (n_components, n_features) or None
        If whiten is 'True', K is the pre-whitening matrix that projects data onto the first n_components principal components. 
        If whiten is 'False', K is 'None'.

    W : ndarray of shape (n_components, n_components)
        The square matrix that unmixes the data after whitening.
        The mixing matrix is the pseudo-inverse of matrix ``W K`` if K is not None, else it is the inverse of W.
    ...
    n_iter : int
        ...
        Else they are just the number of iterations taken to converge. This is returned only when return_n_iter is set to `True`.

    Notes
    -----
    The data matrix X is considered to be a linear combination of non-Gaussian (independent) components i.e. 
    X = AS where columns of S contain the independent components and A is a linear mixing matrix. 
    In short ICA attempts to `un-mix' the data by estimating an un-mixing matrix W where ``S = W K X.``
    While FastICA was proposed to estimate as many sources as features, it is possible to estimate less by setting n_components < n_features. 
    It this case K is not a square matrix and the estimated A is the pseudo-inverse of ``W K``.

    This implementation was originally made for data of shape [n_features, n_samples]. 
    Now the input is transposed before the algorithm is applied. This makes it slightly faster for Fortran-ordered input.
    ...
    """
    est = FastICA(n_components=n_components, algorithm=algorithm, whiten=whiten, fun=fun, fun_args=fun_args, max_iter=max_iter, \
        tol=tol, w_init=w_init, whiten_solver=whiten_solver, random_state=random_state)
...
class FastICA(ClassNamePrefixFeaturesOutMixin, TransformerMixin, BaseEstimator):
    ...
    fun : {'logcosh', 'exp', 'cube'} or callable, default='logcosh'
        ...
        Could be either 'logcosh', 'exp', or 'cube'.  You can also provide your own function. 
        It should return a tuple containing the value of the function, and of its derivative, in the point. 
        The derivative should be averaged along its last dimension.  Example::
    ...
    components_ : ndarray of shape (n_components, n_features)
        ...
        This is equal to the unmixing matrix when ``whiten`` is False,
        and equal to ``np.dot(unmixing_matrix, self.whitening_)`` when ``whiten`` is True.
    ...
    _parameter_constraints: dict = {
        ...
        "whiten": [StrOptions({"arbitrary-variance", "unit-variance"}), Options(bool, {False}) ],
    ...
    def _fit_transform(self, X, compute_sources=False):
        ...
        Returns
        -------
        S : ndarray of shape (n_samples, n_components) or None Sources matrix. `None` if `compute_sources` is `False`.
        """
        ...
        if self.whiten:
            ...
            # X must be casted to floats to avoid typing issues with numpy 2.0 and the line below
    ...
    def inverse_transform(self, X, copy=True):
        ...
        Returns
        -------
        X_new : ndarray of shape (n_samples, n_features), Reconstructed data obtained with the mixing matrix.
        """
        check_is_fitted(self)
