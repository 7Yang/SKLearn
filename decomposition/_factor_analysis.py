...
FactorAnalysis is similar to probabilistic PCA implemented by PCA.score While PCA assumes Gaussian noise 
with the same variance for each feature, the FactorAnalysis model assumes different variances for each of them.

This implementation is based on David Barber's Book, Bayesian Reasoning and Machine Learning,
http://www.cs.ucl.ac.uk/staff/d.barber/brml, Algorithm 21.1
"""
...
class FactorAnalysis(ClassNamePrefixFeaturesOutMixin, TransformerMixin, BaseEstimator):
    ...
    If we would restrict the model further, by assuming that the Gaussian noise is even isotropic (all diagonal entries are the same) 
    we would obtain :class:`PCA`.
    ...
    svd_method : {'lapack', 'randomized'}, default='randomized'
        Which SVD method to use. If 'lapack' use standard SVD from scipy.linalg, if 'randomized' use fast ``randomized_svd`` function.
        Defaults to 'randomized'. For most applications 'randomized' will be sufficiently precise while providing significant speed gains.
        Accuracy can also be improved by setting higher values for `iterated_power`. 
    ...
    rotation : {'varimax', 'quartimax'}, default=None
        If not None, apply the indicated rotation. Currently, varimax and quartimax are implemented. See
        `"The varimax criterion for analytic rotation in factor analysis"
        <https://link.springer.com/article/10.1007%2FBF02289233>`_ H. F. Kaiser, 1958.

        .. versionadded:: 0.24

    random_state : int or RandomState instance, default=0
        Only used when ``svd_method`` equals 'randomized'. Pass an int for reproducible results across multiple function calls.
    ...
    See Also
    --------
    PCA    : Principal component analysis is also a latent linear variable model which however assumes equal noise variance for each feature.
               This extra assumption makes probabilistic PCA faster as it can be computed in closed form.
    ...
    def __init__(self, n_components=None, *, tol=1e-2, copy=True, max_iter=1000, noise_variance_init=None, \
        svd_method="randomized", iterated_power=3, rotation=None, random_state=0):
    ...
    def fit(self, X, y=None):
        ...
        Returns
        -------
        self : object, FactorAnalysis class instance.
        """
        ...
        if self.noise_variance_init is None:
            ...
            if len(self.noise_variance_init) != n_features:
                raise ValueError("noise_variance_init dimension does not with number of features : %d != %d" 
        ...
        # we'll modify svd outputs to return unexplained variance to allow for unified computation of loglikelihood
    ...
    def transform(self, X):
        ...
        Returns
        -------
        X_new : ndarray of shape (n_samples, n_components), The latent variables of X.
        """
    ...
    def get_covariance(self):
        ...
        Returns
        -------
        cov : ndarray of shape (n_features, n_features), Estimated covariance of data.
        """
        check_is_fitted(self)
    ...
    def get_precision(self):
        ...
        Returns
        -------
        precision : ndarray of shape (n_features, n_features), Estimated precision of data.
        """
        check_is_fitted(self)
