...
class TruncatedSVD(ClassNamePrefixFeaturesOutMixin, TransformerMixin, BaseEstimator):
    ...
    In particular, truncated SVD works on term count/tf-idf matrices as returned by the vectorizers in :mod:`sklearn.feature_extraction.text`. 
    ...
    and a "naive" algorithm that uses ARPACK as an eigensolver on `X * X.T` or `X.T * X`, whichever is more efficient.
    ...
    algorithm : {'arpack', 'randomized'}, default='randomized'
        SVD solver to use. Either "arpack" for the ARPACK wrapper in SciPy (scipy.sparse.linalg.svds), 
        or "randomized" for the randomized algorithm due to Halko (2009).

    n_iter : int, default=5
        Number of iterations for randomized SVD solver. Not used by ARPACK. 
        The default is larger than the default in :func:`~sklearn.utils.extmath.randomized_svd` to handle sparse
        matrices that may have large slowly decaying spectrum.

    n_oversamples : int, default=10
        Number of oversamples for randomized SVD solver. Not used by ARPACK.
        See :func:`~sklearn.utils.extmath.randomized_svd` for a complete description.
    ...
    FactorAnalysis : A simple linear generative model with Gaussian latent variables.
    ...
    References
    ----------
    :arxiv:`Halko, et al. (2009). "Finding structure with randomness: Stochastic algorithms for constructing approximate matrix decompositions"
    <0909.4061>`
    ...
    def __init__(self, n_components=2, *, algorithm="randomized", n_iter=5, n_oversamples=10, \
    ...
    def fit_transform(self, X, y=None):
        ...
        Returns
        -------
        X_new : ndarray of shape (n_samples, n_components), Reduced version of X. This will always be a dense array.
        """
        ...
        if self.algorithm == "arpack":
            ...
            U, Sigma, VT = randomized_svd(X, self.n_components, n_iter=self.n_iter, n_oversamples=self.n_oversamples, \
                power_iteration_normalizer=self.power_iteration_normalizer, random_state=random_state)

        self.components_ = VT

        # As a result of the SVD approximation error on X ~ U @ Sigma @ V.T, X @ V is not the same as U @ Sigma
    ...
    def inverse_transform(self, X):
        ...
        Returns
        -------
        X_original : ndarray of shape (n_samples, n_features), Note that this is always a dense array.
        """
        X = check_array(X)
