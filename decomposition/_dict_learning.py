...
def _sparse_encode_precomputed(X, dictionary, *, gram=None, cov=None, algorithm="lasso_lars", regularization=None, \
    copy_cov=True, init=None, max_iter=1000, verbose=0, positive=False):
    """
    ...
    dictionary : ndarray of shape (n_components, n_features)
        The dictionary matrix against which to solve the sparse coding of the data. Some of the algorithms assume normalized rows.

    gram : ndarray of shape (n_components, n_components), default=None
        Precomputed Gram matrix, `dictionary * dictionary'` gram can be `None` if method is 'threshold'.
    ...
    algorithm : {'lasso_lars', 'lasso_cd', 'lars', 'omp', 'threshold'}, default='lasso_lars'
        ...
        * `'lasso_cd'`: uses the coordinate descent method to compute the Lasso solution (`linear_model.Lasso`). 
    ...
    regularization : int or float, default=None
        The regularization parameter. It corresponds to alpha when algorithm is `'lasso_lars'`, `'lasso_cd'` or `'threshold'`.
        Otherwise it corresponds to `n_nonzero_coefs`.

    init : ndarray of shape (n_samples, n_components), default=None
        Initialization value of the sparse code. Only used if `algorithm='lasso_cd'`.
    ...
    Returns
    -------
    code : ndarray of shape (n_components, n_features), The sparse codes.
    """
    ...
    if algorithm == "lasso_lars":
        ...
        try:
            ...
            lasso_lars = LassoLars(alpha=alpha, fit_intercept=False, verbose=verbose, precompute=gram, fit_path=False, \
        ...
        new_code = orthogonal_mp_gram(Gram=gram, Xy=cov, n_nonzero_coefs=int(regularization), tol=None,\
            norms_squared=row_norms(X, squared=True), copy_Xy=copy_cov).T
...
@validate_params({
    ...
    "check_input": ["boolean"],
    "verbose": ["verbose"],
    "positive": ["boolean"] }, prefer_skip_nested_validation=True)
# XXX : could be moved to the linear_model module
def sparse_encode(X, dictionary, *, gram=None, cov=None, algorithm="lasso_lars", n_nonzero_coefs=None, \
    alpha=None, copy_cov=True, init=None, max_iter=1000, n_jobs=None, check_input=True, verbose=0, positive=False):
    """
    Sparse coding.
    ...
    algorithm : {'lasso_lars', 'lasso_cd', 'lars', 'omp', 'threshold'}, default='lasso_lars'
        ...
        * `'lasso_cd'`: uses the coordinate descent method to compute the
            Lasso solution (`linear_model.Lasso`). lasso_lars will be faster if the estimated components are sparse;
        * `'omp'`: uses orthogonal matching pursuit to estimate the sparse solution;
        * `'threshold'`: squashes to zero all coefficients less than regularization from the projection `dictionary * data'`.

    n_nonzero_coefs : int, default=None
        Number of nonzero coefficients to target in each column of the solution. 
        This is only used by `algorithm='lars'` and `algorithm='omp'` and is overridden by `alpha` in the `omp` case. 
    ...
    See Also
    --------
    sklearn.linear_model.lars_path     : Compute Least Angle Regression or Lasso path using LARS algorithm.
    ...
    return _sparse_encode(X, dictionary, gram=gram, cov=cov, algorithm=algorithm, n_nonzero_coefs=n_nonzero_coefs, \
        alpha=alpha, copy_cov=copy_cov, init=init, max_iter=max_iter, n_jobs=n_jobs, verbose=verbose, positive=positive)


def _sparse_encode(X, dictionary, *, gram=None, cov=None, algorithm="lasso_lars", n_nonzero_coefs=None, alpha=None, \
    copy_cov=True, init=None, max_iter=1000, n_jobs=None, verbose=0, positive=False):
    ...
    if effective_n_jobs(n_jobs) == 1 or algorithm == "threshold":
        code = _sparse_encode_precomputed(X, dictionary, gram=gram, cov=cov, algorithm=algorithm, \
            regularization=regularization, copy_cov=copy_cov, init=init, max_iter=max_iter, verbose=verbose, positive=positive)
        return code
    ...
    code_views = Parallel(n_jobs=n_jobs, verbose=verbose)(delayed(_sparse_encode_precomputed)( \
        X[this_slice], dictionary, gram=gram, cov=cov[:, this_slice] if cov is not None else None,
        algorithm=algorithm, regularization=regularization, copy_cov=copy_cov,
        init=init[this_slice] if init is not None else None,
        max_iter=max_iter, verbose=verbose, positive=positive) for this_slice in slices
    )
...
def _update_dict(dictionary, Y, code, A=None, B=None, verbose=False, random_state=None, positive=False):
    ...
    A : ndarray of shape (n_components, n_components), default=None
        Together with `B`, sufficient stats of the online model to update the dictionary.

    B : ndarray of shape (n_features, n_components), default=None
        Together with `A`, sufficient stats of the online model to update the dictionary.

    verbose: bool, default=False
        Degree of output the procedure will print.

    random_state : int, RandomState instance or None, default=None
        Used for randomly initializing the dictionary. Pass an int for reproducible results across multiple function calls.
    ...
    if A is None: A = code.T @ code
    if B is None: B = Y.T @ code
...
def _dict_learning(X, n_components, *, alpha, max_iter, tol, method, n_jobs, dict_init, code_init, callback, \
    verbose, random_state, return_n_iter, positive_dict, positive_code, method_max_iter):
    """
    ...
    # Fortran-order dict better suited for the sparse coding which is the bottleneck of this algorithm.
    ...
    for ii in range(max_iter):
        ...
        # Update code
        code = sparse_encode(X, dictionary, algorithm=method, alpha=alpha, init=code, n_jobs=n_jobs, positive=positive_code, \
...
def _check_warn_deprecated(param, name, default, additional_message=None):
    if param != "deprecated":
        ...
        if additional_message: msg += f" {additional_message}"
...
def dict_learning_online(X, n_components=2, *, alpha=1, n_iter="deprecated", max_iter=None, return_code=True, \
    dict_init=None, callback=None, batch_size=256, verbose=False, shuffle=True, n_jobs=None, method="lars", \
    iter_offset="deprecated", random_state=None, return_inner_stats="deprecated", inner_stats="deprecated", \
    return_n_iter="deprecated", positive_dict=False, positive_code=False, method_max_iter=1000, tol=1e-3, \
    ...
    where V is the dictionary and U is the sparse code. ||.||_Fro stands for the Frobenius norm 
    and ||.||_1,1 stands for the entry-wise matrix norm which is the sum of the absolute values of all the entries in the matrix.
    ...
    n_iter : int, default=100
        ...
        .. deprecated:: 1.1
           `n_iter` is deprecated in 1.1 and will be removed in 1.4. Use `max_iter` instead.

    max_iter : int, default=None
        Maximum number of iterations over the complete dataset before
        stopping independently of any early stopping criterion heuristics.  If ``max_iter`` is not None, ``n_iter`` is ignored.
    ...
    dict_init : ndarray of shape (n_components, n_features), default=None
        ...
        If `None`, the initial values for the dictionary are created with an SVD decomposition of the data via
    ...
    return_inner_stats : bool, default=False
        Return the inner statistics A (dictionary covariance) and B (data approximation). 
        Useful to restart the algorithm in an online setting. If `return_inner_stats` is `True`, `return_code` is ignored.
    ...
    inner_stats : tuple of (A, B) ndarrays, default=None
        ...
        Passing them at initialization is useful in online settings, to avoid losing the history of the evolution.
    ...
    return_inner_stats = _check_warn_deprecated(return_inner_stats, "return_inner_stats", default=False, \
    ...
    return_n_iter = _check_warn_deprecated(return_n_iter, "return_n_iter", default=False, \
        additional_message=("From 1.4 'n_iter' will never be returned. Refer to the 'n_iter_' and "
            "'n_steps_' attributes of the MiniBatchDictionaryLearning object instead."))

    if max_iter is not None:
        transform_algorithm = "lasso_" + method

        est = MiniBatchDictionaryLearning(n_components=n_components, alpha=alpha, n_iter=n_iter, n_jobs=n_jobs, \
            fit_algorithm=method, batch_size=batch_size, shuffle=shuffle, dict_init=dict_init, random_state=random_state, \
            transform_algorithm=transform_algorithm, transform_alpha=alpha, positive_code=positive_code, positive_dict=positive_dict, \
            transform_max_iter=method_max_iter, verbose=verbose, callback=callback, tol=tol, max_no_improvement=max_no_improvement).fit(X)
    ...
    for ii, batch in zip(range(iter_offset, iter_offset + n_iter), batches):
        ...
        this_code = sparse_encode(this_X, dictionary, algorithm=method, alpha=alpha, n_jobs=n_jobs, \
            check_input=False, positive=positive_code, max_iter=method_max_iter, verbose=verbose)
    ...
    if return_code:
        if verbose > 1:
            ...
            print("|", end=" ")
        code = sparse_encode(X, dictionary, algorithm=method, alpha=alpha, n_jobs=n_jobs, \
            check_input=False, positive=positive_code, max_iter=method_max_iter, verbose=verbose)
        if verbose > 1:
            dt = time.time() - t0
...
def dict_learning(X, n_components, *, alpha, max_iter=100, tol=1e-8, method="lars", n_jobs=None, \
    dict_init=None, code_init=None, callback=None, verbose=False, random_state=None, return_n_iter=False, \
    ...
    MiniBatchSparsePCA : Mini-batch Sparse Principal Components Analysis.
    """
    estimator = DictionaryLearning(n_components=n_components, alpha=alpha, max_iter=max_iter, tol=tol, fit_algorithm=method, \
        n_jobs=n_jobs, dict_init=dict_init, callback=callback, code_init=code_init, verbose=verbose, random_state=random_state, \
        positive_code=positive_code, positive_dict=positive_dict, transform_max_iter=method_max_iter)
...
class _BaseSparseCoding(ClassNamePrefixFeaturesOutMixin, TransformerMixin):
    ...
    def __init__(self, transform_algorithm, transform_n_nonzero_coefs, transform_alpha, split_sign, \
        n_jobs, positive_code, transform_max_iter):
    ...
    def _transform(self, X, dictionary):
        ...
        code = sparse_encode(X, dictionary, algorithm=self.transform_algorithm, n_nonzero_coefs=self.transform_n_nonzero_coefs, \
...
class SparseCoder(_BaseSparseCoding, BaseEstimator):
    ...
    split_sign : bool, default=False
        Whether to split the sparse feature vector into the concatenation of its negative part and its positive part. 
        This can improve the performance of downstream classifiers.
    ...
    MiniBatchSparsePCA          : Mini-batch Sparse Principal Components Analysis.
    SparsePCA                   : Sparse Principal Components Analysis.
    sparse_encode               : Sparse coding where each row of the result is the solution to a sparse coding problem.
...
class DictionaryLearning(_BaseSparseCoding, BaseEstimator):
    ...
    ||.||_Fro stands for the Frobenius norm and ||.||_1,1 stands for the entry-wise matrix norm 
    ...
    fit_algorithm : {'lars', 'cd'}, default='lars'
        * `'lars'`: uses the least angle regression method to solve the lasso problem (:func:`~sklearn.linear_model.lars_path`);
        * `'cd'`  : uses the coordinate descent method to compute the Lasso solution (:class:`~sklearn.linear_model.Lasso`). 
                    Lars will be faster if the estimated components are sparse.
    ...
    transform_algorithm : {'lasso_lars', 'lasso_cd', 'lars', 'omp', 'threshold'}, default='omp'
        ...
        - `'lars'`      : uses the least angle regression method (:func:`~sklearn.linear_model.lars_path`);
        - `'lasso_lars'`: uses Lars to compute the Lasso solution.
        - `'lasso_cd'`  : uses the coordinate descent method to compute the Lasso solution (:class:`~sklearn.linear_model.Lasso`). 
                        `'lasso_lars'` will be faster if the estimated components are sparse.
        - `'omp'`       : uses orthogonal matching pursuit to estimate the sparse solution.
        - `'threshold'` : squashes to zero all coefficients less than alpha from the projection ``dictionary * X'``.
    ...
    Attributes
    ----------
    components_ : ndarray of shape (n_components, n_features) dictionary atoms extracted from the data
    ...
    >>> X, dictionary, code = make_sparse_coded_signal(n_samples=30, n_components=15, n_features=20, n_nonzero_coefs=10, random_state=42)
    ...
    We can compare the average squared euclidean norm of the reconstruction error of the sparse coded signal relative to 
    the squared euclidean norm of the original signal:
    ...
    def __init__(self, n_components=None, *, alpha=1, max_iter=1000, tol=1e-8, fit_algorithm="lars", transform_algorithm="omp", \
        transform_n_nonzero_coefs=None, transform_alpha=None, n_jobs=None, code_init=None, dict_init=None, callback=None, \
        verbose=False, split_sign=False, random_state=None, positive_code=False, positive_dict=False, transform_max_iter=1000):
        super().__init__(transform_algorithm, transform_n_nonzero_coefs, transform_alpha, split_sign, n_jobs, positive_code, transform_max_iter)
    ...
    def fit_transform(self, X, y=None):
        ...
        Returns
        -------
        V : ndarray of shape (n_samples, n_components), Transformed data.
        """
        ...
        V, U, E, self.n_iter_ = _dict_learning(X, n_components, alpha=self.alpha, tol=self.tol, max_iter=self.max_iter, \
            method=method, method_max_iter=self.transform_max_iter, n_jobs=self.n_jobs, code_init=self.code_init, dict_init=self.dict_init, \
            callback=self.callback, verbose=self.verbose, random_state=random_state, return_n_iter=True, positive_dict=self.positive_dict, \
...
class MiniBatchDictionaryLearning(_BaseSparseCoding, BaseEstimator):
    ...
    ||.||_Fro stands for the Frobenius norm and ||.||_1,1 stands for the entry-wise matrix norm which is the sum of 
    the absolute values of all the entries in the matrix.
    ...
    transform_n_nonzero_coefs : int, default=None
        Number of nonzero coefficients to target in each column of the solution. 
        This is only used by `algorithm='lars'` and `algorithm='omp'`. If `None`, then `transform_n_nonzero_coefs=int(n_features / 10)`.
    ...
    >>> X, dictionary, code = make_sparse_coded_signal(n_samples=30, n_components=15, n_features=20, n_nonzero_coefs=10, random_state=42)
    >>> dict_learner = MiniBatchDictionaryLearning(n_components=15, batch_size=3, transform_algorithm='lasso_lars', \
    ...
    _parameter_constraints: dict = {
        "n_components": [Interval(Integral, 1, None, closed="left"), None],
        "alpha": [Interval(Real, 0, None, closed="left")],
        "n_iter": [Interval(Integral, 0, None, closed="left"), Hidden(StrOptions({"deprecated"})) ],
    ...
    def __init__(self, n_components=None, *, alpha=1, n_iter="deprecated", max_iter=None, fit_algorithm="lars", \
        n_jobs=None, batch_size=256, shuffle=True, dict_init=None, transform_algorithm="omp", transform_n_nonzero_coefs=None, \
        transform_alpha=None, verbose=False, split_sign=False, random_state=None, positive_code=False, positive_dict=False, \
        transform_max_iter=1000, callback=None, tol=1e-3, max_no_improvement=10):
        super().__init__(transform_algorithm, transform_n_nonzero_coefs, transform_alpha, split_sign, n_jobs, positive_code, transform_max_iter)
        self.n_components  = n_components
        self.alpha         = alpha
        self.n_iter        = n_iter
    ...
    def _minibatch_step(self, X, dictionary, random_state, step):
        ...
        code = _sparse_encode(X, dictionary, algorithm=self._fit_algorithm, alpha=self.alpha, n_jobs=self.n_jobs, \
            positive=self.positive_code, max_iter=self.transform_max_iter, verbose=self.verbose)
    ...
    def _check_convergence(self, X, batch_cost, new_dict, old_dict, n_samples, step, n_steps):
        ...
        - A small change of the dictionary between two minibatch updates. This is controlled by the tol parameter.
        - No more improvement on a smoothed estimate of the objective function for a a certain number of consecutive minibatch updates. 
          This is controlled by the max_no_improvement parameter.
        """
        batch_size = X.shape[0]
        ...
        # Ignore 100 first steps or 1 epoch to avoid initializing the ewa_cost with a too bad value
        if step <= min(100, n_samples / batch_size):
            if self.verbose:
                print(f"Minibatch step {step}/{n_steps}: mean batch cost: {batch_cost}")
            return False

        # Compute an Exponentially Weighted Average of the cost function to monitor the convergence 
        ...
        # Early stopping heuristic due to lack of improvement on smoothed cost function
        ...
        if (self.max_no_improvement is not None and self._no_improvement >= self.max_no_improvement):
            if self.verbose:
                print("Converged (lack of improvement in objective function) at step {step}/{n_steps}")
            return True
    ...
    def fit(self, X, y=None):
        ...
        if self.n_iter != "deprecated":
            warnings.warn(("'n_iter' is deprecated in version 1.1 and will be removed in version 1.4. "\
                "Use 'max_iter' and let 'n_iter' to its default value instead. 'n_iter' is also ignored if 'max_iter' is specified."), FutureWarning)
            n_iter = self.n_iter
        ...
        if self.max_iter is not None:
            ...
            for i, batch in zip(range(n_steps), batches):
                ...
                # XXX callback param added for backward compat in #18975 but a common unified callback API should be preferred
            ...
            for i, batch in zip(range(n_iter), batches):
                ...
                if self.verbose > 10 or trigger_verbose: print(f"{i} batches processed.")
