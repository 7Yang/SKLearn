...
def norm(x):
    ...
    Parameters
    ----------
    x : array-like, Vector for which to compute the norm.
    """
...
def trace_dot(X, Y):
    ...
    Parameters
    ----------
    X : array-like, First matrix.
    Y : array-like, Second matrix.
    """
...
def _beta_divergence(X, W, H, beta, square_root=False):
    ...
    square_root : bool, default=False
        If True, return np.sqrt(2 * res) For beta == 2, it corresponds to the Frobenius norm.

    Returns
    -------
        res : float, Beta divergence of X and np.dot(X, H).
    """
    ...
    # beta-divergence, beta not in (0, 1, 2)
        ...
        if sp.issparse(X):
            # slow loop, but memory efficient computation of : np.sum(np.dot(W, H) ** beta)
            sum_WH_beta = 0
...
def _initialize_nmf(X, n_components, init=None, eps=1e-6, random_state=None):
    ...
    init :  {'random', 'nndsvd', 'nndsvda', 'nndsvdar'}, default=None
        ...
        .. versionchanged:: 1.1
            When `init=None` and n_components is less than n_samples and n_features defaults to `nndsvda` instead of `nndsvd`.

    eps : float, default=1e-6
        Truncate all values less then this in output to zero.

    random_state : int, RandomState instance or None, default=None
        Used when ``init`` == 'nndsvdar' or 'random'. Pass an int for reproducible results across multiple function calls.  
    ...
    References
    ----------
    C. Boutsidis, E. Gallopoulos: SVD based initialization: 
...
def _update_coordinate_descent(X, W, Ht, l1_reg, l2_reg, shuffle, random_state):
    ...
    Update W to minimize the objective function, iterating once over all coordinates. 
    By symmetry, to update H, one can call _update_coordinate_descent(X.T, Ht, W, ...).
...
def _fit_coordinate_descent(X, W, H, tol=1e-4, max_iter=200, l1_reg_W=0, l1_reg_H=0, l2_reg_W=0, \
    l2_reg_H=0, update_H=True, verbose=0, shuffle=False, random_state=None):
    """
    Compute Non-negative Matrix Factorization (NMF) with Coordinate Descent

    The objective function is minimized with an alternating minimization of W and H. 
    Each minimization is done with a cyclic (up to a permutation of the features) Coordinate Descent.
    ...
    random_state : int, RandomState instance or None, default=None
        Used to randomize the coordinates in the CD solver, when ``shuffle`` is set to ``True``. 
    ...
    References
    ----------
    .. [1] :doi:`"Fast local algorithms for large scale nonnegative matrix and tensor factorizations" <10.1587/transfun.E92.A.708>`
       Cichocki, Andrzej, and P. H. A. N. Anh-Huy. IEICE transactions on fundamentals of electronics, 
       communications and computer sciences 92.3: 708-721, 2009.
    """
...
def _multiplicative_update_w(X, W, H, beta_loss, l1_reg_W, l2_reg_W, gamma, H_sum=None, HHt=None, XHt=None, update_H=True):
    ...
    if beta_loss == 2:
        ...
        if beta_loss == 1:
            ...
            # speeds up computation time refer to /numpy/numpy/issues/9363
            WH_safe_X_data **= -1
            WH_safe_X_data **= 2
        ...
        if beta_loss == 1:
            ...
            if sp.issparse(X):
                # memory efficient computation (compute row by row, avoiding the dense matrix WH)
...
def _multiplicative_update_h(X, W, H, beta_loss, l1_reg_H, l2_reg_H, gamma, A=None, B=None, rho=None):
    ...
    if beta_loss == 2:
        ...
        # beta_loss not in (1, 2)
            ...
            if sp.issparse(X):
                # memory efficient computation (compute column by column, avoiding the dense matrix WH)
...
def _fit_multiplicative_update(X, W, H, beta_loss="frobenius", max_iter=200, tol=1e-4, l1_reg_W=0, \
    l1_reg_H=0, l2_reg_W=0, l2_reg_H=0, update_H=True, verbose=0):
    """
    Compute Non-negative Matrix Factorization with Multiplicative Update.

    The objective function is _beta_divergence(X, WH) and is minimized with an alternating minimization of W and H. 
    Each minimization is done with a Multiplicative Update.
    ...
    beta_loss : float or {'frobenius', 'kullback-leibler', 'itakura-saito'}, default='frobenius'
        ...
        Note that for beta_loss <= 0 (or 'itakura-saito'), the input matrix X cannot contain zeros.
    ...
    References
    ----------
    Lee, D. D., & Seung, H., S. (2001). Algorithms for Non-negative Matrix Factorization. Adv. Neural Inform. Process. Syst.. 13.
    ...
    for n_iter in range(1, max_iter + 1):
        # update W H_sum, HHt and XHt are saved and reused if not update_H
        W, H_sum, HHt, XHt = _multiplicative_update_w( X, W, H, beta_loss=beta_loss, l1_reg_W=l1_reg_W, \
            l2_reg_W=l2_reg_W, gamma=gamma, H_sum=H_sum, HHt=HHt, XHt=XHt, update_H=update_H)
...
@validate_params({
    ...
    "W": ["array-like", None],
    "H": ["array-like", None],
    "update_H": ["boolean"] }, prefer_skip_nested_validation=False)
def non_negative_factorization(X, W=None, H=None, n_components="warn", *, init=None, update_H=True, \
    solver="cd", beta_loss="frobenius", tol=1e-4, max_iter=200, alpha_W=0.0, alpha_H="same", l1_ratio=0.0, \
    ...
    The generic norm :math:`||X - WH||_{loss}^2` may represent the Frobenius norm or another supported beta-divergence loss.
    The choice between options is controlled by the `beta_loss` parameter.

    The regularization terms are scaled by `n_features` for `W` and by `n_samples` for `H` to keep their impact balanced 
    with respect to one another and to the data fit term as independent as possible of the size `n_samples` of the training set.
    ...
    Note that the transformed data is named W and the components matrix is named H. In the NMF literature, 
    the naming convention is usually the opposite since the data matrix X is transposed.
    ...
    W : array-like of shape (n_samples, n_components), default=None
        ...
        If `update_H=False`, it is initialised as an array of zeros, unless `solver='mu'`, 
            then it is filled with values calculated by `np.sqrt(X.mean() / self._n_components)`.
    ...
    n_components : int or {'auto'} or None, default=None
        ...
        If `n_components='auto'`, the number of components is automatically inferred from `W` or `H` shapes.
    ...
    init : {'random', 'nndsvd', 'nndsvda', 'nndsvdar', 'custom'}, default=None
        ...
        - 'nndsvd': Nonnegative Double Singular Value Decomposition (NNDSVD) initialization (better for sparseness)
        - 'nndsvda': NNDSVD with zeros filled with the average of X (better when sparsity is not desired)
        - 'nndsvdar': NNDSVD with zeros filled with small random values (generally faster, less accurate alternative to NNDSVDa
          for when sparsity is not desired)
        - 'custom': If `update_H=True`, use custom matrices W and H which must both be provided. 
           If `update_H=False`, then only custom matrix H is used.
    ...
    random_state : int, RandomState instance or None, default=None
        Used for NMF initialisation (when ``init`` == 'nndsvdar' or 'random'), and in Coordinate Descent. 
        Pass an int for reproducible results across multiple function calls.  See :term:`Glossary <random_state>`.
    ...
    .. [2] :doi:`"Algorithms for nonnegative matrix factorization with the
       beta-divergence" <10.1162/NECO_a_00168>` Fevotte, C., & Idier, J. (2011). Neural Computation, 23(9).
    ...
    """
    est = NMF(n_components=n_components, init=init, solver=solver, beta_loss=beta_loss, tol=tol, \
        max_iter=max_iter, random_state=random_state, alpha_W=alpha_W, alpha_H=alpha_H, l1_ratio=l1_ratio, \
...
class _BaseNMF(ClassNamePrefixFeaturesOutMixin, TransformerMixin, BaseEstimator, ABC):
    ...
    _parameter_constraints: dict = {
        "n_components": [Interval(Integral, 1, None, closed="left"), None, StrOptions({"auto"}), Hidden(StrOptions({"warn"}))],
        "init": [StrOptions({"random", "nndsvd", "nndsvda", "nndsvdar", "custom"}), None ],
        "beta_loss": [StrOptions({"frobenius", "kullback-leibler", "itakura-saito"}), Real ],
    ...
    def __init__(self, n_components="warn", *, init=None, beta_loss="frobenius", tol=1e-4, \
    ...
    def _check_params(self, X):
        ...
        if self.n_components == "warn":
            warnings.warn(("The default value of `n_components` will change from `None` to `'auto'` in 1.6. "\
                "Set the value of `n_components` to `None` explicitly to suppress the warning."), FutureWarning)
    ...
    def inverse_transform(self, Xt=None, W=None):
        ...
        Returns
        -------
        X : ndarray of shape (n_samples, n_features), Returns a data matrix of the original shape.
        """
...
class NMF(_BaseNMF):
    ...
    The generic norm :math:`||X - WH||_{loss}` may represent the Frobenius norm or another supported beta-divergence loss.
    The choice between options is controlled by the `beta_loss` parameter.

    The regularization terms are scaled by `n_features` for `W` and by `n_samples` for `H` to keep their impact balanced with 
    respect to one another and to the data fit term as independent as possible of the size `n_samples` of the training set.

    The objective function is minimized with an alternating minimization of W and H.

    Note that the transformed data is named W and the components matrix is named H. 
    In the NMF literature, the naming convention is usually the opposite since the data matrix X is transposed.
    ...
    random_state : int, RandomState instance or None, default=None
        Used for initialisation (when ``init`` == 'nndsvdar' or 'random'), 
        and in Coordinate Descent. Pass an int for reproducible results across multiple function calls.
        See :term:`Glossary <random_state>`.
    ...
    n_components_ : int
        The number of components. It is same as the `n_components` parameter if it was given. 
        Otherwise, it will be same as the number of features.
    ...
    .. [2] :doi:`"Algorithms for nonnegative matrix factorization with the beta-divergence" <10.1162/NECO_a_00168>`
    ...
    def __init__(self, n_components="warn", *, init=None, solver="cd", beta_loss="frobenius", tol=1e-4, max_iter=200, \
        random_state=None, alpha_W=0.0, alpha_H="same", l1_ratio=0.0, verbose=0, shuffle=False):
        super().__init__(n_components=n_components, init=init, beta_loss=beta_loss, tol=tol, max_iter=max_iter, \
            random_state=random_state, alpha_W=alpha_W, alpha_H=alpha_H, l1_ratio=l1_ratio, verbose=verbose)
    ...
    def _check_params(self, X):
        ...
        if self.solver == "mu" and self.init == "nndsvd":
            warnings.warn(("The multiplicative update ('mu') solver cannot update zeros present in the initialization, " \
                "and so leads to poorer results when used jointly with init='nndsvd'. " \
                "You may try init='nndsvda' or init='nndsvdar' instead."), UserWarning)
    ...
    def fit_transform(self, X, y=None, W=None, H=None):
        ...
        Returns
        -------
        W : ndarray of shape (n_samples, n_components), Transformed data.
        """
    ...
    def _fit_transform(self, X, y=None, W=None, H=None, update_H=True):
        ...
        W : array-like of shape (n_samples, n_components), default=None
            If `init='custom'`, it is used as initial guess for the solution.
            If `update_H=False`, it is initialised as an array of zeros, 
                unless `solver='mu'`, then it is filled with values calculated by `np.sqrt(X.mean() / self._n_components)`.
            If `None`, uses the initialisation method specified in `init`.
        ...
        update_H : bool, default=True
            If True, both W and H will be estimated from initial guesses, this corresponds to a call to the 'fit_transform' method.
            If False, only W will be estimated, this corresponds to a call to the 'transform' method.
        ...
        if X.min() == 0 and self._beta_loss <= 0:
            raise ValueError("When beta_loss <= 0 and X contains zeros, the solver may diverge. "\
                    "Please add small values to X, or use a positive beta_loss.")
        ...
        if self.solver == "cd":
            W, H, n_iter = _fit_coordinate_descent(X, W, H, self.tol, self.max_iter, l1_reg_W, \
                l1_reg_H, l2_reg_W, l2_reg_H, update_H=update_H, verbose=self.verbose, shuffle=self.shuffle, \
                random_state=self.random_state)
        elif self.solver == "mu":
            W, H, n_iter, *_ = _fit_multiplicative_update(X, W, H, self._beta_loss, self.max_iter, self.tol, \
                l1_reg_W, l1_reg_H, l2_reg_W, l2_reg_H, update_H, self.verbose)
        else:
...
class MiniBatchNMF(_BaseNMF):
    ...
    tol : float, default=1e-4
        Control early stopping based on the norm of the differences in `H` between 2 steps. 
        To disable early stopping based on changes in `H`, set `tol` to 0.0.
    ...
    forget_factor : float, default=0.7
        Amount of rescaling of past information. Its value could be 1 with finite datasets. 
        Choosing values < 1 is recommended with online learning as more recent batches will weight more than past batches.

    fresh_restarts : bool, default=False
        Whether to completely solve for W at each step. Doing fresh restarts will likely lead to a better solution 
        for a same number of iterations but it is much slower.
    ...
    fresh_restarts_max_iter : int, default=30
        ...
        These iterations may be stopped early based on a small change of W controlled by `tol`.
    ...
    MiniBatchDictionaryLearning : Finds a dictionary that can best be used to represent data using a sparse code.
    ...
    .. [3] :doi:`"Online algorithms for nonnegative matrix factorization with the Itakura-Saito divergence" <10.1109/ASPAA.2011.6082314>`
    ...
    def __init__(self, n_components="warn", *, init=None, batch_size=1024, beta_loss="frobenius", tol=1e-4, \
        max_no_improvement=10, max_iter=200, alpha_W=0.0, alpha_H="same", l1_ratio=0.0, forget_factor=0.7, \
        fresh_restarts=False, fresh_restarts_max_iter=30, transform_max_iter=None, random_state=None, verbose=0):
        """
        """
        super().__init__(n_components=n_components, init=init, beta_loss=beta_loss, tol=tol, max_iter=max_iter, \
            random_state=random_state, alpha_W=alpha_W, alpha_H=alpha_H, l1_ratio=l1_ratio, verbose=verbose)
    ...
    def _solve_W(self, X, H, max_iter):
        ...
        Update W with H being fixed, until convergence. This is the heart of `transform` but it's also used during `fit` when doing fresh restarts.
        """
        avg = np.sqrt(X.mean() / self._n_components)
    ...
    def _minibatch_step(self, X, W, H, update_H):
        ...
        # get scaled regularization terms. Done for each minibatch to take into account variable sizes of minibatches.
        l1_reg_W, l1_reg_H, l2_reg_W, l2_reg_H = self._compute_regularization(X)
        ...
        batch_cost = (_beta_divergence(X, W, H, self._beta_loss) + l1_reg_W * W.sum() + l1_reg_H * H.sum() + 
                l2_reg_W * (W**2).sum() + l2_reg_H * (H**2).sum()) / batch_size

        # update H (only at fit or fit_transform)
        if update_H:
            H[:] = _multiplicative_update_h(X, W, H, beta_loss=self._beta_loss, l1_reg_H=l1_reg_H, l2_reg_H=l2_reg_H, \
                gamma=self._gamma, A=self._components_numerator, B=self._components_denominator, rho=self._rho)
    ...
    def partial_fit(self, X, y=None, W=None, H=None):
        ...
        This is especially useful when the whole dataset is too big to fit in memory at once (see :ref:`scaling_strategies`).
