...
def _cov(X, shrinkage=None, covariance_estimator=None):
    ...
    covariance_estimator : estimator, default=None
        ...
        The object should have a fit method and a ``covariance_`` attribute like the estimators in :mod:`sklearn.covariance``.  if None the shrinkage parameter drives the estimate.

        .. versionadded:: 0.24

    Returns
    -------
    s : ndarray of shape (n_features, n_features), Estimated covariance matrix.
    """
...
def _class_means(X, y):
    ...
    Returns
    -------
    means : array-like of shape (n_classes, n_features), Class means.
    """
    ...
    if is_array_api_compliant:
        ...
        # TODO: Explore the choice of using bincount + add.at as it seems sub optimal from a performance-wise
        cnt = np.bincount(y)
        np.add.at(means, y, X)
        means /= cnt[:, None]
    return means
...
def _class_cov(X, y, priors, shrinkage=None, covariance_estimator=None):
    """
    Compute weighted within-class covariance matrix. The per-class covariance are weighted by the class priors.
    ...
    covariance_estimator : estimator, default=None
        If not None, `covariance_estimator` is used to estimate the covariance matrices instead of relying the empirical covariance estimator (with potential shrinkage).
    ...
    Returns
    -------
    cov : array-like of shape (n_features, n_features), Weighted within-class covariance matrix
    """
    classes  = np.unique(y)
...
class LinearDiscriminantAnalysis(ClassNamePrefixFeaturesOutMixin, LinearClassifierMixin, TransformerMixin, BaseEstimator):
    ...
    solver : {'svd', 'lsqr', 'eigen'}, default='svd'
        ...
        .. versionchanged:: 1.2
            `solver="svd"` now has experimental Array API support. See the :ref:`Array API User Guide <array_api>` for more details.
    ...
    covariance_estimator : covariance estimator, default=None
        If not None, `covariance_estimator` is used to estimate the covariance matrices instead of relying on the empirical covariance estimator (with potential shrinkage).
        The object should have a fit method and a ``covariance_`` attribute like the estimators in :mod:`sklearn.covariance`.
        if None the shrinkage parameter drives the estimate.
    ...
    covariance_ : array-like of shape (n_features, n_features)
        Weighted within-class covariance matrix. It corresponds to `sum_k prior_k * C_k` where `C_k` is the covariance matrix of the samples in class `k`. 
    ...
    def _solve_lstsq(self, X, y, shrinkage, covariance_estimator):
        ...
        The least squares solver computes a straightforward solution of the optimal decision rule based directly on the discriminant functions. 
        It can only be used for classification (with any covariance estimator), because estimation of eigenvectors is not performed. 
        Therefore, dimensionality reduction with the transform is not supported.
        ...
        shrinkage : 'auto', float or None
            ...
            Shrinkage parameter is ignored if  `covariance_estimator` i not None
    ...
    def _solve_eigen(self, X, y, shrinkage, covariance_estimator):
        ...
        The eigenvalue solver computes the optimal solution of the Rayleigh coefficient 
        (basically the ratio of between class scatter to within class scatter). 
        This solver supports both classification and dimensionality reduction (with any covariance estimator).
    ...
    def _solve_svd(self, X, y):
        ...
        # Centers are living in a space with n_classes-1 dim (maximum) Use SVD to find projection in the space spanned by the (n_classes) centers
    ...
    def decision_function(self, X):
        ...
        In a binary classification setting this instead corresponds to the difference `log p(y = 1 | x) - log p(y = 0 | x)`. See :ref:`lda_qda_math`.
...
class QuadraticDiscriminantAnalysis(ClassifierMixin, BaseEstimator):
    ...
    tol : float, default=1.0e-4
        Absolute threshold for a singular value to be considered significant, used to estimate the rank of `Xk` where `Xk` is the centered matrix of samples in class k. 
        This parameter does not affect the predictions. It only controls a warning that is raised when features are considered to be colinear.
    ...
    rotations_ : list of len n_classes of ndarray of shape (n_features, n_k)
        For each class k an array of shape (n_features, n_k), where ``n_k = min(n_features, number of elements in class k)`` It is the rotation of the Gaussian distribution, 
        i.e. its principal axis. It corresponds to `V`, the matrix of eigenvectors coming from the SVD of `Xk = U S Vt` where `Xk` is the centered matrix of samples from class k.

    scalings_ : list of len n_classes of ndarray of shape (n_k,)
        For each class, contains the scaling of the Gaussian distributions along its principal axes, i.e. the variance in the rotated coordinate system. 
        It corresponds to `S^2 / (n_samples - 1)`, where `S` is the diagonal matrix of singular values from the SVD of `Xk`, where `Xk` is the centered matrix of samples from class k.
    ...
    def fit(self, X, y):
        ...
        Fit the model according to the given training data and parameters.
            ...
            .. versionchanged:: 0.19
               ``store_covariances`` has been moved to main constructor as ``store_covariance``
    ...
    def predict(self, X):
        ...
        Returns
        -------
        C : ndarray of shape (n_samples,), Estimated probabilities.
        """
    ...
    def predict_proba(self, X):
        ...
        # compute the likelihood of the underlying gaussian models up to a multiplicative constant.
