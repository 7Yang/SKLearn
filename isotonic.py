...
def check_increasing(x, y):
    """
    Determine whether y is monotonically(单调的) correlated with x.
...
@validate_params({
    ...
    "increasing": ["boolean"]}, prefer_skip_nested_validation=True)
def isotonic_regression(y, *, sample_weight=None, y_min=None, y_max=None, increasing=True):
    """
    Solve the isotonic regression model.  Read more in the :ref:`User Guide <isotonic>`.
    ...
    if y_min is not None or y_max is not None:
        ...
        if y_min is None: y_min = -np.inf
        if y_max is None: y_max =  np.inf
...
class IsotonicRegression(RegressorMixin, TransformerMixin, BaseEstimator):
    ...
    increasing : bool or 'auto', default=True
        Determines whether the predictions should be constrained to increase or decrease with `X`. 
        'auto' will decide based on the Spearman correlation estimate's sign.
    ...
    References
    ----------
    Isotonic Median Regression: A Linear Programming Approach Nilotpal Chakravarti Mathematics of Operations Research
    Vol. 14, No. 2 (May, 1989), pp. 303-308

    Isotone Optimization in R : Pool-Adjacent-Violators Algorithm (PAVA) and Active Set Methods de Leeuw, Hornik, 
    ...
    def _build_y(self, X, y, sample_weight, trim_duplicates=True):
        ...
        if trim_duplicates:
            ...
            # Aside from the 1st and last point, remove points whose y values are equal to both the point before and the point after it.
            keep_data[1:-1] = np.logical_or(np.not_equal(y[1:-1], y[:-2]), np.not_equal(y[1:-1], y[2:]))
            return X[keep_data], y[keep_data]
        else:
            # The ability to turn off trim_duplicates is only used to it make easier to unit test 
            # that removing duplicates in y does not have any impact the resulting interpolation function (besides prediction speed).
            return X, y
    ...
    def fit(self, X, y, sample_weight=None):
        ...
        # It is necessary to store the non-redundant part of the training set on the model to make it possible to support 
        # model persistence via the pickle module as the object built by scipy.interp1d is not picklable directly.
    ...
    def _transform(self, T):
        ...
        Since `transform` is wrapped to output arrays of specific types (e.g.  NumPy arrays, pandas DataFrame), 
        we cannot make `predict` call `transform` directly.

        The above behaviour could be changed in the future, if we decide to output other type of arrays when calling `predict`.
        """
    ...
    # We implement get_feature_names_out here instead of using `ClassNamePrefixFeaturesOutMixin`` because `input_features` are ignored.
    # `input_features` are ignored because `IsotonicRegression` accepts 1d arrays and the semantics of `feature_names_in_` are not clear for 1d arrays.
    def get_feature_names_out(self, input_features=None):
        ...
        Returns
        -------
        feature_names_out : ndarray of str objects, An ndarray with one string i.e. ["isotonicregression0"].
        """
