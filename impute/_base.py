...
def _check_inputs_dtype(X, missing_values):
    ...
    if X.dtype.kind in ("f", "i", "u") and not isinstance(missing_values, numbers.Real):
        raise ValueError("'X' and 'missing_values' types are expected to be both numerical. "\
            "Got X.dtype={} and type(missing_values)={}.".format(X.dtype, type(missing_values)))

def _most_frequent(array, extra_value, n_repeat):
    """
    Compute the most frequent value in a 1d array extended with [extra_value] * n_repeat, 
    where extra_value is assumed to be not part of the array.
    """
    # Compute the most frequent value in array only
    if array.size > 0:
        if array.dtype == object:
            # scipy.stats.mode is slow with object dtype array. Python Counter is more efficient
...
class _BaseImputer(TransformerMixin, BaseEstimator):
    ...
    def _transform_indicator(self, X):
        ...
        Note that X must be the original data as passed to the imputer before any imputation, 
        since imputation may be done inplace in some cases.
        """
        if self.add_indicator:
    ...
    def _concatenate_indicator(self, X_imputed, X_indicator):
        ...
        if sp.issparse(X_imputed):
            # sp.hstack may result in different formats between sparse arrays and matrices; 
            # specify the format to keep consistent behavior
            hstack = partial(sp.hstack, format=X_imputed.format)
        else:
            hstack = np.hstack

        if X_indicator is None:
            raise ValueError("Data from the missing indicator are not provided. "\
...
class SimpleImputer(_BaseImputer):
    ...
    add_indicator : bool, default=False
        ...
        If a feature has no missing values at fit/train time, 
        the feature won't appear on the missing indicator even if there are missing values at transform/test time.
    ...
    In a prediction context, simple imputation usually performs poorly when associated with a weak learner. 
    However, with a powerful learner, it can lead to as good or better performance than complex imputation such as
    ...
    For a more detailed example see :ref:`sphx_glr_auto_examples_impute_plot_missing_values.py`.
    """
    ...
    def _validate_input(self, X, in_fit):
        if self.strategy in ("most_frequent", "constant"):
            # If input is a list of strings, dtype = object.  Otherwise ValueError is raised in SimpleImputer
            # with strategy='most_frequent' or 'constant' because the list is converted to Unicode numpy array
        ...
        if X.dtype.kind not in ("i", "u", "f", "O"):
            raise ValueError("SimpleImputer does not support data with dtype {0}. Please provide either a numeric array " \
    ...
    def fit(self, X, y=None):
        ...
        # default fill_value is 0 for numerical input and "missing_value" otherwise
    ...
    def _sparse_fit(self, X, strategy, missing_values, fill_value):
        ...
        if strategy == "constant":
            # for constant strategy, self.statistics_ is used to store fill_value in each column
    ...
    def _dense_fit(self, X, strategy, missing_values, fill_value):
        ...
        # Most frequent
            ...
            # Avoid use of scipy.stats.mstats.mode due to the required additional overhead and slow benchmarking performance.
        ...
        # Constant
            ...
            # for constant strategy, self.statistcs_ is used to store fill_value in each column
    ...
    def transform(self, X):
        ...
        if self.strategy == "constant" or self.keep_empty_features:
            ...
            if invalid_mask.any():
                ...
                warnings.warn("Skipping features without any observed values: {invalid_features}. " \
                    "At least one non-missing value is needed for imputation with strategy='{self.strategy}'.")
        ...
        if sp.issparse(X):
            if self.missing_values == 0:
                ...
                # if no invalid statistics are found, use the mask computed before, else recompute mask
    ...
    def inverse_transform(self, X):
        ...
        This operation can only be performed after :class:`SimpleImputer` is instantiated with `add_indicator=True`.

        Note that `inverse_transform` can only invert the transform in features that have binary indicators for missing values. 
        If a feature has no missing values at `fit` time, the feature won't have a binary indicator, 
        ...
        if not self.add_indicator:
            raise ValueError("'inverse_transform' works only when 'SimpleImputer' is instantiated with 'add_indicator=True'. "
                f"Got 'add_indicator={self.add_indicator}' instead.")
...
class MissingIndicator(TransformerMixin, BaseEstimator):
    ...
    Note that this component typically should not be used in a vanilla :class:`~sklearn.pipeline.Pipeline` consisting of transformers 
    and a classifier, but rather could be added using a :class:`~sklearn.pipeline.FeatureUnion` or :class:`~sklearn.compose.ColumnTransformer`.
    ...
    error_on_new : bool, default=True
        If `True`, :meth:`transform` will raise an error when there are features with missing values that have no missing values in
        :meth:`fit`. This is applicable only when `features='missing-only'`.

    Attributes
    ----------
    features_ : ndarray of shape (n_missing_features,) or (n_features,)
        The features indices which will be returned when calling :meth:`transform`. They are computed during :meth:`fit`. 
        If `features='all'`, `features_` is equal to `range(n_features)`.
    ...
    def _get_missing_features_info(self, X):
        ...
        X : {ndarray, sparse matrix} of shape (n_samples, n_features)
            The input data with missing values. Note that `X` has been checked in :meth:`fit` and :meth:`transform` before to call this function.
    ...
    def _fit(self, X, y=None, precomputed=False):
        ...
        # Need not validate X again as it would have already been validated in the Imputer calling MissingIndicator
        if not self._precomputed:
            X = self._validate_input(X, in_fit=True)
        else:
    ...
    def transform(self, X):
        ...
        if self.features == "missing-only":
            ...
            if self.error_on_new and features_diff_fit_trans.size > 0:
                raise ValueError("The features {} have missing values in transform but have no missing values in fit."
                    .format(features_diff_fit_trans))
