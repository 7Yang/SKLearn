...
class LocalOutlierFactor(KNeighborsMixin, OutlierMixin, NeighborsBase):
    ...
    one can identify samples that have a substantially lower density than their neighbors. These are considered outliers.
    ...
    leaf_size : int, default=30
        Leaf is size passed to :class:`BallTree` or :class:`KDTree`. 
    ...
    metric : str or callable, default='minkowski'
        ...
        X may be a :term:`sparse graph`, in which case only "nonzero" elements may be considered neighbors.

        If metric is a callable function, it takes two arrays representing 1D vectors as inputs 
    ...
    offset_ : float
        ...
        The offset is set to -1.5 (inliers score around -1), except when a contamination parameter different than "auto" is provided. 
        In that case, the offset is defined in such a way we obtain the expected number of outliers in training.
    ...
    See Also
    --------
    sklearn.svm.OneClassSVM: Unsupervised Outlier Detection using Support Vector Machine.
    ...
    def fit_predict(self, X, y=None):
        ...
        **Not available for novelty detection (when novelty is set to True).**
        ...
        # As fit_predict would be different from fit.predict, fit_predict is only available for outlier detection (novelty=False)
    ...
    def fit(self, X, y=None):
        ...
        Returns
        -------
        self : LocalOutlierFactor, The fitted local outlier factor detector.
        """
        self._fit(X)

        n_samples = self.n_samples_fit_
        if self.n_neighbors > n_samples:
            warnings.warn("n_neighbors (%s) is greater than the total number of samples (%s). n_neighbors "
    ...
    def _check_novelty_predict(self):
        if not self.novelty:
            msg = ("predict is not available when novelty=False, use fit_predict if you want to predict on training data. "
                    "Use novelty=True if you want to use LOF for novelty detection and predict on new unseen data.")
            raise AttributeError(msg)
        return True
    ...
    def predict(self, X=None):
        ...
        **Only available for novelty detection (when novelty is set to True).**
        ...
        Note that the result of ``clf.fit(X)`` then ``clf.predict(X)`` with ``novelty=True`` 
        may differ from the result obtained by ``clf.fit_predict(X)`` with ``novelty=False``.
    ...
    def _predict(self, X=None):
        ...
        X : {array-like, sparse matrix} of shape (n_samples, n_features), default=None
            The query sample or samples to compute the Local Outlier Factor w.r.t. the training samples. 
            If None, makes prediction on the training data without considering them as their own neighbors.
    ...
    def _check_novelty_decision_function(self):
        if not self.novelty:
            msg = ("decision_function is not available when novelty=False. " 
                "Use novelty=True if you want to use LOF for novelty detection and compute decision_function for new unseen data. "
                "Note that the opposite LOF of the training samples is always available by considering the negative_outlier_factor_ attribute.")
            raise AttributeError(msg)
        return True
    ...
    def decision_function(self, X):
        ...
        **Only available for novelty detection (when novelty is set to True).**
        ...
        The argument X is supposed to contain *new data*: if X contains a point from training, it considers the later in its own neighborhood.
    ...
    def _check_novelty_score_samples(self):
        if not self.novelty:
            msg = ("score_samples is not available when novelty=False. "
                "The scores of the training samples are always available through the negative_outlier_factor_ attribute. "
                "Use novelty=True if you want to use LOF for novelty detection and compute score_samples for new unseen data.")
            raise AttributeError(msg)
        return True
    ...
    def score_samples(self, X):
        ...
        **Only available for novelty detection (when novelty is set to True).**
        The argument X is supposed to contain *new data*: if X contains a point from training, it considers the later in its own neighborhood.
        Also, the samples in X are not considered in the neighborhood of any point. 
        Because of this, the scores obtained via ``score_samples`` may differ from the standard LOF scores.
    ...
    def _local_reachability_density(self, distances_X, neighbors_indices):
        ...
        The LRD of a sample is the inverse of the average reachability distance of its k-nearest neighbors.

        Parameters
        ----------
        distances_X : ndarray of shape (n_queries, self.n_neighbors)
            Distances to the neighbors (in the training samples `self._fit_X`) of each query point to compute the LRD.

        neighbors_indices : ndarray of shape (n_queries, self.n_neighbors)
            Neighbors indices (of each query point) among training samples self._fit_X.

        Returns
        -------
        local_reachability_density : ndarray of shape (n_queries,), The local reachability density of each sample.
        """
