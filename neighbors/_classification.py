...
class KNeighborsClassifier(KNeighborsMixin, ClassifierMixin, NeighborsBase):
    ...
    weights : {'uniform', 'distance'}, callable or None, default='uniform'
        ...
        :ref:`sphx_glr_auto_examples_neighbors_plot_classification.py` showing the impact of the `weights` parameter on the decision boundary.
    ...
    metric : str or callable, default='minkowski'
        ...
        and the metrics listed in :class:`~sklearn.metrics.pairwise.distance_metrics` for valid metric values.
        ...
        If metric is a callable function, it takes two arrays representing 1D vectors as inputs and must return one value indicating 
        the distance between those vectors. This works for Scipy's metrics, but is less efficient than passing the metric name as a string.
    ...
    effective_metric_ : str or callble
        The distance metric used. It will be same as the `metric` parameter or a synonym of it, 
        e.g. 'euclidean' if the `metric` parameter set to 'minkowski' and `p` parameter set to 2.

    effective_metric_params_ : dict
        Additional keyword arguments for the metric function. 
        For most metrics will be same with `metric_params` parameter, 
        but may also contain the `p` parameter value if the `effective_metric_` attribute is set to 'minkowski'.
    ...
    def __init__(self, n_neighbors=5, *, weights="uniform", algorithm="auto", leaf_size=30, p=2, \
        ...
        super().__init__(n_neighbors=n_neighbors, algorithm=algorithm, leaf_size=leaf_size, metric=metric, \
    ...
    def predict(self, X):
        ...
        if self.weights == "uniform":
            ...
            # In that case, we do not need the distances to perform the weighting so we do not compute them.
    ...
    def predict_proba(self, X):
        ...
        check_is_fitted(self, "_fit_method")
        if self.weights == "uniform":
            # TODO: systematize this mapping of metric for PairwiseDistancesReductions.
            ...
            if (self._fit_method == "brute" and ArgKminClassMode.is_usable_for(X, self._fit_X, metric) and not self.outputs_2d_):
                ...
                probabilities = ArgKminClassMode.compute(X, self._fit_X, k=self.n_neighbors, weights=self.weights, Y_labels=self._y,
                    unique_Y_labels=self.classes_, metric=metric, metric_kwargs=metric_kwargs,
...
class RadiusNeighborsClassifier(RadiusNeighborsMixin, ClassifierMixin, NeighborsBase):
    ...
    metric : str or callable, default='minkowski'
        ...
        See the documentation of `scipy.spatial.distance
        <https://docs.scipy.org/doc/scipy/reference/spatial.distance.html>`_ and the metrics listed in
        :class:`~sklearn.metrics.pairwise.distance_metrics` for valid metric values.
    ...
    effective_metric_params_ : dict
        Additional keyword arguments for the metric function. 
        For most metrics will be same with `metric_params` parameter, but may also contain the `p` parameter value 
        if the `effective_metric_` attribute is set to 'minkowski'.
    ...
    def fit(self, X, y):
        ...
        Returns
        -------
        self : RadiusNeighborsClassifier, The fitted radius neighbors classifier.
        """
        self._fit(X, y)
        ...
        if self.outlier_label is None:
            ...
            outlier_label_ = []
            # iterate over multi-output, get the most frequent label for each output.
            for k, classes_k in enumerate(classes_):
            ...
            if _is_arraylike(self.outlier_label) and not isinstance(self.outlier_label, str):
                if len(self.outlier_label) != len(classes_):
                    raise ValueError("The length of outlier_label: {} is inconsistent with the output length: {}"
                        .format(self.outlier_label, len(classes_)))
    ...
    def predict(self, X):
        ...
        for k, prob in enumerate(probs):
            # iterate over multi-output, assign labels based on probabilities of each output.
    ...
    def predict_proba(self, X):
        ...
        if self.outlier_label_ is None and outliers.size > 0:
            raise ValueError("No neighbors found for test samples %r, you can try using larger radius, "
                "giving a label for outliers, or considering removing them from your dataset." % outliers)
        ...
        for k, classes_k in enumerate(classes_):
            ...
            if outliers.size > 0:
                ...
                if label_index.size == 1:
                    ...
                    warnings.warn("Outlier label {} is not in training classes. All class probabilities of "
                        "outliers will be assigned with 0.".format(self.outlier_label_[k]))
