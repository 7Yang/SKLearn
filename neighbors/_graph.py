...
def kneighbors_graph(X, n_neighbors, *, mode="connectivity", metric="minkowski", p=2, metric_params=None, include_self=False, n_jobs=None):
    ...
    metric : str, default='minkowski'
        ...
        which results in the standard Euclidean distance when p = 2. See the documentation of `scipy.spatial.distance
    ...
    p : float, default=2
        ...
        and euclidean_distance (l2) for p = 2.  For arbitrary p, minkowski_distance (l_p) is used. This parameter is expected to be positive.
...
def radius_neighbors_graph(X, radius, *, mode="connectivity", metric="minkowski", p=2, metric_params=None, include_self=False, n_jobs=None):
    ...
    metric : str, default='minkowski'
        Metric to use for distance computation. Default is "minkowski", 
        which results in the standard Euclidean distance when p = 2. 
        See the documentation of `scipy.spatial.distance <https://docs.scipy.org/doc/scipy/reference/spatial.distance.html>`_ 
...
class KNeighborsTransformer(ClassNamePrefixFeaturesOutMixin, KNeighborsMixin, TransformerMixin, NeighborsBase):
    ...
    def __init__(self, *, mode="distance", n_neighbors=5, algorithm="auto", leaf_size=30, metric="minkowski", p=2, metric_params=None, n_jobs=None):
        super(KNeighborsTransformer, self).__init__(n_neighbors=n_neighbors, radius=None, algorithm=algorithm, leaf_size=leaf_size, \
    ...
    def fit(self, X, y=None):
        ...
        Returns
        -------
        self : KNeighborsTransformer, The fitted k-nearest neighbors transformer.
        """
        self._fit(X)
    ...
    def transform(self, X):
        ...
        Xt : sparse matrix of shape (n_samples_transform, n_samples_fit)
            ...
            The diagonal is always explicit.  The matrix is of CSR format.
        """
    ...
    def fit_transform(self, X, y=None):
        ...
        Xt : sparse matrix of shape (n_samples, n_samples)
            ...
            Only the neighbors have an explicit value.  The diagonal is always explicit.  The matrix is of CSR format.
        """
...
class RadiusNeighborsTransformer(ClassNamePrefixFeaturesOutMixin, RadiusNeighborsMixin, TransformerMixin, NeighborsBase):
    ...
    metric : str or callable, default='minkowski'
        ...
        If metric is a callable function, it takes two arrays representing 1D vectors as inputs and must return one value 
        indicating the distance between those vectors. This works for Scipy's metrics, 
    ...
    def __init__(self, *, mode="distance", radius=1.0, algorithm="auto", leaf_size=30, metric="minkowski", p=2, metric_params=None, n_jobs=None):
        super(RadiusNeighborsTransformer, self).__init__( n_neighbors=None, radius=radius, algorithm=algorithm, leaf_size=leaf_size, \
            metric=metric, p=p, metric_params=metric_params, n_jobs=n_jobs)
        self.mode = mode
    ...
    def fit(self, X, y=None):
        ...
        Returns
        -------
        self : RadiusNeighborsTransformer, The fitted radius neighbors transformer.
        """
        self._fit(X)
        self._n_features_out = self.n_samples_fit_
        return self
