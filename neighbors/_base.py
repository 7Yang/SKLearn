...
SCIPY_METRICS = [
    ...
    "jaccard", "mahalanobis", "minkowski", "rogerstanimoto", "russellrao", "seuclidean",
...
VALID_METRICS = dict(ball_tree=BallTree.valid_metrics, kd_tree=KDTree.valid_metrics,
...
def _get_weights(dist, weights):
    ...
    Returns
    -------
    weights_arr : array of the same shape as ``dist``, If ``weights == 'uniform'``, then returns None.
    """
    if weights in (None, "uniform"):
        return None

    if weights == "distance":
        # if user attempts to classify a point that was zero distance from one or more training points, 
        # those training points are weighted as 1.0 and the other points as 0.0
        if dist.dtype is np.dtype(object):
            for point_dist_i, point_dist in enumerate(dist):
                # check if point_dist is iterable
                # (ex: RadiusNeighborClassifier.predict may set an element of dist to 1e-6 to represent an 'outlier')
...
def _is_sorted_by_data(graph):
    ...
    graph : sparse matrix of shape (n_samples, n_samples)
        Neighbors graph as given by `kneighbors_graph` or `radius_neighbors_graph`. Matrix should be of format CSR format.
...
def _check_precomputed(X):
    ...
    If the precomputed distance matrix is sparse, it checks that the non-zero entries are sorted by distances. 
    If not, the matrix is copied and sorted.

    Parameters
    ----------
    X : {sparse matrix, array-like}, (n_samples, n_samples)
        Distance matrix to other samples. X may be a sparse matrix, in which case only non-zero elements may be considered neighbors.
...
def sort_graph_by_row_values(graph, copy=False, warn_when_not_sorted=True):
    ...
    ----------
    graph : sparse matrix of shape (n_samples, n_samples)
        Distance matrix to other samples, where only non-zero elements are considered neighbors. 
        Matrix is converted to CSR format if not already.

    copy : bool, default=False
        If True, the graph is copied before sorting. If False, the sorting is performed inplace. 
        If the graph is not of CSR format, `copy` must be True to allow the conversion to CSR format, otherwise an error is raised.

    warn_when_not_sorted : bool, default=True
        If True, a :class:`~sklearn.exceptions.EfficiencyWarning` is raised when the input graph is not sorted by row values.
    ...
    if warn_when_not_sorted:
        warnings.warn(("Precomputed sparse input was not sorted by row values. "
            "Use the function sklearn.neighbors.sort_graph_by_row_values to sort the input by row values, "
            "with warn_when_not_sorted=False to remove this warning."), EfficiencyWarning)
...
def _kneighbors_from_graph(graph, n_neighbors, return_distance):
    ...
    if n_neighbors is not None and row_nnz_min < n_neighbors:
        raise ValueError("%d neighbors per samples are required, but some samples have only %d neighbors in precomputed graph matrix."
            " Decrease number of neighbors used or recompute the graph with more neighbors." % (n_neighbors, row_nnz_min))
...
class NeighborsBase(MultiOutputMixin, BaseEstimator, metaclass=ABCMeta):
    ...
    @abstractmethod
    def __init__(self, n_neighbors=None, radius=None, algorithm="auto", leaf_size=30, metric="minkowski", p=2,
    ...
    def _check_algorithm_metric(self):
        ...
        if callable(self.metric):
            if self.algorithm == "kd_tree":
                ...
                raise ValueError("kd_tree does not support callable metric '%s' Function call overhead will result"
            ...
            raise ValueError("Metric '%s' not valid. Use sorted(sklearn.neighbors.VALID_METRICS['%s']) "
                "to get valid options. Metric can also be a callable function." % (self.metric, alg_check))

        if self.metric_params is not None and "p" in self.metric_params:
            if self.p is not None:
                warnings.warn(("Parameter p is found in metric_params. The corresponding parameter from __init__ is ignored."), 
    ...
    def _fit(self, X, y=None):
        if self._get_tags()["requires_y"]:
            ...
            if is_classifier(self):
                ...
                if y.ndim == 1 or y.ndim == 2 and y.shape[1] == 1:
                    if y.ndim != 1:
                        warnings.warn(("A column-vector y was passed when a 1d array was expected. Please change "
                            "the shape of y to (n_samples,), for example using ravel()."), DataConversionWarning, stacklevel=2)
                ...
                self.classes_ = []
                # Using `dtype=np.intp` is necessary since `np.bincount` (called in _classification.py) fails when dealing
        ...
        if issparse(X):
            ...
            if (self.effective_metric_ not in VALID_METRICS_SPARSE["brute"] and not callable(self.effective_metric_) \
                ...
                raise ValueError("Metric '%s' not valid for sparse input. Use sorted(sklearn.neighbors. VALID_METRICS_SPARSE['brute']) "
                    "to get valid options. Metric can also be a callable function." % (self.effective_metric_))
            self._fit_X = X.copy()
            self._tree = None
        ...
        if self._fit_method == "auto":
            # A tree approach is better for small number of neighbors or small number of features, with KDTree generally faster when available
            if (self.metric == "precomputed" or self._fit_X.shape[1] > 15 or 
                (self.n_neighbors is not None and self.n_neighbors >= self._fit_X.shape[0] // 2)):
                self._fit_method = "brute"
            else:
                if (self.effective_metric_ == "minkowski" and self.effective_metric_params_["p"] < 1):
                    ...
                    # 'minkowski' with weights is not supported by KDTree but is supported byBallTree.
                    self._fit_method = "ball_tree"
        ...
        if (self.effective_metric_ == "minkowski" and self.effective_metric_params_["p"] < 1):
            # For 0 < p < 1 Minkowski distances aren't valid distance metric as they do not satisfy triangular inequality:
            ...
            if self._fit_method == "brute":
                warnings.warn("Mind that for 0 < p < 1, Minkowski metrics are not distance metrics. "
                    "Continuing the execution with `algorithm='brute'`.")
            else:  # self._fit_method in ("kd_tree", "ball_tree")
                raise ValueError(f'algorithm="{self._fit_method}" does not support 0 < p < 1 for the Minkowski metric. '
                'To resolve this problem either set p >= 1 or algorithm="brute".')
        ...
        if self._fit_method == "ball_tree":
            ...
            if (self.effective_metric_ == "minkowski" and self.effective_metric_params_.get("w") is not None):
                raise ValueError("algorithm='kd_tree' is not valid for metric='minkowski' with a weight parameter 'w': "
                    "try algorithm='ball_tree' or algorithm='brute' instead.")
...
def _tree_query_parallel_helper(tree, *args, **kwargs):
    ...
    The Cython method tree.query is not directly picklable by cloudpickle under PyPy.
    """
...
class KNeighborsMixin:
    ...
    def kneighbors(self, X=None, n_neighbors=None, return_distance=True):
        ...
        Examples
        --------
        In the following example, we construct a NearestNeighbors class from an array representing our data set 
        ...
        use_pairwise_distances_reductions = (
            self._fit_method == "brute" and ArgKmin.is_usable_for(X if X is not None else self._fit_X, self._fit_X, self.effective_metric_))
        if use_pairwise_distances_reductions:
            results = ArgKmin.compute(X=X, Y=self._fit_X, k=n_neighbors, metric=self.effective_metric_, \
                metric_kwargs=self.effective_metric_params_, strategy="auto", return_distance=return_distance)
            ...
            chunked_results = list(pairwise_distances_chunked(\
                X, self._fit_X, reduce_func=reduce_func, metric=self.effective_metric_, n_jobs=n_jobs, **kwds))

        elif self._fit_method in ["ball_tree", "kd_tree"]:
            if issparse(X):
                raise ValueError("%s does not work with sparse matrices. Densify the data, or set algorithm='brute'"
                    % self._fit_method)
            chunked_results = Parallel(n_jobs, prefer="threads")(delayed(_tree_query_parallel_helper)(
                self._tree, X[s], n_neighbors, return_distance) for s in gen_even_slices(X.shape[0], n_jobs))
        else:
        ...
        if not query_is_train:
            ...
            # If the query data is the same as the indexed data, we would like to ignore the first nearest neighbor of every sample, 
            # i.e the sample itself.
            if return_distance:
            ...
            # Corner case: When the number of duplicates are more than the number of neighbors, 
            # the first NN will not be the sample, but a duplicate. In that case mask the first duplicate.
    ...
    def kneighbors_graph(self, X=None, n_neighbors=None, mode="connectivity"):
        ...
        X : {array-like, sparse matrix} of shape (n_queries, n_features), or (n_queries, n_indexed) if metric == 'precomputed', default=None
            The query point or points.  If not provided, neighbors of each indexed point are returned.
            In this case, the query point is not considered its own neighbor.  
            For ``metric='precomputed'`` the shape should be (n_queries, n_indexed). Otherwise the shape should be (n_queries, n_features).
        ...
        mode : {'connectivity', 'distance'}, default='connectivity'
            ...
            in 'distance' the edges are distances between points, type of distance depends on the selected metric parameter in NearestNeighbors class.

        Returns
        -------
        A : sparse-matrix of shape (n_queries, n_samples_fit)
            `n_samples_fit` is the number of samples in the fitted data.
            `A[i, j]` gives the weight of the edge connecting `i` to `j`.  The matrix is of CSR format.
        ...
        if mode == "connectivity":
            ...
            raise ValueError('Unsupported mode, must be one of "connectivity", or "distance" but got "{mode}" instead')
...
class RadiusNeighborsMixin:
    ...
    def radius_neighbors(self, X=None, radius=None, return_distance=True, sort_results=False):
        ...
        Return the indices and distances of each point from the dataset lying in a ball with size ``radius`` around the points of the query array. 
        ...
        sort_results : bool, default=False
            If True, the distances and indices will be sorted by increasing distances before being returned. 
            If False, the results may not be sorted. If `return_distance=False`, setting `sort_results=True` will result in an error.
        ...
        the results for multiple query points cannot be fit in a standard data array.
        For efficiency, `radius_neighbors` returns arrays of objects, where each object is a 1D array of indices or distances.
        ...
        while the second array returned contains their indices.  In general, multiple points can be queried at the same time.
        """
        check_is_fitted(self)
        ...
        if use_pairwise_distances_reductions:
            results = RadiusNeighbors.compute(X=X, Y=self._fit_X, radius=radius, metric=self.effective_metric_,
                metric_kwargs=self.effective_metric_params_, strategy="auto", return_distance=return_distance, sort_results=sort_results)
            ...
            chunked_results = pairwise_distances_chunked(\
                    X, self._fit_X, reduce_func=reduce_func, metric=self.effective_metric_, n_jobs=self.n_jobs, **kwds,)
            if return_distance:
    ...
    def radius_neighbors_graph(self, X=None, radius=None, mode="connectivity", sort_results=False):
        ...
        mode : {'connectivity', 'distance'}, default='connectivity'
            Type of returned matrix: 'connectivity' will return the connectivity matrix with ones and zeros, 
            in 'distance' the edges are distances between points, 
            type of distance depends on the selected metric parameter in NearestNeighbors class.

        sort_results : bool, default=False
            If True, in each row of the result, the non-zero entries will be sorted by increasing distances. 
            If False, the non-zero entries may not be sorted. Only used with mode='distance'.
        ...
        -------
        A : sparse-matrix of shape (n_queries, n_samples_fit)
            `n_samples_fit` is the number of samples in the fitted data.  `A[i, j]` gives the weight of the edge connecting `i` to `j`.
