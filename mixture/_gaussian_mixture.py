...
def _check_weights(weights, n_components):
    ...
    if any(np.less(weights, 0.0)) or any(np.greater(weights, 1.0)):
        raise ValueError("The parameter 'weights' should be in the range [0, 1], but got max value %.5f, min value %.5f" 
            % (np.min(weights), np.max(weights)))

    # check normalization
    if not np.allclose(np.abs(1.0 - np.sum(weights)), 0.0):
        raise ValueError("The parameter 'weights' should be normalized, but got sum(weights) = %.5f"
            % np.sum(weights))
    return weights
...
def _estimate_gaussian_covariances_full(resp, X, nk, means, reg_covar):
    ...
    Returns
    -------
    covariances : array, shape (n_components, n_features, n_features), The covariance matrix of the current components.
    """
...
def _estimate_gaussian_covariances_tied(resp, X, nk, means, reg_covar):
    ...
    Returns
    -------
    covariance : array, shape (n_features, n_features), The tied covariance matrix of the components.
    """
    avg_X2 = np.dot(X.T, X)
...
def _estimate_gaussian_covariances_spherical(resp, X, nk, means, reg_covar):
    ...
    Returns
    -------
    variances : array, shape (n_components,), The variance values of each components.
    """
...
def _estimate_gaussian_parameters(X, resp, reg_covar, covariance_type):
    ...
    covariances : array-like
        The covariance matrix of the current components. The shape depends of the covariance_type.
    """
...
def _compute_precision_cholesky(covariances, covariance_type):
    ...
    precisions_cholesky : array-like
        The cholesky decomposition of sample precisions of the current components. The shape depends of the covariance_type.
    """
    estimate_precision_error_message = (
        "Fitting the mixture model failed because some components have "
        "ill-defined empirical covariance (for instance caused by singleton or collapsed samples). "
...
def _compute_precision_cholesky_from_precisions(precisions, covariance_type):
    ...
    As implemented in :func:`_compute_precision_cholesky`, the `precisions_cholesky_` 
    is an upper-triangular matrix for each Gaussian component, 
    which can be expressed as the $UU^T$ factorization of the precision matrix for each Gaussian component, 
    where $U$ is an upper-triangular matrix.

    In order to use the Cholesky decomposition to get $UU^T$, the precision matrix $\Lambda$ 
    needs to be permutated such that its rows and columns are reversed, 
    which can be done by applying a similarity transformation with an exchange matrix $J$,
    where the 1 elements reside on the anti-diagonal and all other elements are 0. 
    In particular, the Cholesky decomposition of the transformed precision matrix is $J\Lambda J=LL^T$, 
    ...
    ----------
    precisions : array-like
        The precision matrix of the current components.  The shape depends on the covariance_type.
    ...
    -------
    precisions_cholesky : array-like
        The cholesky decomposition of sample precisions of the current components. The shape depends on the covariance_type.
    """
    if covariance_type == "full":
...
class GaussianMixture(BaseMixture):
    ...
    precisions_ : array-like
        ...
        A covariance matrix is symmetric positive definite so the mixture of Gaussian 
        can be equivalently parameterized by the precision matrices. 
        Storing the precision matrices instead of the covariance matrices makes it 
        more efficient to compute the log-likelihood of new samples at test time.
        The shape depends on `covariance_type`::
    ...
    def __init__(self, n_components=1, *, covariance_type="full", tol=1e-3, reg_covar=1e-6, max_iter=100, \
        ...
        super().__init__(n_components=n_components, tol=tol, reg_covar=reg_covar, max_iter=max_iter, n_init=n_init, \
            init_params=init_params, random_state=random_state, warm_start=warm_start, verbose=verbose,             \
    ...
    def _initialize_parameters(self, X, random_state):
        # If all the initial parameters are all provided, then there is no need to run the initialization.
    ...
    def bic(self, X):
        ...
        You can refer to this :ref:`mathematical section <aic_bic>` for more details regarding the formulation of the BIC used.
    ...
    def aic(self, X):
        ...
        You can refer to this :ref:`mathematical section <aic_bic>` for more details regarding the formulation of the AIC used.
