...
from ._gaussian_mixture import (_check_precision_matrix, _check_precision_positivity, _compute_log_det_cholesky, \
    _compute_precision_cholesky, _estimate_gaussian_parameters, _estimate_log_gaussian_prob)
...
def _log_dirichlet_norm(dirichlet_concentration):
    ...
    Returns
    -------
    log_dirichlet_norm : float, The log normalization of the Dirichlet distribution.
    """
...
def _log_wishart_norm(degrees_of_freedom, log_det_precisions_chol, n_features):
    ...
    Return
    ------
    log_wishart_norm : array-like of shape (n_components,), The log normalization of the Wishart distribution.
    """
...
class BayesianGaussianMixture(BaseMixture):
    ...
    This class implements two types of prior for the weights distribution: a finite mixture model with Dirichlet distribution 
    and an infinite mixture model with the Dirichlet Process. 
    In practice Dirichlet Process inference algorithm is approximated and uses a truncated distribution 
    with a fixed maximum number of components (called the Stick-breaking representation).
    ...
    n_components : int, default=1
        ...
        the model can decide to not use all the components by setting some component `weights_` to values very close to zero. 
    ...
    init_params : {'kmeans', 'k-means++', 'random', 'random_from_data'}, default='kmeans'
        ...
        .. versionchanged:: v1.1
            `init_params` now accepts 'random_from_data' and 'k-means++' as initialization methods.
    ...
    mean_precision_prior : float or None, default=None
        ...
        Larger values concentrate the cluster means around `mean_prior`.  The value of the parameter must be greater than 0.
    ...
    warm_start : bool, default=False
        ...
        This can speed up convergence when fit is called several times on similar problems.  See :term:`the Glossary <warm_start>`.
    ...
    precisions_ : array-like
        ...
        A covariance matrix is symmetric positive definite so the mixture of Gaussian can be equivalently parameterized by the precision matrices.
        Storing the precision matrices instead of the covariance matrices makes it more efficient to compute the log-likelihood of 
        new samples at test time. The shape depends on ``covariance_type``::
    ...
    precisions_cholesky_ : array-like
        ...
        A precision matrix is the inverse of a covariance matrix.  
        A covariance matrix is symmetric positive definite 
        so the mixture of Gaussian can be equivalently parameterized by the precision matrices.
        Storing the precision matrices instead of the covariance matrices makes it more efficient to 
        compute the log-likelihood of new samples at test time. 
        The shape depends on ``covariance_type``::
    ...
    weight_concentration_prior_ : tuple or float
        ...
        The higher concentration puts more mass in the center and will lead to more components being active, 
        while a lower concentration parameter will lead to more mass at the edge of the simplex.
    ...
    mean_precision_prior_ : float
        ...
        If mean_precision_prior is set to None, `mean_precision_prior_` is set to 1.
    ...
    .. [3] `Blei, David M. and Michael I. Jordan. (2006). "Variational inference for Dirichlet process mixtures". 
       Bayesian analysis 1.1 <https://www.cs.princeton.edu/courses/archive/fall11/cos597C/reading/BleiJordan2005.pdf>`_
    ...
    _parameter_constraints: dict = {
        **BaseMixture._parameter_constraints,
        ...
        "weight_concentration_prior": [None, Interval(Real, 0.0, None, closed="neither") ],
        ...
        "covariance_prior": [None, "array-like", Interval(Real, 0.0, None, closed="neither") ],
    }

    def __init__(self, *, n_components=1, covariance_type="full", tol=1e-3, reg_covar=1e-6, max_iter=100, \
        ...
        degrees_of_freedom_prior=None, covariance_prior=None, random_state=None, warm_start=False, verbose=0, \
        verbose_interval=10):
        super().__init__(n_components=n_components, tol=tol, reg_covar=reg_covar, max_iter=max_iter,  \
            n_init=n_init, init_params=init_params, random_state=random_state, warm_start=warm_start, \
    ...
    def _check_precision_parameters(self, X):
        ...
        if self.degrees_of_freedom_prior is None:
            ...
            raise ValueError("The parameter 'degrees_of_freedom_prior' should be greater than %d, but got %.3f." 
    ...
    def _estimate_weights(self, nk):
        ...
        if self.weight_concentration_prior_type == "dirichlet_process":
            # For dirichlet process weight_concentration will be a tuple containing the two parameters of the beta distribution
    ...
    def _estimate_wishart_full(self, nk, xk, sk):
        ...
        # `degrees_of_freedom_k = degrees_of_freedom_0 + Nk` is the correct formula
        ...
        for k in range(self.n_components):
            ...
            self.covariances_[k] = \
                (self.covariance_prior_ + nk[k] * sk[k] + nk[k] * self.mean_precision_prior_ / self.mean_precision_[k] * np.outer(diff, diff))
    ...
    def _estimate_wishart_tied(self, nk, xk, sk):
        ...
        self.covariances_ = (self.covariance_prior_ + sk * nk.sum() / self.n_components
    ...
    def _estimate_wishart_spherical(self, nk, xk, sk):
        ...
        self.covariances_ = self.covariance_prior_ + \
            nk * (sk + self.mean_precision_prior_ / self.mean_precision_ * np.mean(np.square(diff), 1))
    ...
    def _estimate_log_prob(self, X):
        _, n_features = X.shape
        # We remove `n_features * np.log(self.degrees_of_freedom_)` because the precision matrix is normalized
        log_gauss = _estimate_log_gaussian_prob(X, self.means_, self.precisions_cholesky_, self.covariance_type) \
    ...
    def _compute_lower_bound(self, log_resp, log_prob_norm):
        ...
        The lower bound on the likelihood (of the training data with respect to the model) 
        is used to detect the convergence and has to increase at each iteration.
        ...
        lower_bound : float
        """
        # Contrary to the original formula, we have done some simplification and removed all the constant terms.
        (n_features,) = self.mean_prior_.shape

        # We removed `.5 * n_features * np.log(self.degrees_of_freedom_)` because the precision matrix is normalized.
        log_det_precisions_chol = _compute_log_det_cholesky(self.precisions_cholesky_, self.covariance_type, n_features) \
            - 0.5 * n_features * np.log(self.degrees_of_freedom_)
