...
class BaseMixture(DensityMixin, BaseEstimator, metaclass=ABCMeta):
    ...
    This abstract class specifies an interface for all mixture classes and provides basic common methods for mixture models.
    """
    ...
    def fit(self, X, y=None):
        ...
        The method fits the model ``n_init`` times and sets the parameters with which the model has the largest likelihood or lower bound. 
        Within each trial, the method iterates between E-step and M-step for ``max_iter`` times until the change of likelihood 
        or lower bound is less than ``tol``, otherwise, a ``ConvergenceWarning`` is raised.
        If ``warm_start`` is ``True``, then ``n_init`` is ignored and a single initialization is performed upon the first call. 
    ...
    def fit_predict(self, X, y=None):
        ...
        The method fits the model n_init times and sets the parameters with which the model has the largest likelihood or lower bound. 
        Within each trial, the method iterates between E-step and M-step for `max_iter` times until the change of likelihood 
        or lower bound is less than `tol`, otherwise, a :class:`~sklearn.exceptions.ConvergenceWarning` is raised. 
        ...
        if X.shape[0] < self.n_components:
            raise ValueError("Expected n_samples >= n_components but got n_components = {self.n_components}, n_samples = {X.shape[0]}")
        ...
        # the user is assumed to have used 0-iters initialization to get the initial means.
        if not self.converged_ and self.max_iter > 0:
            warnings.warn("Initialization %d did not converge. Try different init parameters, or increase max_iter, "
                "tol or check for degenerate data." % (init + 1), ConvergenceWarning)
        ...
        # fit_predict(X) are always consistent with fit(X).predict(X) for any value of max_iter and tol (and any random_state).
    ...
    def sample(self, n_samples=1):
        ...
        if self.covariance_type == "full":
            ...
            X = np.vstack([rng.multivariate_normal(mean, self.covariances_, int(sample))
                for (mean, sample) in zip(self.means_, n_samples_comp)])
        else:
            X = np.vstack([mean + rng.standard_normal(size=(sample, n_features)) * np.sqrt(covariance)
                for (mean, covariance, sample) in zip(self.means_, self.covariances_, n_samples_comp)])
    ...
    def _estimate_log_prob_resp(self, X):
        ...
        Compute the log probabilities, weighted log probabilities per component and responsibilities for each sample in X 
        with respect to the current state of the model.
