...
if sp_version >= parse_version("1.7"):
    # Starting in scipy 1.7 pinv2 was deprecated in favor of pinv. pinv now uses the svd to compute the pseudo-inverse.
...
def _pinv2_old(a):
    # Used previous scipy pinv2 that was updated in: https://github.com/scipy/scipy/pull/10067
    # We can not set `cond` or `rcond` for pinv2 in scipy >= 1.3 to keep the same behavior of pinv2 for scipy < 1.3, 
    # because the condition used to determine the rank is dependent on the output of svd.
...
def _get_first_singular_vectors_power_method(X, Y, mode="A", max_iter=500, tol=1e-06, norm_y_weights=False):
    ...
    With norm_y_weights to True and in mode A, this corresponds to the algorithm section 11.3 of the Wegelin's review, 
    except this starts at the "update saliences" part.
    """
    ...
    if mode == "B":
        # Precompute pseudo inverse matrices Basically: X_pinv = (X.T X)^-1 X.T
...
def _svd_flip_1d(u, v):
    ...
    # svd_flip would force us to convert to 2d array and would also return 2d arrays. We don't want that.
...
class _PLS(ClassNamePrefixFeaturesOutMixin, TransformerMixin, RegressorMixin, MultiOutputMixin, BaseEstimator, metaclass=ABCMeta):
    ...
    Main ref: Wegelin, a survey of Partial Least Squares (PLS) methods, with emphasis on the two-block case
    ...
    @abstractmethod
    def __init__(self, n_components=2, *, scale=True, deflation_mode="regression", mode="A", algorithm="nipals", \
    ...
    def fit(self, X, Y):
        ...
        # With PLSRegression n_components is bounded by the rank of (X.T X) see Wegelin page 25. With CCA and PLSCanonical, 
        # n_components is bounded by the rank of X and the rank of Y: see Wegelin page 12
        ...
        # This whole thing corresponds to the algorithm in section 4.1 of the review from Wegelin. 
        # See above for a notation mapping from code to paper.
        Y_eps = np.finfo(Yk.dtype).eps
        for k in range(n_components):
            # Find first left and right singular vectors of the X.T.dot(Y) cross-covariance matrix.
            if self.algorithm == "nipals":
                ...
                try:
                    (x_weights, y_weights, n_iter_) = _get_first_singular_vectors_power_method( \
                        Xk, Yk, mode=self.mode, max_iter=self.max_iter, tol=self.tol, norm_y_weights=norm_y_weights)
                except StopIteration as e:
                    if str(e) != "Y residual is constant": raise
        ...
        # Xi . Gamma.T is a sum of n_components rank-1 matrices. X_(R+1) is whatever is left to fully reconstruct X, 
        # and can be 0 if X is of rank n_components. Similarly, Y was approximated as Omega . Delta.T + Y_(R+1)
    ...
    def fit_transform(self, X, y=None):
        ...
        Returns
        -------
        self : ndarray of shape (n_samples, n_components), Return `x_scores` if `Y` is not given, `(x_scores, y_scores)` otherwise.
        """
...
class PLSRegression(_PLS):
    ...
    copy : bool, default=True
        Whether to copy `X` and `Y` in :term:`fit` before applying centering, and potentially scaling. 
    ...
    def __init__(self, n_components=2, *, scale=True, max_iter=500, tol=1e-06, copy=True):
        super().__init__(n_components=n_components, scale=scale, deflation_mode="regression", mode="A", \
            algorithm="nipals", max_iter=max_iter, tol=tol, copy=copy)
...
class PLSCanonical(_PLS):
    ...
    algorithm : {'nipals', 'svd'}, default='nipals'
        The algorithm used to estimate the first singular vectors of the cross-covariance matrix. 
    ...
    # Results are equal or collinear with the function ``pls(..., mode = "canonical")`` of the "mixOmics" package. 
    # The difference relies in the fact that mixOmics implementation does not exactly implement the Wold algorithm since it does not normalize
    # y_weights to one.

    def __init__(self, n_components=2, *, scale=True, algorithm="nipals", max_iter=500, tol=1e-06, copy=True):
        super().__init__(n_components=n_components, scale=scale, deflation_mode="canonical", mode="A", algorithm=algorithm, \
...
class CCA(_PLS):
    ...
    copy : bool, default=True
        ...
        and potentially scaling. If False, these operations will be done inplace, modifying both arrays.
    ...
    def __init__(self, n_components=2, *, scale=True, max_iter=500, tol=1e-06, copy=True):
        super().__init__(n_components=n_components, scale=scale, deflation_mode="canonical", mode="B", \
            algorithm="nipals", max_iter=max_iter, tol=tol, copy=copy)
...
class PLSSVD(ClassNamePrefixFeaturesOutMixin, TransformerMixin, BaseEstimator):
    ...
    while the targets are projected on the right singular vectors.
    ...
    ----------
    x_weights_ : ndarray of shape (n_features, n_components)
        The left singular vectors of the SVD of the cross-covariance matrix.  Used to project `X` in :meth:`transform`.

    y_weights_ : ndarray of (n_targets, n_components)
        The right singular vectors of the SVD of the cross-covariance matrix.  Used to project `X` in :meth:`transform`.
    ...
    def fit(self, X, Y):
        ...
        # This matrix rank is at most min(n_samples, n_features, n_targets) so n_components cannot be bigger than that.
        n_components = self.n_components
