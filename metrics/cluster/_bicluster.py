...
def _pairwise_similarity(a, b, similarity):
    ...
    result[i, j] is the Jaccard coefficient of a's bicluster i and b's bicluster j.
    """
...
def consensus_score(a, b, *, similarity="jaccard"):
    ...
    Similarity between individual biclusters is computed. Then the best matching between sets is found using the Hungarian algorithm.
    The final score is the sum of similarities divided by the size of the larger set.
    ...
    similarity : 'jaccard' or callable, default='jaccard'
        May be the string "jaccard" to use the Jaccard coefficient, or any function that takes four arguments, 
    ...
    * Hochreiter, Bodenhofer, et. al., 2010. `FABIA: factor analysis for bicluster acquisition 
    ...
    """
    if similarity == "jaccard": similarity = _jaccard
    matrix                   = _pairwise_similarity(a, b, similarity)
    row_indices, col_indices = linear_sum_assignment(1.0 - matrix)
    return matrix[row_indices, col_indices].sum() / max(len(a[0]), len(b[0]))
