...
def check_clusterings(labels_true, labels_pred):
    ...
    if "continuous" in (type_pred, type_label):
        msg = ("Clustering metrics expects discrete values but received {type_label} values for label, and {type_pred} values for target")
...
def _generalized_average(U, V, average_method):
    ...
    if   average_method == "min":        return min(U, V)
    elif average_method == "geometric":  return np.sqrt(U * V)
    elif average_method == "arithmetic": return np.mean([U, V])
    elif average_method == "max":        return max(U, V)
    else: raise ValueError("'average_method' must be 'min', 'geometric', 'arithmetic', or 'max'")


@validate_params(
{
    "labels_true": ["array-like", None], "labels_pred": ["array-like", None], "eps": [Interval(Real, 0, None, closed="left"), None], 
    "sparse": ["boolean"], "dtype": "no_validation"}, prefer_skip_nested_validation=True
)
def contingency_matrix(labels_true, labels_pred, *, eps=None, sparse=False, dtype=np.int64):
    """Build a contingency(意外事件,不测事件) matrix describing the relationship between labels.
    ...
    eps : float, default=None
        If a float, that value is added to all values in the contingency matrix. This helps to stop NaN propagation.
    ...
    contingency : {array-like, sparse}, shape=[n_classes_true, n_classes_pred]
        Matrix :math:`C` such that :math:`C_{i, j}` is the number of samples in true class :math:`i` and in predicted class :math:`j`. 
        If ``eps is None``, the dtype of this array will be integer unless set otherwise with the ``dtype`` argument. 
        If ``eps`` is given, the dtype will be float. Will be a ``sklearn.sparse.csr_matrix`` if ``sparse=True``.
    """
    ...
    # Using coo_matrix to accelerate simple histogram calculation, i.e. bins are consecutive integers Currently, 
    # coo_matrix is faster than histogram2d for simple cases
...
def pair_confusion_matrix(labels_true, labels_pred):
    ...
    The pair confusion matrix :math:`C` computes a 2 by 2 similarity matrix between two clusterings by considering all pairs of samples 
    and counting pairs that are assigned into the same or into different clusters under the true and predicted clusterings.
    ...
    :math:`C_{00}`, false negatives is :math:`C_{10}`, true positives is :math:`C_{11}` and false positives is :math:`C_{01}`.
    ...
    References
    ----------
    .. [1] :doi:`Hubert, L., Arabie, P. "Comparing partitions." Journal of Classification 2, 193–218 (1985).  <10.1007/BF01908075>`
    ...
    Labelings that assign all classes members to the same clusters are complete but may be not always pure, hence penalized, 
    and have some off-diagonal non-zero entries:
...
def rand_score(labels_true, labels_pred):
    ...
    that are assigned in the same or different clusters in the predicted and true clusterings [1]_ [2]_.
    ...
    ----------
    .. [1] :doi:`Hubert, L., Arabie, P. "Comparing partitions." Journal of Classification 2, 193–218 (1985).  <10.1007/BF01908075>`.
    .. [2] `Wikipedia: Simple Matching Coefficient <https://en.wikipedia.org/wiki/Simple_matching_coefficient>`_
    ...
    if numerator == denominator or denominator == 0:
        ...
        # or trivial clustering where each document is assigned a unique cluster. These are perfect matches hence return 1.0.
        return 1.0
...
def adjusted_rand_score(labels_true, labels_pred):
    ...
    .. [Chacon] :doi:`Minimum adjusted Rand index for two clusterings of a given size, 2022, J. E. Chacón and A. I. Rastrojo <10.1007/s11634-022-00491-w>`
...
def homogeneity_completeness_v_measure(labels_true, labels_pred, *, beta=1.0):
    ...
    Those metrics are based on normalized conditional entropy measures of the clustering labeling to evaluate given the knowledge of 
    a Ground Truth class labels of the same samples.
    ...
    V-Measure is identical to :func:`normalized_mutual_info_score` with the arithmetic averaging method.
...
def v_measure_score(labels_true, labels_pred, *, beta=1.0):
    ...
    beta : float, default=1.0
        Ratio of weight attributed to ``homogeneity`` vs ``completeness``.  If ``beta`` is greater than 1, ``completeness`` is weighted more
        strongly in the calculation. If ``beta`` is less than 1, ``homogeneity`` is weighted more strongly.
...
def mutual_info_score(labels_true, labels_pred, *, contingency=None):
    ...
    Where :math:`|U_i|` is the number of the samples in cluster :math:`U_i` and :math:`|V_j|` is the number of the samples in cluster :math:`V_j`, 
    the Mutual Information between clusterings :math:`U` and :math:`V` is given as:
    ...
    ----------
    labels_true : array-like of shape (n_samples,), dtype=integral
        A clustering of the data into disjoint subsets, called :math:`U` in the above formula.

    labels_pred : array-like of shape (n_samples,), dtype=integral
        A clustering of the data into disjoint subsets, called :math:`V` in the above formula.

    contingency : {array-like, sparse matrix} of shape (n_classes_true, n_classes_pred), default=None
        A contingency matrix given by the :func:`~sklearn.metrics.cluster.contingency_matrix` function. 
        If value is ``None``, it will be computed, otherwise the given value is used, with ``labels_true`` and ``labels_pred`` ignored.
...
def adjusted_mutual_info_score(labels_true, labels_pred, *, average_method="arithmetic"):
    ...
    This metric is furthermore symmetric: switching :math:`U` (``label_true``) with :math:`V` (``labels_pred``) will return the same score value. 
    This can be useful to measure the agreement of two independent label assignments strategies on the same dataset when the real ground truth is not known.
    ...
    average_method : {'min', 'geometric', 'arithmetic', 'max'}, default='arithmetic' How to compute the normalizer in the denominator.
    ...
    # Special limit cases: no clustering since the data is not split.
    # It corresponds to both labellings having zero entropy. This is a perfect match hence return 1.0.
    ...
    # normalizer should always be >= emi, but because of floating-point representation, sometimes emi is slightly larger. 
    # Correct this by preserving the sign.
    if denominator < 0:
...
def normalized_mutual_info_score(labels_true, labels_pred, *, average_method="arithmetic"):
    ...
    Normalized Mutual Information (NMI) is a normalization of the Mutual Information (MI) score to scale the results between 0 
    (no mutual information) and 1 (perfect correlation). In this function, mutual information is normalized by some generalized mean of 
    ``H(labels_true)`` and ``H(labels_pred))``, defined by the `average_method`.
    ...
    average_method : {'min', 'geometric', 'arithmetic', 'max'}, default='arithmetic'
        ...
        .. versionadded:: 0.20
        .. versionchanged:: 0.22
           The default value of ``average_method`` changed from 'geometric' to 'arithmetic'.
    ...
    # At this point mi = 0 can't be a perfect match (the special case of a single cluster has been dealt with before).
    # Hence, if mi = 0, the nmi must be 0 whatever the normalization.
    if mi == 0:
        return 0.0
...
def fowlkes_mallows_score(labels_true, labels_pred, *, sparse=False):
    ...
    in both ``labels_true`` and ``labels_pred``), ``FP`` is the number of **False Positive** 
    (i.e. the number of pair of points that belongs in the same clusters in ``labels_true`` and not in ``labels_pred``) 
    and ``FN`` is the number of **False Negative** (i.e the number of pair of points that belongs in the same clusters in ``labels_pred`` and not in ``labels_True``).
...
def entropy(labels):
    ...
    The logarithm used is the natural logarithm (base-e).
    """
    if len(labels) == 0: return 1.0
    label_idx = np.unique(labels, return_inverse=True)[1]
    pi = np.bincount(label_idx).astype(np.float64)
    pi = pi[pi > 0]

    # single cluster => zero entropy
    if pi.size == 1: return 0.0
