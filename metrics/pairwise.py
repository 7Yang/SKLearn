...
def check_pairwise_arrays(X, Y, *, precomputed=False, dtype=None, accept_sparse="csr", force_all_finite=True, copy=False):
    ...
    All distance metrics should use this function first to assert that the given parameters are correct and safe to use.

    Specifically, this function first ensures that both X and Y are arrays, then checks that they are at least two dimensional 
    while ensuring that their elements are floats (or dtype if provided). 
    Finally, the function checks that the size of the second dimension of the two arrays is equal, 
    ...
    precomputed : bool, default=False
        True if X is to be treated as precomputed distances to the samples in Y.

    dtype : str, type, list of type, default=None
        Data type required for X and Y. If None, the dtype will be an appropriate float type selected by _return_float_dtype.
        .. versionadded:: 0.18
    ...
    safe_Y : {array-like, sparse matrix} of shape (n_samples_Y, n_features)
        An array equal to Y if Y was not None, guaranteed to be a numpy array.  If Y was None, safe_Y will be a pointer to X.
    """
    ...
    if precomputed:
        if X.shape[1] != Y.shape[0]:
            raise ValueError("Precomputed metric requires shape (n_queries, n_indexed). Got (%d, %d) for %d indexed." 
...
def check_paired_arrays(X, Y):
    ...
    All paired distance metrics should use this function first to assert that the given parameters are correct and safe to use.

    Specifically, this function first ensures that both X and Y are arrays, 
    then checks that they are at least two dimensional while ensuring that their elements are floats. 
    Finally, the function checks that the size of the dimensions of the two arrays are equal.
...
def euclidean_distances(X, Y=None, *, Y_norm_squared=None, squared=False, X_norm_squared=None):
    ...
    First, it is computationally efficient when dealing with sparse data. Second, if one argument varies but the other remains unchanged, 
    then `dot(x, x)` and/or `dot(y, y)` can be pre-computed.

    However, this is not the most precise way of doing this computation, because this equation potentially suffers from "catastrophic cancellation".
    Also, the distance matrix returned by this function may not be exactly symmetric as required by, e.g., ``scipy.spatial.distance`` functions.
...
def _euclidean_distances(X, Y, X_norm_squared=None, Y_norm_squared=None, squared=False):
    ...
    If norms are passed as float32, they are unused. If arrays are passed as float32, norms needs to be recomputed on upcast chunks.
    ...
    if X.dtype == np.float32:
        # To minimize precision issues with float32, we compute the distance matrix on chunks of X and Y upcast to float64
...
def nan_euclidean_distances(X, Y=None, *, squared=False, missing_values=np.nan, copy=True):
    ...
    When calculating the distance between a pair of samples, this formulation ignores feature coordinates 
    with a missing value in either sample and scales up the weight of the remaining coordinates:
    ...
    References
    ----------
    * John K. Dixon, "Pattern Recognition with Partly Missing Data", IEEE Transactions on Systems, Man, and Cybernetics, Volume: 9, Issue:
      10, pp. 617 - 621, Oct. 1979.  http://ieeexplore.ieee.org/abstract/document/4310090/
...
def _euclidean_distances_upcast(X, XX=None, Y=None, YY=None, batch_size=None):
    ...
    Assumes X and Y have float32 dtype. Assumes XX and YY have float64 dtype or are None.
    X and Y are upcast to float64 by chunks, which size is chosen to limit memory increase by approximately 10% (at least 10MiB).
    """
    ...
    if batch_size is None:
        ...
        maxmem = max(((x_density * n_samples_X + y_density * n_samples_Y) * n_features + (x_density * n_samples_X * y_density * n_samples_Y)) / 10,
            10 * 2**17)
        ...
        # Hence x² + (xd+yd)kx = M, where x=batch_size, k=n_features, M=maxmem, xd=x_density and yd=y_density
    ...
    for i, x_slice in enumerate(x_batches):
        ...
        for j, y_slice in enumerate(y_batches):
            if X is Y and j < i:
                # when X is Y the distance matrix is symmetric so we only need to compute half of it.
...
def _argmin_min_reduce(dist, start):
    # `start` is specified in the signature but not used. 
    # This is because the higher order `pairwise_distances_chunked` function needs reduction functions that are
...
def _argmin_reduce(dist, start):
    # `start` is specified in the signature but not used. 
    # This is because the higher order `pairwise_distances_chunked` function needs reduction functions 
    # that are passed as argument to have a two arguments signature.
    return dist.argmin(axis=1)


_VALID_METRICS = [
    "euclidean", "l2", "l1", "manhattan", "cityblock", "braycurtis", "canberra", "chebyshev",
    "correlation", "cosine", "dice", "hamming", "jaccard", "mahalanobis", "matching", "minkowski",
...
@validate_params(
    {
        ...
        "metric": [StrOptions(set(_VALID_METRICS).union(ArgKmin.valid_metrics())), callable ],
...
def pairwise_distances_argmin_min(X, Y, *, axis=1, metric="euclidean", metric_kwargs=None):
    ...
    pairwise_distances_argmin : Same as `pairwise_distances_argmin_min` but only returns the argmins.
    """
    X, Y = check_pairwise_arrays(X, Y)
    ...
    if ArgKmin.is_usable_for(X, Y, metric):
        ...
        # Joblib-based backend, which is used when user-defined callable are passed for metric.
        ...
        # Turn off check for finiteness because this is costly and because arrays have already been validated.
...
def pairwise_distances_argmin(X, Y, *, axis=1, metric="euclidean", metric_kwargs=None):
    ...
    metric : str or callable, default="euclidean"
        ...
        .. note:: `'kulsinski'` is deprecated from SciPy 1.9 and will be removed in SciPy 1.11.
        .. note:: `'matching'` has been removed in SciPy 1.9 (use `'hamming'` instead).
    ...
    --------
    pairwise_distances : Distances between every pair of samples of X and Y.
    pairwise_distances_argmin_min : Same as `pairwise_distances_argmin` but also returns the distances.
    """
    if metric_kwargs is None:
        metric_kwargs = {}
    ...
    if ArgKmin.is_usable_for(X, Y, metric):
        # This is an adaptor for one "sqeuclidean" specification. For this backend, we can directly use "sqeuclidean".
        if metric_kwargs.get("squared", False) and metric == "euclidean":
            metric = "sqeuclidean"
            metric_kwargs = {}
        ...
        with config_context(assume_finite=True):
            indices = np.concatenate(
                # This returns a np.ndarray generator whose arrays we need to flatten into one.
                list(pairwise_distances_chunked(X, Y, reduce_func=_argmin_reduce, metric=metric, **metric_kwargs))
            )
...
def haversine_distances(X, Y=None):
    ...
    Notes
    -----
    As the Earth is nearly spherical, the haversine formula provides a good approximation of the distance 
    between two points of the Earth surface, with a less than 1% error on average.
...
def manhattan_distances(X, Y=None, *, sum_over_features="deprecated"):
    ...
    sum_over_features : bool, default=True
        ...
        .. deprecated:: 1.2
            ``sum_over_features`` was deprecated in version 1.2 and will be removed in 1.4.

    Returns
    -------
    D : ndarray of shape (n_samples_X * n_samples_Y, n_features) or (n_samples_X, n_samples_Y)
        If sum_over_features is False shape is (n_samples_X * n_samples_Y, n_features) and D contains the
        componentwise L1 pairwise-distances (ie. absolute difference), else shape is (n_samples_X, n_samples_Y) 
        and D contains the pairwise L1 distances.

    Notes
    -----
    When X and/or Y are CSR sparse matrices and they are not already in canonical format,
    this function modifies them in-place to make them canonical.
    ...
    if sum_over_features != "deprecated":
        warnings.warn(("`sum_over_features` is deprecated in version 1.2 and will be removed in version 1.4."), FutureWarning)
    else:
...
def paired_cosine_distances(X, Y):
    ...
    distances : ndarray of shape (n_samples,)
        Returns the distances between the row vectors of `X` and the row vectors of `Y`, 
        where `distances[i]` is the distance between `X[i]` and `Y[i]`.
...
def paired_distances(X, Y, *, metric="euclidean", **kwds):
    ...
    metric : str or callable, default="euclidean"
        ...
        If metric is a string, it must be one of the options specified in PAIRED_DISTANCES, including "euclidean", "manhattan", or "cosine".
    ...
    **kwds : dict
    ...
    See Also
    --------
    sklearn.metrics.pairwise_distances : Computes the distance between every pair of samples.
...
def linear_kernel(X, Y=None, dense_output=True):
    ...
    dense_output : bool, default=True
        ...
        If ``False``, the output is sparse if both input arrays are sparse.

        .. versionadded:: 0.20

    Returns
    -------
    Gram matrix : ndarray of shape (n_samples_X, n_samples_Y) The Gram matrix of the linear kernel, i.e. `X @ Y.T`.
    """
    X, Y = check_pairwise_arrays(X, Y)
...
def polynomial_kernel(X, Y=None, degree=3, gamma=None, coef0=1):
    ...
    Returns
    -------
    Gram matrix : ndarray of shape (n_samples_X, n_samples_Y), The polynomial kernel.
    """
    X, Y = check_pairwise_arrays(X, Y)
    if gamma is None:
        gamma = 1.0 / X.shape[1]
...
def sigmoid_kernel(X, Y=None, gamma=None, coef0=1):
    ...
    Returns
    -------
    Gram matrix : ndarray of shape (n_samples_X, n_samples_Y), Sigmoid kernel between two arrays.
    """
    X, Y = check_pairwise_arrays(X, Y)
    if gamma is None:
        gamma = 1.0 / X.shape[1]
...
def rbf_kernel(X, Y=None, gamma=None):
    ...
    Returns
    -------
    kernel_matrix : ndarray of shape (n_samples_X, n_samples_Y), The RBF kernel.
    """
    X, Y = check_pairwise_arrays(X, Y)
    if gamma is None:
        gamma = 1.0 / X.shape[1]
...
def laplacian_kernel(X, Y=None, gamma=None):
    ...
    Returns
    -------
    kernel_matrix : ndarray of shape (n_samples_X, n_samples_Y), The kernel matrix.
    """
    X, Y = check_pairwise_arrays(X, Y)
    if gamma is None:
        gamma = 1.0 / X.shape[1]
...
def cosine_similarity(X, Y=None, dense_output=True):
    ...
    Returns
    -------
    kernel matrix : ndarray of shape (n_samples_X, n_samples_Y), Returns the cosine similarity between samples in X and Y.
    """
...
def additive_chi2_kernel(X, Y=None):
    ...
    The chi-squared kernel is computed between each pair of rows in X and Y.  
    X and Y have to be non-negative. This kernel is most commonly applied to histograms.
    ...
    * Zhang, J. and Marszalek, M. and Lazebnik, S. and Schmid, C. 
    ...
    International Journal of Computer Vision 2007 https://hal.archives-ouvertes.fr/hal-00171412/document
    """
...
PAIRWISE_DISTANCE_FUNCTIONS = {
    # If updating this dictionary, update the doc in both distance_metrics() and also in pairwise_distances()!
...
def distance_metrics():
    ...
    It exists to allow for a description of the mapping for each of the valid strings.
...
def _check_chunk_size(reduced, chunk_size):
    ...
    if any(_num_samples(r) != chunk_size for r in reduced):
        ...
        raise ValueError("reduce_func returned object of length %s. Expected same length as input: %d." 
...
def pairwise_distances_chunked(X, Y=None, *, reduce_func=None, metric="euclidean", n_jobs=None, working_memory=None, **kwds):
    ...
    In cases where not all of a pairwise distance matrix needs to be stored at once, 
    this is used to calculate pairwise distances in ``working_memory``-sized chunks.  
    ...
    X : {array-like, sparse matrix} of shape (n_samples_X, n_samples_X) or (n_samples_X, n_features)
        ...
        The shape the array should be (n_samples_X, n_samples_X) if metric='precomputed' and (n_samples_X, n_features) otherwise.
    ...
    reduce_func : callable, default=None
        ...
        ``reduce_func(D_chunk, start)`` is called repeatedly, where ``D_chunk`` is a contiguous vertical slice of the pairwise distance matrix, 
        starting at row ``start``.  It should return one of: None; an array, a list, or a sparse matrix of length ``D_chunk.shape[0]``; 
    ...
    metric : str or callable, default='euclidean'
        ...
        or a metric listed in pairwise.PAIRWISE_DISTANCE_FUNCTIONS.  If metric is "precomputed", X is assumed to be a distance matrix.
        Alternatively, if metric is a callable function, it is called on each pair of instances (rows) and the resulting value recorded.
        The callable should take two arrays from X as input and return a value indicating the distance between them.
    ...
    **kwds : optional keyword parameters
    ...
    if metric == "precomputed":
        ...
        #
        # Note:
        #  - this will get at least 1 row, even if 1 row of distances will exceed working_memory.
        #  - this does not account for any temporary memory usage while calculating distances (e.g. difference of vectors in manhattan distance.
    ...
    for sl in slices:
        ...
        if (X is Y or Y is None) and PAIRWISE_DISTANCE_FUNCTIONS.get(metric, None) is euclidean_distances:
            # zeroing diagonal, taking care of aliases of "euclidean", i.e. "l2"
...
def pairwise_distances(X, Y=None, metric="euclidean", *, n_jobs=None, force_all_finite=True, **kwds):
    ...
    If the input is a vector array, the distances are computed. If the input is a distances matrix, it is returned instead.
    ...
    Note that in the case of 'cityblock', 'cosine' and 'euclidean' (which are valid scipy.spatial.distance metrics), 
    the scikit-learn implementation will be used, which is faster and has support for sparse matrices (except for 'cityblock'). 
    For a verbose description of the metrics from scikit-learn, see :func:`sklearn.metrics.pairwise.distance_metrics` function.
    ...
    X : {array-like, sparse matrix} of shape (n_samples_X, n_samples_X) or (n_samples_X, n_features)
        Array of pairwise distances between samples, or a feature array.  
        The shape of the array should be (n_samples_X, n_samples_X) if metric == "precomputed" and (n_samples_X, n_features) otherwise.
    ...
    n_jobs : int, default=None
        The number of jobs to use for the computation. This works by breaking down the pairwise matrix into n_jobs even slices 
        and computing them in parallel.
    ...
    force_all_finite : bool or 'allow-nan', default=True
        ...
        Ignored for a metric listed in ``pairwise.PAIRWISE_DISTANCE_FUNCTIONS``. The possibilities are:
    ...
    **kwds : optional keyword parameters
    ...
    See Also
    --------
    pairwise_distances_chunked : Performs the same calculation as this function, but returns a generator of chunks of the distance matrix, 
        in order to limit memory usage.
    sklearn.metrics.pairwise.paired_distances : Computes the distances between corresponding elements of two arrays.
    """
    if metric == "precomputed":
...
PAIRWISE_BOOLEAN_FUNCTIONS = ["dice", "jaccard", "rogerstanimoto", "russellrao", "sokalmichener", "sokalsneath", "yule" ]
...
def kernel_metrics():
    ...
    This function simply returns the valid pairwise distance metrics.  
    It exists, however, to allow for a verbose description of the mapping for each of the valid strings.
    ...
    Returns
    -------
    kernel_metrics : dict, Returns valid metrics for pairwise_kernels.
    """
...
@validate_params(
    {
        "X": ["array-like", "sparse matrix"],
        "Y": ["array-like", "sparse matrix", None],
        "metric": [ StrOptions(set(PAIRWISE_KERNEL_FUNCTIONS) | {"precomputed"}), callable ],
...
def pairwise_kernels(X, Y=None, metric="linear", *, filter_params=False, n_jobs=None, **kwds):
    ...
    This method takes either a vector array or a kernel matrix, and returns a kernel matrix. 
    If the input is a vector array, the kernels are computed. If the input is a kernel matrix, it is returned instead.

    This method provides a safe way to take a kernel matrix as input, 
    while preserving compatibility with many other algorithms that take a vector array.
    ...
    metric : str or callable, default="linear"
        ...
        This means that callables from :mod:`sklearn.metrics.pairwise` are not allowed, as they operate on matrices, not single samples. 
