...
def _oas(X, *, assume_centered=False):
    ...
    [1] "Shrinkage algorithms for MMSE covariance estimation.", Chen, Y., Wiesel, A., Eldar, Y. C., & Hero, A. O.
        IEEE Transactions on Signal Processing, 58(10), 5016-5029, 2010.  https://arxiv.org/pdf/0907.4698.pdf
    """
    if len(X.shape) == 2 and X.shape[1] == 1:
        # for only one feature, the result is the same whatever the shrinkage
        if not assume_centered:
    ...
    # shrinkage = min(trace(S @ S.T) + trace(S)**2) / ((n + 1) (trace(S @ S.T) - trace(S)**2 / p), 1)
    # where n and p are n_samples and n_features, respectively (cf. Eq. 23 in [1]).
    # The factor 2 / p is omitted since it does not impact the value of the estimator for large p.
    ...
    # The factor 1 / p**2 will cancel out since it is in both the numerator and denominator
...
class ShrunkCovariance(EmpiricalCovariance):
    ...
    assume_centered : bool, default=False
        If True, data will not be centered before computation.  Useful when working with data whose mean is almost, but not exactly zero.
    ...
    def fit(self, X, y=None):
        ...
        # Not calling the parent object to fit, to avoid a potential matrix inversion when setting the precision
...
def ledoit_wolf_shrinkage(X, assume_centered=False, block_size=1000):
    ...
    # A non-blocked version of the computation is present in the tests in tests/test_covariance.py
    ...
    delta /= n_features
    # get final beta as the min between beta and delta We do this to prevent shrinking more than "1", which would invert the value of covariances
    beta = min(beta, delta)
...
class LedoitWolf(EmpiricalCovariance):
    ...
    Ledoit and M. Wolf's formula as described in "A Well-Conditioned Estimator for Large-Dimensional Covariance Matrices",
    ...
    block_size : int, default=1000
        Size of blocks into which the covariance matrix will be split during its Ledoit-Wolf estimation. 
...
def oas(X, *, assume_centered=False):
    ...
    The regularised covariance is:
        (1 - shrinkage) * cov + shrinkage * mu * np.identity(n_features),
        where mu = trace(cov) / n_features and shrinkage is given by the OAS formula (see [1]_).

    The shrinkage formulation implemented here differs from Eq. 23 in [1]_. 
    In the original article, formula (23) states that 2/p (p being the number of features) is multiplied by Trace(cov*cov) 
    in both the numerator and denominator, but this operation is omitted because for a large p, 
    the value of 2/p is so small that it doesn't affect the value of the estimator.

    References
    ----------
    .. [1] :arxiv:`"Shrinkage algorithms for MMSE covariance estimation.",
           Chen, Y., Wiesel, A., Eldar, Y. C., & Hero, A. O.  IEEE Transactions on Signal Processing, 58(10), 5016-5029, 2010. <0907.4698>`
    """
