...
def _rfe_single_fit(rfe, estimator, X, y, train, test, scorer):
    ...
    return rfe._fit(X_train, y_train, \
        lambda estimator, features: _score(estimator, X_test[:, features], y_test, scorer, score_params=None)).scores_


def _estimator_has(attr):
    """
    Check if we can delegate a method to the underlying estimator.
    First, we check the first fitted estimator if available, otherwise we check the unfitted estimator.
    """
...
class RFE(SelectorMixin, MetaEstimatorMixin, BaseEstimator):
    ...
    through any specific attribute or callable.
    ...
    n_features_to_select : int or float, default=None, The number of features to select. 
    ...
    .. [1] Guyon, I., Weston, J., Barnhill, S., & Vapnik, V., "Gene selection for cancer classification using support vector machines",
    ...
    _parameter_constraints: dict = {
        ...
        "n_features_to_select": [None, Interval(RealNotInt, 0, 1, closed="right"), Interval(Integral, 0, None, closed="neither")],
        "step": [ Interval(Integral, 0, None, closed="neither"), Interval(RealNotInt, 0, 1, closed="neither") ],
        "verbose": ["verbose"],
    ...
    def _fit(self, X, y, step_score=None, **fit_params):
        # Parameter step_score controls the calculation of self.scores_ step_score is not exposed to users
        # and is used when implementing RFECV self.scores_ will not be calculated when calling _fit through fit

        tags = self._get_tags()
        X, y = self._validate_data(X, y, accept_sparse="csc", ensure_min_features=2, \
            force_all_finite=not tags.get("allow_nan", True), multi_output=True)
    ...
    def score(self, X, y, **fit_params):
        ...
        **fit_params : dict
        ...
        Returns
        -------
        score : float, Score of the underlying base estimator computed with the selected features returned by `rfe.transform(X)` and `y`.
        """
        check_is_fitted(self)
    ...
    def decision_function(self, X):
        ...
        score : array, shape = [n_samples, n_classes] or [n_samples]
            The decision function of the input samples. The order of the classes corresponds to that in the attribute :term:`classes_`.
...
class RFECV(RFE):
    ...
    estimator : ``Estimator`` instance
        ...
        about feature importance either through a ``coef_`` attribute or through a ``feature_importances_`` attribute.
    ...
    min_features_to_select : int, default=1
        The minimum number of features to be selected. This number of features will always be scored, 
        even if the difference between the original feature count and ``min_features_to_select`` isn't divisible by ``step``.
    ...
    cv : int, cross-validation generator or an iterable, default=None
        ...
        If the estimator is a classifier or if ``y`` is neither binary nor multiclass, :class:`~sklearn.model_selection.KFold` is used.
    ...
    cv_results_ : dict of ndarrays
        ...
        split(k)_test_score : ndarray of shape (n_subsets_of_features,), The cross-validation scores across (k)th fold.
        mean_test_score     : ndarray of shape (n_subsets_of_features,), Mean of scores over the folds.
        std_test_score      : ndarray of shape (n_subsets_of_features,), Standard deviation of scores over the folds.
    ...
    Notes
    -----
    The size of all values in ``cv_results_`` is equal to ``ceil((n_features - min_features_to_select) / step) + 1``,
    ...
    def __init__(self, estimator, *, step=1, min_features_to_select=1, cv=None, scoring=None, \
        verbose=0, n_jobs=None, importance_getter="auto"):
        self.estimator         = estimator
        self.step              = step
        self.importance_getter = importance_getter
        self.cv                = cv
    ...
    def fit(self, X, y, groups=None):
        ...
        groups : array-like of shape (n_samples,) or None, default=None
            ...
            Only used in conjunction with a "Group" :term:`cv` instance (e.g., :class:`~sklearn.model_selection.GroupKFold`).
        ...
        """
        tags = self._get_tags()
        X, y = self._validate_data(X, y, accept_sparse="csr", ensure_min_features=2, force_all_finite=not tags.get("allow_nan", True), \
            multi_output=True)
        ...
        # Build an RFE object, which will evaluate and score each possible feature count, down to self.min_features_to_select
        rfe = RFE(estimator=self.estimator, n_features_to_select=self.min_features_to_select, importance_getter=self.importance_getter, \
            step=self.step, verbose=self.verbose)

        # Determine the number of subsets of features by fitting across the train folds 
        # and choosing the "features_to_select" parameter that gives te least averaged error across all folds.

        # Note that joblib raises a non-picklable error for bound methods even if n_jobs is set to 1 with the default multiprocessing backend.
        # This branching is done so that to make sure that user code that sets n_jobs to 1
        # and provides bound methods as scorers is not broken with the addition of n_jobs parameter in version 0.18.
