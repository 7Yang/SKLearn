...
def _calculate_threshold(estimator, importances, threshold):
    ...
    if threshold is None:
        ...
        is_elasticnet_l1_penalized = "ElasticNet" in est_name and (
            (hasattr(estimator, "l1_ratio_") and np.isclose(estimator.l1_ratio_, 1.0)) or 
...
def _estimator_has(attr):
    ...
    First, we check the fitted estimator if available, otherwise we check the unfitted estimator.
    """
...
class SelectFromModel(MetaEstimatorMixin, SelectorMixin, BaseEstimator):
    ...
    norm_order : non-zero int, inf, -inf, default=1
        Order of the norm used to filter the vectors of coefficients below ``threshold`` in the case 
    ...
    max_features_ : int
        Maximum number of features calculated during :term:`fit`. Only defined if the ``max_features`` is not `None`.
    ...
    _parameter_constraints: dict = {
        ...
        "prefit": ["boolean"],
        "norm_order": [Interval(Integral, None, -1, closed="right"), Interval(Integral, 1, None, closed="left"), Options(Real, {np.inf, -np.inf})],
    ...
    def _get_support_mask(self):
        ...
        if callable(max_features):
            # This branch is executed when `transform` is called directly 
            # and thus `max_features_` is not set and we fallback using `self.max_features` that is not validated
    ...
    def threshold_(self):
        ...
        scores = _get_feature_importances(estimator=self.estimator_, getter=self.importance_getter, \
