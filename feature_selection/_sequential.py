...
class SequentialFeatureSelector(SelectorMixin, MetaEstimatorMixin, BaseEstimator):
    ...
    This Sequential Feature Selector adds (forward selection) or removes (backward selection) features to 
    form a feature subset in a greedy fashion. 
    ...
    In the case of unsupervised learning, this Sequential Feature Selector looks only at the features (X), not the desired outputs (y).
    ...
    cv : int, cross-validation generator or an iterable, default=None
        ...
        :class:`~sklearn.model_selection.StratifiedKFold` is used. In all other cases, :class:`~sklearn.model_selection.KFold` is used. 
    ...
    RFECV : Recursive feature elimination based on importance weights, with automatic selection of the number of features.
    ...
    _parameter_constraints: dict = {
        ...
        "n_features_to_select": [StrOptions({"auto"}), Interval(RealNotInt, 0, 1, closed="right"), Interval(Integral, 0, None, closed="neither")],
    ...
    def fit(self, X, y=None):
        ...
        if self.n_features_to_select == "auto":
            if self.tol is not None:
                # With auto feature selection, `n_features_to_select_` will be updated to `support_.sum()` after features are selected.
        ...
        n_iterations = (self.n_features_to_select_ if self.n_features_to_select == "auto" or self.direction == "forward" \
    ...
    def _get_best_new_feature_score(self, estimator, X, y, cv, current_mask):
        ...
        # i.e. return the best new feature and its score to add (resp. remove) when doing forward selection (resp. backward selection).
        # Feature will be added if the current score and past score are greater than tol when n_feature is auto,
        scores = {}
