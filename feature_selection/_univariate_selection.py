...
# The following function is a rewriting of scipy.stats.f_oneway Contrary to the scipy.stats.f_oneway implementation 
# it does not copy the data while keeping the inputs unchanged.
def f_oneway(*args):
    """
    Perform a 1-way ANOVA(analysis of variance).

    The one-way ANOVA tests the null hypothesis(假设) that 2 or more groups have the same population mean. 
    ...
    *args : {array-like, sparse matrix}
    ...
    Returns
    -------
    f_statistic : float, The computed F-value of the test.
    p_value     : float, The associated p-value from the F-distribution.
    ...
    If these assumptions are not true for a given set of data, it may still be possible to use the Kruskal-Wallis H-test (`scipy.stats.kruskal`_) 
    ...
    References
    ----------
    .. [1] Lowry, Richard.  "Concepts and Applications of Inferential Statistics". Chapter 14.  http://vassarstats.net/textbook
...
def _chisquare(f_obs, f_exp):
    ...
    Version from https://github.com/scipy/scipy/pull/2525 with additional optimizations.
    """
...
def chi2(X, y):
    ...
    so using this function "weeds out" the features that are the most likely to be independent of class and therefore irrelevant for classification.
    ...
    # XXX: we might want to do some of the following in logspace instead for numerical stability.
    # Converting X to float allows getting better performance for the safe_sparse_dot call made below.
    X = check_array(X, accept_sparse="csr", dtype=(np.float64, np.float32))
    if np.any((X.data if issparse(X) else X) < 0):
        raise ValueError("Input X must be non-negative.")

    # Use a sparse representation for Y by default to reduce memory usage when y has many unique classes.
    ...
    if issparse(observed):
        ...
        # XXX: could _chisquare be reimplement to accept sparse matrices for cases 
        # where both n_classes and n_features are large (and X is sparse)?
...
def r_regression(X, y, *, center=True, force_finite=True):
    ...
    force_finite : bool, default=True
        ...
        In the particular case where some features in `X` or the target `y` are constant, the Pearson's R correlation is not defined. 
        When `force_finite=False`, a correlation of `np.nan` is returned to acknowledge this case. 
        When `force_finite=True`, this value will be forced to a minimal correlation of `0.0`.
    ...
    # Compute centered values Note that E[(x - mean(x))*(y - mean(y))] = E[x*(y - mean(y))], so we need not center X
    if center:
        y = y - np.mean(y)
        ...
        # We can reconsider using `isspmatrix` when the minimum version is SciPy >= 1.11
...
def f_regression(X, y, *, center=True, force_finite=True):
    ...
    1. The cross correlation between each regressor and the target is computed using :func:`r_regression` as::
           E[(X[:, i] - mean(X[:, i])) * (y - mean(y))] / (std(X[:, i]) * std(y))

    2. It is converted to an F score and then to a p-value.

    :func:`f_regression` is derived from :func:`r_regression` and will rank features in the same order 
    if all the features are positively correlated with the target.

    Note however that contrary to :func:`f_regression`, :func:`r_regression` values lie in [-1, 1] and can thus be negative. 
    :func:`f_regression` is therefore recommended as a feature selection criterion to identify potentially predictive feature 
    for a downstream classifier, irrespective of the sign of the association with the target variable.

    Furthermore :func:`f_regression` returns p-values while :func:`r_regression` does not.
    ...
    force_finite : bool, default=True
        ...
        - when the target `y` or some features in `X` are constant. 
          In this case, the Pearson's R correlation is not defined leading to obtain `np.nan` values in the F-statistic and p-value. 
          When `force_finite=True`, the F-statistic is set to `0.0` and the associated p-value is set to `1.0`.
        - when a feature in `X` is perfectly correlated (or anti-correlated) with the target `y`. 
          In this case, the F-statistic is expected to be `np.inf`. When `force_finite=True`,
          the F-statistic is set to `np.finfo(dtype).max` and the associated p-value is set to `0.0`.
    ...
    if force_finite and not np.isfinite(f_statistic).all():
        # case where there is a perfect (anti-)correlation f-statistics can be set to the maximum and p-values to zero
        mask_inf = np.isinf(f_statistic)
        f_statistic[mask_inf] = np.finfo(f_statistic.dtype).max
        # case where the target or some features are constant f-statistics would be minimum and thus p-values large
...
# TODO this class should fit on either p-values or scores, depending on the mode.
