"""
The :mod:`sklearn.model_selection._search` includes utilities to fine-tune the parameters of an estimator.
"""
...
class ParameterGrid:
    ...
    param_grid : dict of str to sequence, or sequence of such
        ...
        and is useful to avoid exploring parameter combinations that make no sense or have no effect. See the examples below.
    ...
    See Also
    --------
    GridSearchCV : Uses :class:`ParameterGrid` to perform a full parallelized, parameter search.
    """
        ...
        for grid in param_grid:
            ...
            for key, value in grid.items():
                ...
                if isinstance(value, str) or not isinstance(
                    ...
                    raise TypeError(f"Parameter grid for parameter {key!r} needs to be a list or a numpy array, but got {value!r} (of type "
                        f"{type(value).__name__}) instead. Single values need to be wrapped in a list with one element.")
                if len(value) == 0:
...
class ParameterSampler:
    ...
    Non-deterministic iterable over random candidate combinations for hyper- parameter search. 
    ...
    param_distributions : dict
        ...
        If a list is given, it is sampled uniformly. If a list of dicts is given, first a dict is sampled uniformly, 
    ...
    def __init__(self, param_distributions, n_iter, *, random_state=None):
        if not isinstance(param_distributions, (Mapping, Iterable)):
            raise TypeError("Parameter distribution is not a dict or a list, " \
                "got: {param_distributions!r} of type {type(param_distributions).__name__}")
    ...
    def __iter__(self):
        ...
        # if all distributions are given as lists, we want to sample without replacement
        if self._is_all_lists():
            ...
            if grid_size < n_iter:
                warnings.warn("The total space of parameters %d is smaller than n_iter=%d. Running %d iterations. "
                    "For exhaustive searches, use GridSearchCV." % (grid_size, self.n_iter, grid_size), UserWarning)
                n_iter = grid_size
...
def _check_refit(search_cv, attr):
    if not search_cv.refit:
        raise AttributeError(f"This {type(search_cv).__name__} instance was initialized with `refit=False`. "
            "{attr} is available only after refitting on the best parameters. "
...
def _estimator_has(attr):
    ...
    Calling a prediction method will only be available if `refit=True`. 
    In such case, we check first the fitted best estimator. 
    If it is not fitted, we check the unfitted estimator.

    Checking the unfitted estimator allows to use `hasattr` on the `SearchCV` instance even before calling `fit`.
    """
...
class BaseSearchCV(MetaEstimatorMixin, BaseEstimator, metaclass=ABCMeta):
    ...
    def score(self, X, y=None, **params):
        ...
        **params : dict
            ...
            ..versionadded:: 1.4
                Only available if `enable_metadata_routing=True`. See :ref:`Metadata Routing User Guide <metadata_routing>` for more details.
    ...
    def predict_proba(self, X):
        ...
        y_pred : ndarray of shape (n_samples,) or (n_samples, n_classes)
            ...
            The order of the classes corresponds to that in the fitted attribute :term:`classes_`.
        """
        check_is_fitted(self)
    ...
    def n_features_in_(self):
        ...
        """
        # For consistency with other estimators we raise a AttributeError so that hasattr() fails if the search estimator isn't fitted.
        try:
            check_is_fitted(self)
    ...
    def _run_search(self, evaluate_candidates):
        ...
        This method, implemented in sub-classes, makes it possible to customize the scheduling of evaluations: GridSearchCV 
        and RandomizedSearchCV schedule evaluations for their whole parameter search space at once but other more sequential approaches 
        are also possible: for instance is possible to iteratively schedule evaluations for new regions of the parameter 
        search space based on previously collected evaluation results. 
        This makes it possible to implement Bayesian optimization or more generally sequential model-based optimization 
        by deriving from the BaseSearchCV abstract base class.  
        For example, Successive Halving is implemented by calling `evaluate_candidates` multiples times (once per iteration of the SH process), 
        each time passing a different set of candidates with `X` and `y` of increasing sizes.

        Parameters
        ----------
        evaluate_candidates : callable
            This callback accepts:
                - a list of candidates, where each candidate is a dict of parameter settings.
                - an optional `cv` parameter which can be used to e.g.
                  evaluate candidates on different dataset splits, or evaluate candidates on subsampled data 
                  (as done in the SucessiveHaling estimators). 
                  By default, the original `cv` parameter is used, and it is available as a private `_checked_cv_orig` attribute.
                - an optional `more_results` dict. Each key will be added to the `cv_results_` attribute. 
                  Values should be lists of length `n_candidates`
            ...
            in randomized splitters, and unless the random_state parameter of cv was set to an int, 
            calling cv.split() multiple times will yield different splits. 
            Since cv.split() is called in evaluate_candidates, 
            this means that candidates will be evaluated on different splits each time evaluate_candidates is called. 
            This might be a methodological issue depending on the search strategy that you're implementing. 
            To prevent randomized splitters from being used, you may use _split._yields_constant_splits()

        Examples
        --------
        ::
            def _run_search(self, evaluate_candidates):
                'Try C=0.1 only if C=1 is better than C=10' all_results = evaluate_candidates([{'C': 1}, {'C': 10}])
                score = all_results['mean_test_score'] if score[0] < score[1]: evaluate_candidates([{'C': 0.1}])
        """
        raise NotImplementedError("_run_search not implemented.")

    def _check_refit_for_multimetric(self, scores):
        """Check `refit` is compatible with `scores` is valid"""
        multimetric_refit_msg = ("For multi-metric scoring, the parameter refit must be set to a "
            "scorer key or a callable to refit an estimator with the best parameter setting on the whole data and make the best_* "
            "attributes available for that metric. If this is not needed, refit should be set to False explicitly. {self.refit!r} was passed.")
    ...
    def _select_best_index(refit, refit_metric, results):
        ...
        if callable(refit):
            # If callable, refit is expected to return the index of the best parameter set.
    ...
    def _get_scorers(self, convert_multimetric):
        ...
        convert_multimetric : bool
            Whether to convert a dict of scorers to a _MultimetricScorer. 
            This is used in ``get_metadata_routing`` to include the routing info for multiple scorers.
    ...
    def _get_routed_params_for_fit(self, params):
        ...
        This is a method instead of a snippet in ``fit`` since it's used twice, here in ``fit``, and in ``HalvingRandomSearchCV.fit``.
        """
        if _routing_enabled():
    ...
    def fit(self, X, y=None, **params):
        ...
        **params : dict of str -> object
            ...
            If a fit parameter is an array-like whose length is equal to `num_samples` then it will be split across CV groups along with `X` and `y`.
            For example, the :term:`sample_weight` parameter is split because `len(sample_weights) = len(X)`.
        ...
        # Here we keep a dict of scorers as is, and only convert to a _MultimetricScorer at a later stage. Issue:
        ...
        fit_and_score_kwargs = dict(scorer=scorers, fit_params=routed_params.estimator.fit, score_params=routed_params.scorer.score,
            return_train_score=self.return_train_score, return_n_test_samples=True, return_times=True, return_parameters=False,
        ...
        with parallel:
            ...
            def evaluate_candidates(candidate_params, cv=None, more_results=None):
                ...
                out = parallel(delayed(_fit_and_score)(clone(base_estimator), X, y, train=train, test=test, parameters=parameters, \
                        split_progress=(split_idx, n_splits), candidate_progress=(cand_idx, n_candidates), **fit_and_score_kwargs)
                    for (cand_idx, parameters), (split_idx, (train, test)) in 
                        product(enumerate(candidate_params), enumerate(cv.split(X, y, **routed_params.splitter.split))))

                if len(out) < 1:
                    raise ValueError("No fits were performed. Was the CV iterator empty? Were there no candidates?")
                elif len(out) != n_candidates * n_splits:
                    raise ValueError("cv.split and cv.get_n_splits returned inconsistent results. Expected {} splits, got {}"
                ...
                # The type checking of out will be done in `_insert_error_scores`.
                if callable(self.scoring):
            ...
            # multimetric is determined here because in the case of a callable self.scoring the return type is only known after calling
        ...
        # For multi-metric evaluation, store the best_index_, best_params_ 
        # and best_score_ iff refit is one of the scorer names In single metric evaluation, refit_metric is "score"
        if self.refit or not self.multimetric_:
            self.best_index_ = self._select_best_index(self.refit, refit_metric, results)
            if not callable(self.refit):
                # With a non-custom callable, we can select the best score based on the best index
                self.best_score_ = results[f"mean_test_{refit_metric}"][self.best_index_]
            self.best_params_ = results["params"][self.best_index_]

        if self.refit:
            # here we clone the estimator as well as the parameters, 
            # since sometimes the parameters themselves might be estimators, e.g.
    ...
    def _format_results(self, candidate_params, n_splits, out, more_results=None):
        ...
        def _store(key_name, array, weights=None, splits=False, rank=False):
            ...
            if rank:
                # When the fit/scoring fails `array_means` contains NaNs, 
                # we will exclude them from the ranking process and consider them as tied with the worst performers.
        ...
        # Use one MaskedArray and mask all the places where the param is not applicable for that candidate. 
        # Use defaultdict as each candidate may not contain all the params
        param_results = defaultdict(partial(MaskedArray, np.empty(n_candidates), mask=True, dtype=object))
        for cand_idx, params in enumerate(candidate_params):
            for name, value in params.items():
                # An all masked empty array gets created for the key `"param_%s" % name` at the first occurrence of `name`.
    ...
    def get_metadata_routing(self):
        ...
        routing : MetadataRouter
            A :class:`~sklearn.utils.metadata_routing.MetadataRouter` encapsulating routing information.
        """
...
class GridSearchCV(BaseSearchCV):
    ...
    refit : bool, str, or callable, default=True
        ...
        In that case, the ``best_estimator_`` and ``best_params_`` will be set according to the returned ``best_index_`` 
        while the ``best_score_`` attribute will not be available.
        ...
        to see how to design a custom selection strategy using a callable via `refit`.
    ...
    cv_results_ : dict of numpy (masked) ndarrays
        ...
        For multi-metric evaluation, the scores for all the scorers are available in the ``cv_results_`` dict at the keys ending with 
        that scorer's name (``'_<scorer_name>'``) instead of ``'_score'`` shown above. ('split0_test_precision', 'mean_train_precision' etc.)
    ...
    best_index_ : int
        ...
        The dict at ``search.cv_results_['params'][search.best_index_]`` gives the parameter setting for the best model, 
        that gives the highest mean score (``search.best_score_``).
    ...
    n_features_in_ : int
        Number of features seen during :term:`fit`. Only defined if `best_estimator_` is defined 
        (see the documentation for the `refit` parameter for more details) and that `best_estimator_` exposes
    ...
    feature_names_in_ : ndarray of shape (`n_features_in_`,)
        ...
        Only defined if `best_estimator_` is defined (see the documentation for the `refit` parameter for more details) 
    ...
    train_test_split : Utility function to split the data into a development set 
        usable for fitting a GridSearchCV instance and an evaluation set for its final evaluation.
    ...
    This is done for efficiency reasons if individual jobs take very little time, 
    but may raise errors if the dataset is large and not enough memory is available.  
    A workaround in this case is to set `pre_dispatch`. Then, the memory is copied only `pre_dispatch` many times. 
...
class RandomizedSearchCV(BaseSearchCV):
    ...
    ----------
    estimator : estimator object
        An object of that type is instantiated for each grid point. This is assumed to implement the scikit-learn estimator interface.
        Either estimator needs to provide a ``score`` function, or ``scoring`` must be passed.
    ...
    refit : bool, str, or callable, default=True
        ...
        Where there are considerations other than maximum score in choosing a best estimator, 
        ``refit`` can be set to a function which returns the selected ``best_index_`` given the ``cv_results``. 
        In that case, the ``best_estimator_`` and ``best_params_`` will be set according to the returned ``best_index_`` 
        while the ``best_score_`` attribute will not be available.

        The refitted estimator is made available at the ``best_estimator_`` attribute and permits using ``predict`` directly on this
    ...
    n_features_in_ : int
        Number of features seen during :term:`fit`. Only defined if `best_estimator_` is defined 
        (see the documentation for the `refit` parameter for more details) and that `best_estimator_` exposes `n_features_in_` when fit.

        .. versionadded:: 0.24

    feature_names_in_ : ndarray of shape (`n_features_in_`,)
        Names of features seen during :term:`fit`. Only defined if `best_estimator_` is defined 
        (see the documentation for the `refit` parameter for more details) and that `best_estimator_` exposes `feature_names_in_` when fit.
    ...
    def __init__(self, estimator, param_distributions, *, n_iter=10, scoring=None, n_jobs=None,         \
        refit=True, cv=None, verbose=0, pre_dispatch="2*n_jobs", random_state=None, error_score=np.nan, \
