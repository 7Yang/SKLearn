...
from ..utils import (_approximate_mode, _safe_indexing, check_random_state, indexable, metadata_routing)

__all__ = [
    "BaseCrossValidator", "KFold", "GroupKFold", "LeaveOneGroupOut", "LeaveOneOut", "LeavePGroupsOut",
    "LeavePOut", "RepeatedStratifiedKFold", "RepeatedKFold", "ShuffleSplit", "GroupShuffleSplit", "StratifiedKFold",
    "StratifiedGroupKFold", "StratifiedShuffleSplit", "PredefinedSplit", "train_test_split", "check_cv" ]
...
class BaseCrossValidator(_MetadataRequester, metaclass=ABCMeta):
    ...
    # This indicates that by default CV splitters don't have a "groups" kwarg, unless indicated by inheriting from ``GroupsConsumerMixin``.
    # This also prevents ``set_split_request`` to be generated for splitters which don't support ``groups``.
    ...
    # Since subclasses must implement either _iter_test_masks or _iter_test_indices, neither can be abstract.
...
class LeaveOneOut(BaseCrossValidator):
    ...
    For large datasets one should favor :class:`KFold`, :class:`ShuffleSplit` or :class:`StratifiedKFold`.
...
class LeavePOut(BaseCrossValidator):
    ...
    For large datasets one should favor :class:`KFold`, :class:`StratifiedKFold` or :class:`ShuffleSplit`.
...
class _BaseKFold(BaseCrossValidator, metaclass=ABCMeta):
    ...
    def split(self, X, y=None, groups=None):
        ...
        if self.n_splits > n_samples:
            raise ValueError(("Cannot have number of splits n_splits={0} greater" 
...
class KFold(_BaseKFold):
    ...
    Provides train/test indices to split data in train/test sets. 
    Split dataset into k consecutive(连续) folds (without shuffling by default).

    Each fold is then used once as a validation while the k - 1 remaining folds form the training set.

    Read more in the :ref:`User Guide <k_fold>`.

    For visualisation of cross-validation behaviour and comparison between common scikit-learn split methods
    ...
    StratifiedKFold : Takes class information into account to avoid building
        folds with imbalanced class distributions (for binary or multiclass classification tasks).
...
class StratifiedKFold(_BaseKFold):
    ...
    For visualisation of cross-validation behaviour and comparison between common scikit-learn split methods refer to 
    ...
    * Preserve order dependencies in the dataset ordering, when ``shuffle=False``: all samples from class k in some test set 
      were contiguous in y, or separated in y by samples from classes other than k.
    * Generate test sets where the smallest and largest differ by at most one sample.
    ...
    def _make_test_folds(self, X, y=None):
        ...
        # y_inv encodes y according to lexicographic order. We invert y_idx to map the classes 
        # so that they are encoded by order of appearance: 0 represents the first label appearing in y, 1 the second, etc.
        ...
        if self.n_splits > min_groups:
            warnings.warn("The least populated class in y has only %d members, which is less than n_splits=%d." 
                % (min_groups, self.n_splits), UserWarning)

        # Determine the optimal number of samples from each class in each fold, 
        # using round robin over the sorted y. (This can be done direct from counts, but that code is unreadable.)
        y_order    = np.sort(y_encoded)
        allocation = np.asarray([np.bincount(y_order[i :: self.n_splits], minlength=n_classes) for i in range(self.n_splits)])

        # To maintain the data order dependencies as best as possible within the stratification constraint, 
        # we assign samples from each class in blocks (and then mess that up when shuffle=True).
        test_folds = np.empty(len(y), dtype="i")
        for k in range(n_classes):
            # since the kth column of allocation stores the number of samples of class k in each test set, 
            # this generates blocks of fold indices corresponding to the allocation for class k.
    ...
    def split(self, X, y, groups=None):
        ...
        X : array-like of shape (n_samples, n_features)
            ...
            Note that providing ``y`` is sufficient to generate the splits and hence ``np.zeros(n_samples)`` may be 
...
class StratifiedGroupKFold(GroupsConsumerMixin, _BaseKFold):
    ...
    This cross-validation object is a variation of StratifiedKFold attempts to return stratified folds with non-overlapping groups. 
    The folds are made by preserving the percentage of samples for each class.

    Each group will appear exactly once in the test set across all folds 
    (the number of distinct groups has to be at least equal to the number of folds).

    The difference between :class:`~sklearn.model_selection.GroupKFold` and :class:`~sklearn.model_selection.StratifiedGroupKFold` 
    is that the former attempts to create balanced folds such that the number of distinct groups is approximately the same in each fold, 
    whereas StratifiedGroupKFold attempts to create folds which preserve the percentage of samples for each class as much as possible 
    given the constraint of non-overlapping groups between splits.
    ...
    random_state : int or RandomState instance, default=None
        ...
        Otherwise, leave `random_state` as `None`.  Pass an int for reproducible output across multiple function calls.
        See :term:`Glossary <random_state>`.
    ...
    * Be invariant to class label: relabelling ``y = ["Happy", "Sad"]`` to ``y = [1, 0]`` should not change the indices generated.
    * Stratify based on samples as much as possible while keeping non-overlapping groups constraint. 
      That means that in some cases when there is a small number of groups containing a large number of samples
      the stratification will not be possible and the behavior will be close to GroupKFold.

    See also
    --------
    StratifiedKFold: Takes class information into account to build folds which retain class distributions 
    ...
    def _iter_test_indices(self, X, y, groups):
        ...
        # - Added heuristic for assigning group to the least populated fold in cases when all other criteria are equal
        # - Swtch from using python ``Counter`` to ``np.unique`` to get class distribution
        # - Added scikit-learn checks for input: checking that target is binary or multiclass, 
        #   checking passed random state, checking that number of splits is less than number of members in each class, 
        #   checking that least populated class has more members than there are splits.
        rng = check_random_state(self.random_state)
        y   = np.asarray(y)
        type_of_target_y     = type_of_target(y)
        allowed_target_types = ("binary", "multiclass")
...
class LeaveOneGroupOut(GroupsConsumerMixin, BaseCrossValidator):
    ...
    Provides train/test indices to split data such that each training set is comprised of all samples except ones belonging to one specific group.
    Arbitrary domain specific group information is provided an array integers that encodes the group of each sample.
    ...
    Notes
    -----
    Splits are ordered according to the index of the group left out. The first split has testing set consisting of the group whose index in `groups` is lowest, and so on.
    ...
    def _iter_test_masks(self, X, y, groups):
        ...
        if len(unique_groups) <= 1:
            raise ValueError("The groups parameter contains fewer than 2 unique groups (%s). "
                " LeaveOneGroupOut expects at least 2." % unique_groups)
    ...
    def get_n_splits(self, X=None, y=None, groups=None):
        ...
        groups : array-like of shape (n_samples,)
            Group labels for the samples used while splitting the dataset into train/test set. 
            This 'groups' parameter must always be specified to calculate the number of splits, though the other parameters can be omitted.
...
class LeavePGroupsOut(GroupsConsumerMixin, BaseCrossValidator):
    ...
    The difference between LeavePGroupsOut and LeaveOneGroupOut is that the former builds the test sets 
    with all the samples assigned to ``p`` different values of the groups while the latter uses samples all assigned the same groups.
    ...
    def _iter_test_masks(self, X, y, groups):
        ...
        if self.n_groups >= len(unique_groups):
            raise ValueError("The groups parameter contains fewer than (or equal to) n_groups (%d) numbers of unique groups (%s). "
                "LeavePGroupsOut expects that at least n_groups + 1 (%d) unique groups be " "present" % (self.n_groups, unique_groups, self.n_groups + 1))
...
class _RepeatedSplits(_MetadataRequester, metaclass=ABCMeta):
    ...
    Repeats splits for cross-validators n times with different randomization in each repetition.
    ...
    **cvargs : additional params
        Constructor parameters for cv. Must not contain random_state and shuffle.
    """
...
class GroupShuffleSplit(GroupsConsumerMixin, ShuffleSplit):
    ...
    The difference between LeavePGroupsOut and GroupShuffleSplit is that the former generates splits using all subsets of size ``p`` unique groups,
    ...
    test_size : float, int, default=0.2
        If float, should be between 0.0 and 1.0 and represent the proportion of groups to include in the test split (rounded up). 
        If int, represents the absolute number of test groups. If None, the value is set to the complement of the train size.
        The default will change in version 0.21. It will remain 0.2 only if ``train_size`` is unspecified, 
        otherwise it will complement the specified ``train_size``.

    train_size : float or int, default=None
        If float, should be between 0.0 and 1.0 and represent the proportion of the groups to include in the train split. 
        If int, represents the absolute number of train groups. If None, the value is automatically set to the complement of the test size.
    ...
    def _iter_indices(self, X, y, groups):
        ...
        for group_train, group_test in super()._iter_indices(X=classes):
            # these are the indices of classes in the partition invert them into data indices
...
class StratifiedShuffleSplit(BaseShuffleSplit):
    ...
    def _iter_indices(self, X, y, groups=None):
        ...
        if y.ndim == 2:
            # for multi-label y, map each distinct row to a string repr using join because str(row) uses an ellipsis if len(row) > 1000
        ...
        # Find the sorted list of instances for each class: (np.unique above performs a sort, so code is O(n logn) already)
...
def _validate_shuffle_split(n_samples, test_size, train_size, default_test_size=None):
    """
    Validation helper to check if the test/test sizes are meaningful w.r.t. the size of the data (n_samples).
    """
    ...
    if train_size is not None and train_size_type not in ("i", "f"): raise ValueError("Invalid value for train_size: {}".format(train_size))
    if test_size  is not None and test_size_type  not in ("i", "f"): raise ValueError("Invalid value for test_size: {}".format(test_size))
    ...
    if n_train + n_test > n_samples:
        raise ValueError("The sum of train_size and test_size = %d, should be smaller than the number of samples %d. "
            "Reduce test_size and/or train_size." % (n_train + n_test, n_samples))
...
class PredefinedSplit(BaseCrossValidator):
    ...
    test_fold : array-like of shape (n_samples,)
        ...
        It is possible to exclude sample ``i`` from any test set (i.e. include sample ``i`` in every training set) 
        by setting ``test_fold[i]`` equal to -1.
...
def check_cv(cv=5, y=None, *, classifier=False):
    ...
    cv : int, cross-validation generator or an iterable, default=None
        ...
        For integer/None inputs, if classifier is True and ``y`` is either binary or multiclass, :class:`StratifiedKFold` is used. 
...
@validate_params(
    {
        "test_size" : [Interval(RealNotInt, 0, 1, closed="neither"), Interval(numbers.Integral, 1, None, closed="left"), None ],
        "train_size": [Interval(RealNotInt, 0, 1, closed="neither"), Interval(numbers.Integral, 1, None, closed="left"), None ],
...
# Tell nose that train_test_split is not a test. (Needed for external libraries that may use nose.)
...
def _pprint(params, offset=0, printer=repr):
    ...
    for i, (k, v) in enumerate(sorted(params.items())):
        if isinstance(v, float):
            # use str for representing floating point numbers this way we get consistent representation across architectures and versions.
...
def _build_repr(self):
    ...
    params = dict()
    for key in args:
        # We need deprecation warnings to always be on in order to catch deprecated param values.
        # This is set in utils/__init__.py but it gets overwritten when running under python3 somehow.
...
def _yields_constant_splits(cv):
    # Return True if calling cv.split() always returns the same splits We assume that if a cv doesn't have a shuffle parameter, 
    # it shuffles by default (e.g. ShuffleSplit). If it actually doesn't shuffle (e.g. LeaveOneOut), 
    # then it won't have a random_state parameter anyway, in which case it will default to 0, leading to output=True
