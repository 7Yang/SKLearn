"""
This module gathers tree-based methods, including decision, regression and randomized trees. 
...
from ..utils.validation import (_assert_all_finite_element_wise, _check_sample_weight, assert_all_finite, check_is_fitted)
...
class BaseDecisionTree(MultiOutputMixin, BaseEstimator, metaclass=ABCMeta):
    ...
    _parameter_constraints: dict = {
        ...
        "min_samples_split": [Interval(Integral, 2, None, closed="left"), Interval(RealNotInt, 0.0, 1.0, closed="right") ],
        "min_samples_leaf": [ Interval(Integral, 1, None, closed="left"), Interval(RealNotInt, 0.0, 1.0, closed="neither") ],
        "min_weight_fraction_leaf": [Interval(Real, 0.0, 0.5, closed="both")],
        "max_features": [Interval(Integral, 1, None, closed="left"), Interval(RealNotInt, 0.0, 1.0, closed="right"), StrOptions({"sqrt", "log2"}), None ],
    ...
    @abstractmethod
    def __init__(self, *, criterion, splitter, max_depth, min_samples_split, min_samples_leaf, min_weight_fraction_leaf, max_features, \
        max_leaf_nodes, random_state, min_impurity_decrease, class_weight=None, ccp_alpha=0.0, monotonic_cst=None):
    ...
    def _compute_missing_values_in_feature_mask(self, X, estimator_name=None):
        ...
        Returns
        -------
        missing_values_in_feature_mask : ndarray of shape (n_features,), or None Missing value mask. 
            If missing values are not supported or there are no missing values, return None.
        """
    ...
    def _fit(self, X, y, sample_weight=None, check_input=True, missing_values_in_feature_mask=None):
        ...
        if check_input:
            ...
            # _compute_missing_values_in_feature_mask will check for finite values and compute the missing mask if the tree supports missing values
        ...
        if isinstance(self.max_features, str):
            if self.max_features == "auto":
                if is_classification:
                    ...
                    warnings.warn(("`max_features='auto'` has been deprecated in 1.1 and will be removed in 1.3. "
                        "To keep the past behaviour, explicitly set `max_features='sqrt'`."), FutureWarning)
                else:
                    max_features = self.n_features_in_
                    warnings.warn(("`max_features='auto'` has been deprecated in 1.1 and will be removed in 1.3. "
                        "To keep the past behaviour, explicitly set `max_features=1.0'`."), FutureWarning)
        ...
        if self.monotonic_cst is None:
            ...
            # Check to correct monotonicity constraint' specification, by applying element-wise logical conjunction
            # Note: we do not cast `np.asarray(self.monotonic_cst, dtype=np.int8)` straight away here so as to generate error messages 
            # for invalid values using the original values prior to any dtype related conversion.
            ...
            if not np.all(valid_constraints):
                ...
                raise ValueError("monotonic_cst must be None or an array-like of -1, 0 or 1, but got {unique_constaints_value}")
            ...
            if is_classifier(self):
                ...
                # of the *negative class* in order to make the implementation similar to regression trees.
                # Since self.monotonic_cst encodes constraints on probabilities of the *positive class*, all signs must be flipped.
                monotonic_cst *= -1
        ...
        if max_leaf_nodes < 0:
            ...
            builder = BestFirstTreeBuilder(splitter, min_samples_split, min_samples_leaf, min_weight_leaf, max_depth, max_leaf_nodes,
    ...
    def apply(self, X, check_input=True):
        ...
        Returns
        -------
        X_leaves : array-like of shape (n_samples,) For each datapoint x in X, return the index of the leaf x ends up in. 
...
class DecisionTreeClassifier(ClassifierMixin, BaseDecisionTree):
    ...
    tree_ : Tree instance
        ...
        and :ref:`sphx_glr_auto_examples_tree_plot_unveil_tree_structure.py` for basic usage of these attributes.
    ...
    The :meth:`predict` method operates using the :func:`numpy.argmax` function on the outputs of :meth:`predict_proba`. 
    This means that in case the highest predicted probabilities are tied, 
    the classifier will predict the tied class with the lowest index in :term:`classes_`.
    ...
    def __init__(self, *, criterion="gini", splitter="best", max_depth=None, min_samples_split=2, min_samples_leaf=1,
        ...
        super().__init__(criterion=criterion, splitter=splitter, max_depth=max_depth, min_samples_split=min_samples_split,
            ...
            max_leaf_nodes=max_leaf_nodes, class_weight=class_weight, random_state=random_state,
            min_impurity_decrease=min_impurity_decrease, monotonic_cst=monotonic_cst, ccp_alpha=ccp_alpha)
...
class DecisionTreeRegressor(RegressorMixin, BaseDecisionTree):
    ...
    random_state : int, RandomState instance or None, default=None
        ...
        even if ``splitter`` is set to ``"best"``. When ``max_features < n_features``, 
    ...
    tree_ : Tree instance
        The underlying Tree object. Please refer to ``help(sklearn.tree._tree.Tree)`` for attributes of Tree object and
        :ref:`sphx_glr_auto_examples_tree_plot_unveil_tree_structure.py` for basic usage of these attributes.
    ...
    (e.g. ``max_depth``, ``min_samples_leaf``, etc.) lead to fully grown and unpruned trees which can potentially be very large on some data sets. 
    To reduce memory consumption, the complexity and size of the trees should be controlled by setting those parameter values.
    ...
    _parameter_constraints: dict = {
        **BaseDecisionTree._parameter_constraints,
        "criterion": [StrOptions({"squared_error", "friedman_mse", "absolute_error", "poisson"}), Hidden(Criterion) ],
    }

    def __init__(self, *, criterion="squared_error", splitter="best", max_depth=None, min_samples_split=2, min_samples_leaf=1,
        min_weight_fraction_leaf=0.0, max_features=None, random_state=None, max_leaf_nodes=None, min_impurity_decrease=0.0, ccp_alpha=0.0,
        monotonic_cst=None):
        super().__init__( criterion=criterion, splitter=splitter, max_depth=max_depth, min_samples_split=min_samples_split,  \
            min_samples_leaf=min_samples_leaf, min_weight_fraction_leaf=min_weight_fraction_leaf, max_features=max_features, \
            max_leaf_nodes=max_leaf_nodes, random_state=random_state, min_impurity_decrease=min_impurity_decrease,           \
    ...
    def _more_tags(self):
        # XXX: nan is only support for dense arrays, but we set this for common test to pass, specifically: check_estimators_nan_inf
...
class ExtraTreeClassifier(DecisionTreeClassifier):
    ...
    random splits are drawn for each of the `max_features` randomly selected features and the best split among those is chosen. 
    ...
    random_state : int, RandomState instance or None, default=None
        Used to pick randomly the `max_features` used at each split.  See :term:`Glossary <random_state>` for details.
    ...
    class_weight : dict, list of dict or "balanced", default=None
        Weights associated with classes in the form ``{class_label: weight}``.
        If None, all classes are supposed to have weight one. For multi-output problems, 
    ...
    def __init__(self, *, criterion="gini", splitter="random", max_depth=None, min_samples_split=2, min_samples_leaf=1,
        min_weight_fraction_leaf=0.0, max_features="sqrt", random_state=None, max_leaf_nodes=None, min_impurity_decrease=0.0,
        class_weight=None, ccp_alpha=0.0, monotonic_cst=None):
        super().__init__(criterion=criterion, splitter=splitter, max_depth=max_depth, min_samples_split=min_samples_split,    \
            min_samples_leaf=min_samples_leaf, min_weight_fraction_leaf=min_weight_fraction_leaf, max_features=max_features,  \
            max_leaf_nodes=max_leaf_nodes, class_weight=class_weight, min_impurity_decrease=min_impurity_decrease,            \
            random_state=random_state, ccp_alpha=ccp_alpha, monotonic_cst=monotonic_cst)
...
class ExtraTreeRegressor(DecisionTreeRegressor):
    ...
    def __init__(self, *, criterion="squared_error", splitter="random", max_depth=None, min_samples_split=2, min_samples_leaf=1,\
        min_weight_fraction_leaf=0.0, max_features=1.0, random_state=None, min_impurity_decrease=0.0, max_leaf_nodes=None,      \
