...
__all__ = [ 
    "Binarizer", "KernelCenterer", "MinMaxScaler", "MaxAbsScaler", "Normalizer", "OneHotEncoder", "RobustScaler",
    "StandardScaler", "QuantileTransformer", "PowerTransformer", "add_dummy_feature", "binarize", "normalize",
    "scale", "robust_scale", "maxabs_scale", "minmax_scale", "quantile_transform", "power_transform" 
]

def _is_constant_feature(var, mean, n_samples):
    """
    Detect if a feature is indistinguishable from a constant feature.
    The detection is based on its computed variance and on the theoretical error bounds of the '2 pass algorithm' for variance computation.

    See "Algorithms for computing the sample variance: analysis and recommendations", by Chan, Golub, and LeVeque.
    """
...
def _handle_zeros_in_scale(scale, copy=True, constant_mask=None):
    ...
    Near constant features are detected automatically by identifying scales close to machine precision unless they are precomputed by
    the caller and passed with the `constant_mask` kwarg.

    Typically for standard scaling, the scales are the standard deviation while near constant features are better detected on 
    the computed variances which are closer to machine precision by construction.
    """
    ...
    # scale is an array
        ...
        if constant_mask is None:
            # Detect near constant values to avoid dividing by a very small value 
            # that could lead to surprising results and numerical stability issues.
...
def scale(X, *, axis=0, with_mean=True, with_std=True, copy=True):
    ...
    axis : {0, 1}, default=0
        Axis used to compute the means and standard deviations along. If 0, independently standardize each feature, 
    ...
    copy : bool, default=True
        Set to False to perform inplace row normalization and avoid a copy (if the input is already a numpy array or a scipy.sparse
    ...
    See Also
    --------
    StandardScaler : Performs scaling to unit variance using the Transformer API (e.g. as part of a preprocessing
        :class:`~sklearn.pipeline.Pipeline`).

    Notes
    -----
    This implementation will refuse to center scipy.sparse matrices since it would make them non-sparse and would potentially crash
    the program with memory exhaustion problems.

    Instead the caller is expected to either set explicitly `with_mean=False` (in that case, only variance scaling will be performed on 
    the features of the CSC matrix) or to call `X.toarray()` if he/she expects the materialized dense array to fit in memory.
    ...
    For a comparison of the different scalers, transformers, and normalizers, see: :ref:`sphx_glr_auto_examples_preprocessing_plot_all_scaling.py`.

    .. warning:: Risk of data leak
        Do not use :func:`~sklearn.preprocessing.scale` unless you know what you are doing. 
        ...
        In general, we recommend using :class:`~sklearn.preprocessing.StandardScaler` within a
        :ref:`Pipeline <pipeline>` in order to prevent most risks of data leaking: `pipe = make_pipeline(StandardScaler(), LogisticRegression())`.
    """  # noqa
    X = check_array(X, accept_sparse="csc", copy=copy, ensure_2d=False, estimator="the scale function", \
    ...
    if sparse.issparse(X):
        ...
        # Xr is a view on the original array that enables easy use of broadcasting on the axis in which we are interested in
        Xr = np.rollaxis(X, axis)
        if with_mean:
            Xr -= mean_
            mean_1 = np.nanmean(Xr, axis=0)
            # Verify that mean_1 is 'close to zero'. 
            # If X contains very large values, mean_1 can also be very large, due to a lack of precision of mean_. 
            # In this case, a pre-scaling of the concerned feature is efficient, for instance by its mean or maximum.
            if not np.allclose(mean_1, 0):
                warnings.warn("Numerical issues were encountered when centering the data and might not be solved. "
        ...
        if with_std:
            ...
            if with_mean:
                ...
                # If mean_2 is not 'close to zero', it comes from the fact that scale_ is very small so that mean_2 = mean_1/scale_ > 0,
                # even if mean_1 was close to zero. The problem is thus essentially due to the lack of precision of mean_. 
                # A solution is then to subtract the mean again:
                if not np.allclose(mean_2, 0):
                    warnings.warn("Numerical issues were encountered when scaling the data and might not be solved. "
...
class MinMaxScaler(OneToOneFeatureMixin, TransformerMixin, BaseEstimator):
    ...
    `MinMaxScaler` doesn't reduce the effect of outliers, but it linearily scales them down into a fixed range, 
    where the largest occuring data point corresponds to the maximum value and the smallest one corresponds to the minimum value. 
    For an example visualization, refer to :ref:`Compare MinMaxScaler with other scalers <plot_all_scaling_minmax_scaler_section>`.
    ...
    def partial_fit(self, X, y=None):
        ...
        All of X is processed as a single batch. This is intended for cases when :meth:`fit` is not feasible due to 
        very large number of `n_samples` or because X is read from a continuous stream.
        ...
        X = self._validate_data(X, reset=first_pass, dtype=_array_api.supported_float_dtypes(xp), force_all_finite="allow-nan")
    ...
    def transform(self, X):
        ...
        xp, _ = get_namespace(X)
        X = self._validate_data(X, copy=self.copy, dtype=_array_api.supported_float_dtypes(xp), force_all_finite="allow-nan", reset=False)
    ...
    def inverse_transform(self, X):
        ...
        Returns
        -------
        Xt : ndarray of shape (n_samples, n_features), Transformed data.
        """
        check_is_fitted(self)

        xp, _ = get_namespace(X)
        X = check_array(X, copy=self.copy, dtype=_array_api.supported_float_dtypes(xp), force_all_finite="allow-nan")
...
def minmax_scale(X, feature_range=(0, 1), *, axis=0, copy=True):
    ...
    .. warning:: Risk of data leak
        ...
        Do not use :func:`~sklearn.preprocessing.minmax_scale` unless you know what you are doing. 
        A common mistake is to apply it to the entire data *before* splitting into training and test sets. 
        This will bias the model evaluation because information would have leaked from the test set to the training set.
        In general, we recommend using :class:`~sklearn.preprocessing.MinMaxScaler` 
        within a :ref:`Pipeline <pipeline>` in order to prevent most risks of data leaking: 
        `pipe = make_pipeline(MinMaxScaler(), LogisticRegression())`.

    See Also
    --------
    MinMaxScaler : Performs scaling to a given range using the Transformer
        API (e.g. as part of a preprocessing :class:`~sklearn.pipeline.Pipeline`).
    ...
    """
    # Unlike the scaler object, this function allows 1d input. If copy is required, it will be done inside the scaler object.
...
class StandardScaler(OneToOneFeatureMixin, TransformerMixin, BaseEstimator):
    ...
    For an example visualization, refer to :ref:`Compare StandardScaler with other scalers <plot_all_scaling_standard_scaler_section>`.
    ...
    scale_ : ndarray of shape (n_features,) or None
        ...
        If a variance is zero, we can't achieve unit variance, and the data is left as-is, giving a scaling factor of 1. 
    ...
    n_samples_seen_ : int or ndarray of shape (n_features,)
        ...
        If there are no missing samples, the ``n_samples_seen`` will be an integer, otherwise it will be an array of dtype int. 
        If `sample_weights` are used it will be a float (if no missing data) or an array of dtype float that sums the weights seen so far.
    ...
    def partial_fit(self, X, y=None, sample_weight=None):
        ...
        All of X is processed as a single batch. This is intended for cases when :meth:`fit` is not feasible due to very large number of
        ...
        "Algorithms for computing the sample variance: Analysis and recommendations." The American Statistician 37.3 (1983): 242-247:
        ...
        # if n_samples_seen_ is an integer (i.e. no missing values), 
        # we need to transform it to a NumPy array of shape (n_features,) required by incr_mean_variance_axis and _incremental_variance_axis
            ...
            if self.with_std:
                ...
                # Next passes
                    ...
                    (self.mean_, self.var_, self.n_samples_seen_,) = incr_mean_variance_axis(
                        X, axis=0, last_mean=self.mean_, last_var=self.var_, last_n=self.n_samples_seen_, weights=sample_weight)
                # We force the mean and variance to float64 for large arrays See https://github.com/scikit-learn/scikit-learn/pull/12338
        ...
        # for backward-compatibility, reduce n_samples_seen_ to an integer if the number of samples is the same for each feature 
        # (i.e. no missing values)
        if np.ptp(self.n_samples_seen_) == 0:
            self.n_samples_seen_ = self.n_samples_seen_[0]

        if self.with_std:
            # Extract the list of near constant features on the raw variances, before taking the square root.
    ...
    def inverse_transform(self, X, copy=None):
        ...
        if sparse.issparse(X):
            if self.with_mean:
                raise ValueError("Cannot uncenter sparse matrices: pass `with_mean=False` " 
...
class MaxAbsScaler(OneToOneFeatureMixin, TransformerMixin, BaseEstimator):
    ...
    in the training set will be 1.0. It does not shift/center the data, and thus does not destroy any sparsity.

    This scaler can also be applied to sparse CSR or CSC matrices.

    `MaxAbsScaler` doesn't reduce the effect of outliers; it only linearily scales them down. 
    For an example visualization, refer to :ref:`Compare MaxAbsScaler with other scalers <plot_all_scaling_max_abs_scaler_section>`.
    ...
    def partial_fit(self, X, y=None):
        ...
        first_pass = not hasattr(self, "n_samples_seen_")
        X = self._validate_data(X, reset=first_pass, accept_sparse=("csr", "csc"), \
            dtype=_array_api.supported_float_dtypes(xp), force_all_finite="allow-nan")
    ...
    def transform(self, X):
        ...
        X = self._validate_data(X, accept_sparse=("csr", "csc"), copy=self.copy, reset=False,
            dtype=_array_api.supported_float_dtypes(xp), force_all_finite="allow-nan")
    ...
    def inverse_transform(self, X):
        ...
        X = check_array(X, accept_sparse=("csr", "csc"), copy=self.copy, dtype=_array_api.supported_float_dtypes(xp), force_all_finite="allow-nan")
...
def maxabs_scale(X, *, axis=0, copy=True):
    ...
    .. warning:: Risk of data leak
        Do not use :func:`~sklearn.preprocessing.maxabs_scale` unless you know what you are doing. 
        A common mistake is to apply it to the entire data *before* splitting into training and test sets. 
        This will bias the model evaluation because information would have leaked from the test set to the training set.
        In general, we recommend using :class:`~sklearn.preprocessing.MaxAbsScaler` within a
    ...
    MaxAbsScaler : Performs scaling to the [-1, 1] range using
        the Transformer API (e.g. as part of a preprocessing :class:`~sklearn.pipeline.Pipeline`).
...
class RobustScaler(OneToOneFeatureMixin, TransformerMixin, BaseEstimator):
    ...
    Centering and scaling happen independently on each feature by computing the relevant statistics on the samples in the training set. 
    Median and interquartile range are then stored to be used on later data using the :meth:`transform` method.
    ...
    For an example visualization and comparison to other scalers,
    refer to :ref:`Compare RobustScaler with other scalers <plot_all_scaling_robust_scaler_section>`.
    ...
    with_centering : bool, default=True
        If `True`, center the data before scaling.
        This will cause :meth:`transform` to raise an exception when attempted on sparse matrices, 
        because centering them entails building a dense matrix which in common use cases is likely to be too large to fit in memory.
    ...
    quantile_range : tuple (q_min, q_max), 0.0 < q_min < q_max < 100.0, default=(25.0, 75.0)
        Quantile range used to calculate `scale_`. By default this is equal to the IQR, 
        i.e., `q_min` is the first quantile and `q_max` is the third quantile.
    ...
    unit_variance : bool, default=False
        ...
        In general, if the difference between the x-values of `q_max` and `q_min` for a standard normal distribution is greater than 1, 
        the dataset will be scaled down. If less than 1, the dataset will be scaled up.
    ...
    def fit(self, X, y=None):
        ...
        X = self._validate_data(X, accept_sparse="csc", dtype=FLOAT_DTYPES, force_all_finite="allow-nan")
    ...
    def transform(self, X):
        ...
        """
        check_is_fitted(self)
        X = self._validate_data(X, accept_sparse=("csr", "csc"), copy=self.copy, dtype=FLOAT_DTYPES, reset=False, force_all_finite="allow-nan")
...
def robust_scale(X, *, axis=0, with_centering=True, with_scaling=True, quantile_range=(25.0, 75.0), copy=True, unit_variance=False):
    ...
    copy : bool, default=True
        ...
        (if the input is already a numpy array or a scipy.sparse CSR matrix and if axis is 1).
    ...
    Notes
    -----
    This implementation will refuse to center scipy.sparse matrices since it would make them non-sparse 
    and would potentially crash the program with memory exhaustion problems.

    Instead the caller is expected to either set explicitly `with_centering=False` 
    (in that case, only variance scaling will be performed on the features of the CSR matrix) 
    or to call `X.toarray()` if he/she expects the materialized dense array to fit in memory.
    ...
    .. warning:: Risk of data leak
        ...
        Do not use :func:`~sklearn.preprocessing.robust_scale` unless you know what you are doing. 
        A common mistake is to apply it to the entire data *before* splitting into training and test sets. 
        This will bias the model evaluation because information would have leaked from the test set to the training set.
...
class Normalizer(OneToOneFeatureMixin, TransformerMixin, BaseEstimator):
    ...
    This transformer is able to work both with dense numpy arrays 
    and scipy.sparse matrix (use CSR format if you want to avoid the burden of a copy / conversion).
    ...
    For an example visualization, refer to :ref:`Compare Normalizer with other scalers <plot_all_scaling_normalizer_section>`.
    ...
    However, we recommend to call :meth:`fit_transform` instead of :meth:`transform`, as parameter validation is only performed in :meth:`fit`.
...
def binarize(X, *, threshold=0.0, copy=True):
    ...
    X : {array-like, sparse matrix} of shape (n_samples, n_features)
        ...
        scipy.sparse matrices should be in CSR or CSC format to avoid an un-necessary copy.
    ...
    See Also
    --------
    Binarizer : Performs binarization using the Transformer API (e.g. as part of a preprocessing :class:`~sklearn.pipeline.Pipeline`).
    """
...
class Binarizer(OneToOneFeatureMixin, TransformerMixin, BaseEstimator):
    ...
    or absence(缺席) of a feature rather than a quantified number of occurrences for instance.
    ...
    Notes
    -----
    If the input is a sparse matrix, only the non-zero values are subject to update by the :class:`Binarizer` class.
    ...
    def transform(self, X, copy=None):
        ...
        """
        copy = copy if copy is not None else self.copy
        # TODO: This should be refactored because binarize also calls check_array
...
class KernelCenterer(ClassNamePrefixFeaturesOutMixin, TransformerMixin, BaseEstimator):
    ...
    :math:`\phi(X)` is a function mapping of rows of :math:`X` to a Hilbert space and :math:`K` is of shape `(n_samples, n_samples)`.

    This class allows to compute :math:`\tilde{K}(X, Y)` such that:
    .. math::
        \tilde{K(X, Y)} = \tilde{\phi}(X) . \tilde{\phi}(Y)^{T}

    :math:`\tilde{\phi}(X)` is the centered mapped data in the Hilbert space.

    `KernelCenterer` centers the features without explicitly computing the mapping :math:`\phi(\cdot)`. 
    ...
    def transform(self, K, copy=True):
        ...
        Returns
        -------
        K_new : ndarray of shape (n_samples1, n_samples2), Returns the instance itself.
        """
        check_is_fitted(self)
    ...
    def _n_features_out(self):
        ...
        # Used by ClassNamePrefixFeaturesOutMixin. 
        # This model preserves the number of input features but this is not a one-to-one mapping in the usual sense. 
        # Hence the choice not to use OneToOneFeatureMixin to implement get_feature_names_out for this class.
...
class QuantileTransformer(OneToOneFeatureMixin, TransformerMixin, BaseEstimator):
    ...
    Features values of new/unseen data that fall below or above the fitted range will be mapped to the bounds of 
    the output distribution. Note that this transform is non-linear. 
    It may distort linear correlations between variables measured at the same scale but renders variables measured at different scales 
    more directly comparable.

    For example visualizations, refer to :ref:`Compare QuantileTransformer with other scalers <plot_all_scaling_quantile_transformer_section>`.
    ...
    n_quantiles : int, default=1000 or n_samples
        ...
        If n_quantiles is larger than the number of samples, n_quantiles is set to the number of samples as a larger number of quantiles 
        does not give a better approximation of the cumulative distribution function estimator.
    ...
    subsample : int, default=10_000
        ...
        Note that the subsampling procedure may differ for value-identical sparse and dense matrices.
    ...
    def __init__(self, *, n_quantiles=1000, output_distribution="uniform", ignore_implicit_zeros=False, subsample=10_000, random_state=None, copy=True):
    ...
    def _dense_fit(self, X, random_state):
        ...
        Parameters
        ----------
        X : ndarray of shape (n_samples, n_features), The data used to scale along the features axis.
        """
    ...
    def _sparse_fit(self, X, random_state):
        ...
        for feature_idx in range(n_features):
            ...
            if not column_data.size:
                # if no nnz, an error will be raised for computing the quantiles. Force the quantiles to be zeros.
    ...
    def fit(self, X, y=None):
        ...
        if self.n_quantiles > self.subsample:
            raise ValueError("The number of quantiles cannot be greater than the number of samples used. Got {} quantiles and {} samples."
        ...
        if self.n_quantiles > n_samples:
            warnings.warn("n_quantiles (%s) is greater than the total number of samples (%s). n_quantiles is set to n_samples."
    ...
    def _transform_col(self, X_col, quantiles, inverse):
        ...
        if not inverse:
            # Interpolate in one direction and in the other and take the mean. 
            # This is in case of repeated values in the features and hence repeated quantiles
            #
            # If we don't do this, only one extreme of the duplicated is used 
            # (the upper when we do ascending, and the lower for descending). 
            # We take the mean of these two
            X_col[isfinite_mask] = 0.5 * (np.interp(X_col_finite, quantiles, self.references_) \
        ...
        if not inverse:
            with np.errstate(invalid="ignore"):  # hide NaN comparison warnings
                if output_distribution == "normal":
                    ...
                    # find the value to clip the data to avoid mapping to infinity.
                    # Clip such that the inverse transform will be consistent
                    clip_min = stats.norm.ppf(BOUNDS_THRESHOLD - np.spacing(1))
                    clip_max = stats.norm.ppf(1 - (BOUNDS_THRESHOLD - np.spacing(1)))
                    X_col = np.clip(X_col, clip_min, clip_max)
                # else output distribution is uniform and the ppf is the identity function so we let X_col unchanged
    ...
    def _check_inputs(self, X, in_fit, accept_sparse_negative=False, copy=False):
        ...
        # we only accept positive sparse matrix when ignore_implicit_zeros is false and that we call fit or transform.
        with np.errstate(invalid="ignore"):  # hide NaN comparison warnings
    ...
    def _transform(self, X, inverse=False):
        ...
        Returns
        -------
        X : ndarray of shape (n_samples, n_features), Projected data.
        """
        if sparse.issparse(X):
            for feature_idx in range(X.shape[1]):
    ...
    def transform(self, X):
        ...
        Returns
        -------
        Xt : {ndarray, sparse matrix} of shape (n_samples, n_features), The projected data.
        """
        check_is_fitted(self)
    ...
    def inverse_transform(self, X):
        ...
        Returns
        -------
        Xt : {ndarray, sparse matrix} of (n_samples, n_features), The projected data.
        """
        check_is_fitted(self)
...
def quantile_transform(X, *, axis=0, n_quantiles=1000, output_distribution="uniform", ignore_implicit_zeros=False, subsample=int(1e5), \
    ...
    axis : int, default=0
        ...
        If 0, transform each feature, otherwise (if 1) transform each sample.
    ...
    random_state : int, RandomState instance or None, default=None
        ...
        Please see ``subsample`` for more details. Pass an int for reproducible results across multiple function calls.
        See :term:`Glossary <random_state>`.

    copy : bool, default=True
        Set to False to perform inplace transformation and avoid a copy (if the input is already a numpy array). 
        If True, a copy of `X` is transformed, leaving the original `X` unchanged.
    ...
    See Also
    --------
    QuantileTransformer : Performs quantile-based scaling using the Transformer API (e.g. as part of a preprocessing :class:`~sklearn.pipeline.Pipeline`).
    ...
    .. warning:: Risk of data leak
        ...
        Do not use :func:`~sklearn.preprocessing.quantile_transform` unless you know what you are doing. 
        A common mistake is to apply it to the entire data *before* splitting into training and test sets. 
        This will bias the model evaluation because information would have leaked from the test set to the training set.
        In general, we recommend using :class:`~sklearn.preprocessing.QuantileTransformer` within a
        :ref:`Pipeline <pipeline>` in order to prevent most risks of data leaking:`pipe = make_pipeline(QuantileTransformer(),
        LogisticRegression())`.
...
class PowerTransformer(OneToOneFeatureMixin, TransformerMixin, BaseEstimator):
    ...
    For an example visualization, refer to :ref:`Compare PowerTransformer with other scalers <plot_all_scaling_power_transformer_section>`. 
    To see the effect of Box-Cox and Yeo-Johnson transformations on different distributions, see:
    ...
    .. [1] :doi:`I.K. Yeo and R.A. Johnson, "A new family of power transformations to improve normality or symmetry." 
        Biometrika, 87(4), pp.954-959, (2000). <10.1093/biomet/87.4.954>`

    .. [2] :doi:`G.E.P. Box and D.R. Cox, "An Analysis of Transformations", Journal of the Royal Statistical Society B, 26, 211-252 (1964).
    ...
    def fit_transform(self, X, y=None):
        ...
        Returns
        -------
        X_new : ndarray of shape (n_samples, n_features), Transformed data.
        """
    ...
    def _fit(self, X, y=None, force_transform=False):
        ...
        with np.errstate(invalid="ignore"):  # hide NaN warnings
            ...
            for i, col in enumerate(X.T):
                # For yeo-johnson, leave constant features unchanged lambda=1 corresponds to the identity transformation
    ...
    def transform(self, X):
        ...
        Returns
        -------
        X_trans : ndarray of shape (n_samples, n_features), The transformed data.
        """
        check_is_fitted(self)
    ...
    def inverse_transform(self, X):
        ...
        Returns
        -------
        X : ndarray of shape (n_samples, n_features), The original data.
        """
        check_is_fitted(self)
    ...
    def _yeo_johnson_optimize(self, x):
        """
        Find and return optimal lambda parameter of the Yeo-Johnson transform by MLE, for observed data x.
    ...
    def _check_input(self, X, in_fit, check_positive=False, check_shape=False):
        ...
        in_fit : bool
            Whether or not `_check_input` is called from `fit` or other methods, e.g. `predict`, `transform`, etc.
        ...
        if check_shape and not X.shape[1] == len(self.lambdas_):
            raise ValueError("Input data has a different number of features than fitting data. Should have {n}, data has {m}"
...
def power_transform(X, method="yeo-johnson", *, standardize=True, copy=True):
    ...
    Currently, power_transform supports the Box-Cox transform and the Yeo-Johnson transform. 
    The optimal parameter for stabilizing variance and minimizing skewness is estimated through maximum likelihood.
    ...
    method : {'yeo-johnson', 'box-cox'}, default='yeo-johnson'
        ...
        .. versionchanged:: 0.23
            The default value of the `method` parameter changed from 'box-cox' to 'yeo-johnson' in 0.23.
    ...
    PowerTransformer : Equivalent transformation with the
        Transformer API (e.g. as part of a preprocessing :class:`~sklearn.pipeline.Pipeline`).
    ...
    .. warning:: Risk of data leak.
        Do not use :func:`~sklearn.preprocessing.power_transform` unless you know what you are doing. 
        A common mistake is to apply it to the entire data *before* splitting into training and test sets. 
        This will bias the model evaluation because information would have leaked from the test set to the training set.
        In general, we recommend using :class:`~sklearn.preprocessing.PowerTransformer` within a
        :ref:`Pipeline <pipeline>` in order to prevent most risks of data leaking, 
