...
class _BaseEncoder(TransformerMixin, BaseEstimator):
    """
    Base class for encoders that includes the code to categorize and transform the input features.
    ...
    def _check_X(self, X, force_all_finite=True):
        ...
        - check for missing values for object dtype data (check_array does not do that)
        - return list of features (arrays): this list of features is constructed feature by feature to preserve the data types
          of pandas DataFrame columns, as otherwise information is lost and cannot be used, e.g. for the `categories_` attribute.
        ...
        if not (hasattr(X, "iloc") and getattr(X, "ndim", 0) == 2):
            ...
            # pandas dataframe, do validation later column by column, in order to keep the dtype information to be used in the encoder.
        ...
        for i in range(n_features):
            ...
            if self.categories == "auto":
                ...
                if np.issubdtype(Xi.dtype, np.str_):
                    # Always convert string categories to objects to avoid unexpected string truncation for longer category labels 
                ...
                if (cats.dtype == object and isinstance(cats[0], bytes) and Xi.dtype.kind != "S"):
                    msg = (f"In column {i}, the predefined categories have type 'bytes' "
                        "which is incompatible with values of type '{type(Xi[0]).__name__}'.")
        ...
        for i in range(n_features):
            ...
            if not np.all(valid_mask):
                if handle_unknown == "error":
                    ...
                    X_mask[:, i] = valid_mask
                    # cast Xi into the largest string type necessary to handle different lengths of numpy strings
                    if (self.categories_[i].dtype.kind in ("U", "S") and self.categories_[i].itemsize > Xi.itemsize):
                        Xi = Xi.astype(self.categories_[i].dtype)
                    elif self.categories_[i].dtype.kind == "O" and Xi.dtype.kind == "U":
                        # categories are objects and Xi are numpy strings. Cast Xi to an object dtype to prevent truncation
            ...
            # We use check_unknown=False, since _check_unknown was already called above.
            X_int[:, i] = _encode(Xi, uniques=self.categories_[i], check_unknown=False)
        if columns_with_unknown:
            warnings.warn(("Found unknown categories in columns {columns_with_unknown} during transform. "
                "These unknown categories will be encoded as all zeros"), UserWarning)
    ...
    def _identify_infrequent(self, category_count, n_samples, col_idx):
        ...
        output : ndarray of shape (n_infrequent_categories,) or None
            If there are infrequent categories, indices of infrequent categories. Otherwise None.
        """
    ...
    def _fit_infrequent_category_mapping(self, n_samples, category_counts, missing_indices):
        ...
        For feature `i`, `_default_to_infrequent_mappings[i]` defines the mapping from the integer encoding returned by `super().transform()` 
        into infrequent categories. If `_default_to_infrequent_mappings[i]` is None, there were no infrequent categories in the training set.

        For example if categories 0, 2 and 4 were frequent, while categories 1, 3, 5 were infrequent for feature 7, 
        then these categories are mapped to a single output:
        `_default_to_infrequent_mappings[7] = array([0, 3, 1, 3, 2, 3])`

        Defines private attribute: `_infrequent_indices`. `_infrequent_indices[i]` is an array of indices such that
        `categories_[i][_infrequent_indices[i]]` are all the infrequent category labels. 
        If the feature `i` has no infrequent categories `_infrequent_indices[i]` is None.
        ...
        category_counts: list of ndarray
            `category_counts[i]` is the category counts corresponding to `self.categories_[i]`.
        ...
        self._infrequent_indices = [self._identify_infrequent(category_count, n_samples, col_idx)
        ...
        for feature_idx, infreq_idx in enumerate(self._infrequent_indices):
            ...
            n_cats = len(cats)
            if feature_idx in missing_indices:
                # Missing index was removed from this category when computing infrequent indices, 
                # thus we need to decrease the number of total categories when considering the infrequent mapping.
                n_cats -= 1
    ...
    def _map_infrequent_categories(self, X_int, X_mask, ignore_category_indices):
        ...
        This modifies X_int in-place. Values that were invalid based on `X_mask` are mapped to the infrequent category 
        if there was an infrequent category for that feature.
        ...
        ignore_category_indices : dict
            ...
            Ignored indexes will not be grouped and the original ordinal encoding will remain.
        """
        ...
        for col_idx in range(X_int.shape[1]):
            ...
            if self.handle_unknown == "infrequent_if_exist":
                # All the unknown values are now mapped to the infrequent_idx[0], 
                # which makes the unknown values valid This is needed in `transform` when the encoding is formed using `X_mask`.
                X_mask[:, col_idx] = True

        # Remaps encoding in `X_int` where the infrequent categories are grouped together.
...
class OneHotEncoder(_BaseEncoder):
    ...
    sparse : bool, default=True
        ...
        .. deprecated:: 1.2
           `sparse` is deprecated in 1.2 and will be removed in 1.4. Use `sparse_output` instead.
    ...
    handle_unknown : {'error', 'ignore', 'infrequent_if_exist'}, default='error'
        ...
        - 'infrequent_if_exist' : When an unknown category is encountered during transform, 
           the resulting one-hot encoded columns for this feature will map to the infrequent category if it exists. 
           ...
           If the `'infrequent'` category does not exist, then :meth:`transform` and :meth:`inverse_transform` 
           will handle an unknown category as with `handle_unknown='ignore'`. 
    ...
    max_categories : int, default=None
        Specifies an upper limit to the number of output features for each input feature when considering infrequent categories. 
        If there are infrequent categories,
    ...
    feature_name_combiner : "concat" or callable, default="concat"
        ...
        `feature + "_" + str(category)`.E.g. feature X with values 1, 6, 7 create feature names `X_1, X_6, X_7`.
    ...
    categories_ : list of arrays
        ...
        This includes the category specified in ``drop`` (if any).

    drop_idx_ : array of shape (n_features,)
        - ``drop_idx_[i]`` is the index in ``categories_[i]`` of the category to be dropped for each feature.
        - ``drop_idx_[i] = None`` if no category is to be dropped from 
            the feature with index ``i``, e.g. when `drop='if_binary'` and the feature isn't binary.
        - ``drop_idx_ = None`` if all the transformed features will be retained.

        If infrequent categories are enabled by setting `min_frequency` or `max_categories` to a non-default value 
        and `drop_idx[i]` corresponds to a infrequent category, then the entire infrequent category is dropped.

        .. versionchanged:: 0.23
           Added the possibility to contain `None` values.

    infrequent_categories_ : list of ndarray
        Defined only if infrequent categories are enabled by setting `min_frequency` or `max_categories` to a non-default value.
        `infrequent_categories_[i]` are the infrequent categories for feature `i`. 
        If the feature `i` has no infrequent categories `infrequent_categories_[i]` is None.
    ...
    MultiLabelBinarizer : Transforms between iterable of iterables and a multilabel format, 
        e.g. a (samples x classes) binary matrix indicating the presence of a class label.
    ...
    _parameter_constraints: dict = {
        ...
        "min_frequency": [Interval(Integral, 1, None, closed="left"), Interval(RealNotInt, 0, 1, closed="neither"), None ],
        "sparse": [Hidden(StrOptions({"deprecated"})), "boolean"],  # deprecated
        "sparse_output": ["boolean"],
        "feature_name_combiner": [StrOptions({"concat"}), callable],
    }

    def __init__(self, *, categories="auto", drop=None, sparse="deprecated", sparse_output=True, dtype=np.float64,
    ...
    def _map_drop_idx_to_infrequent(self, feature_idx, drop_idx):
        ...
        If there are no infrequent categories, then `drop_idx` is returned. 
        This method is called in `_set_drop_idx` when the `drop` parameter is an array-like.
        """
        if not self._infrequent_enabled:
            return drop_idx
    ...
    def _set_drop_idx(self):
        ...
        - `'if_binary'`, All zeros if the category is binary and `None` otherwise.
        - array-like, The indices of the categories that match the categories in `self.drop`. 
          If the dropped category is an infrequent category, then the index for the infrequent category is used. 
          This means that the entire infrequent category is dropped.
        ...
        # `_drop_idx_after_grouping` are the categories to drop *after* the infrequent categories are grouped together. 
        # If needed, we remap `drop_idx` back to the categories seen in `self.categories_`.
    ...
    def _compute_n_features_outs(self):
        ...
        # infrequent is enabled, the number of features out are reduced because the infrequent categories are grouped together
        for i, infreq_idx in enumerate(self._infrequent_indices):
            if infreq_idx is None:
                continue
    ...
    def fit(self, X, y=None):
        ...
        if self.sparse != "deprecated":
            warnings.warn(("`sparse` was renamed to `sparse_output` in version 1.2 and will be removed in 1.4. "
                "`sparse_output` is ignored unless you leave `sparse` to its default value."), FutureWarning)
    ...
    def transform(self, X):
        ...
        if transform_output == "pandas" and self.sparse_output:
            raise ValueError("Pandas output does not support sparse data. Set sparse_output=False to"
                " output pandas DataFrames or disable pandas output via" ' `ohe.set_output(transform="default").')
        ...
        if self._drop_idx_after_grouping is not None:
            ...
            # We remove all the dropped categories from mask, and decrement all categories that occur after them to avoid an empty column.
    ...
    def inverse_transform(self, X):
        ...
        ``None`` is used to represent this category. If the feature with the unknown category has a dropped category, 
        the dropped category will be its inverse.
        ...
        for i in range(n_features):
            ...
            # Only happens if there was a column with a unique category. In this case we just fill the column with this unique category value.
            if n_categories == 0:
            ...
            if self.handle_unknown == "ignore" or (self.handle_unknown == "infrequent_if_exist" and infrequent_indices[i] is None):
                ...
                if unknown.any():
                    # if categories were dropped then unknown categories will be mapped to the dropped category
                ...
                if dropped.any():
                    if self._drop_idx_after_grouping is None:
                        ...
                        raise ValueError(f"Samples {all_zero_samples} can not be inverted when drop=None and handle_unknown='error' "
                            "because they contain all zeros")
                    # we can safely assume that all of the nulls in each column are the dropped value
                    drop_idx = self._drop_idx_after_grouping[i]
                    X_tr[dropped, i] = transformed_features[i][drop_idx]

            j += n_categories

        # if ignored are found: potentially need to upcast result to insert None values
        if found_unknown:
    ...
    def _check_get_feature_name_combiner(self):
        if self.feature_name_combiner == "concat":
            ...
            if not isinstance(dry_run_combiner, str):
                raise TypeError("When `feature_name_combiner` is a callable, it should return a Python string. Got {type(dry_run_combiner)} instead.")
...
class OrdinalEncoder(OneToOneFeatureMixin, _BaseEncoder):
    ...
    Read more in the :ref:`User Guide <preprocessing_categorical_features>`.  For a comparison of different encoders, refer to:
    ...
    max_categories : int, default=None
        ...
        If there are infrequent categories, `max_categories` includes the category representing the infrequent categories 
        along with the frequent categories. If `None`, there is no limit to the number of output features.

        `max_categories` do **not** take into account missing or unknown categories. 
        Setting `unknown_value` or `encoded_missing_value` to an integer will increase the number of unique integer codes by one each.
    ...
    OneHotEncoder : Performs a one-hot encoding of categorical features. 
        This encoding is suitable for low to medium cardinality categorical variables, both in supervised and unsupervised settings.
    TargetEncoder : Encodes categorical features using supervised signal in a classification or regression pipeline. 
        This encoding is typically suitable for high cardinality categorical variables.
    LabelEncoder : Encodes target labels with values between 0 and ``n_classes-1``.

    Notes
    -----
    With a high proportion of `nan` values, inferring categories becomes slow with Python versions before 3.10. 
    The handling of `nan` values was improved from Python 3.10 onwards, (c.f. `bpo-43475 <https://github.com/python/cpython/issues/87641>`_).
    ...
    Infrequent categories are enabled by setting `max_categories` or `min_frequency`.
    In the following example, "a" and "d" are considered infrequent and grouped together into a single category, 
    "b" and "c" are their own categories, unknown values are encoded as 3 and missing values are encoded as 4.
    ...
    def fit(self, X, y=None):
        ...
        if self.handle_unknown == "use_encoded_value":
            if is_scalar_nan(self.unknown_value):
                ...
                raise TypeError("unknown_value should be an integer or np.nan when handle_unknown is 'use_encoded_value', got {self.unknown_value}.")
        elif self.unknown_value is not None:
            raise TypeError("unknown_value should only be set when handle_unknown is 'use_encoded_value', got {self.unknown_value}.")
        ...
        if self._infrequent_enabled:
            # Cardinality decreases because the infrequent categories are grouped together
        ...
        # missing values are not considered part of the cardinality when considering unknown categories or encoded_missing_value
        ...
        if self.handle_unknown == "use_encoded_value":
            for cardinality in cardinalities:
                if 0 <= self.unknown_value < cardinality:
                    raise ValueError("The used value for unknown_value {self.unknown_value} "
                        "is one of the values already used for encoding the seen categories.")

        if self._missing_indices:
            if np.dtype(self.dtype).kind != "f" and is_scalar_nan(self.encoded_missing_value):
                raise ValueError("There are missing values in features " f"{list(self._missing_indices)}. For OrdinalEncoder to "
                    f"encode missing values with dtype: {self.dtype}, set " "encoded_missing_value to a non-nan value, or set dtype to a float")

            if not is_scalar_nan(self.encoded_missing_value):
                # Features are invalid when they contain a missing category and encoded_missing_value was already used to encode a known category
                invalid_features = [cat_idx
                ...
                if invalid_features:
                    ...
                    raise ValueError(
                        f"encoded_missing_value ({self.encoded_missing_value}) is already used to encode a known category in features: {invalid_features}")
    ...
    def inverse_transform(self, X):
        ...
        for i in range(n_features):
            ...
            if infrequent_indices is not None and infrequent_indices[i] is not None:
                ...
                # Remap categories to be only frequent categories. The infrequent categories will be mapped to "infrequent_sklearn" later
