...
def _create_expansion(X, interaction_only, deg, n_features, cumulative_size=0):
    ...
    if expanded_col == 0:
        return None
    # This only checks whether each block needs 64bit integers upon expansion. 
    # We prefer to keep int32 indexing where we can, since currently SciPy's CSR construction downcasts when possible,
    # so we prefer to avoid an unnecessary cast. The dtype may still change in the concatenation process if needed.
    ...
    # hence we do not detail the entire bug and all possible avoidance mechnasisms. 
    # Instead we recommend upgrading scipy or shrinking their data.
    cumulative_size += expanded_col
    if (sp_version < parse_version("1.8.0") and cumulative_size - 1 > max_int32 and not needs_int64):
        raise ValueError("In scipy versions `<1.8.0`, the function `scipy.sparse.hstack`"
            " sometimes produces negative columns when the output shape contains `n_cols` too large to be represented by a 32bit signed"
            " integer. To avoid this error, either use a version of scipy `>=1.8.0` or alter the `PolynomialFeatures`"
            " transformer to produce fewer than 2^31 output features."
        )

    # Result of the expansion, modified in place by the `_csr_polynomial_expansion` routine.
...
class PolynomialFeatures(TransformerMixin, BaseEstimator):
    ...
    degree : int or tuple (min_degree, max_degree), default=2
        ...
        If a tuple `(min_degree, max_degree)` is passed, then `min_degree` is the minimum 
        and `max_degree` is the maximum polynomial degree of the generated features. 
    ...
    Notes
    -----
    Be aware that the number of features in the output array scales polynomially in the number of features of the input array, 
    and exponentially in the degree. High degrees can cause overfitting.
    ...
    def _num_combinations(n_features, min_degree, max_degree, interaction_only, include_bias):
        ...
        This should be equivalent to counting the number of terms returned by _combinations(...) but much faster.
        """
    ...
    def powers_(self):
        ...
        combinations = self._combinations(n_features=self.n_features_in_, min_degree=self._min_degree, max_degree=self._max_degree,
    ...
    def fit(self, X, y=None):
        ...
        if isinstance(self.degree, Integral):
            ...
            if not (isinstance(self._min_degree, Integral) and isinstance(self._max_degree, Integral)
                and self._min_degree >= 0 and self._min_degree <= self._max_degree):
                raise ValueError("degree=(min_degree, max_degree) must be non-negative integers that fulfil min_degree <= max_degree, got {self.degree}.")
            ...
            raise ValueError("degree must be a non-negative int or tuple (min_degree, max_degree), got {self.degree}.")

        self.n_output_features_ = self._num_combinations(n_features=n_features, min_degree=self._min_degree, max_degree=self._max_degree,
            interaction_only=self.interaction_only, include_bias=self.include_bias)
        if self.n_output_features_ > np.iinfo(np.intp).max:
            msg = ("The output that would result from the current configuration would have {self.n_output_features_} features "
                "which is too large to be indexed by {np.intp().dtype.name}. Please change some or all of the following:\n- "
                "The number of features in the input, currently {n_features=}\n- The range of degrees to calculate,"
                " currently [{self._min_degree}, {self._max_degree}]\n- Whether to include only interaction terms,"
                " currently {self.interaction_only}\n- Whether to  include a bias term, currently {self.include_bias}.")
            if (np.intp == np.int32 and self.n_output_features_ <= np.iinfo(np.int64).max):  # pragma: nocover
                msg += ("\nNote that the current Python runtime has a limited 32 bit address space "
                        "and that this configuration would have been admissible if run on a 64 bit Python runtime.")
            raise ValueError(msg)
        # We also record the number of output features for _max_degree = 0
        self._n_out_full = self._num_combinations(n_features=n_features, min_degree=0, max_degree=self._max_degree,
            interaction_only=self.interaction_only, include_bias=self.include_bias)
    ...
    def transform(self, X):
        ...
        X : {array-like, sparse matrix} of shape (n_samples, n_features)
            ...
            Prefer CSR over CSC for sparse input (for speed), but CSC is required if the degree is 4 or higher. 
            If the degree is less than 4 and the input format is CSC, it will be converted to CSR, 
            have its polynomial features generated, then converted back to CSC.

            If the degree is 2 or 3, the method described in 
            "Leveraging Sparsity to Speed Up Polynomial Feature Expansions of CSR Matrices Using K-Simplex Numbers" 
            by Andrew Nystrom and John Hughes is used, which is much faster than the method used on CSC input. 
            For this reason, a CSC input will be converted to CSR, and the output will be converted back to CSC prior to being returned, 
            hence the preference of CSR.
        ...
        if sparse.issparse(X) and X.format == "csr":
            ...
            if len(to_stack) == 0:
                ...
                # `scipy.sparse.hstack` breaks in scipy<1.9.2 when `n_output_features_ > max_int32`
                ...
                if (sp_version < parse_version("1.9.2") and self.n_output_features_ > max_int32 and all_int32):
                    raise ValueError(  # pragma: no cover
                        "In scipy versions `<1.9.2`, the function `scipy.sparse.hstack` produces negative columns when:\n1. "
                        "The output shape contains `n_cols` too large to be represented by a 32bit signed integer.\n2. "
                        "All sub-matrices to be stacked have indices of dtype `np.int32`.\nTo avoid this error, "
                        "either use a version of scipy `>=1.9.2` or alter the `PolynomialFeatures` transformer "
                        "to produce fewer than 2^31 output features")
            ...
            combinations = self._combinations(n_features=n_features, min_degree=self._min_degree, max_degree=self._max_degree,\
                interaction_only=self.interaction_only, include_bias=self.include_bias)
            columns = []
            ...
            # Do as if _min_degree = 0 and cut down array after the computation, i.e. use _n_out_full instead of n_output_features_.
            ...
            for _ in range(2, self._max_degree + 1):
                ...
                for feature_idx in range(n_features):
                    ...
                    if next_col <= current_col:
                        break
                    # XP[:, start:end] are terms of degree d - 1 that exclude feature #feature_idx.
...
class SplineTransformer(TransformerMixin, BaseEstimator):
    ...
    Generate a new feature matrix consisting of `n_splines=n_knots + degree - 1` (`n_knots - 1` 
    for `extrapolation="periodic"`) spline basis functions (B-splines) of polynomial order=`degree` for each feature.
    ...
    knots : {'uniform', 'quantile'} or array-like of shape (n_knots, n_features), default='uniform'
        ...
        - If an array-like is given, it directly specifies the sorted knot positions including the boundary knots. 
          Note that, internally, `degree` number of knots are added before the first knot, the same after the last knot.

    extrapolation : {'error', 'constant', 'linear', 'continue', 'periodic'}, default='constant'
        If 'error', values outside the min and max values of the training features raises a `ValueError`.
        If 'constant', the value of the splines at minimum and maximum value of the features is used as constant extrapolation. 
        If 'linear', a linear extrapolation is used. If 'continue', the splines are extrapolated as is, 
        i.e. option `extrapolate=True` in :class:`scipy.interpolate.BSpline`. 
        If 'periodic', periodic splines with a periodicity equal to the distance between the first and last knot are used. 
        Periodic splines enforce equal function values and derivatives at the first and last knot.
        For example, this makes it possible to avoid introducing an arbitrary jump between Dec 31st and Jan 1st in spline features derived from 
        a naturally periodic "day-of-year" input feature. In this case it is recommended to manually set the knot values to control the period.

    include_bias : bool, default=True
        If False, then the last spline element inside the data range of a feature is dropped. 
        As B-splines sum to one over the spline basis functions for each data point, they implicitly include a bias term,
    ...
    n_features_out_ : int
        ...
        where `n_splines` is the number of bases elements of the B-splines, 
        `n_knots + degree - 1` for non-periodic splines and `n_knots - 1` for periodic ones.
        If `include_bias=False`, then it is only `n_features * (n_splines - 1)`.

    See Also
    --------
    KBinsDiscretizer : Transformer that bins continuous data into intervals.
    PolynomialFeatures : Transformer that generates polynomial and interaction features.
    ...
    def _get_base_knot_positions(X, n_knots=10, knots="uniform", sample_weight=None):
        ...
        Base knots such that first knot <= feature <= last knot. 
        For the B-spline construction with scipy.interpolate.BSpline, 2*degree knots beyond the base interval are added.

        Returns
        -------
        knots : ndarray of shape (n_knots, n_features), dtype=np.float64, Knot positions (points) of base interval.
        """
        if knots == "quantile":
            ...
            # knots == 'uniform':
            # Note that the variable `knots` has already been validated and `else` is therefore safe.
    ...
    def fit(self, X, y=None, sample_weight=None):
        ...
        sample_weight : array-like of shape (n_samples,), default = None
            Individual weights for each sample. Used to calculate quantiles if `knots="quantile"`. 
            For `knots="uniform"`, zero weighted observations are ignored for finding the min and max of `X`.
        ...
        if self.sparse_output and sp_version < parse_version("1.8.0"):
            raise ValueError("Option sparse_output=True is only available with scipy>=1.8.0, but here scipy=={sp_version} is used.")

        # number of knots for base interval
        n_knots = base_knots.shape[0]

        if self.extrapolation == "periodic" and n_knots <= self.degree:
            raise ValueError("Periodic splines require degree < n_knots. Got n_knots={n_knots} and degree={self.degree}.")
        ...
        # We have to add degree number of knots below, and degree number knots above the base knots in order to make the spline basis complete.
        if self.extrapolation == "periodic":
            # For periodic splines the spacing of the first / last degree knots needs to be a continuation of the spacing of the last / first base knots.
            period = base_knots[-1] - base_knots[0]
            knots = np.r_[base_knots[-(degree + 1) : -1] - period, base_knots, base_knots[1 : (degree + 1)] + period ]

        else:
            # Eilers & Marx in "Flexible smoothing with B-splines and penalties" https://doi.org/10.1214/ss/1038425655 
            # advice against repeating first and last knot several times,
            # which would have inferior behaviour at boundaries if combined with a penalty (hence P-Spline). 
            # We follow this advice even if our splines are unpenalized. Meaning we do not:
            # knots = np.r_[
            ...
            knots = np.r_[np.linspace(base_knots[0] - degree * dist_min, base_knots[0] - dist_min, num=degree),
                base_knots, np.linspace(base_knots[-1] + dist_max, base_knots[-1] + degree * dist_max, num=degree), ]

        # With a diagonal coefficient matrix, we get back the spline basis elements, i.e. the design matrix of the spline.
    ...
    def transform(self, X):
        ...
        Parameters
        ----------
        X : array-like of shape (n_samples, n_features), The data to transform.
        ...
        # Note: self.bsplines_[0].extrapolate is True for extrapolation in ["periodic", "continue"]
        if scipy_1_10:
        ...
        # Note that scipy BSpline returns float64 arrays and converts input x=X[:, i] to c-contiguous float64.
        ...
        for i in range(n_features):
            ...
            if self.extrapolation in ("continue", "error", "periodic"):
                if self.extrapolation == "periodic":
                    # With periodic extrapolation we map x to the segment [spl.t[k], spl.t[n]].
                    # This is equivalent to BSpline(.., extrapolate="periodic") for scipy>=1.0.0.
                ...
                if use_sparse:
                    ...
                    if self.extrapolation == "periodic":
                        # See the construction of coef in fit. We need to add the last degree spline basis function to the first degree ones 
                        # and then drop the last ones. Note: See comment about SparseEfficiencyWarning below.
                ...
                if use_sparse:
                    mask_inv = ~mask
                    x = X[:, i].copy()
                    # Set some arbitrary values outside boundary that will be reassigned later.
                    ...
                    # scipy.sparse._base.SparseEfficiencyWarning: Changing the sparsity structure of a csr_matrix is expensive. 
            ...
            if self.extrapolation == "error":
                # BSpline with extrapolate=False does not raise an error, but outputs np.nan.
                if (use_sparse and np.any(np.isnan(XBS_sparse.data))) \
                    or (not use_sparse and np.any(np.isnan(XBS[:, (i * n_splines) : ((i + 1) * n_splines)]))):
                    raise ValueError("X contains values beyond the limits of the knots.")
            elif self.extrapolation == "constant":
                # Set all values beyond xmin and xmax to the value of the spline basis functions at those two positions.
                # Only the first degree and last degree number of splines have non-zero values at the boundaries.
                ...
                # Continue the degree first and degree last spline bases linearly beyond the boundaries, 
                # with slope = derivative at the boundary. Note that all others have derivative = value = 0 at the boundaries.

                # spline derivatives = slopes at boundaries
                fp_min, fp_max = spl(xmin, nu=1), spl(xmax, nu=1)
                # Compute the linear continuation.
                if degree <= 1:
                    # For degree=1, the derivative of 2nd spline is not zero at boundary. For degree=0 it is the same as 'constant'.
                    degree += 1
        ...
        if use_sparse:
            # TODO: Remove this conditional error when the minimum supported version of SciPy is 1.9.2
            # `scipy.sparse.hstack` breaks in scipy<1.9.2 when `n_features_out_ > max_int32`
            max_int32 = np.iinfo(np.int32).max
            all_int32 = True
            ...
            if (sp_version < parse_version("1.9.2") and self.n_features_out_ > max_int32 and all_int32):
                raise ValueError(
                    ...
                    "The output shape contains `n_cols` too large to be represented by a 32bit signed integer.\n. "
                    "All sub-matrices to be stacked have indices of dtype `np.int32`.\nTo avoid this error, "
                    "either use a version of scipy `>=1.9.2` or alter the `SplineTransformer` transformer to produce fewer than 2^31 output features"
                )
        ...
        if self.include_bias:
            ...
            # We throw away one spline basis per feature. We chose the last one.
            indices = [j for j in range(XBS.shape[1]) if (j + 1) % n_splines != 0]
            return XBS[:, indices]

    def _more_tags(self):
        return { "_xfail_checks": { "check_estimators_pickle": ("Current Scipy implementation of _bsplines does not support const memory views.") } }
