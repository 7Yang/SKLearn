...
class KBinsDiscretizer(TransformerMixin, BaseEstimator):
    ...
    strategy : {'uniform', 'quantile', 'kmeans'}, default='quantile'
        ...
        For an example of the different strategies see: :ref:`sphx_glr_auto_examples_preprocessing_plot_discretization_strategies.py`.
    ...
    subsample : int or None, default='warn'
        ...
        Defaults to 200_000 when `strategy='quantile'` and to `None` when `strategy='uniform'` or `strategy='kmeans'`.
        `subsample=None` means that all the training samples are used when computing the quantiles that determine the binning thresholds.
        Since quantile computation relies on sorting each column of `X` and that sorting has an `n log(n)` time complexity,
        it is recommended to use subsampling on datasets with a very large number of samples.

        .. versionchanged:: 1.3
            The default value of `subsample` changed from `None` to `200_000` when `strategy="quantile"`.

        .. versionchanged:: 1.5
            The default value of `subsample` changed from `None` to `200_000` when `strategy="uniform"` or `strategy="kmeans"`.

    random_state : int, RandomState instance or None, default=None
        Determines random number generation for subsampling.
        Pass an int for reproducible results across multiple function calls.  See the `subsample` parameter for more details.
    ...
    See Also
    --------
    Binarizer : Class used to bin values as ``0`` or ``1`` based on a parameter ``threshold``.

    Notes
    -----

    For a visualization of discretization on different datasets refer to :ref:`sphx_glr_auto_examples_preprocessing_plot_discretization_classification.py`.
    On the effect of discretization on linear models see: :ref:`sphx_glr_auto_examples_preprocessing_plot_discretization.py`.
    ...
    ``KBinsDiscretizer`` might produce constant features (e.g., when ``encode = 'onehot'`` and certain bins do not contain any data).
    These features can be removed with feature selection algorithms (e.g., :class:`~sklearn.feature_selection.VarianceThreshold`).
    ...
    Sometimes it may be useful to convert the data back into the original feature space. 
    The ``inverse_transform`` function converts the binned data into the original feature space. 
    Each value will be equal to the mean of the two bin edges.
    ...
    _parameter_constraints: dict = {
        ...
        "subsample": [Interval(Integral, 1, None, closed="left"), None, Hidden(StrOptions({"warn"}))],
    ...
    def fit(self, X, y=None, sample_weight=None):
        ...
        if sample_weight is not None and self.strategy == "uniform":
            raise ValueError("`sample_weight` was provided but it cannot be used with strategy='uniform'. Got strategy=" 
                f"{self.strategy!r} instead.")

        if self.strategy in ("uniform", "kmeans") and self.subsample == "warn":
            warnings.warn(("In version 1.5 onwards, subsample=200_000 will be used by default. Set subsample explicitly to "
                "silence this warning in the mean time. Set subsample=None to disable subsampling explicitly."), FutureWarning)
        ...
        for jj in range(n_features):
            ...
            if self.strategy in ("quantile", "kmeans"):
                ...
                if len(bin_edges[jj]) - 1 != n_bins[jj]:
                    warnings.warn("Bins whose width are too small (i.e., <= 1e-8) in feature %d are removed. "
                        "Consider decreasing the number of bins." % jj)
        ...
        if "onehot" in self.encode:
            ...
            # Fit the OneHotEncoder with toy datasets so that it's ready for use after the KBinsDiscretizer is fitted
    ...
    def _validate_n_bins(self, n_features):
        ...
        if violating_indices.shape[0] > 0:
            ...
            raise ValueError("{} received an invalid number of bins at indices {}. Number of bins must be at least 2, and must be an int."
