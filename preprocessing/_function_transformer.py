...
class FunctionTransformer(TransformerMixin, BaseEstimator):
    ...
    A FunctionTransformer forwards its X (and optionally y) arguments to a user-defined function or function object 
    ...
    func : callable, default=None
        The callable to use for the transformation. This will be passed the same arguments as transform, with args and kwargs forwarded.
    ...
    inverse_func : callable, default=None
        ...
        with args and kwargs forwarded. If inverse_func is None, then inverse_func will be the identity function.
    ...
    accept_sparse : bool, default=False
        Indicate that func accepts a sparse matrix as input. If validate is False, this has no effect. Otherwise, if accept_sparse is false,
    ...
    feature_names_out : callable, 'one-to-one' or None, default=None
        ...
        If it is a callable, then it must take two positional arguments: 
        this `FunctionTransformer` (`self`) and an array-like of input feature names (`input_features`). 
        It must return an array-like of output feature names. 
        The `get_feature_names_out` method is only defined if `feature_names_out` is not None.
    ...
    def __init__(self, func=None, inverse_func=None, *, validate=False, accept_sparse=False, check_inverse=True, \
    ...
    def _check_input(self, X, *, reset):
        if self.validate:
            ...
            # We run this only when reset==True to store the attributes but not validate them, because validate=False
    ...
    def _check_inverse_transform(self, X):
        ...
        if not _allclose_dense_sparse(X[idx_selected], X_round_trip):
            warnings.warn(("The provided functions are not strictly inverse of each other. "
    ...
    def fit(self, X, y=None):
        ...
        Returns
        -------
        self : object, FunctionTransformer class instance.
        """
    ...
    def get_feature_names_out(self, input_features=None):
        ...
        input_features : array-like of str or None, default=None
            ...
            - If `input_features` is None, then `feature_names_in_` is used as the input feature names. 
              If `feature_names_in_` is not defined, then names are generated: `[x0, x1, ..., x(n_features_in_ - 1)]`.
        ...
        feature_names_out : ndarray of str objects
            ...
            - If `feature_names_out` is 'one-to-one', the input feature names are returned (see `input_features` above). 
              This requires `feature_names_in_` and/or `n_features_in_` to be defined, which is done automatically if `validate=True`. 
              Alternatively, you can set them in `func`.
            - If `feature_names_out` is a callable, then it is called with two arguments, `self` and `input_features`, 
        ...
        if self.feature_names_out == "one-to-one":
            ...
            raise ValueError(f"feature_names_out={self.feature_names_out!r} is invalid. " 
                'It must either be "one-to-one" or a callable with two ' "arguments: the function transformer and an array-like of "
                "input feature names. The callable must return an array-like of output feature names.")
