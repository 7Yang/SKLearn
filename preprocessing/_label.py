...
class LabelEncoder(TransformerMixin, BaseEstimator, auto_wrap_output_keys=None):
    ...
    def transform(self, y):
        ...
        Returns
        -------
        y : array-like of shape (n_samples,), Labels as normalized encodings.
        """
...
class LabelBinarizer(TransformerMixin, BaseEstimator, auto_wrap_output_keys=None):
    ...
    `LabelBinarizer` makes this easy with the :meth:`inverse_transform` method.
    ...
    y_type_ : str
        Represents the type of the target data as evaluated by :func:`~sklearn.utils.multiclass.type_of_target`. 
        Possible type are 'continuous', 'continuous-multioutput', 'binary', 'multiclass',
    ...
    def fit(self, y):
        ...
        if self.sparse_output and (self.pos_label == 0 or self.neg_label != 0):
            raise ValueError("Sparse binarization is only supported with non zero pos_label and zero neg_label, "
                "got pos_label={self.pos_label} and neg_label={self.neg_label}")
    ...
    def fit_transform(self, y):
        ...
        y : {ndarray, sparse matrix} of shape (n_samples,) or (n_samples, n_classes)
            Target values. The 2-d matrix should only contain 0 and 1, represents multilabel classification. 
            Sparse matrix can be CSR, CSC, COO, DOK, or LIL.
    ...
    def inverse_transform(self, Y, threshold=None):
        ...
        threshold : float, default=None
            ...
            Use 0 when ``Y`` contains the output of :term:`decision_function` (classifier).
        ...
        Notes
        -----
        In the case when the binary labels are fractional (probabilistic), :meth:`inverse_transform` chooses the class with the greatest value. 
        Typically, this allows to use the output of a linear model's :term:`decision_function` method directly 
        as the input of :meth:`inverse_transform`.
        """
        check_is_fitted(self)
...
def label_binarize(y, *, classes, neg_label=0, pos_label=1, sparse_output=False):
    ...
    if sparse_output and (pos_label == 0 or neg_label != 0):
        raise ValueError("Sparse binarization is only supported with non zero pos_label and zero neg_label, "
            "got pos_label={0} and neg_label={1}".format(pos_label, neg_label))
...
class MultiLabelBinarizer(TransformerMixin, BaseEstimator, auto_wrap_output_keys=None):
    ...
    it is unwieldy to process. This transformer converts between this intuitive format and the supported multilabel format: 
    ...
    MultiLabelBinarizer()
    >>> mlb.classes_
    array(['-', 'c', 'd', 'e', 'f', 'h', 'i', 'l', 'm', 'o', 'r', 's', 't', 'y'], dtype=object)
    ...
    def _transform(self, y, class_mapping):
        ...
        Returns
        -------
        y_indicator : sparse matrix of shape (n_samples, n_classes), Label indicator matrix. Will be of CSR format.
        """
